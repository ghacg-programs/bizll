...
-   新增社交帐号登录： Google、Facebook、Microsoft、Twitter、Apple 登录功能
-   新增 Stripe 收款接口
-   新增网站语言前后台分开设置功能，可以前台设置一种语言，后台设置另外一种语言
-   新增多个海外社交平台分享功能，并支持在后台设置启用并排序
-   新增商城系统自动发货的商品，可以设置额外发货内容。例如：可以将卡密的使用教程或者软件下载地址等信息同时发送给用户
-   新增文章参数批量设置中，添加“订单时效”的批量设置选项
-   新增底部页脚板块三，添加小图片的功能
-   新增收款的本地货币参数配置以及收款货币及收款汇率单独设置的功能，可实现美元->人民币收款，或自定义一个货币转人民币收款，例如 R 币：1R 币=5 人民币
-   新增佣金处理使用接口自动付款的功能，当收款接口如果是微信或支付宝企业接口时，开启此功能，就无需再手动扫码付款
-   新增默认头像可设置为 Gravatar、WeAvatar、Cravatar 头像，以及极客族、loli、sep.cc、v2ex Gravatar 加速服务
-   为最近评论的小工具模块增加第二种 UI 样式，看上去更美观一些
-   新增匿名评论，可选择允许输入博客链接的选项及链接跳转(支持外链重定向)功能
-   新增文章时间显示可选择显示更新时间或发布时间的选项
-   新增评论和私信可选择插入链接的功能及选项开关
-   新增幻灯片加图文视频卡片模块：左侧显示图片、视频或幻灯片，右侧可以显示任意数量的图片或视频卡片
-   新增帖子页面，显示作者信息+作者帖子模块的功能及开关选项
-   将小工具公告模块更新为“上下滚动公告信息模块”，新增可直接调用筛选后文章或帖子作为消息内容

-   修复用户中心筛选已提现或未提现的数据时，可能会失效的问题
-   更新 PayPal 的收款接口，采用最新的 API 接口
-   修复后台启用“替换 WP 默认登录页面”功能后，用户登录后仍然可以打开后台登录页面的 bug
-   修复社区论坛快速发布功能可能会无法插入图片的 bug
-   修复了其他一些 bug 以及 UI 细节优化

### V8.9---2026/06/01

-   新增页面可单独设置页面背景颜色或背景图的功能
-   新增所有布局模块可设置背景图的功能
-   新增所有布局模块标题样式可选居中大标题的样式
-   新增所有布局模块可添加标题按钮的功能
-   新增模块按容器导出和导入的功能
-   新增模块布局预置布局模板功能
-   新增布局模块：产品虚拟评价模块
...
-   新增布局模块：产品 3D 交互展示模块
-   新增布局模块：产品选项卡片模块
-   新增原生多语言功能，可使用 wp 多语言插件扩展主题语言，主题内置中文、英语两种语言
-   商城新增让用户手动填写收货地址的功能，方便海外网站使用
-   新增 AI 生成 SEO 标题、简介、关键词的功能（实验性功能，需升级 WP 7.0 以上版本））

-   兼容 WordPress V7.0+版本
-   优化了模块布局的前端性能，由于 wp 可视化布局时会加载大量的 dom 元素，导致页面卡顿，主题深度优化减少 95%的 DOM 直接渲染，改为动态渲染，已大幅度较少卡顿现象
-   优化可视化模块布局的操作细节，增加快捷编辑按钮，模块定位提示等
-   优化了模块入场动画的丝滑度
-   优化的部分 UI 及交互细节
-   修复免登录后可能会不显示内容的 bug
-   修复微信企业 H5 支付在某些情况下可能会失败的 bug
-   修复未读消息可能不显示的 bug

### V8.8.1---2026/05/06

-   修复了几个重要 bug，感谢@人民的骆驼的反馈及测试

### V8.8---2026/05/01

-   新增智能多语言翻译功能，基于 translate 开源项目，为网站添加多语言切换功能，自动翻译，免费且准确率高
-   新增微信公众号获取当前菜单数据、获取已发布消息 article_id 明细的功能，方便设置公众号菜单
-   新增商城商品卡密自动发货商品自动同步库存的功能，无需再手动填写库存
-   新增文章按分类设置查看权限的功能，支持登录查看、按用户等级、会员等级、认证等用户属性进行限制，支持分类层级
-   新增页面单独设置布局宽度的功能
-   新增：复合信息卡片模块，显示图标、标题、标签、简介等信息的卡片
-   新增：横向滚动合作伙伴模块，3 种 UI 样式，可自行添加链接或调用后台链接
-   新增：文本标题模块，更适合做模块标题和简介的模块，支持多种 UI 设置
-   新增：按钮组模块，用于显示一个或多个按钮，支持多种 UI 配置
-   新增：布局间距模块，用于再模块之间添加额外的间距
-   为所有模块新增入场动画功能，当页面滚动到模块时，模块以动画的形式出现，支持多种动画效果
-   新增后台订单手动补单功能，管理员手动为订单设置为已支付状态

-   修复几个重要的漏洞，感谢@人民的骆驼的反馈及测试
-   修复板块未设置发布权限选项时可能会错误的 bug
-   修复后台发帖可能会无法保存的 bug
-   修复消息中心可能会报错的 bug
-   修复快速发帖可能会无法添加额外隐藏内容的 bug
-   修复论坛首页在 ipad 等平板电脑上导航栏显示错位的 bug
-   修复前台添加付费资源时，偶尔可能不显示确认按钮的 bug
