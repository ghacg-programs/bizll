...
function zib_ajax_approve_comment()
{
    ...
    if (empty($_POST['comment_id'])) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('数据传入出错', 'zib_language'))));
        exit();
    }
    ...
    if (!$comment) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('查找数据出错', 'zib_language'))));
        exit();
    }
    ...
    if ('trash' == $current || !$current) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('评论已删除', 'zib_language'))));
        exit();
    }
    ...
    if (in_array($current, array('unapproved', 'spam'), true)) {
        ...
        $status = 'approve';
        $msg    = __('此评论已审核', 'zib_language');
    } else {
        $result = wp_set_comment_status($comment, 'hold', true);
        $status = 'hold';
        $msg    = __('已驳回此评论', 'zib_language');
    }
...
{
    if (empty($_POST['comment_id'])) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('数据传入出错，请刷新页面', 'zib_language'))));
        exit();
    }
    ...
    $comment    = get_comment($comment_id);
    if (!$comment) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('查找数据出错，请刷新页面', 'zib_language'))));
        exit();
    }

    if (!zib_current_user_can('comment_edit', $comment)) {
        zib_send_json_error(__('编辑权限不足', 'zib_language'));
    }

    if ('trash' == $comment->comment_approved) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('该评论已删除，请刷新页面', 'zib_language'))));
        exit();
    }
...
function zib_ajax_comment_set_topping()
{
    ...
    if ($comment->comment_parent) {
        zib_send_json_error(['msg' => __('该评论无法设置置顶', 'zib_language')]);
    }
    ...
    if ($is_topping) {
        ...
        zib_send_json_success(['msg' => __('已取消置顶', 'zib_language'), 'reload' => true]);
    } else {
        update_comment_meta($id, 'topping', 1);
        do_action('comment_is_topping', $comment); //添加挂钩
        //刷新缓存
        zib_send_json_success(['msg' => __('已置顶此评论', 'zib_language'), 'reload' => true]);
    }
...
function zib_ajax_trash_comment()
{
    ...
    if (wp_trash_comment($comment)) {
        echo(json_encode(array('msg' => __('评论已删除', 'zib_language'))));
    } else {
...
function zib_ajax_submit_comment()
{
    ...
    if (empty($_POST['comment']) || zib_new_strlen($_POST['comment']) < 2) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('请输入内容', 'zib_language'))));
        exit();
    }
    ...
    if ($edit_id) {
        ...
        if ($comment->comment_content == $_POST['comment']) {
            echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('内容未修改', 'zib_language'))));
            exit;
        }
        ...
        if (!$update_comment) {
            echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('评论修改失败', 'zib_language'))));
            exit;
        }

        echo(json_encode(array('error' => 0, 'html' => zib_comment_filters($_POST['comment']), 'wait_time' => $wait_time, 'msg' => __('评论已修改', 'zib_language'))));
        exit;
    }
    ...
    if (is_wp_error($comment)) {
        ...
        if (!empty($data)) {
            ...
            echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('评论提交失败', 'zib_language'))));
            exit;
        }
    }
    ...
    $msg = __('评论已提交', 'zib_language');
    $msg .= '0' == $comment->comment_approved ? __('，正在等待审核', 'zib_language') : '';
