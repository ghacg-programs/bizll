...
function zib_ajax_user_signin_nopas()
{
    if (is_user_logged_in()) {
        echo(json_encode(array('error' => 1, 'reload' => 1, 'msg' => __('您已登录，请刷新页面', 'zib_language'))));
        exit;
    }

    $nopas_s = _pz('user_signin_nopas_s');
    if (!$nopas_s) {
        echo(json_encode(array('error' => 1, 'msg' => __('暂未开启此登录方式', 'zib_language'))));
        exit;
    }
    ...
    if ('email' == $captcha_type) {
        ...
        if (!$user) {
            echo(json_encode(array('error' => 1, 'msg' => __('未找到此邮箱注册账户', 'zib_language'))));
            exit();
        }
    } elseif ('phone' == $captcha_type) {
        $user = zib_get_user_by('phone', $captcha_val);
        if (!$user) {
            echo(json_encode(array('error' => 1, 'msg' => __('未找到此手机号注册账户', 'zib_language'))));
            exit();
        }
    }
    if (!$user) {
        echo(json_encode(array('error' => 1, 'msg' => __('未找到您的用户信息', 'zib_language'))));
        exit();
    }
...
{
    if (is_user_logged_in()) {
        echo(json_encode(array('error' => 1, 'reload' => 1, 'msg' => __('你已经登录，请刷新页面', 'zib_language'))));
        exit;
    }
    ...
    if (strlen($_POST['password2']) < 6) {
        echo(json_encode(array('error' => 1, 'msg' => __('密码太短,至少6位', 'zib_language'))));
        exit();
    }
    ...
    if ((!$no_repas || !$captch) && $_POST['password2'] !== $_POST['repassword']) {
        echo(json_encode(array('error' => 1, 'msg' => __('两次密码输入不一致', 'zib_language'))));
        exit();
    }
    ...
    if ($captch) {
        ...
        if ('email' == $captcha_type && email_exists($captcha_val)) {
            echo(json_encode(array('error' => 1, 'msg' => __('该邮箱已注册，请登录', 'zib_language'))));
            exit();
        }
        if ('phone' == $captcha_type && zib_get_user_by('phone', $captcha_val)) {
            echo(json_encode(array('error' => 1, 'msg' => __('该手机号已注册，请登录', 'zib_language'))));
            exit();
        }
    ...
    if (is_wp_error($status)) {
        ...
        echo(json_encode(array('error' => 1, 'status' => $status, 'msg' => __('系统出错，请稍候再试', 'zib_language'))));
        exit();
    }
    ...
    $result = array('error' => 0, 'reload' => 1, 'msg' => sprintf(__('注册成功，欢迎您：%s', 'zib_language'), $sanitized_user_login));
    //重定向返回页面
    if (!empty($_REQUEST['redirect_to'])) {
        $result['goto'] = $_REQUEST['redirect_to'];
    }
...
function zib_ajax_user_signin()
{
    ...
    if (empty($_POST['password'])) {
        echo(json_encode(array('error' => 1, 'msg' => __('请输入密码', 'zib_language'))));
        exit();
    }
    ...
    if (filter_var($_POST['username'], FILTER_VALIDATE_EMAIL)) {
        ...
        if (empty($user_data)) {
            echo(json_encode(array('error' => 1, 'msg' => __('未找到此用户名注册账户', 'zib_language'))));
            exit();
        }
    }
    ...
    if (is_wp_error($user_verify)) {
        $error_message = __('账号或密码错误', 'zib_language');
...
function zib_ajax_signin_captcha()
{
    ...
    if ('email' == $captcha_type && !email_exists($to)) {
        echo(json_encode(array('error' => 1, 'msg' => __('该邮箱尚未注册或绑定账号', 'zib_language'))));
        exit();
    }
    if ('phone' == $captcha_type && !zib_get_user_by('phone', $to)) {
        echo(json_encode(array('error' => 1, 'msg' => __('该手机号尚未注册或绑定账号', 'zib_language'))));
        exit();
    }
...
function zib_ajax_resetpassword_captcha($captcha_type = '', $send = '')
{
    ...
    if ($current_user_id) {
        ...
        if ('email' == $captcha_type) {
            ...
            if (!$user_email) {
                echo(json_encode(array('error' => 1, 'msg' => __('您的账号暂未绑定邮箱', 'zib_language'))));
                exit();
            }
            if ($user_email != $to) {
                echo(json_encode(array('error' => 1, 'msg' => __('您输入邮箱账号与您绑定的邮箱不一致', 'zib_language'))));
                exit();
            }
        }
        if ('phone' == $captcha_type) {
            ...
            if (!$phone) {
                echo(json_encode(array('error' => 1, 'msg' => __('您的账号暂未绑定手机号', 'zib_language'))));
                exit();
            }

            if ($phone != $to) {
                echo(json_encode(array('error' => 1, 'msg' => __('您输入手机号与您绑定的手机号不一致', 'zib_language'))));
                exit();
            }
        }
    }
    ...
    if (!$allow) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('此用户不允许重置密码', 'zib_language'))));
        exit();
    }
...
function zib_ajax_reset_password($captcha_type = '', $send = '')
{
    ...
    if ($current_user_id && (!$user_data || $current_user_id != $user_data->ID)) {
        ...
        $captcha_type_name = array(
            'email' => __('邮箱账号', 'zib_language'),
            'phone' => __('手机号', 'zib_language'),
        );
        echo(json_encode(array('error' => 1, 'msg' => sprintf(__('您的%s输入错误', 'zib_language'), $captcha_type_name[$captcha_type]))));
        exit();
    }

    if (!$user_data) {
        echo(json_encode(array('error' => 1, 'msg' => __('未查询到您的账号信息', 'zib_language'))));
        exit();
    }
    ...
    echo(json_encode(array('error' => 0, 'reload' => 1, 'goto' => esc_url(home_url()), 'msg' => __('密码重设成功！请牢记新密码', 'zib_language'))));
    exit;
}
