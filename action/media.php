...
function zib_ajax_user_upload_image()
{
    ...
    if (!$cuid) {
        echo(json_encode(array('error' => 1, 'error_id' => 'nologged', 'ys' => 'danger', 'msg' => __('请先登录', 'zib_language'))));
        exit;
    }

    if (!wp_verify_nonce($_POST['upload_image_nonce'], 'upload_image')) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('安全验证失败，请刷新页面后重试', 'zib_language'))));
        exit();
    }
    ...
    echo(json_encode(array('error' => '', 'ys' => '', 'msg' => __('图片已上传', 'zib_language'), 'img_url' => $img_url)));
    exit();
}
...
function zib_ajax_download_file()
{
    ...
    if (!isset($_REQUEST['nonce']) || !wp_verify_nonce($_REQUEST['nonce'], 'download_file_' . $file_id)) {
        _default_wp_die_handler(__('下载链接已过期，请刷新页面重新下载！', 'zib_language'), __('下载链接已过期', 'zib_language'));
    }

    $attachment = get_post($file_id);

    if (!isset($attachment->ID) || 'attachment' !== $attachment->post_type) {
        _default_wp_die_handler(__('抱歉！文件不存在或已删除！', 'zib_language'), __('文件不存在', 'zib_language'));
    }
...
function zib_ajax_user_split_upload()
{
    ...
    if (empty($_FILES[$file_id])) {
        zib_send_json_error(__('上传信息错误，请重新选择文件', 'zib_language'));
    }
    ...
    if (!$move_new_file) {
        zib_send_json_error(['msg' => __('文件移动失败，请重新选择', 'zib_language'), 'move_new_file' => $move_new_file, 'chunk_path' => $chunk_path]);
    }
...
function zib_ajax_user_split_upload_merge()
{
    ...
    for ($i = 0; $i <= $split_chunks_count - 1; $i++) {
        ...
        if (!$chunkData || !file_exists($chunkFile)) {
            ...
            zib_send_json_success(['msg' => __('文件缺少分片，正在重新上传缺失分片', 'zib_language'), 'ys' => 'warning', 'chunk_loss' => true, 'progress' => ($uploaded_md5_data['uploaded_count'] / $split_chunks_count), 'uploaded_chunks' => $uploaded_md5_data['uploaded_chunks'], 'uploaded_count' => $uploaded_md5_data['uploaded_count'], 'chunkFile' => $chunkFile]);
        }
...
function zib_ajax_user_split_uploaded_chunk()
{
    ...
    if (!$file_name || $split_chunks_count < 1) {
        zib_send_json_error(__('参数异常', 'zib_language'));
    }
