...
function zib_ajax_post_delete_modal()
{
    ...
    if ('trash' === $post->post_status) {
        zib_ajax_notice_modal('info', __('文章已删除，请刷新页面', 'zib_language'));
    }
    ...
    $desc = '<div class="c-red mb20">' . sprintf(__('当前文章下下，共有%d次评论，确认要删除吗？', 'zib_language'), $all_comment_count) . '</div>';

    $header = zib_get_modal_colorful_header('jb-red', '<i class="fa fa-exclamation-triangle" aria-hidden="true"></i>', __('确认删除此文章？', 'zib_language'));

    $title = $title ? '<b>（' . $title . '）</b>' : '';
    $con   = '';
    $con .= '<div class="em12 mb10">' . sprintf(__('您正在删除文章%s', 'zib_language'), $title) . '</div>';
    $con .= $desc;
    ...
    $footer .= $hidden_html;
    $footer .= '<button type="button" data-dismiss="modal" href="javascript:;" class="but">' . __('取消', 'zib_language') . '</button>';
    $footer .= '<button class="but c-red wp-ajax-submit"><i class="fa fa-trash-o" aria-hidden="true"></i>' . __('确认删除', 'zib_language') . '</button>';
    $footer .= '</div>';
...
function zib_ajax_new_post_delete()
{
    ...
    if ('trash' === $post->post_status) {
        zib_send_json_success(array('msg' => __('文章已经删除', 'zib_language'), 'reload' => true, 'goto' => home_url()));
    }

    if (!zib_current_user_can('new_post_delete', $post)) {
        zib_send_json_error(__('您没有删除此内容的权限', 'zib_language'));
    }
...
function zib_ajax_new_posts()
{
    ...
    if (!_pz('post_article_s')) {
        echo (json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('投稿功能已关闭', 'zib_language'))));
        exit();
    }
    ...
    if (!$posts_id && !zib_current_user_can('new_post_add')) {
        zib_send_json_error(__('抱歉您的权限不足，暂时无法发布', 'zib_language'));
    }

    if ($posts_id && !zib_current_user_can('new_post_edit', $posts_id)) {
        zib_send_json_error(__('抱歉您的权限不足，暂时无法编辑此文章', 'zib_language'));
    }
    ...
    if (empty($title)) {
        zib_send_json_error(__('请填写文章标题', 'zib_language'));
    }
    if (empty($content)) {
        zib_send_json_error(__('还未填写任何内容', 'zib_language'));
    }
    ...
    if ('posts_save' == $action) {
        ...
        if (zib_new_strlen($content) < 10) {
            zib_send_json_error(__('文章内容过少', 'zib_language'));
        }
        if (empty($cat)) {
            zib_send_json_error(__('请选择文章分类', 'zib_language'));
        }
    }
    ...
    if (!$cuid) {
        ...
        if (empty($_POST['user_name'])) {
            zib_send_json_error(__('请输入昵称', 'zib_language'));
        }
        $post_author = _pz('post_article_user', 1);
        $lx          = !empty($_POST['contact_details']) ? sprintf(__(',联系：%s', 'zib_language'), esc_attr($_POST['contact_details'])) : '';
        $title       = $title . sprintf(__('[投稿-姓名：%1$s%2$s]', 'zib_language'), esc_attr($_POST['user_name']), $lx);
    }
    ...
    if (!$in_id) {
        zib_send_json_error(__('文章保存失败，请稍后再试', 'zib_language'));
    }
    ...
    switch ($postarr['post_status']) {
        ...
        case 'draft': //草稿
            ...
            break;
        default:
            $send['msg']    = __('文章已发布', 'zib_language');
            $send['reload'] = true;
            $send['goto']   = $url;
    }
...
function zib_ajax_newpost_save_pay($post_id = null, $echo_success = null)
{
        ...
        if ($pay_modo !== 'points') {
            ...
            if (_pz('post_article_pay_free_s')) {
                ...
                if (!isset($posts_zibpay['pay_price']) || !is_numeric($posts_zibpay['pay_price'])) {
                    zib_send_json_error(array('posts_id' => $post_id, 'msg' => __('请设置付费内容的价格', 'zib_language')));
                }
            } elseif (!$pay_price) {
                zib_send_json_error(array('posts_id' => $post_id, 'msg' => __('请设置付费内容的价格', 'zib_language')));
            }
            ...
            if ($pay_price < 0) {
                zib_send_json_error(array('posts_id' => $post_id, 'msg' => __('请设置正确的价格', 'zib_language')));
            }
            ...
            if ($vip_input_s) {
                if (isset($posts_zibpay['vip_1_price']) && !$posts_zibpay['vip_1_price'] && $posts_zibpay['vip_1_price'] != '0') {
                    zib_send_json_error(array('posts_id' => $post_id, 'msg' => sprintf(__('请设置%s价格', 'zib_language'), _pz('pay_user_vip_1_name'))));
                }
                if (isset($posts_zibpay['vip_2_price']) && !$posts_zibpay['vip_2_price'] && $posts_zibpay['vip_2_price'] != '0') {
                    zib_send_json_error(array('posts_id' => $post_id, 'msg' => sprintf(__('请设置%s价格', 'zib_language'), _pz('pay_user_vip_2_name'))));
                }
                ...
                if ($vip_1_price > $pay_price) {
                    zib_send_json_error(array('posts_id' => $post_id, 'msg' => __('会员价不能高于普通价', 'zib_language')));
                }

                if ($vip_2_price > $vip_1_price) {
                    zib_send_json_error(array('posts_id' => $post_id, 'msg' => sprintf(__('%1$s会员价不能高于%2$s价格', 'zib_language'), _pz('pay_user_vip_2_name'), _pz('pay_user_vip_1_name'))));
                }
            ...
            if (!$points_price) {
                zib_send_json_error(array('posts_id' => $post_id, 'msg' => __('请设置付费内容的积分价格', 'zib_language')));
            }

            if ($points_price < 0) {
                zib_send_json_error(array('posts_id' => $post_id, 'msg' => __('请设置正确的积分价格', 'zib_language')));
            }
            ...
            if ($vip_input_s) {
                if (isset($posts_zibpay['vip_1_points']) && !$posts_zibpay['vip_1_points'] && $posts_zibpay['vip_1_points'] != '0') {
                    zib_send_json_error(array('posts_id' => $post_id, 'msg' => sprintf(__('请设置%s积分售价', 'zib_language'), _pz('pay_user_vip_1_name'))));
                }
                if (isset($posts_zibpay['vip_2_points']) && !$posts_zibpay['vip_2_points'] && $posts_zibpay['vip_2_points'] != '0') {
                    zib_send_json_error(array('posts_id' => $post_id, 'msg' => sprintf(__('请设置%s积分售价', 'zib_language'), _pz('pay_user_vip_2_name'))));
                }
                ...
                if ($vip_2_points > $vip_1_points) {
                    zib_send_json_error(array('posts_id' => $post_id, 'msg' => sprintf(__('%1$s价格不能高于%2$s价格', 'zib_language'), _pz('pay_user_vip_2_name'), _pz('pay_user_vip_1_name'))));
                }
    ...
    if (null === $echo_success ? true : $echo_success) {
        zib_send_json_success(array('msg' => __('文章付费功能保存成功', 'zib_language'), 'reload' => true, 'post_id' => $post_id));
    }
}

//设置特色封面
function zib_ajax_newpost_save_featured($post_id = null, $echo_success = null)
{
    $post_id = $post_id ? $post_id : (!empty($_REQUEST['posts_id']) ? (int) $_REQUEST['posts_id'] : 0);
    ...
    if (null === $echo_success ? true : $echo_success) {
        zib_send_json_success(array('msg' => __('封面保存成功', 'zib_language'), 'reload' => true, 'post_id' => $post_id));
    }
}
