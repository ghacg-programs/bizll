 ...
 * @LastEditTime : 2026-06-22 21:35:15
...
function zib_ajax_user_edit_datas()
{
    ...
    if (!$cuid) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('权限不足', 'zib_language'))));
        exit;
    }
    ...
    if (!empty($_POST['desc'])) {
        if (zib_new_strlen($_POST['desc']) < 4) {
            echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('签名过短，不能低于4个字符', 'zib_language'))));
            exit();
        }
        if (zib_new_strlen($_POST['desc']) > 60) {
            echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('签名过长，不能超过60个字符', 'zib_language'))));
            exit();
        }
    }
    ...
    if ($_POST['url'] && (!zib_is_url($_POST['url']))) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('网址格式错误', 'zib_language'))));
        exit();
    }

    if ($_POST['url'] && !$_POST['url_name']) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('请输入个人网站名称', 'zib_language'))));
        exit();
    }

    if ($_POST['url_name'] && !$_POST['url']) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('请输入个人网站链接', 'zib_language'))));
        exit();
    }

    if ($_POST['address'] && zib_new_strlen($_POST['address']) > 50) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('居住地格式错误', 'zib_language'))));
        exit();
    }

    if ($_POST['weibo'] && (!zib_is_url($_POST['weibo']) || zib_new_strlen($_POST['weibo']) > 50)) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('微博格式错误', 'zib_language'))));
        exit();
    }

    if ($_POST['github'] && (!zib_is_url($_POST['github']) || zib_new_strlen($_POST['github']) > 50)) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('GitHub格式错误', 'zib_language'))));
        exit();
    }

    if ($_POST['qq'] && !preg_match("/^[1-9]\d{4,16}$/", $_POST['qq'])) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('QQ格式错误', 'zib_language'))));
        exit();
    }

    if ($_POST['weixin'] && zib_new_strlen($_POST['weixin']) > 20) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('微信字数过长，限制在30字内', 'zib_language'))));
        exit();
    }
    ...
    echo(json_encode(array('error' => 0, 'ys' => '', 'msg' => __('用户资料修改成功', 'zib_language'))));
    exit();
}
...
function zib_ajax_user_upload_avatar()
{
    ...
    if (!wp_verify_nonce($_POST['upload_avatar_nonce'], 'upload_avatar')) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('安全验证失败，请稍候再试', 'zib_language'))));
        exit();
    }

    if (empty($_FILES['file'])) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('请选择图像', 'zib_language'))));
        exit();
    }
    ...
    echo(json_encode(array('error' => 0, 'hide_modal' => true, 'msg' => __('头像修改成功', 'zib_language'), 'replace_img' => '.avatar-id-' . $cuid, 'img_id' => $img_id, 'url' => $image_url[0])));
    exit();
}
...
function zib_ajax_user_upload_cover()
{
    ...
    echo(json_encode(array('error' => 0, 'hide_modal' => true, 'msg' => __('封面图修改成功', 'zib_language'), 'img_id' => $img_id, 'replace_img' => '.user-cover-id-' . $cuid, 'url' => $image_url[0])));
    exit();
}
...
function zib_ajax_user_change_password()
{
    ...
    if (!$user_id) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('登录状态异常，请刷新后重试', 'zib_language'))));
        exit;
    }
    if (empty($_POST['password']) || empty($_POST['password2'])) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('密码不能为空', 'zib_language'))));
        exit();
    }

    if (strlen($_POST['password']) < 6) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('密码至少6位', 'zib_language'))));
        exit();
    }

    if ($_POST['password'] !== $_POST['password2']) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('两次密码输入不一致', 'zib_language'))));
        exit();
    }
    ...
    if (!$oauth_new) {
        ...
        if (empty($_POST['passwordold'])) {
            echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('请输入原密码', 'zib_language'))));
            exit();
        }

        if ($_POST['passwordold'] == $_POST['password']) {
            echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('新密码和原密码不能相同', 'zib_language'))));
            exit();
        }
        ...
        if (!$valid) {
            echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('原密码错误', 'zib_language'))));
            exit();
        }
    }
    ...
    $msg = $oauth_new ? __('密码设置成功', 'zib_language') : __('修改成功，下次请使用新密码登录', 'zib_language');
...
function zib_ajax_user_oauth_untying()
{
    ...
    echo(json_encode(array('error' => 0, 'ys' => '', 'reload' => true, 'goto' => $goto, 'msg' => __('已解除绑定', 'zib_language'))));
...
function zib_ajax_bind_email_captcha()
{
    ...
    if (empty($_POST['email'])) {
        echo(json_encode(array('error' => 1, 'msg' => __('请输入邮箱账号', 'zib_language'))));
        exit();
    }
    ...
    if ($email == $_POST['email']) {
        echo(json_encode(array('error' => 1, 'msg' => __('修改邮箱不能与现邮箱相同', 'zib_language'))));
        exit();
    }
    ...
    if (email_exists($_POST['email'])) {
        echo(json_encode(array('error' => 1, 'msg' => __('该邮箱已有绑定账号', 'zib_language'))));
        exit();
    }
...
function zib_ajax_user_bind_email()
{
    ...
    if (_pz('user_bind_option', true, 'email_set_captch')) {
        ...
        //执行老邮箱验证判断
        if ($email) {
            zib_ajax_is_verify_user('email', $cuid, __('老邮箱', 'zib_language'));
        }
    }
    ...
    $msg = $email ? __('邮箱修改成功', 'zib_language') : __('邮箱绑定成功', 'zib_language');
    echo(json_encode(array('error' => 0, 'msg' => $msg)));
    exit();
}
...
function zib_ajax_user_bind_phone()
{
    ...
    //执行老手机号码验证判断
    if ($old_phone) {
        zib_ajax_is_verify_user('phone', $cuid, __('老手机号', 'zib_language'));
    }
    ...
    $msg = $old_phone ? __('手机号修改成功', 'zib_language') : __('手机号绑定成功', 'zib_language');
    echo(json_encode(array('error' => 0, 'msg' => $msg)));
    exit();
}
...
function zib_ajax_user_set_modal()
{
    ...
    if (!$cuid) {
        zib_ajax_notice_modal('danger', __('登录状态异常，请刷新后重试', 'zib_language'));
    }
...
function zib_ajax_verify_user_captcha()
{
    ...
    if ('email' == $type) {
        ...
        if (!filter_var($to, FILTER_VALIDATE_EMAIL)) {
            echo(json_encode(array('error' => 1, 'msg' => __('用户邮箱格式错误，请于管理员联系', 'zib_language'))));
            exit();
        }
    } elseif ('phone' == $type) {
        $to = get_user_meta($cuid, 'phone_number', true);
        if (!$to) {
            echo(json_encode(array('error' => 1, 'msg' => __('未找到绑定的手机号', 'zib_language'))));
            exit();
        }
    }
...
function zib_ajax_verify_user()
{
    ...
    if (empty($_REQUEST['captch'])) {
        echo(json_encode(array('error' => 1, 'msg' => __('请输入验证码', 'zib_language'))));
        exit();
    }
    ...
    echo(json_encode(array('error' => 0, 'msg' => __('验证成功', 'zib_language'))));
    exit();
}
...
function zib_ajax_user_rewards_modal()
{
    ...
    $rewards_title = $rewards_title ? $rewards_title : __('文章很赞！支持一下吧', 'zib_language');
    $weixin_img    = '';
    $alipay_img    = '';
    ...
    if ($weixin) {
        $weixin_img = '<li><p class="muted-2-color">' . __('微信扫一扫', 'zib_language') . '</p><div class="rewards-img"><img class="fit-cover" src="' . $weixin . '"></div></li>';
    }
    if ($alipay) {
        $alipay_img = '<li><p class="muted-2-color">' . __('支付宝扫一扫', 'zib_language') . '</p><div class="rewards-img"><img class="fit-cover" src="' . $alipay . '"></div></li>';
    }
