...
if (empty($wxConfig['appid']) || empty($wxConfig['appkey'])) {
    wp_die(__('微信公众号配置错误，请先在后台配置参数', 'zib_language'));
}
...
switch ($action) {
    case 'code_check': //验证验证码是否正确
        ...
        if (!empty($wxConfig['gzh_type']) && $wxConfig['gzh_type'] === 'not') {
            ...
            if (!$code) {
                echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('请输入验证码', 'zib_language'))));
                exit();
            }
            ...
            if (!$user_key) {
                echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('验证码错误或已过期，请重新获取', 'zib_language'))));
                exit();
            }

            if ($user_key === -1) {
                echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('验证码已过期，请重新获取', 'zib_language'))));
                exit();
            }
            ...
            echo(json_encode(array('reload' => 1, 'msg' => __('验证成功', 'zib_language'), 'openid' => $user_key, 'goto' => add_query_arg($goto_uery_arg, $wxConfig['backurl']))));
            exit();

        }
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('功能未启用', 'zib_language'))));
        exit();
        break;
    ...
    case 'code_login':
        ...
        if (!$openId) {
            zib_oauth_die(__('参数传入错误', 'zib_language'));
        }
        ...
        if (empty($_REQUEST['nonce']) || !wp_verify_nonce($_REQUEST['nonce'], 'weixingzh_code_login_nonce')) {
            zib_oauth_die(__('页面已过期，请重新登录', 'zib_language'));
        }
        $oauth_result = zib_oauth_update_user($oauth_data);
    ...
    case 'callback':
        ...
        if (!empty($wxOAuth->callback) && !empty($wxConfig['gzh_type']) && $wxConfig['gzh_type'] === 'not') {
            $wxOAuth->code_keyword        = !empty($wxConfig['code_keyword']) ? $wxConfig['code_keyword'] : __('验证码', 'zib_language');
            $wxOAuth->code_reply_template = !empty($wxConfig['code_reply']) ? $wxConfig['code_reply'] : __("您的验证码为：%code%\n有效期%time%秒，如过期或验证失败可以重新发送“%keyword%”获取验证码", 'zib_language');
