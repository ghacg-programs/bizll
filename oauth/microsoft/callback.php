<?php
/*
 * @Author: Qinver
 * @Url: zibll.com
 */

require_once get_theme_file_path('/oauth/sdk/microsoft/OAuth2.php');

@session_start();

if (empty($_SESSION['YURUN_MICROSOFT_STATE'])) {
    wp_safe_redirect(home_url());
    exit;
}

$microsoftConfig = get_oauth_config('microsoft');
$microsoftOAuth  = new \Zib\OAuthLogin\Microsoft\OAuth2($microsoftConfig['appid'], $microsoftConfig['appkey'], $microsoftConfig['backurl']);

if ($microsoftConfig['agent']) {
    $microsoftOAuth->loginAgentUrl = esc_url(home_url('/oauth/microsoftagent'));
}

try {
    $accessToken = $microsoftOAuth->getAccessToken($_SESSION['YURUN_MICROSOFT_STATE']);
    $userInfo    = $microsoftOAuth->getUserInfo();
    $openid      = $microsoftOAuth->openid;
} catch (Exception $err) {
    zib_oauth_die($err->getMessage());
}

if ($openid && $userInfo) {
    $display_name          = !empty($userInfo['name']) ? $userInfo['name'] : '';
    $userInfo['nick_name'] = $display_name;

    $oauth_data = array(
        'type'        => 'microsoft',
        'openid'      => $openid,
        'name'        => $display_name,
        'avatar'      => '',
        'description' => '',
        'getUserInfo' => $userInfo,
    );

    zib_agent_callback($oauth_data);

    $oauth_result = zib_oauth_update_user($oauth_data);

    if ($oauth_result['error']) {
        zib_oauth_die($oauth_result['msg']);
    }

    $rurl = !empty($_SESSION['oauth_rurl']) ? $_SESSION['oauth_rurl'] : $oauth_result['redirect_url'];
    wp_safe_redirect($rurl);
    exit;
}

zib_oauth_die();
wp_safe_redirect(home_url());
exit;
