...
class OAuth2
{
    ...
    private function getAccessTokenFromRemote()
    {
        ...
        if (!is_array($res)) {
            throw new GZHException(__('AccessToken获取失败：网络连接异常，无法访问微信API接口', 'zib_language'));
        }

        if (isset($res['errcode'])) {
            throw new GZHException(sprintf(__('AccessToken获取失败，错误码：%1$s，错误信息：%2$s', 'zib_language'), $res['errcode'], $this->errmsgToChinese($res['errmsg'])));
        }

        throw new GZHException(sprintf(__('AccessToken获取失败：%s', 'zib_language'), json_encode($res)));
    }
    ...
    public function sendTemplateMsg($open_id, $template_id, $data, $url = '')
    {
        ...
        if (!is_array($result)) {
            throw new GZHException(__('发送模板消息失败：网络连接异常，无法访问微信API接口', 'zib_language'));
        }

        if (isset($result['errcode']) && $result['errcode'] != 0) {
            throw new GZHException(sprintf(__('发送模板消息失败，错误码：%1$s，错误信息：%2$s', 'zib_language'), $result['errcode'], $this->errmsgToChinese($result['errmsg'])));
        }
    ...
    {
        $errmsg_array = array(
            'invalid template_id'             => __('模板ID无效或不存在', 'zib_language'),
            'invalid url size'                => __('不合法的 URL 长度', 'zib_language'),
            'invalid appid'                   => __('不合法的 AppID ，请开发者检查 AppID 的正确性', 'zib_language'),
            'invalid message type'            => __('不合法的消息类型', 'zib_language'),
            'invalid openid'                  => __('不合法的 OpenID ，请开发者确认 OpenID （该用户）是否已关注公众号', 'zib_language'),
            'system error'                    => __('系统繁忙，此时请开发者稍候再试', 'zib_language'),
            'invalid appsecret'               => __('不合法的 AppSecret ，请开发者检查 AppSecret 的正确性', 'zib_language'),
            'not in whitelist'                => __('IP 地址不在白名单中，请在接口开通时将 IP 地址添加到接口 IP 白名单中', 'zib_language'),
            'reach max api daily quota limit' => __('调用超过天级别频率限制', 'zib_language'),
            'argument invalid'                => __('参数内容错误，或参数格式错误', 'zib_language'),
            'require subscribe'               => __('该用户已取消关注公众号', 'zib_language'),
        );
    ...
    public function searchPost($s, $opt = array())
    {
        ...
        if ($lists) {
            return (!empty($option['title']) ? $option['title'] : __('为您查询到以下内容', 'zib_language')) . "\n\n" . $lists . '<a href="' . home_url('/?s=') . esc_attr($s) . '">' . __('【查看更多搜索结果】', 'zib_language') . '</a>';
        }
    ...
    public function getQrcodeById($repeat = true)
    {
        ...
        if (!is_array($result)) {
            throw new GZHException(__('二维码获取失败：网络连接异常，无法访问微信API接口', 'zib_language'));
        }
        ...
        if (isset($result['errcode'])) {
            throw new GZHException(sprintf(__('二维码获取失败，错误码：%1$s，错误信息：%2$s', 'zib_language'), $result['errcode'], $this->errmsgToChinese($result['errmsg'])));
        }

        throw new GZHException(sprintf(__('二维码获取失败：%s', 'zib_language'), json_encode($result)));
    }
    ...
    public function getUserInfo($openId)
    {
        ...
        throw new GZHException(sprintf(__('用户信息获取失败：%s', 'zib_language'), json_encode($data)));
    }
