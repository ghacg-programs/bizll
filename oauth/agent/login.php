...
if (!$type || !$config['url']) {
    echo (json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('代理社交登录配置参数错误', 'zib_language'))));
    exit();
}

if (!in_array($type, $oauth_s)) {
    echo (json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('此登录方式未启用', 'zib_language'))));
    exit();
}
...
if ($type == 'weixingzh') {
    ...
    if ($action === 'check_callback') {
        ...
        $response = $http->timeout(10000)->post($check_url);

        if (empty($response->success)) {
            echo json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('代理登录接口连接超时', 'zib_language'), 'url' => $check_url));
            exit();
        }
    ...
    if (zib_weixingzh_is_qrcode()) {
        ...
        //获取二维码
        $http     = new Yurun\Util\HttpRequest;
        $response = $http->timeout(10000)->post($url);
        //返回请求
        if (empty($response->success)) {
            echo json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('代理登录接口链接超时', 'zib_language'), 'url' => $url));
            exit();
        }
        $result = $response->json(true);
