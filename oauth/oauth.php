 ...
 * @LastEditTime : 2026-06-15 20:31:28
 * @Email         : 770349780@qq.com
 * @Project       : Zibll子比主题
 * @Description   : 一款极其优雅的Wordpress主题|社交账号登录
...
function zib_oauth_update_user($args)
{
    ...
    if (zib_oauth_new_is_to_page()) {
        ...
        $url = add_query_arg('redirect_to', ($rurl), zib_get_sign_url('oauth'));
        header('location:' . $url);
        exit;
    }
...
function zib_get_oauth_bind_tab($args)
{
    ...
    $oauth_box = '<div class="oauth-data-box">
                    <div class="muted-box">
                        <div class="user-info flex ac relative">
                            ...
                            <div class="flex1 ml10"><b>' . sprintf(__('您好！%s', 'zib_language'), esc_html($args['name'])) . '</b><div class="mt6 px12 muted-2-color">' . sprintf(__('您首次使用此%1$s登录', 'zib_language'), '<b class="badg badg-sm c-blue">' . esc_html($type_name) . esc_html__('账号', 'zib_language') . '</b>') . '<br>' . __('请先绑定已有账号或创建新账号', 'zib_language') . '</div></div>
                        </div>
                    </div>
                </div>';
    ...
    $tab_html .= '<div class="title-h-left em14">' . esc_html__('绑定账号', 'zib_language') . '</div>';
    $tab_html .= '<a class="muted-color px12" href="#tab-sign-up" data-toggle="tab">' . esc_html__('没有账号？创建新账号', 'zib_language') . '<i class="em12 ml3 fa fa-angle-right"></i></a>';
    $tab_html .= '</div>';
    ...
    $tab_html .= '<div class="box-body">';
    $tab_html .= '<div class="title-h-left em14">' . esc_html__('创建新账号', 'zib_language') . '</div>';
    $tab_html .= '<a class="muted-color px12" href="#tab-sign-in" data-toggle="tab">' . esc_html__('已有账号，立即绑定', 'zib_language') . '<i class="em12 ml3 fa fa-angle-right"></i></a>';
    $tab_html .= '</div>';
...
function zib_oauth_update_user_meta($args, $is_new = false)
{
    ...
    if (!empty($args['name'])) {
        $getUserInfo['nick_name'] = $args['name'];
    }
    ...
    //自定义头像，无则添加
    $custom_avatar = zib_get_user_meta($args['user_id'], 'custom_avatar', true);
    if ($args['avatar'] && !$custom_avatar) {
        zib_update_user_meta($args['user_id'], 'custom_avatar', $args['avatar']);
    }
    ...
    if ($is_new) {
        ...
        if (zib_is_username_judgment($nickname)['error']) {
            //判断用户名是否合法
            $nickname = $args['login_name'] ? str_replace('user', __('用户', 'zib_language'), $args['login_name']) : __('用户', 'zib_language') . mt_rand(1000, 9999) . mt_rand(1000, 9999);
        }
...
function zib_oauth_page_template()
{
    ...
    if ($oauth) {
        if (in_array($oauth, array('clogin', 'agent', 'gitee', 'giteeagent', 'alipay', 'alipayagent', 'baidu', 'baiduagent', 'qq', 'qqagent', 'weixin', 'weixinagent', 'weixingzh', 'weibo', 'weiboagent', 'github', 'githubagent', 'google', 'googleagent', 'facebook', 'facebookagent', 'microsoft', 'microsoftagent', 'twitter', 'twitteragent', 'apple', 'appleagent'))):
            ...
            $template          = get_theme_file_path($template_file);
 ...
 * @return {*}
 */
function zib_oauth_die($msg = null, $t = null)
{

    if (null === $msg) {
        $msg = __('异常错误，请重试', 'zib_language');
    }
    if (null === $t) {
        $t = get_current_user_id() ? __('绑定失败', 'zib_language') : __('登录失败', 'zib_language');
    }
...
function zib_agent_login()
{
    ...
    if (!empty($_REQUEST['agent_back_url']) && !empty($_REQUEST['sign'])) {
        ...
        if ($oauth_agent != 'server') {
            echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('错误：未启用代理登录服务', 'zib_language'))));
            exit();
        }

        $agent_oauth = new \agent\OAuth2($config);
        if (!$agent_oauth->verifySign()) {
            echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('错误：代理登录签名错误或密钥错误', 'zib_language'))));
            exit();
        }
