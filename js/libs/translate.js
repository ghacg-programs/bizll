...
if (typeof translate == 'object' && typeof translate.version == 'string') {
    throw new Error('translate.js 已经加载');
}
var translate = {
    /**
     * 当前的版本
		...
		*/
        set: function (data) {
            ...
            if (typeof data.visual.webPageLoadTranslateBeforeHiddenText.use === 'boolean' && data.visual.webPageLoadTranslateBeforeHiddenText.use === true) {
                if (translate.visual.webPageLoadTranslateBeforeHiddenText_use === false) {
                    //没有启动，才会启动
        ...
        input: {
			...
			*/
            start: function () {
                ...
                // 1. 保存原生value属性描述符
                translate.listener.input.originalValueDescriptor = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value');
			...
			*/
            reset: function () {
                ...
                Object.defineProperty(HTMLInputElement.prototype, 'value', translate.listener.input.originalValueDescriptor);
        ...
        rectsSpaceEliminateOverlap: function (inputRects) {
            ...
            for (let i = 0; i < n; i++) {
                ...
                for (let j = i + 1; j < n; j++) {
                    ...
                    // ---- 优先检测包含（几何方式，带阈值）
                    const A_contains_B = Ai.left <= Bj.left + pixelThreshold && Ai.top <= Bj.top + pixelThreshold && Ai.right >= Bj.right - pixelThreshold && Ai.bottom >= Bj.bottom - pixelThreshold;

                    const B_contains_A = Bj.left <= Ai.left + pixelThreshold && Bj.top <= Ai.top + pixelThreshold && Bj.right >= Ai.right - pixelThreshold && Bj.bottom >= Ai.bottom - pixelThreshold;
		...
		*/
        ...
        afterAddSpace: function (adjacentPairs) {
            ...
            adjacentPairs.forEach((pair) => {
                ...
                // 获取计算样式
                const beforeStyle = window.getComputedStyle(beforeNode.nodeType === Node.TEXT_NODE ? beforeNode.parentElement : beforeNode);

                const afterStyle = window.getComputedStyle(afterNode.nodeType === Node.TEXT_NODE ? afterNode.parentElement : afterNode);

                // 检查间距是否由CSS属性引起
                const hasRightSpacing = parseFloat(beforeStyle.marginRight) > 0 || parseFloat(beforeStyle.paddingRight) > 0;

                const hasLeftSpacing = parseFloat(afterStyle.marginLeft) > 0 || parseFloat(afterStyle.paddingLeft) > 0;
