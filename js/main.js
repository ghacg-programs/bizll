...
/** 检测 AJAX 响应是否含 WordPress 致命错误 */
function zib_is_fatal_error_response(text) {
    return text && (text.indexOf('Fatal error') > -1 || text.indexOf('致命错误') > -1);
}

/**
 * 简单 sprintf：key 对应带 %s、%1$s 占位符的文案
 */
function zib_sprintf(key) {
...
$.fn.countdown = function () {
    ...
    $(this).each(function () {
        ...
        _this.attr('title', zib__('countdown_end_time') + endtime);
        ...
        function updateClock() {
            ...
            if (_this.find('.second').length) {
                ...
                var html = '';
                if (t.days > 0) {
                    html += '<span class="day">' + t.days + '</span><span class="day-unit">' + zib__('countdown_day') + '</span>';
                }
                if (t.hours > 0 || t.days > 0) {
                    html += '<span class="hour">' + t.hours + '</span><span class="hour-unit">' + zib__('countdown_hour') + '</span>';
                }
                if (t.minutes > 0 || t.hours > 0 || t.days > 0) {
                    html += '<span class="minute">' + t.minutes + '</span><span class="minute-unit">' + zib__('countdown_minute') + '</span>';
                }

                html += '<span class="second">' + t.seconds + '</span><span class="second-unit">' + zib__('countdown_second') + '</span>';
                _this.html(html);
            }

            if (t.total <= 0) {
                clearInterval(timeinterval);
                var over_text = _this.attr('data-over-text') || zib__('countdown_ended');
...
function post_ajax(_this, con, jcon, item, loader, pag, next, trigger, replace, nomore, top, ajax_trigger) {
    ...
    jcon = jcon || con;
    ajax_trigger = ajax_trigger || (_win.ajax_trigger ? _win.ajax_trigger : '<i class="fa fa-arrow-right"></i>' + zib__('load_more'));
    replace = _this.attr('ajax-replace') || replace;
    nomore = nomore || _win.ajax_nomore;

    var $loader = find('.post_ajax_loader');
    if (!$loader.length) {
    ...
    //3级目录find
    function find(selector, $parent, con_selector) {
        $parent = $parent && $parent.length ? $parent : $con;
        con_selector = con_selector || con;
        ...
        //二级
        $element = $parent.find('>*:not(' + con_selector + ')>' + selector);
        if ($element.length) {
            return $element;
        }

        //三级
        $element = $parent.find('>*:not(' + con_selector + ')>*:not(' + con_selector + ')>' + selector);
        if ($element.length) {
            return $element;
        }

        //四级
        $element = $parent.find('>*:not(' + con_selector + ')>*:not(' + con_selector + ')>*:not(' + con_selector + ')>' + selector);
        if ($element.length) {
            return $element;
        }
    ...
    href &&
        $.ajax({
            ...
            dataType: 'html',
            beforeSend: function () {
                if (replace && $item.length) {
                    $item.remove();
                }

                $con.attr('ing', true);

                find('.post_ajax_trigger').remove();
                find('.no-more').remove();
                find('.post_ajax_loader').remove();
                find('.noajax-pag').remove();
                find(pag).remove();

                $item = find(item); //必须重新获取列表，因为可能已经删除了
                if ($item.length) {
            ...
            success: function (a) {
                ...
                var c_c = find(item, $jcon, jcon); //列表
                var c_p = find(pag, $jcon, jcon); //下一页
                ...
                //添加头部
                var $item_header = find('.ajax-item-header', $jcon, jcon);
                if ($item_header.length) {
                    find('.ajax-item-header').remove(); //删除列表头部内容
                ...
                if ($item.length) {
                    ...
                    if (top) {
                        //追加方向为顶部
                        $item.first().before(c_p).before(c_c);
                    } else {
                        //追加方向为底部
                        $item.last().after(c_p).after(c_c);
                    }
                } else {
                    //原本无列表则添加
                    if (top) {
                ...
                $con.removeAttr('ing').trigger('post_ajax.ed', $(a));
                find('.post_ajax_loader', $con).hide();
...
_win.bd.on('click', '[ajax-tab],[ajax-target]', function () {
    ...
    console.log(target);
    $(target).find('.post_ajax_trigger').remove();
...
_win.bd.on('click', '.ajax-next', function (e) {
    ...
    var _loader = '<div class="text-center muted-2-color mt20"><i class="loading mr10"></i>' + zib__('loading') + '</div>';
...
_win.bd.on('click', '.oauth-untying', function () {
    if (confirm(zib__('confirm_unbind')) == true) {
        var _this = $(this),
            data = {
...
_win.bd.on('click', '.medal-add-link', function () {
    if (confirm(zib__('confirm_grant_badge')) == true) {
...
_win.bd.on('click', '.medal-remove-link', function () {
    if (window.confirm(zib__('confirm_revoke_badge'))) {
        zib_ajax($(this));
        return false;
    }
});
...
_win.bd.on('click', '.toggle-theme', function (event) {
    ...
    function toggle() {
        ...
        $('img[white-src],img[dark-src]').each(function () {
            var _this = $(this);
            var _s_src = isDark ? _this.attr('white-src') : _this.attr('dark-src');

            if (!_s_src) {
                return;
            }
            _this.attr('src', _s_src);
        });
...
_win.bd.on('click', '[data-action]', function () {
    ...
    if (!_win.is_signin) {
        var t = lcs.get(_type) || '';
        if (-1 !== t.indexOf(',' + s + ',')) return notyf(zib__(type == 'like' ? 'like_already_post' : 'like_already_comment'), 'warning');
    ...
    action_ajax(_this, data, s, type, zib__('like_thanks'));
});
...
_win.bd.on('click', '.qrcode-signin', function () {
    ...
    $.post(
        ...
        null,
        function (n) {
            n || notyf(zib__('qrcode_failed'), 'danger');
    ...
    if (multiple > 1) {
        ...
        if (is_active) {
            ...
            if (Object.keys(ii).length >= multiple) {
                return notyf(zib_sprintf('select_max', multiple), 'danger');
            }
            //不存在-添加
...
_win.bd.on(
    ...
    debounce(function () {
        ...
        if (_this.val().length < 1) {
            ...
            text = _this.attr('warning-max') || zib__('input_max');
            _warning.html(text.replace('1$', max)).fadeIn(150);
        } else if (val < min) {
            text = _this.attr('warning-min') || zib__('input_min');
...
$.fn.AutoSearch = function () {
    return this.each(function () {
        ...
        var min_length_remind = zib_sprintf('search_min_chars', min_length);
        var loading_icon = '<i class="loading em12"></i>';
        var loading_remind = loading_icon + '<span class="ml6">' + zib__('search_loading') + '</span>';
        var is_on = 'is-on';
...
_win.bd.on('click', '.trash-history-search', function () {
    if (confirm(zib__('confirm_clear_search')) == true) {
        $('.history-search')
            .slideUp()
            .delay(1000, function () {
                $(this).remove();
...
function zib_ajax(_this, data, success, noty, no_loading) {
    ...
    if (window.captcha) {
        if (window.captcha._this) {
            delete window.captcha._this;
        }
    ...
    var _loading = no_loading ? _text : '<i class="loading mr6"></i><text>' + zib__('please_wait') + '</text>';
    noty != 'stop' && notyf(noty || zib__('processing'), 'load', '', 'wp_ajax');
    ...
    $.ajax({
        ...
        success: function (n) {
            ...
            if (noty != 'stop') {
                notyf(n.msg || zib__('done'), ys, '', 'wp_ajax');
            } else if (n.msg) {
                notyf(n.msg, ys);
            }
...
function posts_limit_height() {
    if ($('.limit-height').length) {
        ...
        if (~~r_h >= ~~r_m + 79) {
            var nn = '<div class="read-more"><a href="javascript:;" class="read-more-open">' + zib__('read_more') + '<i class="fa ml10 fa-angle-down"></i></a></div>';
 ...
 *
 * 用法：class带有obs-animate ani-fade的元素，当滚动到视口时，会自动触发此动画
 *
 * class带有 obs-animate-repeat 的元素或者 data-animation-repeat="true" 的元素，当滚动到视口时，会重复触发此动画
 */
(function () {
    ...
    var REPEAT_CLS = 'obs-animate-repeat';
    ...
    function execute(entries) {
        ...
        var isIntersecting_count = 0;
        for (var j = 0; j < entries.length; j++) {
            if (entries[j].isIntersecting) {
                isIntersecting_count++;
            }
        }
        ...
        for (var i = 0; i < entries.length; i++) {
            ...
            var is_repeat = el.hasAttribute('data-animation-repeat') || el.classList.contains(REPEAT_CLS);
            if (entry.isIntersecting) {
                var delay = c_i * 0.11;
                if (isIntersecting_count > 8) {
                    delay = delay / (isIntersecting_count / 8);
                }
    ...
    function removeAnimation(el) {
        ...
        $(el)
            .removeClass(BASE_CLS + ' ' + SELECTOR_class + ' ' + animation_class + ' ' + REPEAT_CLS)
            .removeAttr('data-animation')
            .removeAttr('data-animation-repeat')
...
function number_stats_init() {
    function animateCount(el, duration) {
        ...
        var _duration = ~~el.getAttribute('data-duration');
        duration = _duration > 10 ? _duration : duration;
    ...
    var wraps = document.querySelectorAll('.stat-number-wrap');
    if (!wraps.length) {
        return;
    }
    ...
    if (!('IntersectionObserver' in window)) {
        wraps.forEach(function (wrap) {
            wrap.querySelectorAll('.stat-number').forEach(function (el) {
...
_win.bd.on('click', '.produck-wid-feature-intro .fi-tab', function () {
    var _this = $(this);
    ...
    var panels = _this.closest('.produck-wid-feature-intro').find('.fi-panel');
    panels.removeClass('is-active');
    panels.eq(_this.index()).addClass('is-active');
});

//3D交互展示
function tilt_cases_init() {
    ...
    function setTranslate3d(imgs, translate3d) {
        //先获取原有的transform
        var transform = imgs[0].style.transform;
        if (transform.includes('translate3d')) {
            //移出translate*()空格
            transform = transform.replace(/translate3d\(.*?\)/g, '').trim();
        }

        transform = transform + ' ' + translate3d;
        imgs.css('transform', transform);
    }

    tbquire(['tilt'], function () {
        $('.vanilla-tilt:not(.is-initialized)').each(function () {
            var _this = $(this);
            var imgs = _this.find('img');

            var maxTilt = parseFloat(_this.attr('data-tilt-max')) || 10;
            var n = imgs.length;
            var gain = 1.22;

            var hoverZone = _this.closest('.tilt-cases-container') || _this;
            var mes = _this.data('tilt-mouse-event-element');

            if (mes) {
                _this.removeAttr('data-tilt-mouse-event-element');
                var elZone = _this.closest(mes);
                hoverZone = elZone.length ? elZone : hoverZone;
            }

            var opt = {};
            if (hoverZone.length) {
                opt['mouse-event-element'] = hoverZone[0];
            }

            _this.addClass('is-initialized');
            VanillaTilt.init(this, opt);
            if (n <= 1) {
                return;
            }

            // 图片视差
            this.addEventListener('tiltChange', function (e) {
                var detail = e.detail || {};
                var tx = parseFloat(detail.tiltX) || 0;
                var ty = parseFloat(detail.tiltY) || 0;
                imgs.each(function (i) {
                    var $img = $(this);
                    var base = parseFloat($img.attr('data-tilt-depth'));
                    if (isNaN(base)) {
                        base = 20;
                    }
                    if (base === 0) {
                        setTranslate3d($img, 'translate3d(0,0,0)');
                        return;
                    }

                    // 单图：仅略加强；多图：从第一层到最后一层系数约 0.48x～1.62x，拉开前后景
                    var layerMul = 1;
                    if (n > 1) {
                        layerMul = 0.48 + (i / (n - 1)) * 1.14;
                    }

                    var effective = base * gain * layerMul;
                    var dx = (tx / maxTilt) * effective * -1;
                    var dy = (ty / maxTilt) * effective;

                    setTranslate3d($img, 'translate3d(' + dx + 'px, ' + dy + 'px, 0)');
                });
            });

            // 鼠标离开与 vanilla-tilt 相同热区时复位
            if (hoverZone) {
                hoverZone.on('mouseleave', function () {
                    imgs.each(function () {
                        setTranslate3d($(this), 'translate3d(0,0,0)');
                    });
                });
            }
        });
...
//小工具预览模式
_win.bd.on('click', '.customize-preview-widget-edit', function (e) {
    if (!wp || !wp.customize || !wp.customize.widgetsPreview) {
        return;
    }

    var _this = $(this);
    var box = _this.closest('.customize-preview-widget-box');

    e.preventDefault();
    wp.customize.widgetsPreview.preview.send('focus-widget-control', box.prop('id'));
});
