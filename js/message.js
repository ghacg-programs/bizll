 ...
 * @LastEditTime : 2026-06-09 13:58:19
...
_win.bd.on('click', '.send-private', function () {
    ...
    zib_ajax(_this, 0, function (n) {
        if (n.html) {
            var _par = _this.parents('.private-window');
...
_win.bd.on('click', '.private-next', function () {
    ...
    post_ajax($(this).attr('no-scroll', true), '.private-window-content', '.private-window-content', '.private-item', _loader, '.private-pag', '.private-next', 'private-next px12', !1, '<span class="px12">' + zib__('message_all_loaded') + '</span>', !0, '<i class="fa fa-angle-up mr10"></i>' + zib__('message_load_more'));
    return !1;
});

/*对话列表翻页 */
_win.bd.on('click', '.chat-next', function () {
    post_ajax($(this), '#user_chat_lists', '#user_chat_lists', '.chat-lists', '', '.chat-pag', '.chat-next');
    return !1;
});
...
_win.bd.on('click', '.ajax-blacklist,.ajax-clear-msg,.ajax-readed', function () {
    ...
    if (confirm(zib_sprintf('confirm_action', $text)) == 1) {
        zib_ajax(_this, 0, function (n) {
            if (n.text) {
