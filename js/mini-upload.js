 ...
 * @LastEditTime : 2026-05-05 23:27:54
...
function miniupload() {
    ...
    _body.on('change', input_selector, function (e) {
        ...
        $.each(files, function (key, val) {
            ...
            if (accept_yanz && -1 == val.type.indexOf(accept_yanz)) {
                //如果限制文件格式
                notyf(zib_sprintf('upload_format_error', name), 'danger'), _this.val('');
            } else if (size_max && val.size > size_max * 1048567) {
                //限制文件大小
                notyf(zib_sprintf('upload_size_exceeded', name, size_max), 'danger');
        ...
        $.each(mini_file[upload_id], function (key, val) {
            if (multiple_max && multiple_max < ii) {
                //限制文件数量
                notyf(zib_sprintf('upload_count_exceeded', multiple_max), 'danger');
                return false;
            } else {
                ii++;
                a_file.push(val);
    ...
    _body.on('click', submit_selector, function (e) {
        ...
        //必须选择图片判断
        if (!is_files) return notyf(zib__('upload_select_first'), 'danger');
        ...
        //人机验证
        if (formData.get('captcha_mode') && is_captcha(formData.get('captcha_mode'))) {
            tbquire(['captcha'], function () {
                CaptchaOpen(_this, formData.get('captcha_mode'));
            });
            return !1;
        }
        ...
        if (window.captcha) {
            ...
            formData.append('captcha', JSON.stringify(window.captcha));
        ...
        notyf(zib__('processing_wait'), 'load', '', notyf_id);
        _this.attr('disabled', true).html('<span class="miniupload-ing"><i class="loading mr3 c-white"></i><span class="loading-text c-white">' + zib__('uploading') + '<count class="px12 ml3"></count></span><div class="progress progress-striped active"><div class="progress-bar progress-bar-success" role="progressbar" style="width:0;"></div></div></span>');
        ...
        $.ajax({
            ...
            xhr: jqXhr(function (e) {
                ...
                percent >= 100 && _this.find('.loading-text').html(zib__('upload_processing') + ' ...');
            ...
            success: function (n) {
                ...
                notyf(n.msg || zib__('operation_success'), ys, '', notyf_id);
    ...
    //图片转Base64
    function ToB64(e, n) {
        if ('undefined' == typeof FileReader) return notyf(zib__('upload_unsupported'), 'danger');
