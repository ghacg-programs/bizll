 ...
 * @LastEditTime : 2026-06-18 12:30:18
...
var $author = $('#comment-user-info');
var $submit = $('#commentform #submit');
var $com_ajax_url = _win.ajax_url;
...
_win.bd.on('click', '.comment-trash-link', function () {
    ...
    if (confirm(zib__('comment_confirm_delete')) == 1) {
        zib_ajax(
            _this,
            trash_data,
            function (n) {
...
_win.bd.on('click', '.comment-approve-link', function () {
    ...
    zib_ajax(_this, trash_data, function (n) {
        if (!n.error) {
            var _text = n.status == 'hold' ? zib__('comment_approve') : zib__('comment_reject');
            var _class = n.status == 'hold' ? 'approve' : 'unapprove';
            var _badg = n.status == 'hold' ? '<span class="badg c-red badg-sm">' + zib__('comment_pending_review') + '</span>' : '';

            if (n.status == 'hold') {
                reply.remove();
            } else {
                reply.length || _this.parents('.dropdown').before('<span class="reply-link"><a class="comment-reply-link" data-commentid="' + commentid + '" href="javascript:;">' + zib__('comment_reply') + '</a></span>');
            }
...
_win.bd.on('click', '#commentform #submit', function () {
    ...
    if ($author.length && $author.attr('require_name_email')) {
        if (inputs.author.length < 2 || inputs.email.length < 4) {
            return notyf(zib__('comment_enter_name_email'), 'warning'), $author.addClass('open').find('[name="author"]').focus(), !1;
        }
        if (!is_mail(inputs.email)) {
            return notyf(zib__('comment_email_invalid'), 'warning'), $author.addClass('open').find('[name="email"]').focus(), !1;
        }
    }
    if (inputs.comment.length < 4) {
        return notyf(zib__('comment_too_short'), 'warning'), $('#comment').focus(), !1;
    }
            ...
            if (inputs.edit_comment_ID) {
                ...
                if ($('[reply-show]').length && n.comment) {
                    if (~~n.comment.comment_approved > 0) {
                        ...
                        setTimeout(function () {
                            notyf(zib__('comment_hidden_after_review'), 'warning');
                        }, 200);
                    }
                }
...
var addComment = {
    moveForm: function (_this, commId, parentId, respondId, postId, num) {
        ...
        if (num) {
            ...
            action_text_r.html(zib__('comment_edit_content'));
            t.I('comment').value = ''; //清空内容
            $('#comment_parent').attr('name', 'edit_comment_ID');
            $submit.attr('disabled', true).html('<i class="loading mr6"></i><span>' + zib__('loading') + '</span>');
            $('#comment').attr('disabled', true).val(zib__('comment_fetching_content'));
            $('#' + commId)
                .find('.comment-footer')
                .hide();
            var data = {
