 ...
 * @LastEditTime : 2026-07-02 11:32:11
...
_win.bd.on('click', '.dropdown-code [type="submit"]', function () {
    ...
    if (val.length < 2) return void notyf(zib__('enter_code'), 'warning');
...
_win.bd.on('click', '.dropdown-image [type="submit"]', function () {
    ...
    if (val.length < 6) return void notyf(zib__('enter_valid_image_url'), 'warning');
    val = '[img=' + val + ']\n';
    return grin(_this, val);
});
...
//编辑框添加链接
_win.bd.on('click', '.dropdown-link [type="submit"]', function () {
    var _this = $(this);
    var val = _this.closest('.dropdown-link').find('textarea').val();
    if (val.length < 6) return void notyf(zib__('enter_valid_link_url'), 'warning');

    //判断URL合法性
    if (!zib_is_url(val)) {
        return void notyf(zib__('enter_valid_link_url'), 'warning');
    }

    var target = '';
    if (_this.closest('.dropdown-link').find('input[name="target"]').is(':checked')) {
        target = 'target=_blank';
    }

    var text = _this.closest('.dropdown-link').find('.link-text').val() || val;

    var link_val = '[link=' + val + ' ' + target + ']' + text + '[/link]';

    return grin(_this, link_val);
});
...
_win.bd.on('click', '.quick-reply-item', function () {
    ...
    _this
        ...
        .append('<div class="' + quick_reply_action + '"><div class="modal-buts but-average"><button type="button" class="but quick-reply-btn-insert" href="javascript:;">' + zib__('insert') + '</button><button type="button" class="but c-blue padding-lg quick-reply-btn-send">' + zib__('send') + '</button></div></div>');
});
