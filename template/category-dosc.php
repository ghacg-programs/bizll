...
function zib_dosc_cat_cover($cat_id = '')
{
    ...
    if (is_super_admin() && !$desc) {
        $desc = __('请在Wordress后台-文章-文章分类中添加分类描述！', 'zib_language');
    }

    $desc .= zib_get_term_admin_edit(__('编辑此分类', 'zib_language'));
    ...
    <div class="dosc-search">
        <div class="search-input">
            <form method="get" class="line-form" action="<?php echo esc_url(home_url('/')); ?>">
                ...
                <input type="text" name="s" class="line-form-input" tabindex="2" placeholder="<?php echo esc_attr(sprintf(__('搜索%s', 'zib_language'), $cat_obj->cat_name)); ?>">
