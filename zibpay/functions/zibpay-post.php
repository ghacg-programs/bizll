 ...
 * @LastEditTime : 2026-05-25 17:51:07
 ...
 * @return {*}
 */
function zibpay_get_post_cashier_link($post_id = 0, $class = 'but jb-red', $con = null, $query_arg = array())
{
    if (null === $con) {
        $con = __('立即购买', 'zib_language');
    }
...
function zibpay_pay_points_cashier_modal($post_id = 0, $price_type = '')
{
    ...
    $con .= $pay_limit > 0 ? '' : '<div class="flex jsb ab"><span class="muted-2-color">' . __('价格', 'zib_language') . '</span><div><span>' . $mark . '<span class="em14">' . $price . '</span></span></div></div>';
    $con .= $vip_price ? '<div class="flex jsb ab"><span class="muted-2-color">' . __('会员价', 'zib_language') . zibpay_get_vip_icon($vip_level, 'ml3') . '</span><span class="c-red">' . $mark . '<span class="em14">' . $pay_price . '</span></span></div>' : '';
    $con .= '</div>';
    ...
    $form .= '<button class="but jb-yellow padding-lg btn-block radius wp-ajax-submit mt10" >' . __('立即支付', 'zib_language') . '<span class="ml6 px12">' . zibpay_get_points_mark() . '</span>' . $pay_price . '</button>';
    $form .= '</form>';
    ...
    $header = zib_get_modal_colorful_header('jb-blue', '<i class="fa fa-cart-plus"></i>', __('确认购买', 'zib_language'));
...
function zibpay_pay_cashier_modal($post_id = 0, $price_type = '')
{
    ...
    $con = '<div class="mb10 order-type-' . $pay_type . '"><span class="pay-tag badg badg-sm mr6">' . $order_type_name . '</span><span>' . $pay_title . '</span></div>';
    $con .= '<div class="mb10 muted-box padding-h6 line-16">';
    $con .= $pay_limit > 0 ? '' : '<div class="flex jsb ab"><span class="muted-2-color">' . __('价格', 'zib_language') . '</span><div>' . $original_price . '<span>' . $mark . '<span class="em14">' . $price . '</span></span></div></div>';
    $con .= $vip_price ? '<div class="flex jsb ab"><span class="muted-2-color">' . __('会员价', 'zib_language') . zibpay_get_vip_icon($vip_level, 'ml3') . '</span><span>' . $mark . '<span class="em14">' . $vip_price . '</span></span></div>' : '';
    $con .= $rebate_discount ? '<div class="flex jsb ab"><span class="muted-2-color">' . __('推广折扣', 'zib_language') . '</span><div><span>' . '<span class="em14">-' . $rebate_discount . '</span></span></div></div>' : '';
    $con .= $rebate_discount || $vip_price ? '<div class="flex jsb ab"><span class="muted-2-color">' . __('支付金额', 'zib_language') . '</span><div class="font-bold c-red"><span>' . $mark . '<span class="em14">' . ($pay_price - $rebate_discount) . '</span></span></div></div>' : '';
    $con .= '</div>';
...
function zibpay_get_demo_link($pay_mate = array(), $post_id = 0, $class = 'but c-yellow padding-lg btn-block em09')
{
    ...
    $text   = !empty($pay_mate['demo_link']['text']) ? $pay_mate['demo_link']['text'] : __('查看演示', 'zib_language');
...
function zibpay_get_sales_volume($pay_mate = array(), $post_id = 0)
{
    ...
    $cuont = zibDB::name(zibpay::$order_table_name)->where('post_id', $post_id)->where('status', 1)->count();
...
function zibpay_posts_free_logged_show_box($pay_mate, $post_id = '')
{
    ...
    $order_type_name = str_replace(__('付费', 'zib_language'), __('免费', 'zib_language'), $order_type_name);

    $cuont        = '';
    $cuont_volume = zibpay_get_sales_volume($pay_mate, $post_id);
    if (_pz('pay_show_paycount', true) && $cuont_volume) {
        $cuont = '<badge class="img-badge hot jb-blue px12">' . sprintf(__('已售 %s', 'zib_language'), $cuont_volume) . '</badge>';
    }
    $mark = zibpay_get_pay_mark();
    ...
    if (5 != $pay_mate['pay_type']) {
        if (6 == $pay_mate['pay_type']) {
            ...
            $video_pic      = !empty($pay_mate['video_pic']) ? '<img ' . $lazy_attr . 'src="' . esc_attr($pay_mate['video_pic']) . '" alt="' . esc_attr(sprintf(__('付费视频-%s', 'zib_language'), $pay_title)) . '">' : zib_post_thumbnail();
        ...
        $product_graphic = str_replace(__('请付费后', 'zib_language'), __('请登录后', 'zib_language'), $product_graphic);
    }
    ...
    if (zib_is_close_sign()) {
        ...
        $button  = '<div class=""><span class="badg padding-lg btn-block c-red em09"><i class="fa fa-info-circle mr10"></i>' . __('登录功能已关闭，暂时无法查看', 'zib_language') . '</span></div>';
    } else {
        $pay_doc = !empty($pay_mate['pay_doc']) ? $pay_mate['pay_doc'] : sprintf(__('此内容为%s，请登录后查看', 'zib_language'), str_replace(__('付费', 'zib_language'), __('免费', 'zib_language'), zibpay_get_pay_type_name($pay_mate['pay_type'])));
        $button  = '<div class=""><a href="javascript:;" class="but signin-loader padding-lg btn-block jb-blue"><i class="fa fa-sign-in"></i> ' . __('登录查看', 'zib_language') . '</a></div>';
    }
...
function zibpay_posts_paid_box($pay_mate, $paid, $post_id = '')
{
    ...
    switch ($pay_type) {
        ...
        case 2:
            ...
            if (_pz('pay_type_option', 0, 'down_alone_page')) {
                ...
                $down_link = '<a target="_blank" href="' . add_query_arg('post', $post_id, zib_get_template_page_url('pages/download.php')) . '" class="but jb-blue padding-lg btn-block"><i class="fa fa-download fa-fw" aria-hidden="true"></i>' . __('资源下载', 'zib_language') . '</a>';
                ...
                $down_box  = '<div class="hidden-box show"><div class="hidden-text"><i class="fa fa-download mr6" aria-hidden="true"></i>' . __('资源下载', 'zib_language') . '</div>' . $down_buts . '</div>';
        ...
        case 5:
            ...
            if ($pay_gallery) {
                ...
                $header_box .= '<div class="absolute blur-10 opacity3"><img ' . $lazy_attr . 'src="' . esc_attr($attachment) . '" alt="' . esc_attr(sprintf(__('付费图片-%s', 'zib_language'), $pay_title)) . '"></div>';
                $header_box .= '<div style="margin-top: -20px;" class="relative mb6"><span class="badg b-theme badg-sm"> ' . sprintf(__('共%s张图片', 'zib_language'), $all_count) . ' </span></div>';
                $header_box .= $gallery;
                $header_box .= '</div>';
            } else {
                $header_box = '<div class="b-red text-center" style="padding: 30px 10px;"><i class="fa fa-fw fa-info-circle mr10"></i>' . sprintf(__('暂无图片内容，%s', 'zib_language'), (is_super_admin() ? __('请在后台添加', 'zib_language') : __('请与管理员联系', 'zib_language'))) . '</div>';
            }
        ...
        case 6:
            ...
            if ($video_url) {
                ...
                if ($episode_array && is_array($episode_array)) {
                    foreach ($episode_array as $episode) {
                        if (!empty($episode['url'])) {
                            $episode_index++;
                            $episode_title = $episode['title'] ? $episode['title'] : sprintf(__('第%s集', 'zib_language'), $episode_index);
                ...
                $episode_html = '';
                if ($episode_lists) {
                    $episode_title = $pay_mate['video_title'] ? $pay_mate['video_title'] : __('第1集', 'zib_language');
                ...
                $header_box = '<div class="b-red text-center" style="padding: 30px 10px;"><i class="fa fa-fw fa-info-circle mr10"></i>' . sprintf(__('暂无视频内容，%s', 'zib_language'), (is_super_admin() ? __('请在后台添加', 'zib_language') : __('请与管理员联系', 'zib_language'))) . '</div>';
            }
            break;
    }
    ...
    if ('free' == $paid_type) {
        ...
        $mark      = zibpay_get_pay_mark();
        $mark      = '<span class="pay-mark">' . $mark . '</span>';
        $paid_info = '<div class="flex jsb"><span>' . __('订单号', 'zib_language') . '</span><span>' . zibpay_get_order_num_link($paid['order_num']) . '</span></div>';
        $paid_info .= '<div class="flex jsb"><span>' . __('支付时间', 'zib_language') . '</span><span>' . $paid['pay_time'] . '</span></div>';
        $paid_info .= '<div class="flex jsb"><span>' . __('支付金额', 'zib_language') . '</span><span>' . zibpay_get_order_pay_price($paid) . '</span></div>';

        //到期时间
        $order_expired_time = $paid['order_expired_time'] ?? 0;
        $order_expired_html = '';
        if ($paid['paid_type'] == 'paid' && $order_expired_time) {
            $order_expired_html = '<div class="flex jsb"><span>' . __('订单有效期', 'zib_language') . '</span><span data-toggle="tooltip" title="' . esc_attr(sprintf(__('当前订单有效期截止到%s，到期后需重新购买', 'zib_language'), $order_expired_time)) . '">' . $order_expired_time . '<i class="fa fa-question-circle-o ml6"></i></span></div>';
        }
...
function zibpay_posts_pay_box($pay_mate = array(), $post_id = 0)
{
    ...
    //简介
    $pay_doc = !empty($pay_mate['pay_doc']) ? $pay_mate['pay_doc'] : sprintf(__('此内容为%s，请付费后查看', 'zib_language'), zibpay_get_pay_type_name($pay_mate['pay_type']));
    ...
    if (5 != $pay_mate['pay_type']) {
        if (6 == $pay_mate['pay_type']) {
            ...
            if (is_array($episode_array)) {
                ...
                if ($episode_count > 1) {
                    $pay_title = '<span class="badg badg-sm b-theme mr6"><i class="fa fa-play-circle mr3" aria-hidden="true"></i>' . sprintf(__('共%s集', 'zib_language'), $episode_count) . '</span>' . $pay_title;
                }
            }
        } else {
            $post_thumbnail = zib_post_thumbnail();
...
function zibpay_get_posts_pay_gallery_box_image_slide($id, $i, $all_count, $pay_title = '', $attachment = '')
{
    ...
    $slide .= '<img ' . $lazy_attr . 'src="' . esc_attr($attachment) . '" data-full-url="' . esc_attr($attachment_full) . '" alt="' . esc_attr(sprintf(__('付费图片-%s', 'zib_language'), $pay_title)) . '">';
    ...
    if (!empty($pay_mate['pay_gallery'])) {
        ...
        if ($show >= 1 && $all_count > $show) {
            ...
            if ($i <= $all_count) {
                ...
                $slide .= '<div class="swiper-slide mr10" style="width: 150px;">';
                $slide .= '<div class="graphic" style="padding-bottom: 100%!important;">';
                $slide .= '<img ' . $lazy_attr . 'src="' . esc_attr($attachment) . '" alt="' . esc_attr(sprintf(__('付费图片-%s', 'zib_language'), $pay_title)) . '">';
                $slide .= '<div class="absolute graphic-mask" style="opacity: 0.3;"></div>';
                $slide .= '<div class="abs-center text-center">' . sprintf(__('请付费后查看', 'zib_language') . '<br>' . __('剩余%s张图片', 'zib_language'), ($all_count - $i + 1)) . '</div>';
                $slide .= '</div>';
                $slide .= '</div>';
            }
            ...
            $product_graphic .= '<div class="abs-center text-center">' . sprintf(__('共%s张图片', 'zib_language') . '<br>' . __('请付费后查看', 'zib_language'), $all_count) . '</div>';
        ...
        $product_graphic .= '<div class="abs-center text-center">' . (is_super_admin() ? __('暂无图片，请在后台添加付费图片', 'zib_language') : __('暂无可查看图片，请与站长联系', 'zib_language')) . '</div>';
        $product_graphic .= $discount_tag ? '<div class="padding-6 jb-red px12 abs-center text-center left-bottom">' . $discount_tag . '</div>' : '';
        $product_graphic .= '</div></div>';
    }
...
function zibpay_get_show_price($pay_mate = array(), $post_id = 0, $class = 'px13')
{
    ...
    //价格
    $original_price = !empty($pay_mate['pay_original_price']) && round((float) $pay_mate['pay_original_price'], 2) > round((float) $pay_mate['pay_price'], 2) ? '<span class="original-price" title="' . esc_attr(sprintf(__('原价 %s', 'zib_language'), round((float) $pay_mate['pay_original_price'], 2))) . '">' . $mark . round((float) $pay_mate['pay_original_price'], 2) . '</span>' : '';
    //促销标签
    ...
    if ($pay_limit > 0 && (_pz('pay_user_vip_1_s', true) || _pz('pay_user_vip_2_s', true))) {
        $title = array(
            '1' => sprintf(__('%s及以上会员可购买', 'zib_language'), _pz('pay_user_vip_1_name')),
            '2' => sprintf(__('仅%s可购买', 'zib_language'), _pz('pay_user_vip_2_name')),
        );
        $vip_icon = zib_get_svg('vip_' . $pay_limit, '0 0 1024 1024', 'mr3');

        $price = '<div class="' . $class . ' padding-h10"><b data-toggle="tooltip" title="' . esc_attr($title[$pay_limit]) . '" class="badg radius jb-vip' . $pay_limit . '" style="padding: 5px 20px;">' . $vip_icon . __('会员专属资源', 'zib_language') . '</b></div>';
    } elseif (zibpay_post_is_points_modo($pay_mate)) {
        //积分
        $mark = zibpay_get_points_mark();

        $price = '<div class="c-yellow ' . $class . '"><span class="em12 mr3">' . $mark . '</span><b class="em3x">' . $pay_mate['points_price'] . '</b><span class="px12 ml3">' . __('积分', 'zib_language') . '</span></div>';
    }
...
function zibpay_get_pay_form_but($pay_mate = array(), $post_id = 0)
{
    ...
    if (!$user_id) {
        if (!_pz('pay_no_logged_in', true) || '0' != $pay_limit) {
            if (zib_is_close_sign()) {
                $pay_button = '<span class="badg px12 c-yellow">' . __('登录功能已关闭，暂时无法购买', 'zib_language') . '</span>';
            } else {
                //登录按钮
                $pay_button = '<a href="javascript:;" class="but jb-blue signin-loader padding-lg"><i class="fa fa-sign-in mr10" aria-hidden="true"></i>' . __('登录购买', 'zib_language') . '</a>';
            }
        } else {
    ...
    if ($pay_limit > 0 && (_pz('pay_user_vip_1_s', true) || _pz('pay_user_vip_2_s', true))) {
        ...
        if (!$user_vip_level) {
            ...
            $pay_vip_text = sprintf(__('升级%s', 'zib_language'), _pz('pay_user_vip_' . $pay_limit . '_name'));
        }

        if (!$user_vip_level || $user_vip_level < $pay_limit) {
            $pay_button = '<div class="badg c-yellow em09 mb6"><i class="fa fa-fw fa-info-circle fa-fw mr6" aria-hidden="true"></i>' . sprintf(__('您暂无购买权限，请先%s', 'zib_language'), $pay_vip_text) . '</div>';
...
function zibpay_get_posts_vip_price($pay_mate, $hide = 0)
{
    ...
    for ($vi = 1; $vi <= 2; $vi++) {
        ...
        $vip_price = $vip_price ? $mark . $vip_price : __('免费', 'zib_language');
        ...
        if ($action_class) {
            $vip_price_con[] = '<span href="javascript:;" class="but vip-price ' . $action_class . '" vip-level="' . $vi . '" data-toggle="tooltip" title="' . esc_attr(sprintf(__('开通%s', 'zib_language'), _pz('pay_user_vip_' . $vi . '_name'))) . '">' . $vip_icon . $vip_price . '</span>';
        } else {
