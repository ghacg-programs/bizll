 ...
 * @LastEditTime : 2026-05-05 18:18:11
 ...
 * @return {*}
 */
function zibpay_get_points_pay_link($class = '', $con = null)
{
    if (null === $con) {
        $con = __('购买积分', 'zib_language');
    }
    $user_id = get_current_user_id();
...
function zibpay_update_user_points($user_id, $data)
{
    ...
    $old_value = 0;
    if (!zibpay_user_balance_or_points_save_db('points', $user_id, $data['value'], $old_value)) {
        return false;
    }

    $data['points'] = $old_value + $data['value']; //记录当前
    ...
    zib_update_user_meta($user_id, 'points_record', $new_record);
    return true;
}
...
function zibpay_get_points_pay_modal($user_id)
{
    ...
    $default_pay_price = 0;
    $custom_limit_html = !empty($custom_limit['min']) ? sprintf(__('最低购买%s积分', 'zib_language'), $custom_limit['min']) : '';
    $custom_limit_html .= $custom_limit_html ? '，' : '';
    $custom_limit_html .= !empty($custom_limit['max']) ? sprintf(__('最高购买%s积分', 'zib_language'), $custom_limit['max']) : '';
    ...
    <div class="relative flex ab">
        ...
        <input class="line-form-input em16 key-color" style="padding: 1px;" name="custom" type="number" ' . (!empty($custom_limit['min']) ? ' limit-min="' . $custom_limit['min'] . '"' : '') . (!empty($custom_limit['max']) ? ' limit-max="' . $custom_limit['max'] . '"' : '') . ' warning-max="' . esc_attr__('最高可购买1$积分', 'zib_language') . '" warning-min="' . esc_attr__('最低需购买1$积分', 'zib_language') . '">
        <i class="line-form-line"></i>
    </div>
    <div class="muted-2-color em09 mt6">' . $custom_limit_html . '</div></div>';
    $header = '<div class="mb10 touch"><button class="close" data-dismiss="modal">' . zib_get_svg('close', null, 'ic-close') . '</button><b class="modal-title flex ac"><span class="mr6 em14">' . zib_get_svg('points-color') . '</span>' . __('购买积分', 'zib_language') . '</b></div>';
    ...
    if ($product_html) {
        $product_html = '<div class="muted-color mb6">' . __('请选择需购买的积分', 'zib_language') . '</div>' . $product_html;
        if ($custom_s) {
            $product_html .= '<div class="muted-color mt20 mb6">' . sprintf(__('自定义积分数量（1%s=%s积分）', 'zib_language'), zibpay_get_currency_unit(), _pz('pay_points_rate')) . '</div>' . $custom_product;
        }
    }

    if (!$product_html) {
        $product_id   = 'custom';
        $product_html = '<div class="muted-color mb6">' . __('请输入需购买的积分数额', 'zib_language') . '</div>' . $custom_product;
    }
...
function zibpay_get_user_points_record_lists($user_id = 0)
{
    ...
    foreach ($record as $k => $v) {
        if (isset($v['value']) && isset($v['points'])) {
            ...
            $lists .= '</div>';
            $lists .= '<div class="flex jsb xx text-right flex0 ml10 ab"><b class="em12 ' . $_class . '">' . ($v['value'] < 0 ? $v['value'] : '+' . $v['value']) . '</b><div class="em09 muted-2-color">' . sprintf(__('积分：%s', 'zib_language'), $v['points']) . '</div></div>';
            $lists .= '</div>';
        }
    }

    if (!$lists) {
        $lists = zib_get_null(__('暂无积分记录', 'zib_language'), 42, 'null-order.svg');
    } else {
        if (count($record) > 49) {
...
function zib_get_points_free_lists($user_id)
{
    ...
    if ($to_day) {
        ...
        $max_desc = $day_max > $to_day ? sprintf(__('每日可免费获取%s积分，超过后将不再获取', 'zib_language'), $day_max) : sprintf(__('今日获取的免费积分已超过%s，将不再获取', 'zib_language'), $day_max);
        $lists .= '<div class="border-bottom padding-h10"><div class="flex jsb ac"><div class="flex1 mr20"><div class="font-bold mb6">' . sprintf(__('今日累计 %1$s', 'zib_language'), '<span class="focus-color">+' . esc_html($to_day) . '</span>') . '</div><div class="muted-2-color em09">' . $max_desc . '</div></div><a class="muted-2-color shrink0" data-toggle="tab" href="#tab_points_date">' . __('每日详情', 'zib_language') . '<i class="fa fa-angle-right ml6 em12"></i></a></div></div>';
    }
...
function zib_get_user_points_add_options($key)
{
    ...
    if ($_options === null) {
        ...
        $_options['checkin'] = array(__('签到奖励', 'zib_language'), 0, '', '');
    }
