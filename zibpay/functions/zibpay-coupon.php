 ...
 * @LastEditTime : 2026-05-13 11:12:34
...
function zibpay_is_coupon_available($coupon_code, $order_type = null, $args_id = 0)
{
    ...
    if (!$coupon) {
        return array('error' => true, 'msg' => __('优惠码不存在或已使用', 'zib_language'));
    }

    if ($args_id && $coupon['post_id'] && $coupon['post_id'] != $args_id) {
        return array('error' => true, 'msg' => __('优惠码不能用于当前商品', 'zib_language'));
    }

    //判断使用次数
    if ($coupon['reuse'] && $coupon['used_count'] >= $coupon['reuse']) {
        return array('error' => true, 'msg' => __('优惠码已达到使用次数上限', 'zib_language'));
    }

    //判断有效期
    if ($coupon['expire_time'] && current_time('timestamp') > strtotime($coupon['expire_time'])) {
        return array('error' => true, 'msg' => __('优惠码已过期', 'zib_language'));
    }
