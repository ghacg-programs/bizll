 ...
 * @LastEditTime : 2026-06-02 22:14:31
...
function zibpay_posts_paid_widget_box($pay_mate = array(), $post_id = 0, $paid = array(), $theme = 'jb-red')
{
    ...
    $get_link_name = array(
        1 => '<i class="fa fa-dot-circle-o fa-fw" aria-hidden="true"></i>' . __('查看内容', 'zib_language'),
        2 => '<i class="fa fa-download fa-fw" aria-hidden="true"></i>' . __('获取下载地址', 'zib_language'),
        5 => '<i class="fa fa-file-image-o fa-fw" aria-hidden="true"></i>' . __('查看图片', 'zib_language'),
        6 => '<i class="fa fa-play-circle fa-fw" aria-hidden="true"></i>' . __('查看视频', 'zib_language'),
    );
    $get_link = '<div class="mt10"><a href="javascript:(scrollTopTo(\'#posts-pay\',-50));" class="but padding-lg btn-block ' . $theme . '">' . (!empty($get_link_name[$pay_mate['pay_type']]) ? $get_link_name[$pay_mate['pay_type']] : '<i class="fa fa-dot-circle-o fa-fw" aria-hidden="true"></i>' . __('查看内容', 'zib_language')) . '</a></div>';
    ...
    if (stristr($paid_type, 'vip')) {
        //价格
        $paid_info = '<div class="flex jsb ab"><span class="em12">' . __('售价：', 'zib_language') . '</span><span>' . zibpay_get_show_price($pay_mate, $post_id, 'text-center') . '</span></div>';
        ...
        if ($paid['paid_type'] == 'paid' && $order_expired_time) {
            $order_expired_html = '<div class="flex jsb"><span>' . __('有效期', 'zib_language') . '</span><span data-toggle="tooltip" title="' . esc_attr(sprintf(__('当前订单有效期截止到%s，到期后需重新购买', 'zib_language'), $order_expired_time)) . '">' . $order_expired_time . '<i class="fa fa-question-circle-o ml6"></i></span></div>';
        }
        ...
        if (zib_is_close_sign()) { //是否开启登录功能
            $get_link = '<div class="mt10"><span class="badg padding-lg btn-block c-red em09"><i class="fa fa-info-circle mr10"></i>' . __('登录功能已关闭，暂时无法查看', 'zib_language') . '</span></div>';
        } else {
            $get_link = '<div class="mt10"><a href="javascript:;" class="but signin-loader padding-lg btn-block jb-blue"><i class="fa fa-sign-in"></i> ' . __('登录查看', 'zib_language') . '</a></div>';
        }
    }
