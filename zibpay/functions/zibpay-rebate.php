 ...
 * @LastEditTime : 2026-06-17 16:52:14
...
function zibpay_get_user_rebate_data($user_id, $status = 'all')
{
    ...
    $db = ZibDB::name(zibpay::$order_table_name)->field('GROUP_CONCAT(id) as ids,COUNT(*) as count,SUM(rebate_price) as sum')->where('referrer_id', $user_id)->where('status', 1)->where('rebate_price', '>', 0);
...
function zibpay_get_user_rebate_type($rebate_type, $delimiter = '<\br>')
{
    if (!$rebate_type || !is_array($rebate_type)) {
        return __('暂未参与', 'zib_language');
    }

    if (in_array('all', $rebate_type)) {
        $name = __('全部订单', 'zib_language');
    } else {
        $i    = 1;
        $name = '';
...
function zibpay_user_content_rebate($user_id = '')
{
    ...
    $text_details_title = _pz('pay_rebate_text_details_title', __('返佣规则及说明', 'zib_language'));
    $text_details       = _pz('pay_rebate_text_details');
    $pay_vip_but        = '';

    //顶部标题
    $title = '<div class="box-body">';
    $title .= '<div class="title-h-left"><b>' . __('推荐奖励', 'zib_language') . '</b></div>';
    $title .= $text_desc ? '<div class="muted-2-color">' . $text_desc . '</div>' : '';
    $title .= '</div>';

    // 佣金比例卡片
    $card = '<div class="col-md-6"><div class="zib-widget jb-red relative-h" style="background-size:120%;">';
    ...
    $card .= '<p class="opacity8">' . __('佣金比例', 'zib_language') . '</p>';
    ...
    // 提现按钮
    $withdraw_but = '<div class="abs-right">' . zibpay_get_withdraw_link('but radius c-white', (!empty($withdraw_ing['meta']) ? __('提现处理中', 'zib_language') : __('立即提现', 'zib_language')) . '<i style="margin:0 0 0 6px;" class="fa fa-angle-right em12"></i>') . '</div>';

    $c_dec = '<span>' . sprintf(__('累计%1$s%2$s', 'zib_language'), zibpay_get_pay_mark(), floatval($rebate_price_all)) . '</span>';
    if (!empty($withdraw_ing['meta']['withdraw_detail']['rebate'])) {
        $c_dec .= zibpay_get_withdraw_link('icon-spot', sprintf(__('提现中%1$s%2$s', 'zib_language'), zibpay_get_pay_mark(), floatval($withdraw_ing['meta']['withdraw_detail']['rebate'])));
    } else {
        $c_dec .= $rebate_price_invalid ? '<span class="icon-spot" data-toggle="tooltip" title="' . esc_attr__('查看提现记录', 'zib_language') . '">' . zibpay_get_withdraw_record_link('', sprintf(__('已提现%1$s%2$s', 'zib_language'), zibpay_get_pay_mark(), floatval($rebate_price_invalid)) . '<i style="margin:0 0 0 6px;" class="fa fa-angle-right em12"></i>') . '</span>' : '<span class="icon-spot">' . sprintf(__('已提现%1$s%2$s', 'zib_language'), zibpay_get_pay_mark(), floatval($rebate_price_invalid)) . '</span>';
    }

    $card .= '<div class="col-md-6"><div class="zib-widget jb-blue relative-h">';
    ...
    $card .= '<div class="relative">';
    $card .= '<p class="opacity8">' . __('我的佣金', 'zib_language') . '</p>';
    $card .= '<p class="em12">' . zibpay_get_pay_mark() . ' <b class="em2x mr10">' . floatval($rebate_price_effective) . '</b>' . ($rebate_count_effective ? $rebate_count_effective . __('笔', 'zib_language') : '') . '</p>';
    ...
    // tab按钮
    $tab_but = '';
    $tab_but .= '<li class="active"><a data-toggle="tab" href="#rebate_tab_main">' . __('佣金详情', 'zib_language') . '</a></li>';
    $tab_but .= '<li class=""><a data-toggle="tab" data-ajax="" href="#rebate_tab_detail">' . __('佣金明细', 'zib_language') . '</a></li>'; //佣金明细
    $tab_but .= $show_user_s ? '<li class=""><a data-toggle="tab" data-ajax="" href="#rebate_tab_user">' . __('推荐用户', 'zib_language') . '</a></li>' : ''; //推荐用户
    ...
    if (_pz('pay_rebate_poster_s', 1)) {
        $rebate_poster = '<a poster-share="rebate_' . $user_id . '" data-user="' . $user_id . '" href="javascript:;" class="clip-aut mb10 but c-cyan ml10"><i class="fa fa-qrcode"></i>' . __('推广海报', 'zib_language') . '</a>';
    }

    $tab_con   = '';
    $info_lits = '';
    $info_lits .= '<div class="mb10 mt10"><div class="author-set-left">' . __('推广链接', 'zib_language') . '</div><div class="author-set-right"><b data-clipboard-tag="' . esc_attr__('推广链接', 'zib_language') . '" data-clipboard-text="' . $rebate_url . '" class="but mb10 c-red clip-aut mr10">' . $rebate_url . '</b><a data-clipboard-tag="' . esc_attr__('推广链接', 'zib_language') . '" data-clipboard-text="' . $rebate_url . '" class="clip-aut mb10 but c-yellow">' . __('复制链接', 'zib_language') . '</a>' . $rebate_poster . '</div></div>';
    $info_lits .= '<div class="mb20"><div class="author-set-left">' . __('佣金比例', 'zib_language') . '</div><div class="author-set-right"><b class="badg mr10 c-red-2">' . $rebate_ratio . '%</b>' . $pay_vip_but . '</div></div>';
    $info_lits .= '<div class="mb20"><div class="author-set-left">' . __('返佣订单', 'zib_language') . '</div><div class="author-set-right"><b class="badg">' . $rebate_type . '</b></div></div>';
    $info_lits .= '<div class="mb20"><div class="author-set-left">' . __('累计佣金', 'zib_language') . '</div><div class="author-set-right"><b class="badg c-blue mr6 mb6">' . sprintf(__('累计%1$s%2$s', 'zib_language'), zibpay_get_pay_mark(), $rebate_price_all . zibpay_get_currency_unit()) . '</b><span class="badg mr6 mb6">' . sprintf(__('待提现%1$s%2$s', 'zib_language'), zibpay_get_pay_mark(), $rebate_price_effective . zibpay_get_currency_unit()) . '</span></div></div>';
    $info_lits .= $rebate_price_invalid ? '<div class="mb20"><div class="author-set-left">' . __('已提现', 'zib_language') . '</div><div class="author-set-right"><b class="badg c-blue mr6 mb6">' . zibpay_get_pay_mark() . $rebate_price_invalid . zibpay_get_currency_unit() . '</b>' . zibpay_get_withdraw_record_link('but mb6') . '</div></div>' : '';
    ...
    if ($rebate_count_all) {
        ...
        $detail .= '<a ajax-replace="1" ajax-href="' . $detail_ajax_href . '" class="but mr10 ajax-next">' . sprintf(__('全部 %s', 'zib_language'), $rebate_count_all) . '</a>';
        $detail .= $rebate_count_invalid ? '<a ajax-replace="1" ajax-href="' . esc_url(add_query_arg('rebate_status', '1', $detail_ajax_href)) . '" class="but ajax-next mr10">' . sprintf(__('已提现 %s', 'zib_language'), $rebate_count_invalid) . '</a>' : '<span class="badg mr10">' . sprintf(__('已提现 %s', 'zib_language'), $rebate_count_invalid) . '</span>';
        $detail .= $rebate_count_effective ? '<a ajax-replace="1" ajax-href="' . esc_url(add_query_arg('rebate_status', '0', $detail_ajax_href)) . '" class="but ajax-next mr10">' . sprintf(__('未提现 %s', 'zib_language'), $rebate_count_effective) . '</a>' : '<span class="badg mr10">' . sprintf(__('未提现 %s', 'zib_language'), $rebate_count_effective) . '</span>';
        $detail .= '</div>';
        ...
        //如果没有则显示无
        $detail .= zib_get_null(__('暂无佣金订单', 'zib_language'), 40, 'null-money.svg');
    }
