 ...
 * @LastEditTime : 2026-06-17 17:17:49
...
function zibpay_ajax_rebate_user_detail()
{
    ...
    if ($db_order) {
        foreach ($db_order as $order) {
            ...
            $rebate_status = $order->rebate_status ? '<span class="c-blue badg badg-sm">' . __('已提现', 'zib_language') . '</span>' : '<span class="c-yellow badg badg-sm">' . __('未提现', 'zib_language') . '</span>';
            if ($order->rebate_status == 3) {
                $rebate_status = '<span class="c-yellow badg badg-sm">' . __('提现待处理', 'zib_language') . '</span>';
            }
            ...
            $lists .= '</div>';
            $lists .= '<div class="felx0 flex xx jsb ab"><div class="c-yellow"><span class="mr3 px12">' . zibpay_get_pay_mark() . '</span><b class="em14">' . floatval($order->rebate_price)  . '</b></div><div class="text-right">' . $rebate_status . '</div></div>';
            $lists .= '</div>';
        }
...
function zibpay_ajax_rebate_user_user_lists()
{
    ...
    if ($query) {
                    ...
                    <ul class="list-inline relative">
                        ...
                        <li>
                            <dl>
                                ...
                                <dd class="mt6 em09 muted-color text-ellipsis">' . zib_get_time_ago($desc) . __('注册', 'zib_language') . '</dd>
                            </dl>
                        </li>
                    </ul>
                    </div>';
        ...
        $lists .= zib_get_ajax_null(__('暂无推荐用户', 'zib_language'), 50, 'null-user.svg');
    }
