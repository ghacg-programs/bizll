...
function zibpay_ajax_pay_transfer_modal()
{
    ...
    if (($type === 'points' && (!_pz('points_s') || !_pz('points_transfer_s'))) || ($type === 'balance' && (!_pz('pay_balance_s') || !_pz('pay_balance_transfer_s')))) {
        zib_ajax_modal_error(__('当前功能已关闭', 'zib_language'));
    }
...
function zibpay_ajax_transfer_user_search()
{
    ...
    if ($lists) {
        zib_send_json_success(array('data' => $lists, 'remind' => __('请选择用户转账', 'zib_language')));
    } else {
...
function zibpay_ajax_pay_transfer()
{
    ...
    //判断功能是否关闭
    if (($type === 'points' && (!_pz('points_s') || !_pz('points_transfer_s'))) || ($type === 'balance' && (!_pz('pay_balance_s') || !_pz('pay_balance_transfer_s')))) {
        zib_send_json_error(__('当前功能已关闭', 'zib_language'));
    }
    ...
    if (empty($current_user->ID)) {
        zib_send_json_error(__('登录状态失效，请刷新页面后重新登录', 'zib_language'));
    }
    ...
    if ($price <= 0) {
        zib_send_json_error(__('请输入有效的转账金额', 'zib_language'));
    }
    ...
    if (!isset($recipient_user->display_name)) {
        zib_send_json_error(__('转账用户不存在，请重新选择', 'zib_language'));
    }

    //不能自己给自己转
    if ($current_user_id == $recipient_user->ID) {
        zib_send_json_error(__('无法转账给自己', 'zib_language'));
    }
    ...
    if ($max_val < $price) {
        zib_send_json_error(sprintf(__('您的%s不足，最多可转账%s', 'zib_language'), $type_name, $max_val));
    }
    ...
    zib_send_json_success(['msg' => __('转账成功', 'zib_language'), 'reload' => true]);
}
...
function zibpay_ajax_admin_update_user_balance_or_points()
{
    ...
    if (!$type) {
        zib_send_json_error(__('请选择添加或扣除', 'zib_language'));
    }

    if ($val <= 0) {
        zib_send_json_error(__('请输入数额', 'zib_language'));
    }

    if (!$user_id) {
        zib_send_json_error(__('数据或环境异常', 'zib_language'));
    }
    ...
    if ($action === 'admin_update_user_balance') {
        ...
        if (!_pz('pay_balance_s')) {
            zib_send_json_error(__('余额功能已关闭', 'zib_language'));
        }
        if (!zibpay_update_user_balance($user_id, $data)) {
            zib_send_json_error(__('余额不足，无法进行操作', 'zib_language'));
        }
    } else {
        //积分管理
        if (!_pz('points_s')) {
            zib_send_json_error(__('积分功能已关闭', 'zib_language'));
        }
        if (!zibpay_update_user_points($user_id, $data)) {
            zib_send_json_error(__('积分不足，无法进行操作', 'zib_language'));
        }
    }

    zib_send_json_success(__('操作成功，请刷新页面后查看最新数据', 'zib_language'));
}
