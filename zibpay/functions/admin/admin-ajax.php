...
function zibpay_ajax_admin_statistics_data()
{
    ...
    $send_data['mini_card_data'] = array(
        ...
        array(
            [
                'title' => __('总佣金', 'zib_language'),
                'data'  => zibpay_get_rebate_statistics_totime('all')['sum'],
                'unit'  => $unit,
            ],
            [
                'title' => __('待提现', 'zib_language'),
...
function zibpay_ajax_admin_type_pie_data()
{
    ...
    $data = array(
        1  => array('value' => 0, 'name' => __('付费阅读', 'zib_language')), //文章，帖子
        2  => array('value' => 0, 'name' => __('付费下载', 'zib_language')), //文章
        5  => array('value' => 0, 'name' => __('付费图片', 'zib_language')), //文章
        6  => array('value' => 0, 'name' => __('付费视频', 'zib_language')), //文章
        4  => array('value' => 0, 'name' => __('购买会员', 'zib_language')), //用户
        8  => array('value' => 0, 'name' => __('余额充值', 'zib_language')), //用户
        9  => array('value' => 0, 'name' => __('购买积分', 'zib_language')), //用户
        10 => array('value' => 0, 'name' => __('购买商品', 'zib_language')), //商城，商品
    );
    ...
    if ($other > 0) {
        $data[] = array('value' => $other, 'name' => __('其他', 'zib_language'));
    }
...
function zibpay_ajax_admin_hot_product_data()
{
    ...
    foreach ($data as $key => $value) {
        ...
        if (!$post_title) {
            $post_title = sprintf(__('商品已删除[ID:%s]', 'zib_language'), $value['post_id']);
        }
...
function zibpay_get_user_ranking_data($type = 'points')
{
    ...
    foreach ($db_data as $key => $value) {
        ...
        $db_data[$key]['user_name']   = $user_data->display_name ?? sprintf(__('用户[ID:%s]', 'zib_language'), $value['user_id']);
...
function zibpay_ajax_admin_order_table_list()
{
    ...
    $statistics_data = [
        [
            __('现金销量', 'zib_language') => (int) ($data_1['count'] ?: 0),
            __('积分销量', 'zib_language') => (int) ($data_2['count'] ?: 0),
        ],
        [
            __('现金金额', 'zib_language') => zib_floatval_round($data_1['order_price'] ?: 0),
            __('积分金额', 'zib_language') => (int) ($data_2['order_price'] ?: 0),
        ],
        [
            __('佣金合计', 'zib_language') => zib_floatval_round($data_3['rebate_price']),
            __('待提现', 'zib_language')  => zib_floatval_round($data_5['rebate_price']),
        ],
        [
            __('分成合计', 'zib_language') => zib_floatval_round($data_3['income_price']),
            __('待提现', 'zib_language')  => zib_floatval_round($data_4['income_price']),
        ],
    ];
...
function zibpay_ajax_admin_clear_order()
{
    ...
    if ($result) {
        zib_send_json_success(sprintf(__('清理完成[共清理：%s个订单]', 'zib_language'), $result));
    } else {
        zib_send_json_success(__('没有需要清理的订单', 'zib_language'));
    }
}
        ...
        foreach ($db_data['orders'] as $key => $order) {
            ...
            if ($order['user_id']) {
                ...
                $user_info = [
                    ...
                    'name'      => $udata ? $udata->display_name : sprintf(__('用户[ID:%s]', 'zib_language'), $order['user_id']),
            ...
            if ($order['post_id']) {
                ...
                $product_info['title']     = $product->post_title ?? sprintf(__('商品已删除[ID:%s]', 'zib_language'), $order['post_id']);
            }

            $author_info = [
                'name' => __('无', 'zib_language'),
            ];
            ...
            if ($order['post_author']) {
                ...
                $author_info = [
                    ...
                    'name'           => $author->display_name ?? sprintf(__('用户[ID:%s]', 'zib_language'), $order['post_author']),
            ...
            if ($order['referrer_id'] && $order['rebate_price'] > 0) {
                ...
                $rebate_info = [
                    ...
                    'referrer_info' => [
                        'name'   => $referrer->display_name ?? sprintf(__('用户[ID:%s]', 'zib_language'), $order['referrer_id']),
...
function zibpay_ajax_admin_payment_submit()
{
    ...
    if (!$payment_method) {
        zib_send_json_error(__('请选择支付方式', 'zib_language'));
    }

    if (!$pay_num) {
        zib_send_json_error(__('请输入支付单号', 'zib_language'));
    }
    ...
    if ($order) {
        ...
        zib_send_json_success(__('补单成功', 'zib_language'));
    } else {
        zib_send_json_error(__('补单失败，未找到可补单的订单', 'zib_language'));
    }
}
