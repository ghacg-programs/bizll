 ...
 * @LastEditTime : 2026-06-17 17:19:52
...
function zibpay_get_user_income_order_lists($user_id)
{
    ...
    $ice_perpage   = 10;
    $income_status = isset($_REQUEST['income_status']) ? (int) $_REQUEST['income_status'] : '';

    $db = ZibDB::name(zibpay::$order_table_name)->where('status', 1)->where('post_author', $user_id)->where('income_price', '>', 0)->order('pay_time', 'DESC')->page($paged, $ice_perpage);
    if (isset($_REQUEST['income_status'])) {
        $db->where('income_status', $income_status);
    }
    ...
    if ($db_order) {
        foreach ($db_order as $order) {
            ...
            $income_status = $order->income_status ? '<span class="c-blue badg badg-sm">' . __('已提现', 'zib_language') . '</span>' : '<span class="c-yellow badg badg-sm">' . __('未提现', 'zib_language') . '</span>';
            if ($order->income_status == 3) {
                $income_status = '<span class="c-yellow badg badg-sm">' . __('提现待处理', 'zib_language') . '</span>';
            }

            $lists .= '<div class="jsb flex border-bottom padding-h10 ajax-item ' . $class . '">';
            $lists .= '<div class="">';
            $lists .= '<div class="mb6">' . $pay_title . '</div>';
            $lists .= '<div class="muted-2-color em09">' . sprintf(__('订单号：%s', 'zib_language'), $order_num) . '</div>';
            $lists .= '<div class="muted-2-color em09">' . sprintf(__('时间：%s', 'zib_language'), $pay_time) . '</div>';
            $lists .= '</div>';
            $lists .= '<div class="felx0 flex xx jsb ab"><div class="c-yellow"><span class="mr3 px12">' . zibpay_get_pay_mark() . '</span><b class="em14">' . floatval($order->income_price) . '</b></div><div class="text-right">' . $income_status . '</div></div>';
            $lists .= '</div>';
        }
        ...
        $lists .= zib_get_ajax_null(__('暂无订单', 'zib_language'), 60, 'null-order.svg');
    }
...
function zibpay_get_user_income_post_lists($user_id)
{
    ...
    if ($posts_query && $posts_query->have_posts()) {
        while ($posts_query->have_posts()): $posts_query->the_post();
            ...
            $lists .= '<div class="zib-widget ajax-item mb10"><div class="muted-color"><a target="_blank" class="block text-ellipsis mb6" href="' . $permalink . '">' . $type_badge . $posts_title . '</a></div><div class="em09 muted-2-color">' . __('合计销售：', 'zib_language') . '<span class="c-yellow">' . sprintf(__('%s笔', 'zib_language'), ($all_income['count'] ?: 0)) . '</span></div><div class="flex jsb ac em09 muted-2-color"><div class="mr20">' . __('合计收入：', 'zib_language') . '<span class="c-yellow">' . zibpay_get_pay_mark() . (floatval($all_income['income']) ?: 0) . zibpay_get_currency_unit() . '</span></div><div class="">' . zib_get_svg('time', null, 'mr6') . get_the_modified_time('Y-m-d H:i:s') . '</div></div></div>';
        endwhile;
        //帖子分页paginate
...
function zibpay_get_post_income_data($post_id)
{
    ...
    if ($all_data === null) {
        $db       = ZibDB::name(zibpay::$order_table_name)->field('post_id,COUNT(*) as count,SUM(income_price) as income')->where('status', 1)->where('income_price', '>', 0)->group('post_id');
        $all_data = $db->select()->result();
    }
...
function zibpay_get_user_today_income_data($user_id, $status = 'all')
{
    ...
    $db = ZibDB::name(zibpay::$order_table_name)->field('COUNT(*) as count,SUM(income_price) as sum')->where('post_author', $user_id)->where('status', 1)->where('income_price', '>', 0)->whereTime('pay_time', 'today');
    if ($income_status !== null) {
        $db->where('income_status', $income_status);
    }

    $data = $db->select()->find()->toArray();
    if (!isset($data['count'])) {
        return $error;
    }
...
function zibpay_get_user_income_data($user_id, $status = 'all')
{
    ...
    $db = ZibDB::name(zibpay::$order_table_name)->field('GROUP_CONCAT(id) as ids,COUNT(*) as count,SUM(income_price) as sum')->where('post_author', $user_id)->where('status', 1)->where('income_price', '>', 0);
    if ($income_status !== null) {
        $db->where('income_status', $income_status);
    }
    $data = $db->select()->find()->toArray();
...
function zibpay_get_user_income_ratio($user_id)
{
    ...
    //查询独立设置的比例，有则返回
    $user_income_ratio = zib_get_user_meta($user_id, 'income_rule', true);
...
function zibpay_update_order_income_price($pay_order)
{
    ...
    $income_detail['withdraw_time'] = current_time('Y-m-d H:i:s');
    $income_detail['withdraw_type'] = 'points_auto';

    $updata_data = [
        'income_status' => 1,
        'income_detail' => maybe_serialize($income_detail),
    ];

    ZibDb::name('zibpay_order')->where('id', $pay_order->id)->update($updata_data);
}
add_action('payment_order_success', 'zibpay_update_order_income_price', 5);
...
function zibpay_user_content_income($user_id = 0)
{
    ...
    $card = '<div class="col-md-6"><div class="colorful-bg c-cyan zib-widget"><div class="colorful-make"></div>';
    $card .= '<div class="relative">';
    $card .= '<p class="opacity8">' . __('今日收入', 'zib_language') . '</p>';
    $card .= '<p class="em12">' . zibpay_get_pay_mark() . '<b class="em2x mr10">' . (floatval($today_data['sum']) ?: '0') . '</b> ' . ($today_data['count'] && $today_data['sum'] ? sprintf(__('%s笔', 'zib_language'), $today_data['count']) : '') . '</p>';
    $card .= '<div class="em09">' . sprintf(__('现金分成 %s%%', 'zib_language'), ($income_ratio ?: '0')) . (_pz('points_s') ? '<span class="icon-spot">' . sprintf(__('积分分成 %s%%', 'zib_language'), ($income_points_ratio ?: '0')) . '</span>': '') . '</div>';
    $card .= '</div>';
    $card .= '</div></div>';
    ...
    // 提现按钮
    $withdraw_but = '<div class="abs-right">' . zibpay_get_withdraw_link('but radius c-red', (!empty($withdraw_ing['meta']) ? __('提现处理中', 'zib_language') : __('立即提现', 'zib_language')) . '<i style="margin:0 0 0 6px;" class="fa fa-angle-right em12"></i>') . '</div>';
    $c_dec        = sprintf(__('累计%1$s%2$s', 'zib_language'), zibpay_get_pay_mark(), (floatval($all_data['sum']) ?: '0'));
    if (!empty($withdraw_ing['meta']['withdraw_detail']['income'])) {
        $c_dec .= zibpay_get_withdraw_link('icon-spot', sprintf(__('提现中%1$s%2$s', 'zib_language'), zibpay_get_pay_mark(), floatval($withdraw_ing['meta']['withdraw_detail']['income'])));
    } else {
        $__invalid = floatval($income_price_invalid['sum']);
        $c_dec .= $__invalid ? '<span class="icon-spot" data-toggle="tooltip" title="' . esc_attr__('查看提现记录', 'zib_language') . '">' . zibpay_get_withdraw_record_link('', sprintf(__('已提现%1$s%2$s', 'zib_language'), zibpay_get_pay_mark(), $__invalid) . '<i style="margin:0 0 0 6px;" class="fa fa-angle-right em12"></i>') . '</span>' : '<span class="icon-spot">' . sprintf(__('已提现%1$s%2$s', 'zib_language'), zibpay_get_pay_mark(), ($__invalid ?: '0')) . '</span>';
    }

    $card .= '<div class="col-md-6"><div class="colorful-bg c-red zib-widget"><div class="colorful-make" style="transform: rotate(36deg) scale(0.8);"></div>';
    $card .= '<div class="relative">';
    $card .= '<p class="opacity8">' . __('我的收入', 'zib_language') . '</p>';
    $card .= '<p class="em12">' . zibpay_get_pay_mark() . '<b class="em2x mr10">' . (floatval($income_price_effective['sum']) ?: '0') . '</b> ' . ($income_price_effective['count'] && $income_price_effective['sum'] ? sprintf(__('%s笔', 'zib_language'), $income_price_effective['count']) : '') . '</p>';
    ...
    //tab内容
    $tab_but = '';
    $tab_but .= '<li class="active"><a data-toggle="tab" href="#income_tab_main">' . __('创作分成', 'zib_language') . '</a></li>';
    //分成明细
    $tab_but .= '<li class=""><a data-toggle="tab" data-ajax="" href="#income_tab_post">' . __('我的商品', 'zib_language') . '</a></li>';
    $tab_but .= '<li class=""><a data-toggle="tab" data-ajax="" href="#income_tab_order">' . __('收入明细', 'zib_language') . '</a></li>';
    ...
    if (_pz('pay_income_desc_page_s')) {
        $tab_content .= '<div><a class="but c-blue" href="' . get_permalink(_pz('pay_income_desc_page')) . '">' . __('查看详细说明', 'zib_language') . '</a></div>';
    }
    $tab_content .= '</div>';
    ...
    if ($all_data['count']) {
        ...
        $tab_content .= '<a ajax-replace="1" ajax-href="' . $order_ajax_href . '" class="but mr10 ajax-next">' . sprintf(__('全部 %s', 'zib_language'), $all_data['count']) . '</a>';
        $tab_content .= $income_price_invalid['count'] ? '<a ajax-replace="1" ajax-href="' . esc_url(add_query_arg('income_status', '1', $order_ajax_href)) . '" class="but ajax-next mr10">' . sprintf(__('已提现 %s', 'zib_language'), $income_price_invalid['count']) . '</a>' : '<span class="badg mr10">' . sprintf(__('已提现 %s', 'zib_language'), '0') . '</span>';
        $tab_content .= $income_price_effective['count'] ? '<a ajax-replace="1" ajax-href="' . esc_url(add_query_arg('income_status', '0', $order_ajax_href)) . '" class="but ajax-next mr10">' . sprintf(__('未提现 %s', 'zib_language'), $income_price_effective['count']) . '</a>' : '<span class="badg mr10">' . sprintf(__('未提现 %s', 'zib_language'), '0') . '</span>';
        $tab_content .= '</div>';
        ...
        //如果没有则显示无
        $tab_content .= zib_get_null(__('暂无收入订单', 'zib_language'), 40, 'null-money.svg');
    }
    $tab_content .= '</div>';
...
//获取分成统计数据
function zibpay_get_income_statistics_totime($time_type = 'all', $status = 'all')
{
