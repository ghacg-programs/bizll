 ...
 * @LastEditTime : 2026-06-29 19:21:17
 ...
 * @return {*}
 */
function zibpay_get_balance_charge_link($class = '', $con = null)
{
    if (null === $con) {
        $con = __('充值', 'zib_language');
    }
...
function zibpay_user_balance_or_points_save_db($type, $user_id, $val, &$old_value = 0)
{
    ...
    if (empty($old_meta->user_id)) {
        ...
        if ($is_balance) {
            //余额：以两位小数字符串写入，避免 %f 浮点字面量污染 LONGTEXT
            $sql      = "INSERT INTO {$wpdb->usermeta} (user_id, meta_key, meta_value) VALUES (%d, %s, %s)";
            $sql_args = [$user_id, $meta_key, zib_floatval_round($abs_value)];
        } else {
            $sql      = "INSERT INTO {$wpdb->usermeta} (user_id, meta_key, meta_value) VALUES (%d, %s, %d)";
            $sql_args = [$user_id, $meta_key, (int) $abs_value];
        }
    } else {
        $old_value = $is_balance ? floatval(round((float) $old_meta->meta_value, 2)) : (int) $old_meta->meta_value;

        //直接更新数据库，避免并发；余额用 DECIMAL 定点运算，防止 IEEE 754 误差累积
        if ($val > 0) {
            if ($is_balance) {
                $rounded  = zib_floatval_round($abs_value);
                $sql      = "UPDATE {$wpdb->usermeta} SET meta_value = ROUND(CAST(meta_value AS DECIMAL(20,2)) + CAST(%s AS DECIMAL(20,2)), 2) WHERE user_id = %d AND meta_key = %s";
                $sql_args = [$rounded, $user_id, $meta_key];
            } else {
                $sql      = "UPDATE {$wpdb->usermeta} SET meta_value = CAST(meta_value AS SIGNED) + %d WHERE user_id = %d AND meta_key = %s";
                $sql_args = [(int) $abs_value, $user_id, $meta_key];
            }
        } else {
            if ($is_balance) {
                $rounded  = zib_floatval_round($abs_value);
                $sql      = "UPDATE {$wpdb->usermeta} SET meta_value = ROUND(CAST(meta_value AS DECIMAL(20,2)) - CAST(%s AS DECIMAL(20,2)), 2) WHERE user_id = %d AND meta_key = %s AND CAST(meta_value AS DECIMAL(20,2)) >= CAST(%s AS DECIMAL(20,2))";
                $sql_args = [$rounded, $user_id, $meta_key, $rounded];
            } else {
                $sql      = "UPDATE {$wpdb->usermeta} SET meta_value = CAST(meta_value AS SIGNED) - %d WHERE user_id = %d AND meta_key = %s AND CAST(meta_value AS SIGNED) >= %d";
                $sql_args = [(int) $abs_value, $user_id, $meta_key, (int) $abs_value];
            }
        }
    }
...
function zibpay_get_balance_charge_modal($user_id)
{
    ...
    $custom_limit_html = !empty($custom_limit['min']) ? sprintf(__('最低充值%s%s', 'zib_language'), $mark, $custom_limit['min']) : '';
    $custom_limit_html .= $custom_limit_html ? '，' : '';
    $custom_limit_html .= !empty($custom_limit['max']) ? sprintf(__('最高充值%s%s', 'zib_language'), $mark, $custom_limit['max']) : '';
    ...
    <div class="relative flex ab">
        ...
        <input class="line-form-input em16 key-color" style="padding: 1px;" name="custom_price" type="number" ' . (!empty($custom_limit['min']) ? ' limit-min="' . $custom_limit['min'] . '"' : '') . (!empty($custom_limit['max']) ? ' limit-max="' . $custom_limit['max'] . '"' : '') . ' warning-max="' . esc_attr(sprintf(__('最高可充值1$%s', 'zib_language'), zibpay_get_currency_unit())) . '" warning-min="' . esc_attr(sprintf(__('最低需充值1$%s', 'zib_language'), zibpay_get_currency_unit())) . '">
        <i class="line-form-line"></i>
    </div>
    <div class="muted-2-color em09 mt6">' . $custom_limit_html . '</div></div>';

    $header = '<div class="mb10 touch"><button class="close" data-dismiss="modal">' . zib_get_svg('close', null, 'ic-close') . '</button><b class="modal-title flex ac"><span class="mr6 em14">' . zib_get_svg('money-color-2') . '</span>' . __('余额充值', 'zib_language') . '</b></div>';
    ...
    if ($product && is_array($product)) {
        ...
        <div class="c-red"><span class="px12">' . $mark . '</span><span class="">' . $show_price . '</span>' . ($show_price < $price ? '<span class="ml6 c-yellow smail">' . sprintf(__('省%s', 'zib_language'), zibpay_format_local_price_text($price - $show_price)) . '</span>' : '') . '</div>
        </div>';
        }
    }

    if ($product_html) {
        $product_html = '<div class="muted-color mb6">' . __('请选择充值金额', 'zib_language') . '</div>' . $product_html;
        if ($custom_s) {
            $product_html .= '<div class="muted-color mt20 mb6">' . __('自定义充值金额', 'zib_language') . '</div>' . $custom_product;
        }
    }

    if (!$product_html) {
        $product_id   = 'custom';
        $product_html = '<div class="muted-color mb6">' . __('请输入充值金额', 'zib_language') . '</div>' . $custom_product;
    }
...
function zibpay_get_user_balance_record_lists($user_id = 0)
{
    ...
    foreach ($record as $k => $v) {
        if (isset($v['value']) && isset($v['balance'])) {
            ...
            $lists .= $v['order_num'] ? '<div class="em09">' . sprintf(__('订单号：%s', 'zib_language'), $v['order_num']) . '</div>' : '';
            $lists .= $v['time'] ? '<div class="em09">' . sprintf(__('时间：%s', 'zib_language'), $v['time']) . '</div>' : '';
            $lists .= '</div>';
            $lists .= '<div class="flex jsb xx text-right flex0 ml10 ab"><b class="em12 ' . $_class . '">' . ($v['value'] < 0 ? $v['value'] : '+' . $v['value']) . '</b><div class="em09 muted-2-color">' . sprintf(__('余额：%s', 'zib_language'), $v['balance']) . '</div></div>';
            $lists .= '</div>';
        }
    }

    if (!$lists) {
        $lists = zib_get_null(__('暂无余额记录', 'zib_language'), 42, 'null-order.svg');
    } else {
        if (count($record) > 49) {
            $lists .= '<div class="text-center mt20 muted-3-color">' . __('最多显示近50条记录', 'zib_language') . '</div>';
        }
    }
    return $lists;
}
...
function zib_get_user_wallet_mini_box($user_id = 0, $class = '')
{
    ...
    if ($pay_balance_s) {
        ...
        $box .= '<a rel="nofollow" style="flex: 1;" class="muted-box padding-6 flex1 radius4" href="' . zib_get_user_center_url('balance') . '">
                    <div class="muted-2-color px12">' . __('余额', 'zib_language') . '<i class="ml6 fa fa-angle-right em12"></i></div>
    ...
    if ($points_s) {
        ...
        $box .= '<a rel="nofollow" style="flex: 1;" class="muted-box padding-6 flex1 radius4" href="' . zib_get_user_center_url('balance') . '">
                    <div class="muted-2-color px12">' . __('积分', 'zib_language') . '<i class="ml6 fa fa-angle-right em12"></i></div>
...
function zibpay_user_content_balance($user_id = 0)
{
    ...
    if ($pay_balance_s) {
        ...
        $charge_link            = zibpay_get_balance_charge_link('but b-blue-2', __('充值', 'zib_language'));
        ...
        if ($pay_balance_withdraw_s) {
            ...
            if (!$transfer_link) {
                $transfer_link = zibpay_get_withdraw_link('but c-blue-2 mr6 hollow', __('提现', 'zib_language') . ($withdraw_ing ? __('处理中', 'zib_language') . '<i style="margin:0 0 0 6px;" class="fa fa-angle-right em12"></i>' : ''));
            } else {
                $right_top_btn = zibpay_get_withdraw_link('muted-color px12', __('提现', 'zib_language') . ($withdraw_ing ? __('处理中', 'zib_language') : '') . '<i style="margin:0 0 0 6px;" class="fa fa-angle-right em12"></i>');
            }
        }
        $card .= '<div class="col-md-6">
                <div class="zib-widget relative" style="padding-left: 24px;">
                    <div class="flex jsb">
                        <div class="mb6">
                        ' . zib_get_svg('money-color-2', null, 'em12 mr6') . __('余额', 'zib_language') . '
                        </div>
                        ' . $right_top_btn . '
                    </div>
                    ...
                    <div class="muted-2-color em09">' . ($add_ing ? '<span data-toggle="tooltip" title="' . esc_attr__('提现待转入', 'zib_language') . '">' . sprintf(__('待转入 %s', 'zib_language'), $add_ing) . '</span>' : '<span>' . sprintf(__('待转入 %s', 'zib_language'), $add_ing) . '</span>') . $withdraw_record_link . '</div>
                </div>
            </div>';

        // tab-列表内容：余额变动
        $tab_but .= '<li class="active"><a data-toggle="tab" href="#record_tab_balance">' . __('余额记录', 'zib_language') . '</a></li>';
    ...
    if ($points_s) {
        ...
        $points_pay_link = zibpay_get_points_pay_link('but c-green', __('购买积分', 'zib_language'));
        $transfer_link   = zibpay_get_transfer_link('points', 'but c-green mr6 hollow');
        $right_top_btn   = zib_get_user_checkin_btn('muted-color px12', __('签到领积分', 'zib_language') . '<i class="fa fa-angle-right em12 ml3"></i>', __('今日已签到', 'zib_language') . '<i class="fa fa-angle-right em12 ml3"></i>');
        $bottom_link     = zibpay_get_vip_points_exchange_link('muted-2-color em09', __('积分兑换会员', 'zib_language') . '<i style="margin:0 0 0 6px;" class="fa fa-angle-right em12"></i>', 'div') ?: '<div class="muted-2-color em09 pointer" data-onclick="[href=\'#record_tab_points_free\']">' . __('做任务赚积分', 'zib_language') . '<i style="margin:0 0 0 6px;" class="fa fa-angle-right em12"></i></div>';

        $card .= '<div class="col-md-6">
                    <div class="zib-widget relative" style="padding-left: 24px;">
                        <div class="flex jsb">
                            <div class="mb6">
                            ' . zib_get_svg('points-color', null, 'em12 mr6') . __('积分', 'zib_language') . '
                            </div>
                            ' . $right_top_btn . '
                        </div>
                        <div class="flex jsb ac withdraw-amount-box">
        ...
        // tab-列表内容：积分变动
        $tab_but .= '<li class="' . (!$pay_balance_s ? 'active' : '') . '"><a data-toggle="tab" href="#record_tab_points">' . __('积分记录', 'zib_language') . '</a></li>';
        $tab_but .= '<li class=""><a data-toggle="tab" href="#record_tab_points_free">' . __('积分任务', 'zib_language') . '</a></li>';
    ...
    if ($card_i < 2) {
        $card = str_replace('class="col-md-6"', 'class="col-sm-12"', $card);
    }
 ...
 * @return {*}
 */
function zibpay_get_transfer_link($type = 'points', $class = '', $recipient = 0, $con = null)
{
    if (null === $con) {
        $con = __('转账', 'zib_language');
    }
...
function zibpay_get_pay_transfer_modal($type = 'points', $recipient = 0)
{
    ...
    if (!zib_current_user_can($type . '_transfer')) {
        return '<div class="touch">' . zib_get_nocan_info($user_id, $type . '_transfer', __('无法转账', 'zib_language'), 20, 0, 220) . '</div>';
    }

    if ($type === 'balance') {
        $type_name      = __('余额', 'zib_language');
        ...
        $desc .= $service_charge ? '<div class="mt6 c-yellow">' . sprintf(__('转账时将扣除%s%%的手续费', 'zib_language'), $service_charge) . '</div>' : '';
    } else {
        $type_name      = __('积分', 'zib_language');
    ...
    //余额不足，无法转账
    if (!$max_val) {
        return zib_get_modal_colorful_header('c-yellow', $icon_svg, sprintf(__('%s转账', 'zib_language'), $type_name)) . '<div class="em12 text-center c-red" style="padding: 30px 0;">' . sprintf(__('您的%s不足，暂时无法转账', 'zib_language'), $type_name) . '</div>';
    }
    ...
    $search .= '</div>';
    $search .= '<div class="mt20 mb10 muted-3-color separator search-remind em09">' . __('搜索用户进行转账', 'zib_language') . '</div>';
    ...
    $form = '<div class="mb20"><a data-value="0" data-for="recipient" data-toggle="tab" href="#pay_transfer_modal_1" aria-expanded="true" class="muted-color"><i class="fa fa-angle-left em12 mr6"></i>' . __('重新选择用户', 'zib_language') . '</a></div>
        <div class="transfer-user"><span class="flex ac muted-box" name="recipient">' . $recipient_user . '</span></div>';

    $form .= '<div class="muted-color mt20 mb6">' . __('请输入转账金额', 'zib_language') . '</div>
    <div class="" data-for="balance_product" data-value="custom">
    <div class="relative flex ab">
        <span class="ml6 mr10 muted-color shrink0">' . $mark . '</span>
        <input class="line-form-input em16 key-color" style="padding: 1px;" name="price" type="number" limit-max="' . $max_val . '" limit-min="0.00001" warning-max="' . esc_attr__('最高可转账1$', 'zib_language') . '" warning-min="' . esc_attr__('金额不能为0', 'zib_language') . '">
        <i class="line-form-line"></i>
    </div>
    <div class="muted-2-color em09 mt6 mb10">' . sprintf(__('最多可转账%s', 'zib_language'), $max_val) . '</div></div>';
    ...
    <button class="mt6 but jb-yellow btn-block radius padding-lg wp-ajax-submit" data-confirm="' . esc_attr__('确认要转账给此用户吗？ 此操作无法撤回，涉及到资金安全，请再次确认转账用户及金额正确！', 'zib_language') . '"><i class="fa fa-check" aria-hidden="true"></i>' . __('确认转账', 'zib_language') . '</button>
            </div>';
    ...
    $header = '<div class="mb10 touch"><button class="close" data-dismiss="modal">' . zib_get_svg('close', null, 'ic-close') . '</button><b class="modal-title flex ac"><span class="mr6 em14">' . $icon_svg . '</span>' . sprintf(__('%s转账', 'zib_language'), $type_name) . '</b></div>';
...
function zibpay_get_transfer_recipient_user_card($user_id, $is_con = false)
{
    ...
    $btn          = '<div class="muted-color">' . __('给他转账', 'zib_language') . '<i style="margin:0 0 0 6px;" class="fa fa-angle-right em12"></i></div>';
