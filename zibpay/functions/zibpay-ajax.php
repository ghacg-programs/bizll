 ...
 * @LastEditTime : 2026-06-17 15:47:32
...
function zibpay_initiate_pay_session_data()
{
    ...
    if (!empty($_REQUEST['openid'])) {
        ...
        if (!empty($_SESSION['ZIBPAY_POST'])) {
            ...
            zib_send_json_error(__('PHP session 数据获取失败', 'zib_language'));
        }
    }
}
...
function zibpay_ajax_initiate_pay($payment_id = 0)
{
    ...
    if (empty($payment_id)) {
        zib_send_json_error(['code' => 'error', 'msg' => __('参数错误', 'zib_language')]);
    }

    $payment_data = zibpay::get_payment($payment_id);
    if (empty($payment_data)) {
        zib_send_json_error(['code' => 'error', 'msg' => __('支付数据不存在', 'zib_language')]);
    }

    if ($payment_data['status'] == '1') {
        zib_send_json_success(['code' => 'success', 'msg' => __('订单已支付，请刷新页面', 'zib_language')]);
    }

    //判断订单是否已经关闭
    if ($payment_data['status'] == '-1') {
        zib_send_json_error(['code' => 'error', 'msg' => __('订单已关闭，请刷新页面', 'zib_language')]);
    }

    //判断订单是否失效
    $time_remaining = zibpay_get_payment_pay_over_time($payment_data);
    if ($time_remaining == 'over') {
        zib_send_json_error(['code' => 'error', 'msg' => __('订单已失效，请刷新页面重新下单', 'zib_language')]);
    }

    //根据$payment_id获取订单数据
    $order_data = zibpay::get_order_by_payment_id($payment_id);
    if (empty($order_data[0])) {
        zib_send_json_error(['code' => 'error', 'msg' => __('订单错误，请重新下单', 'zib_language')]);
    }
    ...
    if (!zibpay::update_payment($payment_update_data)) {
        zib_send_json_error(['code' => 'update_payment_error', 'msg' => __('支付数据更新失败', 'zib_language')]);
    }
...
function zibpay_check_pay()
{
    ...
    if (empty($_POST['order_num'])) {
        echo(json_encode(array('error' => 1, 'msg' => __('还未生成订单', 'zib_language'))));
        exit();
    }
    ...
    if (empty($order_check_data['order_num'])) {
        echo(json_encode(array('error' => 1, 'msg' => __('数据传入错误', 'zib_language'))));
        exit();
    }

    $order_check_data['over_time'] = zibpay_get_payment_pay_over_time($order_check_data);
    if ($order_check_data['status'] == -1) {
        echo(json_encode(array('error' => 1, 'status' => -1, 'msg' => __('订单已关闭', 'zib_language'))));
        exit();
    }
...
function zibpay_initiate_pay($order_data)
{
    ...
    $defaults = array(
        ...
        'ip_address'     => zib_get_remote_ip_addr(), //IP地址
        'order_name'     => sprintf(__('%s支付', 'zib_language'), get_bloginfo('name')), //订单名称
    ...
    if (empty($order_data['order_num'])) {
        return array('error' => 1, 'msg' => __('订单创建失败', 'zib_language'));
    }

    $order_data['order_price'] = zib_floatval_round($order_data['order_price']);
    $order_data['local_price'] = $order_data['order_price']; //记录本地金额

    //汇率换算
    if (zibpay_need_settle_convert($order_data['payment_method'])) {
        if ('alipay' === $order_data['payment_method']) {
            $order_data['calculated_settle_price'] = zibpay_convert_for_alipay($order_data['local_price']);
        } else {
            $order_data['calculated_settle_price'] = zibpay_convert_for_wechat($order_data['local_price']);
        }
        $order_data['order_price'] = $order_data['calculated_settle_price'];
    }
    ...
    if ((float) $order_data['order_price'] <= 0 && $order_data['payment_method'] !== 'card_pass') {
        ...
        return array('error' => 0, 'reload' => true, 'msg' => __('支付成功', 'zib_language'), 'payok' => 1, 'return_url' => $order_data['return_url']);
    }
    ...
    if (!$pay_sdk || 'null' == $pay_sdk) {
        return array('error' => 1, 'msg' => __('当前订单不支持此方式支付，请联系客服', 'zib_language'));
    }
    ...
    //结算金额换算
    if (zibpay_need_settle_convert($order_data['payment_method'])) {
        $sdk_amount = zibpay_extract_sdk_settle_amount($payresult, $order_data['calculated_settle_price'] ?? null);
        $payresult  = zibpay_build_settle_scan_response($order_data, $payresult, $sdk_amount); //扫码支付返回结果换算
    }
...
function zibpay_initiate_paypal($order_data)
{
    ...
    if (!empty($config['rest_s'])) {
        return zibpay_initiate_paypal_rest($order_data, $config);
    }

    return zibpay_initiate_paypal_nvp($order_data, $config);
}

/**PayPal REST 新接口 */
function zibpay_initiate_paypal_rest($order_data, $config = array())
{
    if (!$config) {
        $config = zibpay_get_payconfig('paypal');
    }
    ...
    if (empty($config['client_id']) || empty($config['client_secret'])) {
        return array('error' => 1, 'msg' => __('PayPal REST 接口缺少 Client ID 或 Client Secret', 'zib_language'));
    }
    ...
    $cancel_url  = add_query_arg(array('return_url' => urlencode($return_url), 'cancel' => 'cancel'), ZIB_TEMPLATE_DIRECTORY_URI . '/zibpay/shop/paypal/return.php');
    $success_url = add_query_arg(array(
        'return_url' => urlencode($return_url),
        'order_num'  => $order_data['order_num'],
        'api'        => 'rest',
    ), ZIB_TEMPLATE_DIRECTORY_URI . '/zibpay/shop/paypal/return.php');

    $paypal = new PayPalRest($config);
    $result = $paypal->createOrder($total, $config['currency'], $order_data['order_num'], $order_data['order_name'], $success_url, $cancel_url);

    if (is_wp_error($result)) {
        return array('error' => 1, 'msg' => $result->get_error_message());
    }

    return array('open_url' => true, 'url' => $result['approve_url']);
}

/**PayPal NVP 旧接口 */
function zibpay_initiate_paypal_nvp($order_data, $config = array())
{
    if (!$config) {
        $config = zibpay_get_payconfig('paypal');
    }
    ...
    if (empty($config['username']) || empty($config['password']) || empty($config['signature'])) {
        return array('error' => 1, 'msg' => __('PayPal接口缺少配置参数', 'zib_language'));
    }
    ...
    if (!is_string($result)) {
        return array('error' => 1, 'result' => $result, 'msg' => isset($result['L_LONGMESSAGE0']) ? sprintf(__('PayPal接口错误：%s', 'zib_language'), $result['L_LONGMESSAGE0']) : __('PayPal配置错误，或网络连接失败', 'zib_language'));
    }
...
//Stripe发起支付
function zibpay_initiate_stripe($order_data)
{
    $config = zibpay_get_payconfig('stripe');
    if (empty($config['secret_key'])) {
        return array('error' => 1, 'msg' => __('Stripe 接口缺少 Secret Key', 'zib_language'));
    }
    ...
    $total       = bcmul($order_data['order_price'], $config['rates'], 2);
    $return_url  = !empty($order_data['return_url']) ? $order_data['return_url'] : home_url();
    $cancel_url  = add_query_arg(array('return_url' => urlencode($return_url), 'cancel' => 'cancel'), ZIB_TEMPLATE_DIRECTORY_URI . '/zibpay/shop/stripe/return.php');
    $success_url = add_query_arg(array(
        'return_url' => urlencode($return_url),
        'order_num'  => $order_data['order_num'],
    ), ZIB_TEMPLATE_DIRECTORY_URI . '/zibpay/shop/stripe/return.php');
    $success_url .= (strpos($success_url, '?') !== false ? '&' : '?') . 'session_id={CHECKOUT_SESSION_ID}';

    $stripe = new StripeCheckout($config);
    $result = $stripe->createSession($total, $config['currency'], $order_data['order_num'], $order_data['order_name'], $success_url, $cancel_url);
...
function zibpay_initiate_vmqphp($order_data)
{
    ...
    if (empty($config['apiurl']) || empty($config['key'])) {
        return array('error' => 1, 'msg' => __('V免签接口缺少配置参数', 'zib_language'));
    }
    ...
    if (isset($get_json['code']) && 1 == $get_json['code'] && !empty($get_json['data']['payUrl'])) {
        ...
        $reallyPrice           = !empty($get_json['data']['reallyPrice']) ? zib_floatval_round($get_json['data']['reallyPrice']) : $order_data['order_price'];
        $result['order_price'] = $reallyPrice;
        return $result;
    }
    $msg = !empty($get_json['msg']) ? $get_json['msg'] : __('接口请求错误', 'zib_language');
...
function zibpay_initiate_epay($order_data)
{
    ...
    $config = zibpay_get_payconfig('epay');
    if (empty($config['apiurl']) || empty($config['partner']) || empty($config['key'])) {
        return array('error' => 1, 'msg' => __('易支付缺少配置参数', 'zib_language'));
    }
    ...
    if ($is_mobile || empty($config['qrcode'])) {
        ...
        if (isset($result['code']) && 1 == $result['code'] && (!empty($result['code_url']) || !empty($result['qrcode']))) {
            ...
            if (!empty($result['money'])) {
                $result['order_price'] = zib_floatval_round($result['money']);
            }
            ...
            $result['error'] = 1;
            $result['msg']   = !empty($result['msg']) ? $result['msg'] : __('收款码请求失败', 'zib_language');
        }
...
function zibpay_initiate_official_alipay($order_data = array())
{
    ...
    if (wp_is_mobile() && $config['h5'] && $config['webappid'] && $config['webprivatekey']) {
        if (empty($config['publickey'])) {
            return array('error' => 1, 'msg' => __('缺少支付宝公钥参数', 'zib_language'));
        }
        /**支付宝企业支付-手机网站支付产品 */
        // 公共配置
        $params        = new \Yurun\PaySDK\AlipayApp\Params\PublicParams;
        ...
        if (empty($data['sign'])) {
            return array('error' => 1, 'msg' => __('APPID或应用私钥错误，导致签名失败', 'zib_language'));
        }
        ...
        if (empty($config['privatekey']) || empty($config['appid'])) {
            return array('error' => 1, 'msg' => __('支付宝后台配置无效', 'zib_language'));
        }
        if (empty($config['publickey'])) {
            return array('error' => 1, 'msg' => __('缺少支付宝公钥参数', 'zib_language'));
        }
        ...
        try {
            ...
            if (strpos($e_msg, '同步返回数据验证失败') !== false) {
                $e_msg = __('同步返回数据验证失败，请检查支付宝公钥是否正确', 'zib_language');
            }
        ...
        if (!empty($data['alipay_trade_precreate_response']['qr_code'])) {
            ...
            $data['alipay_trade_precreate_response']['msg']        = __('处理完成，请扫码支付', 'zib_language');
            if (wp_is_mobile()) {
                $data['alipay_trade_precreate_response']['more_html'] = '<a href="' . esc_url($data['alipay_trade_precreate_response']['qr_code']) . '" class="but btn-block c-blue em09 padding-h10">' . __('跳转到支付宝APP付款', 'zib_language') . '</a>';
            }
...
function zibpay_initiate_official_wechat($order_data = array())
{
    ...
    $config = zibpay_get_payconfig('official_wechat');
    if (empty($config['merchantid']) || empty($config['appid']) || empty($config['key'])) {
        return array('error' => 1, 'msg' => __('微信支付后台配置无效', 'zib_language'));
    }
    ...
    if (wp_is_mobile() && $config['h5'] && !$zibpay_is_wechat_app) {
        ...
        $request->total_fee        = zibpay_settle_price_to_fen($order_data['order_price']); // 订单总金额，单位为：分
        $request->spbill_create_ip = !empty($order_data['ip_address']) ? $order_data['ip_address'] : zib_get_remote_ip_addr(); // 客户端ip，必须传正确的用户ip，否则会报错
...
function zibpay_initiate_xunhupay($order_data = array())
{
    ...
    if ('wechat' == $payment && empty($config['wechat_appid']) && empty($config['wechat_appsecret'])) {
        return array('error' => 1, 'msg' => __('未设置appid或者appsecret', 'zib_language'));
    }
...
function zibpay_initiate_payjs($order_data)
{
    ...
    if (empty($config['mchid']) || empty($config['key'])) {
        return array('error' => 1, 'msg' => __('未设置mchid或者key', 'zib_language'));
    }
    ...
    $data           = [
        'mchid'        => $mchid, //商户号
        'total_fee'    => zibpay_settle_price_to_fen($order_data['order_price']), //金额。单位：分
        'out_trade_no' => $order_data['order_num'], //本地订单号
    ...
    if (isset($result['return_code']) && isset($result['qrcode'])) {
        ...
        $result = array('error' => 1, 'result' => $result, 'msg' => !empty($result['return_msg']) ? $result['return_msg'] : __('PayJS收款接口连接失败', 'zib_language'));
    }
    return $result;
}
...
function zibpay_initiate_xhpay($order_data)
{
    ...
    $config = zibpay_get_payconfig('xhpay');
    if (empty($config['mchid']) || empty($config['key'])) {
        return array('error' => 1, 'msg' => __('未设置商户号或者API秘钥', 'zib_language'));
    }
...
function zibpay_initiate_codepay($order_data = array())
{
    ...
    if (empty($config['id']) || empty($config['key']) || empty($config['token'])) {
        return array('error' => 1, 'msg' => __('码支付配置错误', 'zib_language'));
    }
    ...
    if (isset($resultData['status']) && 0 == $resultData['status']) {
        //返回真实金额
        $money = !empty($resultData['money']) ? zib_floatval_round($resultData['money']) : $order_data['order_price'];
        return array('url_qrcode' => $resultData['qrcode'], 'order_price' => $money);
    }

    $msg = !empty($resultData['msg']) ? $resultData['msg'] : __('码支付接口请求错误', 'zib_language');
    return array('error' => 1, 'msg' => $msg);
}
...
function zibpay_initiate_balance($order_data = array())
{
    ...
    if (empty($order_data['user_id'])) {
        return array('error' => 1, 'msg' => __('请先登录', 'zib_language'));
    }
    ...
    if ($order_type && !zibpay_is_allow_balance_pay($order_type)) {
        return array('error' => 1, 'msg' => __('当前交易不支持余额支付', 'zib_language'));
    }
...
function zibpay_initiate_card_pass($order_data = array())
{
    ...
    if (!isset($zibpay_card_pass->id)) {
        return array('error' => 1, 'msg' => __('异常错误', 'zib_language'));
    }
    ...
    $msg = ($order_data['order_type'] ?? 0) == 8 ? sprintf(__('卡密充值成功，充值金额：%s', 'zib_language'), $order_data['order_price']) : __('卡密兑换成功', 'zib_language');
...
function zibpay_points_initiate_order()
{
    ...
    switch ($order_type) {
        case 4: //会员开通、升级、续费
            if (!isset($_REQUEST['product_id']) || $_REQUEST['product_id'] == 'null') {
                zib_send_json_error(__('请选择需要兑换的会员', 'zib_language'));
            }
            ...
            if (empty($lists_opt[$product_id]['points'])) {
                zib_send_json_error(__('会员商品不存在，请重新选择', 'zib_language'));
            }
        ...
        default: //文章类型的
            ...
            if (empty($post->ID) || empty($pay_mate['pay_type']) || 'no' == $pay_mate['pay_type'] || !zibpay_post_is_points_modo($pay_mate)) {
                zib_send_json_error(__('商品数据获取错误', 'zib_language'));
            }
            ...
            if ($__price <= 0) {
                zib_send_json_error(__('商品售价错误', 'zib_language'));
            }
    ...
    if ($__price > $user_points) {
        zib_send_json_error(__('积分不足，暂时无法支付', 'zib_language'));
    }
    ...
    if (!$order) {
        zib_send_json_error(__('订单创建失败', 'zib_language'));
    }
    ...
    zib_send_json_success(['reload' => true, 'msg' => __('购买成功', 'zib_language')]);
}
...
function zibpay_points_initiate_pay(array $order_data)
{
    ...
    if ($order_data['order_price'] > $user_points) {
        return ['error' => 1, 'msg' => __('积分不足，暂时无法支付', 'zib_language')];
    }
    ...
    if ($order_data['order_price'] > 0) {
        ...
        $update_points_data = array(
            'order_num' => $order_data['order_num'], //订单号
            'value'     => -$order_data['order_price'], //值 整数为加，负数为减去
            'type'      => __('积分支付', 'zib_language'), //类型说明
            'desc'      => $order_data['desc'] ?? __('购买商品', 'zib_language'), //说明
        );

        $update_points = zibpay_update_user_points($user_id, $update_points_data);

        if (!$update_points) {
            return ['error' => 1, 'msg' => __('支付失败，请检查积分是否足够，或刷新后重试', 'zib_language')];
        }
    }
    ...
    return ['error' => 0, 'msg' => __('支付成功', 'zib_language')];
}
