 ...
 * @LastEditTime : 2026-05-17 14:49:12
 * @Project      : Zibll子比主题
        ...
        case 9: //购买积分
            ...
            if ($payment_method === 'card_pass') {
                if (_pz('points_pass_exchange_s')) {
                    ...
                    if (empty($card_db->id)) {
                        zib_send_json_error(__('卡号或密码错误', 'zib_language'));
                    }

                    if ($card_db->status != '0') {
                        zib_send_json_error(__('该卡密已使用', 'zib_language'));
                    }

                    $card_price = zibpay_get_pass_exchange_points($card_db);

                    if (!$card_price) {
                        zib_send_json_error(__('当前卡密可兑换的积分为0', 'zib_language'));
                    }
                    ...
                    zib_send_json_error(__('该功能已关闭', 'zib_language'));
                }
            } else {
                ...
                if ($product_id === 'custom') {
                    //自定义数额
                    if ($custom_points <= 0) {
                        zib_send_json_error(__('请输入积分数额', 'zib_language'));
                    }
                    $custom_limit = zibpay_get_pay_points_product_custom_limit();
                    if (!empty($custom_limit['min']) && $custom_points < $custom_limit['min']) {
                        zib_send_json_error(sprintf(__('最低购买%s积分', 'zib_language'), $custom_limit['min']));
                    }
                    if (!empty($custom_limit['max']) && $custom_points > $custom_limit['max']) {
                        zib_send_json_error(sprintf(__('最高可购买%s积分', 'zib_language'), $custom_limit['max']));
                    }
                    ...
                    if (!isset($product[$product_id])) {
                        zib_send_json_error(__('商品信息错误，请刷新页面后重试', 'zib_language'));
                    }
        ...
        case 8: //余额充值
            ...
            if ($payment_method === 'card_pass') {
                if (_pz('pay_balance_pass_charge_s')) {
                    ...
                    if (!$card_price) {
                        zib_send_json_error(__('当前卡密可充值金额为0', 'zib_language'));
                    }
                ...
                if ($balance_product === 'custom') {
                    //自定义数额
                    if ($custom_price <= 0) {
                        zib_send_json_error(__('请输入充值金额', 'zib_language'));
                    }

                    $custom_limit = zibpay_get_pay_balance_product_custom_limit();
                    if (!empty($custom_limit['min']) && $custom_price < $custom_limit['min']) {
                        zib_send_json_error(sprintf(__('最低充值%s', 'zib_language'), $custom_limit['min']));
                    }
                    if (!empty($custom_limit['max']) && $custom_price > $custom_limit['max']) {
                        zib_send_json_error(sprintf(__('最高充值%s', 'zib_language'), $custom_limit['max']));
                    }
            ...
            if ($payment_method === 'card_pass') {
                if (_pz('pay_vip_pass_charge_s')) {
                    ...
                    if (!$vip_exchange_card_data['level'] && !$vip_exchange_card_data['time']) {
                        zib_send_json_error(__('当前卡密无法兑换会员', 'zib_language'));
                    }
                ...
                if (empty($vip_product_id[0]) || !isset($vip_product_id[1]) || !isset($vip_product_id[2])) {
                    zib_send_json_error(__('会员数据传入错误', 'zib_language'));
                }
                ...
                if (!_pz('pay_user_vip_' . $vip_level . '_s', true)) {
                    zib_send_json_error(__('暂未提供此功能', 'zib_language'));
                }
                ...
                if ('renewvip' == $vip_action) {
                    ...
                    if (!_pz('vip_opt', true, 'vip_upgrade')) {
                        zib_send_json_error(__('功能已关闭', 'zib_language'));
                    }

                    //当前会员信息判断
                    $user_vip_level = zib_get_user_vip_level($user_id);
                    if (!$user_vip_level || $user_vip_level >= $vip_level) {
                        zib_send_json_error(__('无法升级', 'zib_language'));
                    }
            ...
            if ($user_id) {
                ...
                if ($vip_level && _pz('pay_user_vip_' . $vip_level . '_s', true) && isset($pay_mate['vip_' . $vip_level . '_price'])) {
                    ...
                    if (!$pay_mate['vip_' . $vip_level . '_price']) {
                        zib_send_json_error(__('会员免费，请刷新页面', 'zib_language'), 'info');
                    }
