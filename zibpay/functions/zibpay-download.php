 ...
 * @LastEditTime : 2026-05-25 17:49:59
...
function zibpay_get_post_down_buts($pay_mate, $paid_type = 'pay', $post_id = '')
{
    if (empty($pay_mate['pay_download'])) {
        return '<div class="muted-2-color text-center">' . __('暂无可下载资源', 'zib_language') . '</div>';
    }
    ...
    if ($down) {
        foreach ($down as $key => $down_v) {
            ...
            $down_name      = !empty($down_v['name']) ? $down_v['name'] : __('本地下载', 'zib_language');
            $icon           = '<i class="fa fa-download" aria-hidden="true"></i>';
            $class          = 'b-theme';
            if (stripos($down_v['link'], 'weiyun.')) {
                $class .= ' weiyun';
                $down_name = $down_v['name'] ? $down_v['name'] : __('腾讯微云', 'zib_language');
                $icon      = zib_get_svg('weiyun', '0 0 1400 1024');
            } elseif (stripos($down_v['link'], 'baidu')) {
                $class .= ' baidu';
                $down_name = $down_v['name'] ? $down_v['name'] : __('百度网盘', 'zib_language');
                $icon      = zib_get_svg('pan_baidu');
            } elseif (stripos($down_v['link'], 'lanzou')) {
                $down_name = $down_v['name'] ? $down_v['name'] : __('蓝奏云', 'zib_language');
                $class .= ' lanzou';
                ...
                $down_name = $down_v['name'] ? $down_v['name'] : __('移动云盘', 'zib_language');
                $class .= ' b-blue';
                $icon = zib_get_svg('yidongyun');
            } elseif (stripos($down_v['link'], '.189.')) {
                $down_name = $down_v['name'] ? $down_v['name'] : __('天翼云', 'zib_language');
                $class .= ' tianyi';
                $icon = zib_get_svg('tianyi');
            } elseif (stripos($down_v['link'], 'ctfile')) {
                $down_name = $down_v['name'] ? $down_v['name'] : __('城通网盘', 'zib_language');
                $class .= ' ctfile';
                $icon = zib_get_svg('ctfile', '0 0 1260 1024');
            } elseif (stripos($down_v['link'], 'pan.xunlei')) {
                $down_name = $down_v['name'] ? $down_v['name'] : __('迅雷云盘', 'zib_language');
                $class .= ' b-blue';
                ...
                $down_name = $down_v['name'] ? $down_v['name'] : __('123云盘', 'zib_language');
                $class .= ' b-blue';
                $icon = zib_get_svg('123pan');
            } elseif (stripos($down_v['link'], 'alipan')) {
                $down_name = $down_v['name'] ? $down_v['name'] : __('阿里云盘', 'zib_language');
                $class .= ' alipan';
                $icon = zib_get_svg('alipan');
            } elseif (stripos($down_v['link'], 'quark')) {
                $down_name = $down_v['name'] ? $down_v['name'] : __('夸克网盘', 'zib_language');
                $class .= ' baidu';
                $icon = zib_get_svg('quark');
            } elseif (stripos($down_v['link'], 'yunpan')) {
                $down_name = $down_v['name'] ? $down_v['name'] : __('360云盘', 'zib_language');
                $class .= ' b-blue';
                $icon = zib_get_svg('360yunpan');
            } elseif (stripos($down_v['link'], 'uc.cn')) {
                $down_name = $down_v['name'] ? $down_v['name'] : __('UC网盘', 'zib_language');
                $class .= ' b-black';
            ...
            $copy_key  = !empty($down_v['copy_key']) ? $down_v['copy_key'] : (!empty($down_v['copy_val']) ? __('更多内容', 'zib_language') : '');
            ...
            if (!empty($down_v['copy_val'])) {
                ...
                $down_more = '<a href="javascript:;" ' . ($copy_key ? 'data-toggle="tooltip" title="' . esc_attr(sprintf(__('点击复制%s', 'zib_language'), $copy_key)) . '"' : '') . 'data-clipboard-tag="' . $copy_key . '" data-clipboard-text="' . $copy_val . '" class="but">' . $down_v['more'] . '</a>';
            }
    ...
    if ($user_id && stristr($paid_type, 'free')) {
        ...
        if ($download_limit) {
            ...
            if ($surplus <= 0) {
                ...
                $down_but = '<div class=""><span class="badg c-red btn-block">' . sprintf(__('您今日下载免费资源个数已超限，请明日再下载%s', 'zib_language'), ($is_allow_buy ? __('或购买此商品', 'zib_language') : '')) . '</span></div>';
                $down_but .= $is_allow_buy ? zibpay_get_post_cashier_link($post_id, 'but jb-red mt20', __('立即购买', 'zib_language'), array('price_type' => 'download_limit_over')) : '';
            } else {
                $_text    = $user_vip_level ? sprintf(__('您是尊贵的%s，', 'zib_language'), _pz('pay_user_vip_' . $user_vip_level . '_name')) : __('您', 'zib_language');
                $down_but = '<div class=""><span class="badg c-red btn-block">' . sprintf(__('%s今日还可下载%s个免费资源', 'zib_language'), $_text, $surplus) . '</span></div>' . $down_but;
            }
        }
        ...
        if ($download_limit || $download_limit_vip_1 || $download_limit_vip_2) {
            $download_limit_html = '<div class="muted-box mb10">';
            $download_limit_html .= '<div class="mb6">' . __('免费资源每日可下载：', 'zib_language') . '</div>';
            $download_limit_html .= $download_limit ? '<div class="mb6">' . sprintf(__('普通用户：%s', 'zib_language'), ($download_limit ? sprintf(__('%s个', 'zib_language'), $download_limit) : __('不限制', 'zib_language'))) . '</div>' : '';
            $download_limit_html .= $download_limit_vip_1 ? '<div class="mb6">' . _pz('pay_user_vip_1_name') . '：' . ($download_limit_vip_1 ? sprintf(__('%s个', 'zib_language'), $download_limit_vip_1) : __('不限制', 'zib_language')) . '</div>' : '';
            $download_limit_html .= $download_limit_vip_2 ? '<div class="">' . _pz('pay_user_vip_2_name') . '：' . ($download_limit_vip_2 ? sprintf(__('%s个', 'zib_language'), $download_limit_vip_2) : __('不限制', 'zib_language')) . '</div>' : '';
