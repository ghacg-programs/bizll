...
(function ($) {
    ...
    function init() {
        ...
        <div class="modal-dialog" role="document">\
            ...
            '/zibpay/assets/img/wechat-sys.png">\
                    ...
                    <div class="row-5">\
                    <div class="pay-qrcon">\
                        <button class="but cir hollow abs-center right-top nowave" data-dismiss="modal"><svg class="ic-close" aria-hidden="true"><use xlink:href="#icon-close"></use></svg></button>\
                        <div class="qrcon">\
                            <div class="pay-logo-header mb10 flex jc"><span class="pay-logo"></span><span class="pay-logo-name t-wechat">' + zib__('pay_alipay') + '</span><span class="pay-logo-name t-alipay">' + zib__('pay_wechat') + '</span><span class="pay-over-time em09"></span></div>\
                            <div class="pay-title em09 muted-2-color padding-h6"></div>\
                            <div><span class="em09 pay-settle-mark"></span><span class="pay-price em14"></span><span class="em09 pay-settle-unit"></span></div>\
                            <div class="pay-qrcode">\
                                <img src="" alt="pay-qrcode">\
                            </div>\
                        </div>\
                    <div class="pay-switch"></div>\
                    <div class="pay-notice"><div class="notice load">' + zib__('pay_order_creating') + '</div></div>\
                    </div>\
				</div>\
                </div>\
            </div>\
        ...
        _modal = $('#' + modal_id).modal({ backdrop: 'static', show: false, keyboard: false });

        //倒计时结束
        _modal.on('countdown-over', '.pay-over-time>*', countdownOver);
    }

    function countdownOver() {
        var msg = zib__('pay_order_timeout');
        _modal.find('.pay-qrcode img').css('filter', 'blur(4px)');
        qrcode_notice(msg, 'danger');
        notyf(msg, 'danger');
    ...
    function coupon_submit() {
        ...
        if (!ajax_data.coupon) {
            return notyf(zib__('pay_enter_coupon'), 'warning'), !1;
        }
        ...
        zib_ajax(
            ...
            function (n) {
                ...
                if (n.discount.type == 'subtract') {
                    discount_html = '<span class="muted-2-color">' + zib__('pay_discount_reduce') + '</span><span class="font-bold c-red em14">-' + n.discount.val + '</span>';
                    if (_actual_price) {
                        _actual_price = (_actual_price - n.discount.val).toFixed(2);
                    }
                } else if (n.discount.type == 'multiply') {
                    if (_actual_price) {
                        discount_html = '<span class="c-red">' + zib_sprintf('pay_discount_off', n.discount.val * 10) + '</span><span class="font-bold c-red em14">-' + parseFloat((_actual_price * (1 - n.discount.val)).toFixed(2)) + '</span>';
                        _actual_price = (_actual_price * n.discount.val).toFixed(2);
                    } else {
                        discount_html = '<span class="muted-2-color">' + zib__('pay_discount') + '</span><span class="font-bold c-red em14">' + zib_sprintf('pay_discount_fold', n.discount.val * 10) + '</span>';
                    }
                }

                discount_html = discount_html ? '<div class="discount-box flex jsb ab">' + discount_html + '</div>' : '';
                if (n.expire_time) {
                    discount_html += '<div class="coupon-time flex jsb ab"><span class="muted-2-color">' + zib__('pay_valid_until') + '</span><span class="muted-color">' + n.expire_time + '</span></div>';
                }

                var box_html = '<div class="mt10 coupon-box mb10 muted-box padding-h6 line-16"><div><span class="badg badg-sm mr6 jb-blue">' + zib__('pay_coupon_available') + '</span>' + (n.title ? '<span>' + n.title + '</span>' : '') + '</div>' + discount_html + '</div>';
    ...
    function ajax_send(data, _this) {
        data.openid && notyf(zib__('pay_initiating'), 'load', '', 'pay_ajax'); //微信JSAPI支付
        ...
        zib_ajax(
            ...
            function (n) {
                ...
                if (n.float_btn) {
                    ...
                    if (float_box.length) {
                        float_box.prop('outerHTML', n.float_btn);
                    } else {
                        _body.append(n.float_btn);
                    }
                }
                ...
                if (n.error) {
                    if (n.modal) {
                        ...
                        $('.shop-confirm-modal.in,#modal_pay_uservip.in').modal('hide');
                    }
                    return;
                }
                ...
                if (n.jsapiParams) {
                    ...
                    notyf(zib__('pay_complete'), '', '', data.openid ? 'pay_ajax' : '');
                    return;
                }
                ...
                if (n.url_qrcode) {
                    ...
                    qrcode_notice(zib__('pay_scan_qrcode'), '');
                    ...
                    var settleMark = n.settle_mark || _win.pay_mark || zib__('currency_yuan') || '￥';
                    var settlePrice = n.settle_price || n.order_price || '';
                    var settleUnit = n.settle_unit || '';
                    _modal.find('.pay-settle-mark').html(settleMark);
                    _modal.find('.pay-price').html(settlePrice);
                    _modal.find('.pay-settle-unit').html(settleUnit);
                    n.payment_method && _modal.find('.pay-payment').removeClass('wechat alipay').addClass(n.payment_method);
                    n.over_time && _modal.find('.pay-over-time').html('<span class="ml6 badg badg-sm" int-second="1" max-unit="minute" data-over-text="' + zib__('pay_trade_closed') + '" data-countdown="' + n.over_time + '"><span class="minute"></span>:<span class="second"></span></span>');
    ...
    function verify_pay() {
        if (order_result.order_num) {
            $.ajax({
                ...
                success: function (n) {
                    if (n.status == '1') {
                        qrcode_notice(zib__('pay_success_redirect'), 'success');
                        setTimeout(function () {
    ...
    function vip_pay() {
        ...
        var _modal =
            '<div class="modal fade flex jc' +
            (is_mobile ? ' bottom' : '') +
            '" id="modal_pay_uservip" tabindex="-1" role="dialog" aria-hidden="false">\
    <div class="modal-dialog full-sm" role="document">' +
            (is_mobile ? ' <div class="touch-close"></div>' : '') +
            '\
    ...
    <div class="modal-body"><h4 style="padding:20px;" class="text-center"><i class="loading zts em2x"></i></h4></div>\
        ...
        if (!$('#modal_pay_uservip').length) {
            _body.append(_modal);
            if (is_mobile) {
                $('#modal_pay_uservip').minitouch({
                    direction: 'bottom',
                    selector: '.modal-dialog',
                    start_selector: '.modal-colorful-header,.touch-close,.touch',
                    onEnd: function () {
                        $('#modal_pay_uservip').modal('hide');
                    },
                });
            }
        ...
        if (modal.find('.payvip-modal').length) {
            ...
            notyf(zib__('pay_vip_loading'), 'load', '', 'payvip_ajax');
            $.ajax({
                type: 'POST',
                url: pay_ajax_url,
                data: {
                ...
                dataType: 'json',
                success: function (n) {
                    var msg = n.msg || zib__('pay_select_vip_option');
                    if (msg.indexOf('登录') != -1) {
                        modal.remove();
