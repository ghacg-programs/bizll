 ...
 * @LastEditTime : 2026-06-18 12:03:06
...
function zib_get_repas_link($is_link = false, $class = 'muted-2-color', $text = null)
{
    if (null === $text) {
        $text = __('找回密码', 'zib_language');
    }
...
function zib_user_signtab_content($tab = 'signin', $is_page = false, $echo = true)
{
    ...
    if ($signin_form) {
        ...
        $tab_html .= '<div class="title-h-left fa-2x">' . __('登录', 'zib_language') . '</div>';
        $tab_html .= $signup_form ? '<a class="muted-color px12" href="#tab-sign-up" data-toggle="tab">' . __('没有账号？立即注册', 'zib_language') . '<i class="em12 ml3 fa fa-angle-right"></i></a>' : '';
        $tab_html .= '</div>';
    ...
    if ($signup_form) {
        ...
        $tab_html .= '<div class="box-body">';
        $tab_html .= '<div class="title-h-left fa-2x">' . __('注册', 'zib_language') . '</div>';
        $tab_html .= $signin_form ? '<a class="muted-color px12" href="#tab-sign-in" data-toggle="tab">' . __('已有账号，立即登录', 'zib_language') . '<i class="em12 ml3 fa fa-angle-right"></i></a>' : '';
        $tab_html .= '</div>';
    ...
    if ($is_page) {
        ...
        $tab_html .= '<div class="box-body">';
        $tab_html .= '<div class="title-h-left fa-2x">' . __('找回密码', 'zib_language') . '</div>';
        $tab_html .= '<a class="muted-color px12" href="#tab-sign-in" data-toggle="tab">' . __('登录', 'zib_language') . '</a><i class="icon-spot"></i><a class="muted-color px12" href="#tab-sign-up" data-toggle="tab">' . __('注册', 'zib_language') . '</a>';
        $tab_html .= '</div>';
    ...
    if (zib_is_oauth_qrcode_s()) {
        ...
        $tab_html .= '<div class="box-body">';
        $tab_html .= '<div class="title-h-left fa-2x">' . __('扫码登录', 'zib_language') . '</div>';
        $tab_html .= '<span class="muted-3-color px12">' . __('使用', 'zib_language') . '<a class="muted-color" href="#tab-sign-in" data-toggle="tab">' . __('其它方式登录', 'zib_language') . '</a>' . ($signup_form ? __('或', 'zib_language') . '<a class="muted-color" href="#tab-sign-up" data-toggle="tab">' . __('注册', 'zib_language') . '</a>' : '') . '</span>';
        ...
        $tab_html .= '</div>';
        $agreement = zib_get_user_agreement(__('扫码登录即表示同意', 'zib_language'));
        if ($agreement) {
...
function zib_get_img_verification_input($id = 'img_verification', $name = 'canvas_yz')
{
    ...
    $input .= '<div class="scale-placeholder">' . __('图形验证码', 'zib_language') . '</div>';
    $input .= '<span class="yztx abs-right pointer imagecaptcha" imagecaptcha-id="' . $id . '" data-toggle="tooltip" title="' . esc_attr__('点击刷新', 'zib_language') . '"></span>';
...
function zib_resetpassword_form($type = '')
{
    ...
    $input .= '<div class="scale-placeholder">' . __('设置新密码', 'zib_language') . '</div>';
    ...
    $input .= '<div class="scale-placeholder">' . __('重复密码', 'zib_language') . '</div>';
    $input .= '<div class="abs-right passw muted-2-color"><i class="fa-fw fa fa-eye"></i></div><i class="line-form-line"></i>';
    $input .= '</div>';
    //按钮
    ...
    $input .= '<button type="button" class="but radius jb-green padding-lg signsubmit-loader btn-block">' . __('确认提交', 'zib_language') . '</button>';
...
function zib_signup_form($oauth_bind = false)
{
    ...
    $input .= '<div class="scale-placeholder">' . __('设置用户名', 'zib_language') . '</div>';
    $input .= '</div>';
    ...
    $input .= '<div class="relative line-form mb10">';
    $input .= '<input type="password" name="password2" class="line-form-input" tabindex="3" placeholder="">';
    $input .= '<div class="scale-placeholder">' . __('设置密码', 'zib_language') . '</div>';
    $input .= '<div class="abs-right passw muted-2-color"><i class="fa-fw fa fa-eye"></i></div><i class="line-form-line"></i>';
    $input .= '</div>';
    ...
    if ($invit_code === 'open') {
        ...
        $input .= '<div class="scale-placeholder">' . __('邀请码(选填)', 'zib_language') . '</div>';
    ...
    $input .= '<button type="button" class="but radius jb-green padding-lg signsubmit-loader btn-block">' . zib_get_svg('signup', '0 0 1024 1024', 'icon mr10') . ($oauth_bind ? __('绑定', 'zib_language') : '') . __('注册', 'zib_language') . '</button>';

    $agreement = zib_get_user_agreement(__('注册即表示同意', 'zib_language'));
    if ($agreement) {
        if (_pz('user_agreement_check')) {
            $input .= zib_get_agreement_input(__('已阅读并同意', 'zib_language'), 'muted-color mt10 text-center px12 opacity8', false);
        } else {
    ...
    //必须输入邀请码，才能注册
    if ($invit_code === 'must') {
        $invit_code_desc = _pz('invit_code_must_desc') ?: __('请输入邀请码', 'zib_language');
...
function zib_get_user_invit_code_must_verify_from()
{
    ...
    $input .= '<div class="scale-placeholder">' . __('请填写邀请码', 'zib_language') . '</div>';
    $input .= wp_nonce_field($action, '_wpnonce', false, false); //安全效验
    $input .= '</div>';
    $input .= '</div>';

    $input .= '<input type="hidden" name="action" value="' . $action . '">';
    $input .= '<div class="box-body"><button type="button" class="but radius jb-blue padding-lg btn-block wp-ajax-submit invit-code-verify" next-tab="tab-signup-signup" tabindex="2">' . __('继续', 'zib_language') . '<i style="margin: 0 0 0 .5em;" class="em12 fa fa-angle-right"></i></button></div>';
...
function zib_signin_form($oauth_bind = false)
{
    ...
    if (!zib_is_close_signin() || $oauth_bind) {
        ...
        if (!$nopas_s || $nopas_active_type !== 'only_nopas') {
            //用户名
            $name_placeholder = $phone_s ? __('用户名/手机号/邮箱', 'zib_language') : __('用户名或邮箱', 'zib_language');
            $input .= '<div class="relative line-form mb10">';
            ...
            $input .= '<div class="scale-placeholder">' . __('登录密码', 'zib_language') . '</div>';
            $input .= '<div class="abs-right passw muted-2-color"><i class="fa-fw fa fa-eye"></i></div><i class="line-form-line"></i>';
            $input .= '</div>';
            ...
            $input .= '<span class="muted-color form-checkbox"><input type="checkbox" id="remember" checked="checked" tabindex="4" name="remember" value="forever"><label for="remember" class="ml3">' . __('记住登录', 'zib_language') . '</label></span>';
            ...
            //免密登录
            if ($nopas_s) {
                $input .= '<span class="opacity5"> | </span><a class="muted-2-color" data-toggle="tab" href="#tab-signin-nopas">' . __('免密登录', 'zib_language') . '</a> ';
            }
            $input .= '</span>';
            $input .= '</div>';
            ...
            $input .= '<button type="button" class="but radius jb-blue padding-lg signsubmit-loader btn-block"><i class="fa fa-sign-in mr10"></i>' . ($oauth_bind ? __('绑定', 'zib_language') : '') . __('登录', 'zib_language') . '</button>';
            $input .= '</div>';
        ...
        if ($nopas_s) {
            ...
            $nopas_input .= '<span class="muted-color form-checkbox"><input type="checkbox" id="remember2" checked="checked" tabindex="4" name="remember" value="forever"><label for="remember2" class="ml3">' . __('记住登录', 'zib_language') . '</label></span>';

            //找回密码
            $nopas_input .= '<span class="pull-right muted-2-color">';
            //免密登录
            $nopas_input .= $nopas_active_type !== 'only_nopas' ? '<a class="muted-2-color" data-toggle="tab" href="#tab-signin-pas">' . __('账号密码登录', 'zib_language') . '</a> ' : '';
            $nopas_input .= '</span>';
            $nopas_input .= '</div>';
            ...
            $nopas_input .= '<button type="button" class="but radius jb-blue padding-lg signsubmit-loader btn-block"><i class="fa fa-sign-in mr10"></i>' . ($oauth_bind ? __('绑定', 'zib_language') : '') . __('登录', 'zib_language') . '</button>';
            $nopas_input .= '</div>';
    ...
    if (!$oauth_bind) {
        ...
        if ($social_login) {
            $html .= '<p class="social-separator separator muted-3-color em09">' . __('社交账号登录', 'zib_language') . '</p>';
            $html .= '<div class="social_loginbar">';
            $html .= $social_login;
            $html .= '</div>';
            $agreement = zib_get_user_agreement(__('使用社交账号登录即表示同意', 'zib_language'));
            if ($agreement) {
...
function zib_get_sign_captch($captcha_type = 'email', $action = 'signup_captcha')
{
    ...
    $input_placeholder = __('邮箱', 'zib_language');
    if ('phone' == $captcha_type) {
        $input_placeholder = __('手机号', 'zib_language');
    } elseif ('email_phone' == $captcha_type) {
        $input_placeholder = __('手机号或邮箱', 'zib_language');
    }
    $input = '';
    $input .= '<div class="relative line-form mb10">';
    ...
    $input .= '<div class="scale-placeholder">' . __('验证码', 'zib_language') . '</div>';
    $input .= '<span class="yztx abs-right"><button type="button" form-action="' . $action . '" class="but c-blue captchsubmit">' . __('发送验证码', 'zib_language') . '</button></span>';
...
function zib_signout_modal()
{
    ...
    $args = array(
        ...
        'title'           => __('退出登录', 'zib_language'),
        'content'         => '<div class="ml10">
            <h4>' . sprintf(__('您好！ %s', 'zib_language'), $current_user->display_name) . '</h4><p class="c-red">' . __('确认要退出当前登录吗？', 'zib_language') . '</p></div>',
...
function zib_get_user_agreement($before = '', $glue = '、')
{
    ...
    if ($user_agreement_s) {
        $agreement_args[] = '<a class="focus-color" target="_blank" href="' . get_permalink(_pz('user_agreement_page')) . '">' . __('用户协议', 'zib_language') . '</a>';
    }

    $user_privacy_s = _pz('user_privacy_s');
    if ($user_privacy_s) {
        $agreement_args[] = '<a class="focus-color" target="_blank" href="' . get_permalink(_pz('user_privacy_page')) . '">' . __('隐私声明', 'zib_language') . '</a>';
    }
 ...
 * @return {*}
 */
function zib_get_agreement_input($before = null, $class = 'muted-color mb20', $checked = true)
{
    if (null === $before) {
        $before = __('已阅读并同意', 'zib_language');
    }
...
function zib_get_user_verify_from($type = 'email')
{
    ...
    $name = array(
        'email' => __('邮箱', 'zib_language'),
        'phone' => __('手机', 'zib_language'),
    );
    ...
    $input .= '<div class="scale-placeholder">' . __('验证码', 'zib_language') . '</div>';
    $input .= '<span class="yztx abs-right"><button type="button" form-action="' . $action . '_captcha" class="but c-blue captchsubmit">' . __('发送验证码', 'zib_language') . '</button></span>';
    $input .= '<div class="abs-right match-ok muted-color"><i class="fa-fw fa fa-check-circle"></i></div>';
    ...
    $input .= '<input type="hidden" name="action" value="' . $action . '">';
    $input .= '<button type="button" class="but jb-blue padding-lg btn-block user-verify-submit" next-tab="" tabindex="2"><i class="fa fa-shield"></i> ' . __('立即验证', 'zib_language') . '</button>';
...
function zib_get_user_bind_from($type = 'email', $user_id = 0)
{
    ...
    if ('email' == $type && !_pz('user_bind_option', true, 'email_set_captch')) {
        ...
        $input .= '<div class="scale-placeholder">' . __('请输入邮箱', 'zib_language') . '</div>';
        $input .= '</div>';
    ...
    $input .= '<button type="button" class="but jb-blue padding-lg btn-block signsubmit-loader"><i class="fa fa-check"></i> ' . __('确认提交', 'zib_language') . '</button>';
...
function zib_get_user_center_bind_tab($tab = 'email', $user_id = '')
{
    ...
    $tab_html .= '<a href="#tab-bind-email" data-toggle="tab">' . __('修改邮箱', 'zib_language') . '</a>';
    $tab_html .= '<a href="#tab-change-password" data-toggle="tab">' . __('修改密码', 'zib_language') . '</a>';
    $tab_html .= '<a href="#tab-bind-phone" data-toggle="tab">' . __('绑定手机', 'zib_language') . '</a>';
    $tab_html .= '</div>';

    //email
    $user_email      = $udata->user_email;
    $title           = $user_email ? __('修改邮箱', 'zib_language') : __('绑定邮箱', 'zib_language');
    ...
    if ($user_email && _pz('user_bind_option', true, 'email_set_captch')) {
        ...
        $bind_email_html .= '<li class="active"><a href="#tab-bind-email-verify" data-toggle="tab">' . __('验证老邮箱', 'zib_language') . '</a></li>';
        $bind_email_html .= '<li><span>' . __('验证新邮箱', 'zib_language') . '</span><a href="#tab-bind-email-bind" data-toggle="tab" class="hide"></a></li>';
        $bind_email_html .= '<li><span>' . __('修改成功', 'zib_language') . '</span></li>';
        $bind_email_html .= '</ul>';

        $bind_email_html .= '<div class="tab-pane fade active in" id="tab-bind-email-verify">';
        $bind_email_html .= '<p class="muted-2-color">' . sprintf(__('请在下方获取验证码以验证您的老邮箱%s', 'zib_language'), '<span class="badg">' . $user_email . '</span>') . '</p>';
        $bind_email_html .= zib_get_user_verify_from('email', $user_email);
        $bind_email_html .= '</div>';

        $bind_email_html .= '<div class="tab-pane fade" id="tab-bind-email-bind">';
        $bind_email_html .= '<p class="muted-2-color">' . __('请输入您需要修改的新邮箱账号', 'zib_language') . '</p>';
    ...
    if (_pz('user_bind_option', false, 'bind_phone')) {
        ...
        $title = $phone ? __('修改手机', 'zib_language') : __('绑定手机', 'zib_language');
        ...
        if ($phone) {
            ...
            $bind_phone_html .= '<li class="active"><a href="#tab-bind-phone-verify" data-toggle="tab">' . __('验证老手机', 'zib_language') . '</a></li>';
            $bind_phone_html .= '<li><span>' . __('验证新手机', 'zib_language') . '</span><a href="#tab-bind-phone-bind" data-toggle="tab" class="hide"></a></li>';
            $bind_phone_html .= '<li><span>' . __('修改成功', 'zib_language') . '</span></li>';
            $bind_phone_html .= '</ul>';

            $bind_phone_html .= '<div class="tab-pane fade active in" id="tab-bind-phone-verify">';
            $bind_phone_html .= '<p class="muted-2-color">' . sprintf(__('请在下方获取验证码以验证您的老手机号%s', 'zib_language'), '<br/><span class="badg">' . $phone . '</span>') . '</p>';
            $bind_phone_html .= zib_get_user_verify_from('phone');
            $bind_phone_html .= '</div>';

            $bind_phone_html .= '<div class="tab-pane fade" id="tab-bind-phone-bind">';
            $bind_phone_html .= '<p class="muted-2-color">' . __('请输入您需要修改的新手机号码', 'zib_language') . '</p>';
    ...
    $title = $oauth_new ? __('设置密码', 'zib_language') : __('修改密码', 'zib_language');
    ...
    if (!$oauth_new) {
        ...
        $tab_html .= '<div class="scale-placeholder">' . __('请输入原密码', 'zib_language') . '</div>';
    ...
    $tab_html .= '<div class="scale-placeholder">' . __('请输入新密码', 'zib_language') . '</div>';
    $tab_html .= '<div class="abs-right passw muted-2-color"><i class="fa-fw fa fa-eye"></i></div><i class="line-form-line"></i>';
    $tab_html .= '</div>';
    $tab_html .= '<div class="relative line-form mb10">';
    $tab_html .= '<input type="password" name="password2" class="line-form-input" tabindex="3" placeholder="">';
    $tab_html .= '<div class="scale-placeholder">' . __('请再次输入新密码', 'zib_language') . '</div>';
    $tab_html .= '<div class="abs-right passw muted-2-color"><i class="fa-fw fa fa-eye"></i></div><i class="line-form-line"></i>';
    $tab_html .= '</div>';
    ...
    $tab_html .= '<button type="button" class="but jb-blue padding-lg btn-block signsubmit-loader"><i class="fa fa-check"></i> ' . __('确认提交', 'zib_language') . '</button>';
    $tab_html .= '</form>';
    if (!$oauth_new && ($user_email || $phone)) {
        $tab_html .= '<div class="text-right mt6">';
        $tab_html .= '<a data-toggle="tab" href="#tab-reset-password" class="em09 muted-2-color">' . __('忘记原密码？点击重设密码', 'zib_language') . '</a>';
        $tab_html .= '</div>';
    }
    $tab_html .= '</div>';
    ...
    if (!$oauth_new && ($user_email || $phone)) {
        ...
        $tab_html .= zib_get_modal_colorful_header('jb-yellow', '<i class="fa fa-unlock-alt"></i>', __('重设密码', 'zib_language'));
        $tab_html .= '<div class="text-riht mb10 ml10">';
        $tab_html .= '<a data-toggle="tab" href="#tab-change-password" class="em09 but p2-10"><i class="fa fa-angle-left em12"></i> ' . __('返回使用原密码修改密码', 'zib_language') . '</a>';
        $tab_html .= '</div>';
        $tab_html .= zib_resetpassword_form();
        $tab_html .= '</div>';
    }
 ...
 * @return {*}
 */
function zib_get_user_home_link($id, $class = '', $text = null)
{
    if (null === $text) {
        $text = __('个人主页', 'zib_language');
    }
...
function zib_get_post_user_box($user_id, $desc = '', $class = '')
{
    ...
    $Private_but  = Zib_Private::get_but($user_id, zib_get_svg('private') . __('私信', 'zib_language'), 'ml6 but c-blue px12-sm'); //私信
...
//获取用户头像
function zib_get_avatar_box($id_or_email, $class = 'avatar-img', $link = true, $vip = true)
{
    $user_id    = zib_get_avatar_user_id($id_or_email);
    $avatar_img = zib_get_data_avatar($id_or_email);
    $vip_icon   = '';
    if ($vip && $user_id) {
