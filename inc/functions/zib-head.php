 ...
 * @LastEditTime : 2026-05-18 18:32:54
...
function zib_head_css_layout_bg()
{
    //全局背景图片
    $styles = '';
    if (is_page()) {
        global $post;
        $post_id   = $post->ID;
        $layout_bg = zib_get_post_meta($post_id, 'layout_bg', true);

        if (!empty($layout_bg['img_white']['background-color'])) {
            $styles .= 'body{background-color: ' . $layout_bg['img_white']['background-color']. ';}';
        }
        if (!empty($layout_bg['img_white']['background-image'])) {
            $styles .= 'body{
                background-image: url("' . $layout_bg['img_white']['background-image'] . '");
                ' . ($layout_bg['img_white']['background-position'] ? 'background-position: ' . $layout_bg['img_white']['background-position'] . ';' : '') . '
                ' . ($layout_bg['img_white']['background-repeat'] ? 'background-repeat: ' . $layout_bg['img_white']['background-repeat'] . ';' : '') . '
                ' . ($layout_bg['img_white']['background-attachment'] ? 'background-attachment: ' . $layout_bg['img_white']['background-attachment'] . ';' : '') . '
                ' . ($layout_bg['img_white']['background-size'] ? 'background-size: ' . $layout_bg['img_white']['background-size'] . ';' : '') . '
                ' . ($layout_bg['img_white']['background-origin'] ? 'background-origin: ' . $layout_bg['img_white']['background-origin'] . ';' : '') . '
            }';
        }

        if (!empty($layout_bg['img_dark']['background-color'])) {
            $styles .= 'body.dark-theme{background-color: ' . $layout_bg['img_dark']['background-color'] . ';}';
        }

        if (!empty($layout_bg['img_dark']['background-image'])) {
            $styles .= 'body.dark-theme{
                background-image: url("' . $layout_bg['img_dark']['background-image'] . '");
                ' . ($layout_bg['img_dark']['background-position'] ? 'background-position: ' . $layout_bg['img_dark']['background-position'] . ';' : '') . '
                ' . ($layout_bg['img_dark']['background-repeat'] ? 'background-repeat: ' . $layout_bg['img_dark']['background-repeat'] . ';' : '') . '
                ' . ($layout_bg['img_dark']['background-attachment'] ? 'background-attachment: ' . $layout_bg['img_dark']['background-attachment'] . ';' : '') . '
                ' . ($layout_bg['img_dark']['background-size'] ? 'background-size: ' . $layout_bg['img_dark']['background-size'] . ';' : '') . '
                ' . ($layout_bg['img_dark']['background-origin'] ? 'background-origin: ' . $layout_bg['img_dark']['background-origin'] . ';' : '') . '
            }';
        }
    }
    ...
    if (!empty($theme_img_bg['background-color'])) {
        $styles .= 'body{background-color: ' . $theme_img_bg['background-color'] . ';}';
    }
    ...
    if (!empty($theme_img_bg['background-color'])) {
        $styles .= '.dark-theme{background-color: ' . $theme_img_bg['background-color'] . ';}';
    }
    if (!empty($theme_img_bg['background-image'])) {
        $styles .= '.dark-theme{
               background-image: url("' . $theme_img_bg['background-image'] . '");
...
function zib_title($echo = true)
{
    ...
    if (is_home()) {
        if (_pz('hometitle')) {
            $html = _pz('hometitle');
            if ($paged > 1) {
                $html .= _get_delimiter() . __('最新发布', 'zib_language');
            }
        } else {
            if ($paged > 1) {
                $html .= _get_delimiter() . __('最新发布', 'zib_language');
    ...
    if ($paged > 1) {
        $html .= _get_delimiter() . sprintf(__('第%s页', 'zib_language'), $paged);
    }
    if ($echo) {
...
function zib_description($echo = true)
{
    ...
    if (is_singular()) {
        ...
        $description = sprintf(__('%s: \'%s\' 的搜索结果', 'zib_language'), $blog_name, $s);
    } else {
        $description = $blog_name . "'" . trim(wp_title('', false)) . "'";
    }
