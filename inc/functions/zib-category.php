 ...
 * @LastEditTime : 2026-05-22 17:49:49
...
function zib_get_term_search_btn($term_id, $class = '', $text = '')
{
    ...
    $args = array(
        ...
        'type'        => 'post',
        'placeholder' => __('在', 'zib_language') . zib_get_taxonomy_name($term->taxonomy) . '[' . $term->name . ']' . __('中搜索', 'zib_language'),
    );
...
function zib_topics_cover($cat_id = '')
{
    ...
    if (_pz('topics_post_count_s', false) && $wp_query->get('paged') <= 1) {
        ...
        $title .= '<span class="icon-spot">' . sprintf(__('共%s篇', 'zib_language'), $count) . '</span>';
    }
...
function zib_get_cat_allow_view_btn($cat_id, $class = '', $text = '')
{
    ...
    $title = '<i class="fa fa-unlock-alt mr6"></i>' . __('文章阅读限制', 'zib_language');

    return '<a class="' . $class . '" href="javascript:;"  data-html="1" title="' . esc_attr(strip_tags($title)) . '" data-content="' . esc_attr($con) . '" data-trigger="hover" data-placement="auto bottom" data-container="body" data-toggle="popover">' . $text . '</a>';
...
function zib_page_cover($title, $img, $desc, $more = '', $center = false)
{
    ...
    $attr  = '';
    if ($paged && $paged > 1) {
        $title .= ' <small class="icon-spot">' . sprintf(__('第%s页', 'zib_language'), $paged) . '</small>';
    } else {
...
function zib_ajax_option_menu($page = 'home', $class = 'ajax-option ajax-replace', $link_class = 'ajax-next', $attr = 'win-ajax-replace="filter"')
{
    ...
    if ($page_args[$page]['cat']) {
        $con .= zib_get_option_terms_but($page_args[$page]['cat_option'], $link_class, __('分类', 'zib_language'));
    }
    if ($page_args[$page]['topics']) {
        $con .= zib_get_option_terms_but($page_args[$page]['topics_option'], $link_class, __('专题', 'zib_language'));
    }
    if ($page_args[$page]['tag']) {
        $con .= zib_get_option_terms_but($page_args[$page]['tag_option'], $link_class, __('标签', 'zib_language'));
    }
...
function zib_get_option_orderby_but($option = array(), $link_class = 'ajax-next')
{
    ...
    $d_but       = $is_dropdown ? '<a href="javascript:;" data-toggle="dropdown"><span name="cat">' . __('排序', 'zib_language') . '</span><i class="fa fa-fw fa-sort opacity5" aria-hidden="true"></i></a>' : __('排序', 'zib_language');
...
function zib_get_option_terms_but($option = array(), $link_class = 'ajax-next', $text = null)
{
    if (null === $text) {
        $text = __('分类', 'zib_language');
    }
    $defaults = array(
        'lists'          => array(),
        'dropdown'       => false,
        ...
        foreach ($lists as $term) {
            ...
            if (in_array($term->term_id, $this_id_s)) {
                ...
                if (!empty($children_ibj[$term->term_id])) {
                    ...
                    $child_name = array(
                        'category' => __('子分类', 'zib_language'),
                        'topics'   => __('子专题', 'zib_language'),
                        'post_tag' => __('子标签', 'zib_language'),
                    )[$_object->taxonomy];
                }
            }
...
function zib_get_option_custom_filter_but($link_class = 'ajax-next', $filters = 'all')
{
    ...
    foreach ($filter_args as $filter) {
        ...
        if ($filters === 'all' || (is_array($filters) && in_array($filter['key'], $filters))) {
            ...
            if ($is_focus) {
                $vals_but = '<a rel="nofollow" ajax-replace="true" class="' . $link_class . '" href="' . add_query_arg($filter['key'], false, $uri) . '">' . __('全部', 'zib_language') . '</a>' . $vals_but;
            }
        }
        if ($vals_but) {
            $html .= '<div class="flex ac">';
...
function zib_term_aggregation($args = array(), $echo = false)
{
    ...
    if (!$description && is_super_admin()) {
        $description = sprintf(__('请在Wordress后台-文章-文章%s中添加描述！', 'zib_language'), $but_name) . zib_get_term_admin_edit(__('立即编辑', 'zib_language'), $term);
    }
    ...
    $meta       = '<sapn class="px12 muted-3-color text-ellipsis"><i class="fa fa-file-text-o fa-fw"></i>' . sprintf(__('%s篇文章', 'zib_language'), $count) . '</sapn>';
    $meta .= '<a class="but p2-10 px12 c-blue hide-sm shrink0"' . $target_blank . ' href="' . $href . '"><i class="fa fa-angle-right"></i>' . __('更多文章', 'zib_language') . '</a>';
...
function zib_get_taxonomy_name($taxonomy)
{
    $taxonomy_names = array(
        ...
        'forum_topic'   => __('话题', 'zib_language'),
...
function zib_get_post_single_cat_allow_view($cat_id)
{
    ...
    switch ($data['type']) {
        ...
        case 'follow':
            $data['html'] = '<p class="separator muted-3-color em09">' . __('收藏后可查看', 'zib_language') . '</p>';
            break;
        ...
        case 'roles':
            ...
            if (!$user_id) {
                ...
                if (!$data['allow']) {
                    $title = __('此分类内容已隐藏', 'zib_language');
                    $roles = '';
                    $roles .= $vip ? $vip : '';
                    $roles .= $level ? $level : '';
                    $roles .= $auth ? $auth : '';
