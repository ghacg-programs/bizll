 ...
 * @LastEditTime : 2026-05-18 23:10:47
...
function zib_get_post_more_dropdown($post, $class = '', $con_class = 'but cir post-drop-meta', $con = '', $direction = 'down')
{
    ...
    $user_ban = zib_get_edit_user_ban_link($post->post_author, '', '<i class="fa fa-ban mr6 fa-fw c-red"></i>' . __('禁封用户', 'zib_language'));
    if (!$user_ban) {
        $user_ban = zib_get_report_link($post->post_author, get_permalink($post), '', '<i class="fa fa-exclamation-triangle mr6 fa-fw c-red"></i>' . __('举报内容', 'zib_language'));
    }
    $action .= $user_ban ? '<li>' . $user_ban . '</li>' : '';

    //编辑
    $edit = zib_get_post_edit_link($post, '', '<i class="fa fa-fw fa-edit mr6"></i>' . __('编辑文章', 'zib_language'));
    $action .= $edit ? '<li>' . $edit . '</li>' : '';

    //删除
    $del = zib_bbs_post_delete_link($post, 'c-red', '<i class="fa fa-trash-o mr6 fa-fw"></i>' . __('删除文章', 'zib_language'));
 ...
 * @return {*}
 */
function zib_get_post_edit_link($post = null, $class = 'but c-blue', $con = null)
{
    if (null === $con) {
        $con = __('编辑文章', 'zib_language');
    }
 ...
 */
function zib_bbs_post_delete_link($post = null, $class = '', $con = null, $tag = 'a')
{
    if (null === $con) {
...
function zib_get_query_mate_orderby_keys()
{
    return apply_filters('query_mate_orderby_keys', array(
        'value_num' => array('sales_count','followed-user-count', 'score', 'plate_id', 'posts_count', 'reply_count', 'today_reply_count', 'follow_count', 'follow', 'views', 'like', 'favorite', 'favorite_count', 'zibpay_price', 'price', 'zibpay_points_price', 'sales_volume', 'balance', 'points', 'phone_number', 'vip_level', 'level', 'referrer_id'),
