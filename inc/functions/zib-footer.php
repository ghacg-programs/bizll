...
function zib_win_var()
{
    ...
    <script type="text/javascript">
        window._win = {
            ...
            pay_mark: '<?php echo esc_js(function_exists('zibpay_get_pay_mark') ? zibpay_get_pay_mark() : (_pz('pay_mark') ?: '￥')); ?>',
            pay_currency_unit: '<?php echo esc_js(function_exists('zibpay_get_currency_unit') ? zibpay_get_currency_unit() : __('元', 'zib_language')); ?>',
            ...
            ajax_trigger: '<?php echo esc_js(_pz('ajax_trigger', '加载更多')); ?>',
            ajax_nomore: '<?php echo esc_js(_pz('ajax_nomore', '没有更多了')); ?>',
            qj_loading: '<?php echo esc_js(_pz('qj_loading', '加载中...')); ?>',
...
function zib_float_right()
{
    ...
    foreach ($option as $key => $opt) {
        ...
        switch ($key) {
            case 'theme_mode':
                $btn .= '<a' . $style . ' class="float-btn toggle-theme hover-show"' . $tooltip . ' data-placement="left" title="' . esc_attr__('切换主题', 'zib_language') . '" href="javascript:;"><i class="fa fa-toggle-theme"></i>
                </a>';
                break;
            case 'shop_cart':
                if (_pz('shop_s') && $user_id && !apply_filters('is_shop_cart_page', false)) {
                    ...
                    $btn .= '<a' . $style . ' class="float-btn float-btn-shop-cart nowave"' . $tooltip . ' data-placement="left" title="' . esc_attr__('购物车', 'zib_language') . '" href="' . $url . '">
            ...
            case 'back_top':
                ...
                $btn .= '<a' . $style . ' class="float-btn ontop fade"' . $tooltip . ' data-placement="left" title="' . esc_attr__('返回顶部', 'zib_language') . '" href="' . $scrollTo . '"><i class="fa fa-angle-up em12"></i></a>';
                break;
            case 'service_qq':
                $href = $opt['qq'] ? 'https://wpa.qq.com/msgrd?v=3&uin=' . $opt['qq'] . '&site=qq&menu=yes' : '';
                $html = '<a' . $style . ' class="float-btn service-qq"' . $tooltip . ' data-placement="left" title="' . esc_attr__('QQ联系', 'zib_language') . '" target="_blank" href="' . $href . '"><i class="fa fa-qq"></i></a>';
                $btn .= $href ? $html : '';
                break;
            case 'pay_vip':
                ...
                if (!zib_is_close_sign() && (_pz('pay_user_vip_1_s', true) || $pay_user_vip_2_s)) {
                    ...
                    if ($user_id) {
                        ...
                        $title     = (1 == $vip_level ? __('升级会员', 'zib_language') : __('开通会员', 'zib_language'));
                        ...
                        $btn .= '<a' . $style . ' class="float-btn signin-loader"' . $tooltip . ' data-placement="left" title="' . esc_attr__('开通会员', 'zib_language') . '" href="javascript:;">' . $icon . '</a>';
                    }
                }
            ...
            case 'service_wechat':
                ...
                if ($wechat_img) {
                    ...
                    $s_img .= '<img class="radius4 relative" width="100%" ' . $lazy_attr . 'src="' . $wechat_img . '"  alt="' . esc_attr(sprintf(__('扫码添加微信%s', 'zib_language'), zib_get_delimiter_blog_name())) . '">';
                    $s_img .= '</div>';
                    $btn .= '<a' . $style . ' class="float-btn service-wechat hover-show nowave" title="' . esc_attr__('扫码添加微信', 'zib_language') . '" href="javascript:;"><i class="fa fa-wechat"></i>' . $s_img . '</a>';
                }
                break;
...
//获取发布模态框的按钮
function zib_get_add_modal_link($class = '', $text = null, $args = array())
{
    if (null === $text) {
        $text = __('发布', 'zib_language');
    }
    $defaults = array(
        'class'         => $class,
        'mobile_bottom' => true,
...
function zib_get_single_footer_tabbar($btn)
{
    ...
    if (apply_filters('zibpay_is_show_paybutton', false)) {
        $btn .= '<div class="tabbar-item but jb-red single-pay-tabbar">' . __('立即购买', 'zib_language') . '</div>';
        if ($comments_open) {
...
function zib_footer_con()
{
    ...
    $html   = '<ul class="flex ac gap20 footer-contact-box list-inline">' . $box . '</ul>';
    $html .= $c_code;
    echo $html;
}
...
function zib_footer_con_2()
{
    ...
    if ((!wp_is_mobile() || _pz('footer_contact_m_s', true)) && _pz('footer_contact_wechat_img')) {
        ...
        $s_img .= '<img style="box-shadow: 0 5px 10px rgba(0,0,0,.2); border-radius:4px;" height="100" ' . $lazy_attr . 'src="' . _pz('footer_contact_wechat_img') . '" alt="' . esc_attr(sprintf(__('扫一扫加微信%s', 'zib_language'), zib_get_delimiter_blog_name())) . '">';
        $s_img .= '</div>';

        $html .= '<a class="toggle-radius hover-show nowave" href="javascript:;">' . zib_get_svg('d-wechat') . $s_img . '</a>';
    }
    if (_pz('footer_contact_qq')) {
        $html .= '<a class="toggle-radius" data-toggle="tooltip" target="_blank" title="' . esc_attr__('QQ联系', 'zib_language') . '" href="https://wpa.qq.com/msgrd?v=3&uin=' . _pz('footer_contact_qq') . '&site=qq&menu=yes">' . zib_get_svg('d-qq', '-50 0 1100 1100') . '</a>';
    }
    if (_pz('footer_contact_weibo')) {
        $html .= '<a class="toggle-radius" data-toggle="tooltip" title="' . esc_attr__('微博', 'zib_language') . '" target="_blank" href="' . _pz('footer_contact_weibo') . '">' . zib_get_svg('d-weibo') . '</a>';
    }
    if (_pz('footer_contact_email')) {
        $html .= '<a class="toggle-radius" data-toggle="tooltip" title="' . esc_attr__('发邮件', 'zib_language') . '" href="mailto:' . _pz('footer_contact_email') . '">' . zib_get_svg('d-email', '-20 80 1024 1024') . '</a>';
    }
...
function zib_footer_con_3()
{
    ...
    if ($imgs) {
        $html .= zib_get_footer3_img_box($imgs, 'footer-miniimg flex at hh gap6');
    }

    $imgs_2 = (array) _pz('footer_mini_img_2');
    if ($imgs_2) {
        $html .= zib_get_footer3_img_box($imgs_2, 'footer-miniimg-2 flex at hh gap6');
    }
...
//生成footer3 小图片
function zib_get_footer3_img_box($imgs, $class = '')
{
    ...
    foreach ($imgs as $img) {
        ...
        $url    = !empty($img['link']['url']) ? $img['link']['url'] : '';
        $target = $url ? (!empty($img['link']['target']) ? ' target="' . $img['link']['target'] . '"' : '') : '';

        $text_html = $text ? '<div class="opacity8 em09 mt6 text-center">' . $text . '</div>' : '';
        $img_html  = '<div ' . ($text ? ' data-toggle="tooltip" title="' . $text . '"' : '') . '>
                        <img ' . $lazy_attr . 'src="' . $img['image'] . '" alt="' . $text . zib_get_delimiter_blog_name() . '">
                        ' . $text_html . '
                    </div>';

        $html .= $url ? '<a href="' . $url . '"' . $target . '>' . $img_html . '</a>' : $img_html;
    }
