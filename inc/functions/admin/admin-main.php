...
function zib_admin_environmental_dependence_reminders()
{
    ...
    if (!extension_loaded('mbstring')) {
        $error .= '<p>' . sprintf(__('PHP缺少%1$s基础组件，将会导致异常报错！', 'zib_language'), '<code>mbstring</code>') . ' ' . sprintf('<a href="%s" target="_blank">%s</a>', esc_url('https://www.zibll.com/forum-post/41851.html'), __('查看安装教程', 'zib_language')) . '</p>';

    }

    if (!extension_loaded('curl')) {
        $error .= '<p>' . sprintf(__('PHP缺少%1$s基础组件，将会导致部分功能无法使用！', 'zib_language'), '<code>curl</code>') . __('请在php.ini中开启', 'zib_language') . '<code>curl</code>' . __('扩展', 'zib_language') . '</p>';
    }

    $is_throttle = zib_fun_throttle(60, 'zib_admin_throttle_check');
    if (is_wp_error($is_throttle)) {
        $error .= '<p>' . __('文件夹权限异常，导致核心函数无法正常运行，请及时处理！', 'zib_language') . '</p><p>' . sprintf(__('错误信息：%s', 'zib_language'), $is_throttle->get_error_message()) . '</p>';
    }

    if ($error) {
        echo '<div class="notice notice-error is-dismissible">
        <h3>' . __('错误警告', 'zib_language') . '</h3>' . $error . '
        </div>';
    }
}
...
{
    return array_merge($row_actions, ['term_id' => __('ID：', 'zib_language') . $term->term_id]);
}
...
function zib_admin_add_term_img_form_field()
{
    ...
    echo '<div class="form-field">
		<label for="taxonomy_image">' . __('封面图像', 'zib_language') . '</label>
		<input type="text" name="taxonomy_image" id="taxonomy_image" value="" />
        <br/>
        <p>' . __('设置封面图，建议尺寸为2000x800,需要在主题设置-分类、标签页：开启分类、标签封面显示功能', 'zib_language') . '</p>
		<button class="z_upload_image_button button">' . __('上传/添加图像', 'zib_language') . '</button>
...
function zib_admin_add_term_img_form_field_edit($taxonomy)
{
    ...
    echo '<tr class="form-field">
		<th scope="row" valign="top"><label for="taxonomy_image">' . __('图像', 'zib_language') . '</label></th>
		<td><img class="taxonomy-image" src="' . $image_text . '"/><br/><input type="text" name="taxonomy_image" id="taxonomy_image" value="' . $image_text . '" /><br />
        <p>' . __('设置封面图，建议尺寸为2000x800,需要在主题设置-分类、标签页：开启分类、标签封面显示功能', 'zib_language') . '</p>
        <button class="z_upload_image_button button">' . __('上传/添加图像', 'zib_language') . '</button>
		<button class="z_remove_image_button button">' . __('删除图像', 'zib_language') . '</button>
		</td>
...
{
    $new_columns          = array();
    $new_columns['thumb'] = __('图像', 'zib_language');
...
{
    if ('thumb' == $column) {
        $columns = '<span><img src="' . zib_get_taxonomy_img_url($id, null, Z_IMAGE_PLACEHOLDER) . '" alt="' . esc_attr__('Thumbnail', 'zib_language') . '" class="wp-post-image" /></span>';
    }
...
function zib_restrict_manage_comments()
{
    ...
    echo '<select id="filter-by-comment-type" name="post_type">
                <option value="">' . __('全部文章类型', 'zib_language') . '</option>
                ' . $options . '
            </select>';
}
            ...
            return array_merge(
                array(
                    array(
                        ...
                        'title' => __('Zibll主题模块', 'zib_language'),
                    ),
                ),
                $categories
            );
...
class zib_admin_add_term_seo
{
    ...
    {
        echo '<div id="term-seo-field">
        <div class="form-field">
            <label for="term_meta[title]">' . __('SEO 标题', 'zib_language') . '</label>
            <input class="term-seo-title" type="text" name="term_meta[title]" id="term_meta[title]" />
        </div>
        <div class="form-field">
            <label for="term_meta[keywords]">' . __('SEO 关键字keywords）（用英文逗号分开）', 'zib_language') . '</label>
            <input class="term-seo-keywords" type="text" name="term_meta[keywords]" id="term_meta[keywords]" />
        </div>
        <div class="form-field">
            <label for="term_meta[keywords]">' . __('SEO 描述（description）', 'zib_language') . '</label>
            <textarea class="term-seo-description" name="term_meta[description]" id="term_meta[description]" rows="4" cols="40"></textarea>
            <p>' . __('主题默认会自动设置SEO内容，如需单独设置，请在此输入', 'zib_language') . '</p>
        </div>
        </div>
        ';
    }
    ...
    public function edit_tax_field($term)
    {
      ...
      <tr class="form-field">
        <th scope="row">
            <label for="term_meta[title]">' . __('SEO 标题', 'zib_language') . '</label>
            <td>
                <input class="term-seo-title" type="text" name="term_meta[title]" id="term_meta[title]" value="' . $meta_title . '" />
            </td>
        </th>
    </tr>
    <tr class="form-field">
        <th scope="row">
            <label for="term_meta[keywords]">' . __('SEO 关键字（keywords）', 'zib_language') . '</label>
            <td>
                <input class="term-seo-keywords" type="text" name="term_meta[keywords]" id="term_meta[keywords]" value="' . $meta_keywords . '" />
            </td>
        </th>
    </tr>
    <tr class="form-field">
        <th scope="row">
            <label for="term_meta[description]">' . __('SEO 描述（description）', 'zib_language') . '</label>
            <td>
                <textarea class="term-seo-description" name="term_meta[description]" id="term_meta[description]" rows="4">' . $meta_description . '</textarea>
                <p>' . __('主题默认会自动设置SEO内容，如需单独设置，请在此输入', 'zib_language') . $ai_seo_btn . '</p>
            </td>
        </th>
    </tr>
    ';
