...
if (!$zib_bbs->s) {
    ...
    {
        global $zib_bbs;
        return zib_bbs_get_plate_add_link(0, 'btn-newadd', '<icon class="jb-yellow">' . zib_get_svg('plate-fill') . '</icon><text>' . sprintf(__('创建%s', 'zib_language'), $zib_bbs->plate_name) . '</text>', 'a');
    }
    ...
    {
        global $zib_bbs;
        return zib_bbs_get_topic_edit_link(0, 'btn-newadd', '<icon class="jb-pink">' . zib_get_svg('topic') . '</icon><text>' . sprintf(__('创建%s', 'zib_language'), $zib_bbs->topic_name) . '</text>', 'a');
    }
    ...
    {
        global $zib_bbs;
        return zib_bbs_get_posts_add_page_link(0, 'btn-newadd', '<icon class="jb-blue">' . zib_get_svg('posts') . '</icon><text>' . sprintf(__('发布%s', 'zib_language'), $zib_bbs->posts_name) . '</text>', 'a');
    }
    ...
    function zib_bbs_user_count_badges_filter($html, $user_id)
    {
        ...
        global $zib_bbs;
        $html = '<a class="but c-blue-2 tag-forum-post" data-toggle="tooltip" title="' . esc_attr(sprintf(__('共%d篇%s', 'zib_language'), $post_n, $zib_bbs->posts_name)) . '" href="' . zib_get_user_home_url($user_id, array('tab' => 'forum')) . '">' . zib_get_svg('posts') . $post_n . '</a>' . $html;
    ...
    function zib_bbs_get_excerpt_filter($excerpt, $post)
    {
        ...
        if ($allow_view && !strstr($allow_view, __('部分内容已隐藏', 'zib_language'))) {
            return '';
        }
...
{
    global $zib_bbs;
    $options['bbs_plate'] = sprintf(__('[%1$s]创建%2$s', 'zib_language'), $zib_bbs->forum_name, $zib_bbs->plate_name);
    $options['bbs_topic'] = sprintf(__('[%1$s]创建%2$s', 'zib_language'), $zib_bbs->forum_name, $zib_bbs->topic_name);
    $options['bbs_posts'] = sprintf(__('[%1$s]发布%2$s', 'zib_language'), $zib_bbs->forum_name, $zib_bbs->posts_name);
    return $options;
}
