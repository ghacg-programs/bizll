...
function zib_bbs_ajax_posts_score()
{
    ...
    if (!$id) {
        wp_send_json_error(['msg' => __('参数传入错误', 'zib_language'), 'code' => 'no_id']);
    }

    //执行安全验证检查，验证不通过自动结束并返回提醒
    if (!isset($_REQUEST['_wpnonce']) || !wp_verify_nonce($_REQUEST['_wpnonce'], $action)) {
        wp_send_json_error(['msg' => __('环境异常，请刷新页面后稍候再试', 'zib_language'), 'code' => 'env_error']);
    }
    ...
    switch ($action) {
        ...
        case 'score_deduct': //减分
            ...
            if ('score_extra' == $action) {
                ...
                if (!$max) {
                    wp_send_json_error(['msg' => __('您暂无加分权限', 'zib_language')]);
                }
                $new_user_score = $user_score + 1;
                if ($new_user_score > $max) {
                    wp_send_json_error(['msg' => sprintf(__('已为该内容加满%d分', 'zib_language'), $max), 'ys' => 'success']);
                }
            } else {
                $max = apply_filters('bbs_user_score_deduct_max', 5, $user_id); //用户最多减去几分
                if (!$max) {
                    wp_send_json_error(['msg' => __('您暂无减分权限', 'zib_language')]);
                }
                $new_user_score = $user_score - 1;
                if ($new_user_score < -$max) {
                    wp_send_json_error(['msg' => sprintf(__('已为该内容扣满%d分', 'zib_language'), $max), 'ys' => 'warning']);
                }
            }
...
function zib_bbs_ajax_posts_meta_save()
{
    ...
    if (!zib_bbs_current_user_can($action, $post_id)) {
        echo json_encode((array('error' => 1, 'ys' => 'danger', 'msg' => __('暂无此权限', 'zib_language'))));
        exit;
    }
    ...
    switch ($action) {
        case 'posts_essence_set':
            ...
            $msg    = $cancel ? __('已取消此内容的精华称号', 'zib_language') : __('已将此内容设置为精华', 'zib_language');
            $reload = true;
        ...
        case 'posts_topping_set':
            ...
            if ('0' == $topping) {
                $msg = __('已取消置顶', 'zib_language');
            } else {
                $name = zib_bbs_get_posts_topping_options();
                $name = isset($name[$topping]) ? $name[$topping] : __('置顶', 'zib_language');
                $msg  = sprintf(__('已%s此内容', 'zib_language'), $name);
            }
...
function zib_bbs_ajax_posts_topping_set_modal()
{
    ...
    if (!zib_bbs_current_user_can('posts_topping_set', $post->ID)) {
        zib_ajax_notice_modal('danger', __('您没有该操作权限', 'zib_language'));
    }
...
function zib_bbs_ajax_posts_allow_view_set_modal()
{
    ...
    $header         = zib_get_modal_colorful_header('jb-yellow', '<i class="fa fa-unlock-alt"></i>', __('设置阅读权限', 'zib_language'));
    $hidden_html    = '';
...
function zib_bbs_ajax_posts_plate_move()
{
    ...
    if (!$plate_id) {
        echo json_encode((array('error' => 1, 'ys' => 'danger', 'msg' => sprintf(__('请选择%s', 'zib_language'), $plate_name))));
        exit;
    }
    ...
    $post = get_post($id);
    if (empty($post->ID)) {
        echo json_encode((array('error' => 1, 'ys' => 'danger', 'msg' => __('内容不存在或参数传入错误', 'zib_language'))));
        exit;
    }
    //权限验证
    if (!zib_bbs_current_user_can('posts_plate_move', $post)) {
        echo json_encode((array('error' => 1, 'ys' => 'danger', 'msg' => __('您没有该操作权限', 'zib_language'))));
        exit;
    }

    //新版块选择权限验证
    if (!zib_bbs_current_user_can('select_plate', $plate_id, $post)) {
        zib_send_json_error(sprintf(__('抱歉！您暂无选择此%s的权限，请重新选择%s', 'zib_language'), $plate_name, $plate_name));
    }

    $old_id = zib_bbs_get_plate_id($id);
    if ($old_id == $plate_id) {
        echo json_encode((array('error' => 1, 'ys' => 'danger', 'msg' => sprintf(__('所选%s未做任何修改', 'zib_language'), $plate_name))));
        exit;
    }
    ...
    echo(json_encode(array('error' => 0, 'ys' => '', 'reload' => true, 'msg' => __('切换成功', 'zib_language'))));
    exit;
}
...
function zib_bbs_ajax_posts_plate_move_modal()
{
    ...
    if (!zib_bbs_current_user_can('posts_plate_move', $post)) {
        echo zib_get_null(__('抱歉！暂无此权限', 'zib_language'), 30, 'null-cap.svg', '', 200, 150);
        exit;
    }
...
function zib_bbs_ajax_delete_modal()
{
    ...
    if (!$id) {
        zib_ajax_notice_modal('danger', __('参数传入错误', 'zib_language'));
    }
    ...
    switch ($action) {
        ...
        case 'posts_delete_modal':
            ...
            if (!zib_bbs_current_user_can('posts_delete', $post)) {
                zib_ajax_notice_modal('danger', __('您没有删除此内容的权限', 'zib_language'));
            }
            if ('forum_post' != $post->post_type) {
                zib_ajax_notice_modal('danger', __('类型错误或参数传入错误', 'zib_language'));
            }
        ...
        case 'plate_audit_modal':
            ...
            if ('forum_post' === $post->post_type && !zib_bbs_current_user_can('posts_audit', $post)) {
                zib_ajax_notice_modal('danger', __('您没有审核的权限', 'zib_language'));
            }
...
function zib_bbs_ajax_posts_comment_close_modal()
{
    ...
    if (!zib_bbs_current_user_can('comment_close', $post)) {
        zib_ajax_notice_modal('danger', __('您没有关闭评论的权限', 'zib_language'));
    }
...
function zib_bbs_ajax_posts_comment_close()
{
    ...
    if (is_wp_error($post_updated)) {
        zib_send_json_error(sprintf(__('操作失败：%s', 'zib_language'), $post_updated->get_error_message()));
    }

    zib_send_json_success(array('msg' => __('评论状态已更新', 'zib_language'), 'reload' => true, 'hide_modal' => true));
}
...
function zib_bbs_ajax_plate_or_posts_audit()
{
    ...
    if (empty($post->ID)) {
        zib_send_json_error(__('内容不存在或参数传入错误', 'zib_language'));
    }
    //权限判断
    if ('forum_post' === $post->post_type && !zib_bbs_current_user_can('posts_audit', $post)) {
        zib_send_json_error(sprintf(__('您没有审核%s的权限', 'zib_language'), $zib_bbs->posts_name));
    }
    if ('plate' === $post->post_type && !zib_bbs_current_user_can('plate_audit', $post)) {
        zib_send_json_error(sprintf(__('您没有审核%s的权限', 'zib_language'), $zib_bbs->plate_name));
    }
    ...
    if ('pending' === $post->post_status) {
        ...
        if ('reject' === $method) {
            $post_status = 'pending';
            $mag         = __('已驳回此内容，并已通知作者', 'zib_language');
            $send_msg    = !empty($_REQUEST['msg']) ? trim(strip_tags($_REQUEST['msg'])) : '';
            if (!$send_msg) {
                zib_send_json_error(__('请填写驳回原因或修改建议', 'zib_language'));
            }
            zib_newmsg_publish_to_pending($post);
        } else {
            $post_status = 'publish';
            $mag         = __('内容已审核发布', 'zib_language');
        }
    } else {
        $post_status = 'pending';
        $mag         = __('内容已驳回审核', 'zib_language');
    }
    ...
    if ($post_status !== $post->post_status) {
        ...
        if (!$post_updated) {
            zib_send_json_error(__('操作失败，请刷新页面重试', 'zib_language'));
        }
...
function zib_bbs_ajax_plate_or_posts_delete_revoke()
{
    ...
    if ('plate_delete_revoke' === $action) {
        ...
        if (!zib_bbs_current_user_can('plate_edit', $plate)) {
            echo json_encode((array('error' => 1, 'ys' => 'danger', 'msg' => sprintf(__('您没有恢复此%s的权限', 'zib_language'), $name))));
            exit;
        }
    } else {
    ...
    echo(json_encode(array('error' => 0, 'ys' => '', 'reload' => true, 'goto' => $goto, 'msg' => sprintf(__('该%s已移出回收站', 'zib_language'), $name))));
    exit;
}
...
function zib_bbs_ajax_edit_posts()
{
    ...
    if ($is_new) {
        if (!zib_bbs_current_user_can('posts_add')) {
            zib_send_json_error(__('暂无发布权限', 'zib_language'));
        }

    } else {
        if (!zib_bbs_current_user_can('posts_edit', $post_id)) {
            zib_send_json_error(__('您没有编辑此内容的权限', 'zib_language'));
        }
    }
    ...
    //内容长度验证
    if (!$post_title) {
        zib_send_json_error(__('请填写标题', 'zib_language'));
    }
    ...
    if ($title_strlen_limit['max'] && zib_new_strlen($post_title) > $title_strlen_limit['max']) {
        zib_send_json_error(sprintf(__('标题太长了，不能超过%d个字', 'zib_language'), $title_strlen_limit['max']));
    }

    if ('bbs_posts_save' == $action) {
        //提交保存、审核、发布
        if ($title_strlen_limit['min'] && zib_new_strlen($post_title) < $title_strlen_limit['min']) {
            zib_send_json_error(__('标题太短！', 'zib_language'));
        }

        if (!$plate) {
            zib_send_json_error(sprintf(__('请选择%s', 'zib_language'), $zib_bbs->plate_name));
        }
    }
    ...
    if (!empty($_REQUEST['is_quick'])) {
        ...
        if ($hide_content) {
            ...
            $type_desc = array(
                ...
                'vip1'    => __('会员可见', 'zib_language'),
                'payshow' => __('付费可见', 'zib_language'),
                'vip2'    => _pz('pay_user_vip_2_name') . __('可见', 'zib_language'),
            );
            ...
            $post_content .= '<div class="tinymce-hide" contenteditable="false"><p class="hide-before">[hidecontent type="' . $hide_type . '" desc="' . sprintf(__('隐藏内容：%s', 'zib_language'), $type_desc[$hide_type]) . '"]</p><div contenteditable="true">' . $hide_content . '</div><p class="hide-after">[/hidecontent]</p></div>';
        }
    }
    ...
    switch ($post_status) {
        ...
        case 'draft': //草稿
            if ($cuid == $insert_args['post_author']) {
                //我的草稿
                $send['msg']  = __('草稿已保存', 'zib_language');
                $send['html'] = '<div class="muted-2-color flex ac mt6 mr10"><a href="' . zib_get_user_home_url($insert_args['post_author'], array('tab' => 'forum', 'status' => 'draft')) . '" class="but mr10 p2-10 px12">' . __('我的草稿', 'zib_language') . '</a>' . __('草稿已保存', 'zib_language') . ' </div><div class="muted-3-color px12 mt6">' . sprintf(__('最后更新：%s', 'zib_language'), get_the_modified_time('Y-m-d H:i:s', $insert_id)) . '</div>';
            } else {
                $send['msg']  = __('内容已修改，保存为草稿', 'zib_language');
                $send['html'] = '<div class="muted-2-color flex ac mt6 mr10">' . __('草稿已保存', 'zib_language') . ' </div><div class="muted-3-color px12 mt6">' . sprintf(__('最后更新：%s', 'zib_language'), get_the_modified_time('Y-m-d H:i:s', $insert_id)) . '</div>';
            }

            break;
        default:
            $send['msg']    = sprintf(__('%s已发布', 'zib_language'), $zib_bbs->posts_name);
            $send['reload'] = true;
    }
...
function zib_bbs_add_posts_pending_msg($post)
{
    ...
    $title      = sprintf(__('%1$s[%2$s]有新的%3$s待审核：%4$s', 'zib_language'), $plate_name, zib_str_cut(trim(strip_tags($plate->post_title)), 0, 20), $posts_name, zib_str_cut(trim(strip_tags($post->post_title)), 0, 20));
    $send_user  = 'admin';

    $message = sprintf(__('有新的%s待审核', 'zib_language'), $posts_name) . '<br />';
    $message .= sprintf(__('标题：%s', 'zib_language'), trim(strip_tags($post->post_title))) . '<br>';
    $message .= __('内容摘要：', 'zib_language') . '<br />';
    ...
    $message .= sprintf(__('用户：%s', 'zib_language'), zib_get_user_name_link($user_id)) . '<br>';
    $message .= sprintf(__('%s：', 'zib_language'), $plate_name) . sprintf('<a href="%1$s">%2$s</a>', esc_url($plate_url), esc_html($plate->post_title)) . '<br />';
    $message .= sprintf(__('提交时间：%s', 'zib_language'), get_the_time('Y-m-d H:i:s', $post)) . '<br>';

    $message .= '<br>';
    $message .= __('您可以', 'zib_language') . sprintf('<a href="%1$s">%2$s</a>', esc_url($plate_url), __('点击此处', 'zib_language')) . __('进入版块页面的待审核栏目，审核此内容', 'zib_language') . '<br />';
...
function zib_bbs_add_posts_publish_msg($post)
{
    ...
    $plate      = get_post($plate_id);
    $plate_url  = get_permalink($plate);
    $title      = sprintf(__('您管理的%1$s[%2$s]收到了新%3$s：%4$s', 'zib_language'), $plate_name, zib_str_cut(trim(strip_tags($plate->post_title)), 0, 20), $posts_name, zib_str_cut(trim(strip_tags($post->post_title)), 0, 20));
    $send_user  = 'admin';

    $message = sprintf(__('有新的%s已发布', 'zib_language'), $posts_name) . '<br />';
    $message .= sprintf(__('标题：%s', 'zib_language'), trim(strip_tags($post->post_title))) . '<br>';
    $message .= __('内容摘要：', 'zib_language') . '<br />';
    $message .= '<div class="muted-box" style="padding: 10px 15px; border-radius: 8px; background: rgba(125, 125, 125, 0.06); line-height: 1.7;">' . zib_str_cut(trim(strip_tags($post->post_content)), 0, 200, '...') . '</div>';
    $message .= sprintf(__('用户：%s', 'zib_language'), zib_get_user_name_link($user_id)) . '<br>';
    ...
    $message .= '<br>';
    $message .= __('您可以', 'zib_language') . sprintf('<a href="%1$s">%2$s</a>', esc_url(get_permalink($post)), __('点击此处', 'zib_language')) . __('查看此内容', 'zib_language') . '<br />';
    ...
    if ($s) {
        ...
        foreach ($opt['options'] as $v) {
            ...
            if (in_array($v, $new_options)) {
                zib_send_json_error(array('post_id' => $post_id, 'msg' => __('投票选项的内容不能相同', 'zib_language')));
            }
        ...
        if (count($opt['options']) < 2) {
            //如果选项小于2
            zib_send_json_error(array('post_id' => $post_id, 'msg' => __('投票选项太少或选项内容字数太短', 'zib_language')));
        }

        if ($opt['title'] && zib_new_strlen($opt['title']) > 20) {
            zib_send_json_error(array('post_id' => $post_id, 'msg' => __('投票标题太长，最多20个字', 'zib_language')));
        }
        ...
        if ($echo_success) {
            zib_send_json_success(array('msg' => __('投票信息已保存', 'zib_language'), 'vote_option' => $opt, 'post_id' => $post_id));
        }
    } else {
        //保存开关
        update_post_meta($post_id, 'vote', false);
        if ($echo_success) {
            zib_send_json_success(array('post_id' => $post_id, 'msg' => __('投票已关闭', 'zib_language')));
        }
    }
}
...
function zib_bbs_ajax_edit_allow_view($post_id = 0, $echo_success = true, $is_new = false)
{
    ...
    if (($is_new && !zib_bbs_current_user_can('posts_allow_view_add')) || (!$is_new && !zib_bbs_current_user_can('posts_allow_view_edit', $post_id))) {
        zib_send_json_error(array('post_id' => $post_id, 'msg' => __('权限不足', 'zib_language')));
    }

    switch ($allow_view) {
        case 'roles':
            if (!$allow_view_roles) {
                zib_send_json_error(array('post_id' => $post_id, 'msg' => __('请至少选择一项允许查看的用户类型', 'zib_language')));
            }
            break;
        ...
        case 'points':
            ...
            if (!$points_price) {
                zib_send_json_error(array('post_id' => $post_id, 'msg' => __('请设置阅读限制的积分价格', 'zib_language')));
            }
            if ($points_price < 0) {
                zib_send_json_error(array('post_id' => $post_id, 'msg' => __('请设置正确的积分价格', 'zib_language')));
            }
            ...
            if ($points_price_limit) {
                ...
                if ($points_price_limit_min && $points_price < $points_price_limit_min) {
                    zib_send_json_error(array('posts_id' => $post_id, 'msg' => sprintf(__('积分价格不能低于：%s', 'zib_language'), $points_price_limit_min)));
                }
                if ($points_price_limit_max && $points_price > $points_price_limit_max) {
                    zib_send_json_error(array('posts_id' => $post_id, 'msg' => sprintf(__('积分价格不能高于：%s', 'zib_language'), $points_price_limit_max)));
                }
            }

            if ($vip_input_s) {
                if (isset($posts_zibpay['vip_1_points']) && !$posts_zibpay['vip_1_points'] && $posts_zibpay['vip_1_points'] != '0') {
                    zib_send_json_error(array('post_id' => $post_id, 'msg' => sprintf(__('请设置%s积分售价', 'zib_language'), _pz('pay_user_vip_1_name'))));
                }
                if (isset($posts_zibpay['vip_2_points']) && !$posts_zibpay['vip_2_points'] && $posts_zibpay['vip_2_points'] != '0') {
                    zib_send_json_error(array('post_id' => $post_id, 'msg' => sprintf(__('请设置%s积分售价', 'zib_language'), _pz('pay_user_vip_2_name'))));
                }
                ...
                if ($vip_1_points > $points_price) {
                    zib_send_json_error(array('post_id' => $post_id, 'msg' => __('会员价不能高于普通价', 'zib_language')));
                }

                if ($vip_2_points > $vip_1_points) {
                    zib_send_json_error(array('post_id' => $post_id, 'msg' => sprintf(__('%1$s价格不能高于%2$s价格', 'zib_language'), _pz('pay_user_vip_2_name'), _pz('pay_user_vip_1_name'))));
                }
        ...
        case 'pay':
            ...
            if (!$pay_price) {
                zib_send_json_error(array('post_id' => $post_id, 'msg' => __('请设置阅读限制的支付金额', 'zib_language')));
            }
            if ($pay_price < 0) {
                zib_send_json_error(array('post_id' => $post_id, 'msg' => __('请设置正确的价格', 'zib_language')));
            }
            ...
            if ($pay_price_limit) {
                ...
                if ($pay_price_limit_min && $pay_price < $pay_price_limit_min) {
                    zib_send_json_error(array('posts_id' => $post_id, 'msg' => sprintf(__('价格不能低于：%s', 'zib_language'), $pay_price_limit_min)));
                }
                if ($pay_price_limit_max && $pay_price > $pay_price_limit_max) {
                    zib_send_json_error(array('posts_id' => $post_id, 'msg' => sprintf(__('价格不能高于：%s', 'zib_language'), $pay_price_limit_max)));
                }
            }

            if ($vip_input_s) {
                if (isset($posts_zibpay['vip_1_price']) && !$posts_zibpay['vip_1_price'] && $posts_zibpay['vip_1_price'] != '0') {
                    zib_send_json_error(array('post_id' => $post_id, 'msg' => sprintf(__('请设置%s价格', 'zib_language'), _pz('pay_user_vip_1_name'))));
                }
                if (isset($posts_zibpay['vip_2_price']) && !$posts_zibpay['vip_2_price'] && $posts_zibpay['vip_2_price'] != '0') {
                    zib_send_json_error(array('post_id' => $post_id, 'msg' => sprintf(__('请设置%s价格', 'zib_language'), _pz('pay_user_vip_2_name'))));
                }
                ...
                if ($vip_2_price > $vip_1_price) {
                    zib_send_json_error(array('post_id' => $post_id, 'msg' => sprintf(__('%1$s会员价不能高于%2$s价格', 'zib_language'), _pz('pay_user_vip_2_name'), _pz('pay_user_vip_1_name'))));
                }
    ...
    if ($echo_success) {
        zib_send_json_success(array('msg' => __('阅读权限已保存', 'zib_language'), 'reload' => true, 'allow_view_roles' => $allow_view_roles, 'allow_view' => $allow_view, 'post_id' => $post_id));
    }
}
...
function zib_bbs_ajax_submit_vote()
{
    ...
    if (!$voted) {
        zib_send_json_error(__('请选择投票选项', 'zib_language'));
    }
...
function zib_bbs_ajax_apply_moderator_modal()
{
    ...
    if (zib_bbs_is_the_moderator($plate_id)) {
        zib_ajax_notice_modal('info', sprintf(__('您已是该%1$s的%2$s', 'zib_language'), $zib_bbs->plate_name, $zib_bbs->plate_moderator_name));
    }
    ...
    if ($processing) {
        ...
        $header         = zib_get_modal_colorful_header('jb-yellow', zib_get_svg('plate-fill'), sprintf(__('申请成为%s', 'zib_language'), $moderator_name));

        $plate_title = get_the_title($processing->meta['plate_id']);
        $con         = '<h5 class="c-red mb20">' . sprintf(__('您的%s申请审核处理中，请耐心等待！', 'zib_language'), $moderator_name) . '</h5>';
        $con .= '<div class="mb10 muted-2-color">' . sprintf(__('申请%s：%s', 'zib_language'), $zib_bbs->plate_name, $plate_title) . '</div>';
        $con .= '<div class="mb10 muted-2-color">' . sprintf(__('提交说明：%s', 'zib_language'), $processing->meta['desc']) . '</div>';
        $con .= '<div class="mb20 muted-2-color">' . sprintf(__('提交时间：%s', 'zib_language'), $processing->meta['time']) . '</div>';
        $con .= '<div class="modal-buts but-average"><a type="button" data-dismiss="modal" class="but" href="javascript:;">' . __('取消', 'zib_language') . '</a></div>';
...
function zib_bbs_ajax_comment_set_hot()
{
    ...
    if (empty($comment->comment_ID)) {
        zib_send_json_error(['msg' => __('参数错误或评论已删除', 'zib_language')]);
    }

    if (!zib_bbs_current_user_can('comment_set_hot', $comment)) {
        zib_send_json_error(['msg' => __('权限不足', 'zib_language')]);
    }
    ...
    if ($is_hot) {
        ...
        zib_send_json_success(['msg' => __('已取消此评论的神评称号', 'zib_language'), 'reload' => true]);
    } else {
        update_comment_meta($id, 'is_hot', 1);
        do_action('comment_is_hot', $comment); //添加挂钩
        //刷新缓存
        zib_send_json_success(['msg' => __('已将此评论设为神评', 'zib_language'), 'reload' => true]);
    }
