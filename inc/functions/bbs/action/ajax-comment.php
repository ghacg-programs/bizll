...
function zib_bbs_ajax_answer_adopt()
{
    ...
    if (!$comment) {
        zib_send_json_error(__('当前回答不存在或者参数错误', 'zib_language'));
    }
    ...
    if (!zib_bbs_current_user_can('question_answer_adopt', $posts_id)) {
        zib_ajax_notice_modal('danger', __('您没有采纳回答的权限', 'zib_language'));
    }

    //评论状态判断
    if (wp_get_comment_status($comment) !== 'approved') {
        zib_send_json_error(__('当前回答暂未通过审核', 'zib_language'));
    }
    ...
    if ($adopted_badeg) {
        zib_send_json_success(array('msg' => __('该回答已采纳', 'zib_language'), 'badeg' => $adopted_badeg, 'comment_id' => $comment_id, 'hide_modal' => true));
    }
    ...
    $adopted_badeg = zib_bbs_get_comment_adopted_badeg($comment_id);
    zib_send_json_success(array('msg' => __('已采纳该回答', 'zib_language'), 'badeg' => $adopted_badeg, 'comment_id' => $comment_id, 'hide_modal' => true));
}
