...
function zib_bbs_ajax_term_delete()
{
    ...
    echo(json_encode(array('error' => 0, 'ys' => '', 'reload' => true, 'goto' => $goto, 'msg' => sprintf(__('%1$s（%2$s）已删除', 'zib_language'), $name, esc_attr($term->name)))));
    exit;
}
...
function zib_bbs_ajax_save_term()
{
    ...
    $name     = zib_bbs_get_taxonomy_name($taxonomy);
    $name     = $name ? $name : __('内容', 'zib_language');
    ...
    if (zib_new_strlen($content) > 50) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('简介太长，不能超过50个字', 'zib_language'))));
        exit();
    }
    ...
    $text = $term_id ? __('编辑', 'zib_language') : __('创建', 'zib_language');
