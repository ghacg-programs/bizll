...
function zib_bbs_get_plate_order_options()
{
    ...
    $args = array(
        ...
        'date'              => __('创建时间', 'zib_language'),
        'modified'          => __('更新时间', 'zib_language'),
        'last_post'         => __('最新发帖', 'zib_language'),
        'last_reply'        => __('最新回帖', 'zib_language'),
        'posts_count'       => __('最多帖子', 'zib_language'),
        'reply_count'       => __('最多回复', 'zib_language'),
        'today_reply_count' => __('今日最多回复', 'zib_language'),
        'views'             => __('最多查看(热门)', 'zib_language'),
        'follow_count'      => sprintf(__('最多%s', 'zib_language'), $zib_bbs->plate_follow_name),
...
function zib_bbs_get_plate_header($plate_id = 0, $class = '', $show_cat = true)
{
    ...
    $ount_mate .= '<item class="mate-posts mr10">' . sprintf(__('帖子 %s', 'zib_language'), $all_posts_count) . '</item>';

    $reply_count = zib_bbs_get_plate_reply_cut_count($plate_id);
    $ount_mate .= $reply_count ? '<item class="mate-reply mr10">' . sprintf(__('互动 %s', 'zib_language'), zib_bbs_get_plate_reply_cut_count($plate_id)) . '</item>' : '';
    ...
    if ($follow_count) {
        ...
        $ount_mate .= '<item class="mate-views mr10">' . sprintf(__('阅读 %s', 'zib_language'), zib_bbs_get_plate_views_cut_count($plate_id)) . '</item>';
    }
...
function zib_bbs_get_plate_header_moderator_btns($post)
{
    ...
    if ($moderator_avatar) {
        ...
        $moderator_html .= $moderator_avatar . sprintf(__('%d名%s', 'zib_language'), count($moderator) + 1, $zib_bbs->plate_moderator_name) . '<i class="ml6 fa fa-angle-right em12" style="margin-right: 0;"></i>';
        ...
        $moderator_link = zib_bbs_get_apply_moderator_link($post, $class, sprintf(__('申请%s', 'zib_language'), $zib_bbs->plate_moderator_name) . '<i class="ml6 fa fa-angle-right em12" style="margin-right: 0;"></i>');
    }
    if (!$moderator_link) {
        $moderator_link = zib_bbs_get_add_plate_moderator_link($plate_id, $class, sprintf(__('添加%s', 'zib_language'), $zib_bbs->plate_moderator_name) . '<i class="ml6 fa fa-angle-right em12" style="margin-right: 0;"></i>');
    }
    $btns .= $moderator_link;
    $btns .= zib_bbs_get_posts_add_page_link(array('plate_id' => $plate_id), $class, zib_get_svg('add') . __('发布', 'zib_language'));
...
function zib_bbs_get_plate_add_link($cat = 0, $class = 'ml20 but hollow c-blue p2-10 px12 flex0', $con = '', $tag = 'botton')
{
    ...
    if (!$con) {
        global $zib_bbs;
        $con = zib_get_svg('add') . sprintf(__('创建%s', 'zib_language'), $zib_bbs->plate_name);
    }
 ...
 * @return {*}
 */
function zib_bbs_get_plate_edit_link($plate_id = 0, $cat_id = 0, $class = '', $con = null, $tag = 'botton')
{
    if (null === $con) {
        $con = '<i class="fa fa-edit"></i>' . __('版块设置', 'zib_language');
    }
...
//获取版主设置发帖权限的模态框链接
function zib_bbs_get_plate_set_add_limit_link($id = 0, $class = '', $con = null, $tag = 'a')
{
    if (null === $con) {
        $con = '<i class="fa fa-fw fa-unlock-alt mr6"></i>' . __('设置发布权限', 'zib_language');
    }
...
function zib_bbs_get_plate_follow_pay_set_link($plate_id = 0, $class = '', $con = '', $tag = 'a')
{
    ...
    if (!$con) {
        global $zib_bbs;
        $con = '<i class="fa fa-fw fa-money mr6"></i>' . sprintf(__('设置付费%s', 'zib_language'), $zib_bbs->plate_follow_name);
    }
 ...
 * @return {*}
 */
function zib_bbs_get_plate_allow_view_set_link($plate_id = 0, $class = '', $con = null, $tag = 'a')
{
    if (null === $con) {
        $con = '<i class="fa fa-fw fa-eye-slash mr6"></i>' . __('设置查看权限', 'zib_language');
    }
 ...
 * @return {*}
 */
function zib_bbs_get_set_add_limit_link($type = 'plate', $id = 0, $class = '', $con = null, $tag = 'a')
{
    if (null === $con) {
        $con = '<i class="fa fa-fw fa-unlock-alt mr6"></i>' . __('设置发布权限', 'zib_language');
    }
...
function zib_bbs_get_plate_follow_pay_cashier_modal($plate_id)
{
    ...
    $pay_title = sprintf(__('请确认付费%1$s%2$s[%3$s]', 'zib_language'), $zib_bbs->plate_follow_name, $name, get_the_title($plate_id));
    ...
    $con .= '<div class="flex jsb ab"><span class="muted-2-color">' . __('价格', 'zib_language') . '</span><div><span>' . $mark . '<span class="em14">' . $show_price . '</span></span></div></div>';
    $con .= $vip_price !== 'no' && $vip_price < $show_price ? '<div class="flex jsb ab"><span class="muted-2-color">' . __('会员价', 'zib_language') . zibpay_get_vip_icon($vip_level, 'ml3') . '</span><span class="c-red">' . $mark . '<span class="em14">' . $vip_price . '</span></span></div>' : '';
    $con .= '</div>';

    if ($pay_price == 0) {
        $con .= '<input type="hidden" name="action" value="follow_plate_confirm">';
        $con .= '<input type="hidden" name="type" value="free">';
        $con .= '<button class="but jb-blue padding-lg btn-block radius wp-ajax-submit mt10" >' . sprintf(__('免费%s', 'zib_language'), $zib_bbs->plate_follow_name) . '</button>';
    } else {
        if ($is_points) {
            //我的积分卡片
            $user_points = zibpay_get_user_points($user_id);
            $con .= '<div class="mb10 muted-box">';
            $con .= '<div class="flex jsb ab"><span class="muted-2-color">' . zib_get_svg('points-color', null, 'em12 mr6') . __('我的积分', 'zib_language') . '</span><div><span class="c-green">' . $mark . '<span class="em14">' . $user_points . '</span></span></div></div>';
            $con .= '</div>';
            ...
            if ($pay_price > $user_points) {
                ...
                $con .= '<div class="badg c-red btn-block mb20">' . __('抱歉，您的积分不足，暂时无法支付', 'zib_language') . '</div>';
                $con .= '<div class="modal-buts but-average"><a rel="nofollow" type="button" class="but padding-lg" href="' . $points_user_url . '">' . __('我的积分', 'zib_language') . '</a>' . $points_pay_link . '</div>';
            } else {
                $con .= '<input type="hidden" name="payment_method" value="points">';
                $con .= '<input type="hidden" name="action" value="submit_order">';
                $con .= '<button class="but jb-yellow padding-lg btn-block radius wp-ajax-submit mt10" >' . sprintf(__('立即支付 %s', 'zib_language'), $mark . $pay_price) . '</button>';
            }
    ...
    $header = zib_get_modal_colorful_header('jb-blue', '<i class="fa fa-cart-plus"></i>', sprintf(__('付费%1$s%2$s', 'zib_language'), $zib_bbs->plate_follow_name, $name));
    return $header . $con;
}
...
function zib_bbs_initiate_order_data_follow_plate($__data, $post_data)
{
    ...
    $user_id  = get_current_user_id();
    if (empty($post->ID)) {
        zib_send_json_error(sprintf(__('%s数据获取错误', 'zib_language'), $zib_bbs->plate_name));
    }
    ...
    if ($pay_price <= 0) {
        zib_send_json_error(sprintf(__('当前%1$s不需要付费%2$s，请刷新页面后重试', 'zib_language'), $zib_bbs->plate_name, $zib_bbs->plate_follow_name));
    }
    ...
    if ($is_points) {
        ...
        if ($payment_method === 'points') {
            zib_send_json_error(__('支付方式错误，请刷新页面后重试', 'zib_language'));
        }
    }
...
function zib_bbs_get_plate_follow_cancel_modal($plate_id)
{
    ...
    $header = zib_get_modal_colorful_header('jb-yellow', '<i class="fa fa-times"></i>', sprintf(__('取消%1$s此%2$s', 'zib_language'), $zib_bbs->plate_follow_name, $zib_bbs->plate_name));
    $con    = '<div class="">
    <span class="muted-2-color">' . sprintf(__('当前%1$s为付费%2$s，', 'zib_language'), $zib_bbs->plate_name, $zib_bbs->plate_follow_name) . '</span>
    <span class="muted-2-color">' . sprintf(__('取消后如需再次%1$s，需要重新付费！', 'zib_language'), $zib_bbs->plate_follow_name) . '</span>
    <div class="c-red mt10">' . sprintf(__('请确认真的要取消%1$s此%2$s吗？', 'zib_language'), $zib_bbs->plate_follow_name, $zib_bbs->plate_name) . '</div>

        <button class="but jb-red padding-lg btn-block radius wp-ajax-submit mt20" form-action="follow_plate_confirm" form-data="' . esc_attr(json_encode(['id' => $plate_id])) . '">' . sprintf(__('确认取消%s', 'zib_language'), $zib_bbs->plate_follow_name) . '</button>
    </div>';
...
function zib_bbs_follow_plate_toggle($id, $user_id)
{
    ...
    if ($is_follow_ed) {
        ...
        //关注
        $type   = 'add';
        $text   = sprintf(__('已%s', 'zib_language'), $zib_bbs->plate_follow_name);
        $active = true;
...
function zib_bbs_get_add_limit_btn($type = 'plate', $id = 0, $class = '', $text = '')
{
    ...
    if ('plate' === $type) {
        ...
        $desc = sprintf(__('可在此%1$s分类创建%2$s', 'zib_language'), $zib_bbs->plate_name, $zib_bbs->plate_name);
        ...
        $desc = sprintf(__('可在此%1$s发布%2$s', 'zib_language'), $zib_bbs->plate_name, $zib_bbs->posts_name);
    }

    if ($add_limit == 'follow') {
        $name        = '';
        $roles_lists = '<div class="mb6 muted-2-color">' . sprintf(__('%1$s此%2$s后可发布', 'zib_language'), $zib_bbs->plate_follow_name, $zib_bbs->plate_name) . '</div>';
    } else {
        $_id         = 'bbs_' . $type . '_add_limit_' . $add_limit;
        $_name       = _pz('user_cap', array(), $_id);
        $name        = !empty($_name['name']) ? $_name['name'] : sprintf(__('限制%s', 'zib_language'), $add_limit);
        $roles_lists = zib_bbs_get_cap_roles_lists($type . '_add_limit_' . $add_limit);
        $roles_lists = $roles_lists ? '<div class="mb6 muted-2-color">' . sprintf(__('以下用户组%s', 'zib_language'), $desc) . '</div>' . zib_str_remove_lazy($roles_lists) : '<div class="c-yellow">' . sprintf(__('仅管理员%s', 'zib_language'), $desc) . '</div>';
    }
    ...
    $title = '<i class="fa fa-unlock-alt mr6"></i>' . __('权限限制', 'zib_language');
...
function zib_bbs_get_plate_badge_popover($post = null, $class = '', $text = '')
{
    ...
    if ($add_limit) {
        if ($add_limit == 'follow') {
            $roles_lists = '<div class="separator c-yellow">' . sprintf(__('%1$s此%2$s后可发布%3$s', 'zib_language'), $zib_bbs->plate_follow_name, $zib_bbs->plate_name, $zib_bbs->posts_name) . '</div>';
        } else {
            $desc = sprintf(__('可在此%1$s发布%2$s', 'zib_language'), $zib_bbs->plate_name, $zib_bbs->posts_name);
            if (!$text) {
            ...
            $roles_lists = $roles_lists ? '<p class="separator muted-3-color mb6">' . sprintf(__('以下用户组%s', 'zib_language'), $desc) . '</p>' . $roles_lists : '<div class="separator c-yellow">' . sprintf(__('仅管理员%s', 'zib_language'), $desc) . '</div>';
        }
    ...
    $allow_view_con = str_replace(__('可查看', 'zib_language'), sprintf(__('可查看此%s内容', 'zib_language'), $zib_bbs->plate_name), $allow_view_con);
 ...
 * @return {*}
 */
function zib_bbs_get_plate_delete_link($plate_id = 0, $class = '', $con = null, $revoke_con = null, $tag = 'a')
{
    if (null === $con) {
        $con = '<i class="fa fa-trash-o fa-fw"></i>' . __('删除', 'zib_language');
    }
    if (null === $revoke_con) {
        $revoke_con = '<i class="fa fa-trash-o fa-fw mr6"></i>' . __('撤销删除', 'zib_language');
    }
...
function zib_bbs_get_plate_count_mates($plate_id = 0)
{
    ...
    //有互动优先显示互动，没有互动显示帖子
    if ($all_reply_count) {
        $all_reply_count = '<item class="mate-posts mr10" data-toggle="tooltip" title="' . esc_attr__('互动', 'zib_language') . '"><icon class="">' . zib_get_svg('comment') . '</icon>' . $all_reply_count . '</item>';
    } else {
    ...
    $follow_count = zib_bbs_get_plate_follow_cut_count($plate_id);
    $follow_count = $follow_count ? '<item class="mate-follow" data-toggle="tooltip" title="' . sprintf(__('已%s', 'zib_language'), $zib_bbs->plate_follow_name) . '"><icon class=""><i class="fa fa-heart-o"></i></icon>' . $follow_count . '</item>' : '';

    $views_count = zib_bbs_get_plate_views_cut_count($plate_id);
    $views_count = '<item class="mate-views mr10" data-toggle="tooltip" title="' . esc_attr__('热度', 'zib_language') . '"><icon class="">' . zib_get_svg('hot') . '</icon>' . $views_count . '</item>';

    $author_id = get_the_author_meta('ID');
    $author    = zib_get_avatar_box($author_id, 'avatar-mini');
    $author    = '<item class="mate-author" data-toggle="tooltip" title="' . esc_attr__('作者', 'zib_language') . '">' . $author . '</item>';
...
function zib_bbs_get_plate_card($class = '', $follow = true, $today = false)
{
    ...
    $title_attr = $follow_count ? sprintf(__('%1$s人%2$s', 'zib_language'), $follow_count, $zib_bbs->plate_follow_name) : '';
    $title_attr .= $title_attr ? ' ' : '';
    $title_attr .= $all_reply_count ? sprintf(__('%s次互动', 'zib_language'), $all_reply_count) : '';
    ...
    if ($today) {
        ...
        $today             = $today_reply_count ? '<badge class="spot abs-tr" data-toggle="tooltip" title="' . sprintf(__('今日有%s次新互动', 'zib_language'), $today_reply_count) . '"></badge>' : '';
    }
    ...
    $html .= '<div class="mt3 px12 muted-2-color text-ellipsis count"' . $title_attr . '>' . sprintf(__('%s热度', 'zib_language'), $views_count) . '</div>';
    $html .= '</a>';
    $html .= $follow_btn;
    $html .= '</div>';
...
function zib_bbs_get_main_plate($class = '')
{
    ...
    $today_reply_count = $today_reply_count ? '<span class="badg but-plate hide-sm flex0" title="' . sprintf(__('今日互动 %s', 'zib_language'), $today_reply_count) . '">' . zib_get_svg('comment-fill', null, 'icon c-red em12') . ' ' . sprintf(__('今日：%s', 'zib_language'), $today_reply_count) . '</span>' : '';
    $target_blank      = _pz('plate_target_blank') ? ' target="_blank"' : '';
...
function zib_bbs_get_all_plate_follow_btn($plate_id, $class = 'but jb-pink', $text = '', $ok_text = '')
{
    ...
    if (!$ok_text) {
        $ok_text = sprintf(__('已%s', 'zib_language'), $zib_bbs->plate_follow_name);
    }
...
function zib_bbs_get_plate_posts_count($post = null, $today = false)
{
    ...
    if (!isset($bbs_plate_posts_count[$type])) {
        ...
        $db->field('_m.meta_value as id, count(_p.ID) as count');
        $db->join([$wpdb->postmeta, '_m'], '_p.ID = _m.post_id and _m.meta_key = "plate_id"');
        $db->where('_p.post_type', 'forum_post');
        $db->where('_p.post_status', 'publish');
        if ($today) {
            $db->whereTime('_p.post_date_gmt', 'today');
        }

        $counts = $db->group('_m.meta_value')->select()->result();
        //保存到全局变量
    ...
    if ($counts) {
        foreach ($counts as $count_item) {
            if ($count_item->id) {
                wp_cache_set($count_item->id, $count_item->count, 'plate_posts_count_' . $type);
            }
        }
    }
...
function zib_bbs_get_plate_reply_count($id = 0, $today = false)
{
    ...
    if (!isset($bbs_plate_reply_count[$type])) {
        /**
         * SELECT _m.meta_value as id, count(*) as count FROM aaa_comments AS _c
         * INNER JOIN aaa_posts AS _p ON (_c.comment_post_ID = _p.ID AND _c.comment_approved = "1")
         * INNER JOIN aaa_postmeta AS _m ON (_p.ID = _m.post_id and _m.meta_key = "plate_id")
         * WHERE (_p.post_type = 'forum_post' AND _p.post_status = 'publish' AND _c.comment_date_gmt BETWEEN '2026-05-05 00:00:00' AND '2026-05-05 23:59:59')
         * GROUP BY _m.meta_value
         */

        $db = zibDB::table($wpdb->comments)->alias('_c');
        $db->field('_m.meta_value as id, count(*) as count');
        $db->join([$wpdb->posts, '_p'], '_c.comment_post_ID = _p.ID AND _c.comment_approved = "1"');
        $db->join([$wpdb->postmeta, '_m'], '_p.ID = _m.post_id and _m.meta_key = "plate_id"');
        $db->where('_p.post_type', 'forum_post');
        $db->where('_p.post_status', 'publish');
        if ($today) {
            $db->whereTime('_c.comment_date_gmt', 'today');
        }
        $counts = $db->group('_m.meta_value')->select()->result();
        //保存到全局变量
...
function zib_bbs_get_plate_info_mini_box($post = null, $class = '')
{
    ...
    $ount_mate .= '<item class="mate-views"><div class="em09 opacity5 mb6">' . __('互动', 'zib_language') . '</div><div class="em14"> ' . zib_bbs_get_plate_reply_cut_count($plate_id) . '</div></item>';
    $ount_mate .= '<item class="mate-views"><div class="em09 opacity5 mb6">' . __('阅读', 'zib_language') . '</div><div class="em14"> ' . zib_bbs_get_plate_views_cut_count($plate_id) . '</div></item>';
    $ount_mate = '<div class="count-mates text-center">' . $ount_mate . '</div>';

    $follow_count = zib_bbs_get_plate_follow_cut_count($plate_id);
    $follow_html  = $follow_count ? '<badge class="img-badge px12 follow-info">' . sprintf(__('%1$s人已%2$s', 'zib_language'), $follow_count, $zib_bbs->plate_follow_name) . '</badge>' : '';
...
function zib_bbs_get_plate_search_btn($plate_id, $class = '')
{
    ...
    $args = array(
        ...
        'type'        => 'forum',
        'placeholder' => sprintf(__('在%1$s[%2$s]中搜索%3$s', 'zib_language'), $zib_bbs->plate_name, $post_title, $zib_bbs->posts_name),
    );
...
function zib_bbs_get_plate_header_more_btn($plate_id, $class = '', $show_follow = false)
{
    ...
    $html = '';
    if ($show_follow) {
        $follow_btn = zib_bbs_get_all_plate_follow_btn($plate_id, $class, '<i title="' . esc_attr($zib_bbs->plate_follow_name) . '" class="fa fa-heart-o mr6 em12"></i>', '<i title="' . esc_attr(sprintf(__('已%s', 'zib_language'), $zib_bbs->plate_follow_name)) . '" class="fa fa-heart mr6 em12"></i>');
...
function zib_bbs_get_plate_more_dropdown($plate_id, $class = '', $con_class = '', $con = '')
{
    ...
    $add = zib_bbs_get_plate_add_link(false, '', zib_get_svg('add', null, 'icon mr6 fa-fw') . sprintf(__('创建新%s', 'zib_language'), $name), 'a');
    ...
    $edit = zib_bbs_get_edit_plate_moderator_link($plate_id, '', zib_get_svg('circle', null, 'icon mr6 fa-fw') . sprintf(__('管理%s', 'zib_language'), $zib_bbs->plate_moderator_name));
    $action .= $edit ? '<li>' . $edit . '</li>' : '';

    $del = zib_bbs_get_plate_delete_link($plate_id, 'c-red', '<i class="fa fa-trash-o mr6 fa-fw"></i>' . sprintf(__('删除此%s', 'zib_language'), $name));
...
function zib_bbs_get_search_plate()
{
    ...
    if ($lists) {
        ...
        $lists = zib_get_ajax_null(__('未找到相关内容', 'zib_language'), '75', 'null-search.svg');
    }
...
function zib_bbs_plate_page_locate()
{
    ...
    if ('trash' === $post_status) {
        ...
        $plate_id = $post->ID;
        $name     = $zib_bbs->plate_name;
        $con      = '<p class="muted-2-color box-body separator" style="margin:60px 0;">' . sprintf(__('当前%s已删除', 'zib_language'), $zib_bbs->plate_name) . '</p>';
        $edit     = zib_bbs_get_plate_edit_link($plate_id, false, 'but hollow c-blue padding-lg', sprintf(__('重新发布此%s', 'zib_language'), $name) . '<b><i class="fa fa-angle-right ml10 em12"></i></b>', 'a');
...
function zib_bbs_plate_page_content()
{
    ...
    if ($is_hide) {
        echo '<div class="plate-tab zib-widget">' . zib_get_null(__('抱歉！您暂无查看此版块内容的权限', 'zib_language'), 10, 'null-cap.svg', '') . $is_hide . '</div>';
    } else {
...
function zib_bbs_get_plate_allow_view_btn($post = 0, $class = '', $not_show = true)
{
    ...
    $title = '<i class="fa fa-unlock-alt mr6"></i>' . sprintf(__('%s阅读限制', 'zib_language'), $type_name);
    ...
    if ($post && (zib_bbs_current_user_can('posts_audit', $post) || zib_bbs_current_user_can('posts_edit_other', $post))) {
        if (zib_bbs_current_user_can('posts_audit', $post)) {
            ...
            $tabs_options['pending'] = array(
                'title'       => zib_get_svg('approve', null, 'icon mr3') . __('待审核', 'zib_language'),
                'post_status' => 'pending',
                'show'        => array('pc_s', 'm_s'),
            );
        }
        if (zib_bbs_current_user_can('posts_edit_other', $post)) {
            ...
            $tabs_options['trash'] = array(
                'title'       => '<i class="fa fa-trash-o mr3"></i>' . __('回收站', 'zib_language'),
                'post_status' => 'trash',
                'show'        => array('pc_s', 'm_s'),
            );
        }
...
function zib_bbs_get_plate_tab_content_manage($html = '', $option = array())
{
    ...
    if (!$lists) {
        $msg_args = array(
            'trash'   => __('回收站暂无内容', 'zib_language'),
            'pending' => __('暂无待审核内容', 'zib_language'),
        );
...
function zib_bbs_get_plate_tab_content_plate($html = '', $option = array())
{
    ...
    if (!$lists) {
        $lists = zib_get_ajax_null(__('内容空空如也', 'zib_language'));
        if (1 == $index && 1 == $paged) {
            global $zib_bbs;
            $posts_add = zib_bbs_get_posts_add_page_link(array('plate_id' => get_queried_object_id()), 'ml10 mr10 mt10 but hollow c-green padding-lg', zib_get_svg('add') . sprintf(__('发布%s', 'zib_language'), $zib_bbs->posts_name));
