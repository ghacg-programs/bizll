 ...
 * @LastEditTime : 2026-06-09 13:14:07
...
class zib_bbs_edit
{
    ...
    public static function desc_text($inargs = array())
    {
        ...
        if ($inargs['post_status'] && 'draft' == $inargs['post_status']) {
            $submit_text = '<div class="muted-2-color">' . __('正在编辑草稿', 'zib_language') . '</div>';
        }
        ...
        if ($inargs['post_id']) {
            if ($inargs['post_author'] && $current_user_id != $inargs['post_author']) {
                ...
                $submit_text = '<div class="muted-2-color mt6 mr10">' . sprintf(__('正在修改[%s]提交的内容', 'zib_language'), zib_get_user_name("id=$post_author&auth=0&level=0&medal=0&class=&name_class=focus-color")) . '</div>';
            }
            $submit_text .= '<div class="muted-3-color px12 mt6"><span class="badg badg-sm mr6">' . __('编辑内容', 'zib_language') . '</span>' . sprintf(__('最后更新：%s', 'zib_language'), get_the_modified_time('Y-m-d H:i:s', $inargs['post_id'])) . '</div>';
        }
        ...
        if (!$submit_text) {
            ...
            if ($draft_post_count) {
                $submit_text = '<a target="_blank" href="' . zib_get_user_home_url($current_user_id, array('tab' => 'forum', 'status' => 'draft')) . '" class="but mr10 p2-10 px12">' . sprintf(__('您已有%d篇草稿 点击查看', 'zib_language'), $draft_post_count) . '<i class="ml6 fa fa-angle-right em12"></i></a>';
            }
        }
    ...
    public static function posts_submit($inargs = array())
    {
        ...
        if (!get_current_user_id()) {
            return '<div class="but-average mb20 main-radius"><botton type="button" action="bbs_posts_draft" name="submit" class="but jb-green signin-loader padding-lg"><i class="fa fa-fw fa-dot-circle-o"></i>' . __('保存草稿', 'zib_language') . '</botton><botton type="button" action="bbs_posts_save" class="but jb-blue padding-lg signin-loader "><i class="fa fa-fw fa-check-square-o"></i>' . __('提交发布', 'zib_language') . '</botton></div>';
        }
        ...
        if ('publish' !== $post_status && 'pending' !== $post_status && (!$post_id || !zib_current_user_can('posts_audit', $post_id))) {
            ...
            $_draft = '<botton type="button" action="bbs_posts_draft" name="submit" class="but jb-green bbs-posts-submit padding-10"><i class="fa fa-fw fa-dot-circle-o"></i>' . __('保存草稿', 'zib_language') . '</botton>';
        }
        ...
        $submit_text = $post_id ? __('保存', 'zib_language') : __('发布', 'zib_language');
        $btns        = $verification_input;
        $btns .= '<div class="but-average mb20 main-radius">';
        $btns .= $_draft;
        $btns .= '<botton type="button" action="bbs_posts_save" name="submit" class="but jb-blue bbs-posts-submit padding-10"><i class="fa fa-paper-plane-o fa-fw"></i>' . sprintf(__('提交%s', 'zib_language'), $submit_text) . '</botton>';
        $btns .= $hidden;
        $btns .= '</div>';
    ...
    public static function posts_title($title = '', $class = '', $name = 'post_title')
    {
        ...
        $placeholder = _pz('bbs_t_placeholder', __('请输入标题', 'zib_language'));
        $class       = $class ? ' ' . $class : '';
    ...
    public static function posts_editor($in_args = array(), $id = 'post_content', $settings = array())
    {
        ...
        $defaults = array(
            ...
            'teeny'          => false,
            'tinymce'        => array(
                'placeholder' => _pz('bbs_c_placeholder', __('请输入内容', 'zib_language')),
            ),
        );
    ...
    public static function quick_posts($args = array())
    {
        ...
        $content_input = '<div class="relative edit-posts-content"><textarea type="text" class="" name="post_content" tabindex="2" placeholder="' . esc_attr(_pz('bbs_c_placeholder', __('请输入内容', 'zib_language'))) . '" autoheight="true" maxheight="200" style=""></textarea></div>';
        $upload_input  = '';
        ...
        $topic_input = '<div class="topic-drop drop-select dropdown mr20">
            ...
            <div class="dropdown-menu fluid">
                <div class="flex ac jsb padding-10">
                    <div data-for="topic" data-value="null" class="pointer but p2-10"><i class="fa fa-ban mr6"></i>' . sprintf(__('不添加%s', 'zib_language'), $zib_bbs->topic_name) . '</div>
                </div>
        ...
        if (zib_bbs_current_user_can('posts_allow_view_add')) {
            ...
            <div data-target=".allow-view-drop" data-toggle-class="open" class="flex ac jsb drop-btn"><icon class="mr3">' . zib_get_svg('view', null, '') . '</icon><span name="allow_view">' . __('公开', 'zib_language') . '</span></div>
            <div class="dropdown-menu fluid dependency-box">
                <div class="mini-scrollbar padding-10 select-drop-box">
                <div class="em09 muted-3-color mb10">' . sprintf(__('设置该%s的阅读权限', 'zib_language'), $zib_bbs->posts_name) . '</div>
        ...
        if ($user_id && zib_bbs_current_user_can('posts_hide')) {
            ...
            <div data-target=".hide-type-drop" data-toggle-class="open" class="flex ac jsb drop-btn"><icon class="mr3"><i class="fa fa-eye-slash"></i></icon><span>' . __('隐藏内容', 'zib_language') . '</span></div>
            <div class="dropdown-menu fluid dependency-box">
                <div class="mini-scrollbar padding-10 select-drop-box">
                <div class="em09 muted-3-color mb10">' . __('添加额外的隐藏内容', 'zib_language') . '</div>
                <div class="flex hh">' . $items . '</div>
                <input type="hidden" name="hide_type" value="logged">
                <textarea rows="6" name="hide_content" class="form-control input-textarea" placeholder="' . esc_attr__('在此输入需要隐藏的内容', 'zib_language') . '"></textarea>
                </div>
            </div>
        </div>';
        }
        ...
        <div data-target=".plate-drop" data-toggle-class="open" class="flex ac jsb drop-btn">
            <div class="flex ac jsb but-plate but">
                <div class="flex ac">
                    <span  name="plate" class="flex ac"><icon class="mr6">' . zib_get_svg('plate-fill') . '</icon><span>' . __('请选择', 'zib_language') . '</span>' . $zib_bbs->plate_name . '</span>
                </div>
        ...
        <div class="flex ac jsb drop-btn">
            ...
            $allow_view_input = '<div class="flex ac jsb drop-btn mr20 signin-loader"><icon class="mr3">' . zib_get_svg('view', null, '') . '</icon><span name="allow_view">' . __('公开', 'zib_language') . '</span></div>';
            $hide_input       = '<div class="flex ac jsb drop-btn mr20 signin-loader"><icon class="mr3"><i class="fa fa-eye-slash"></i></icon><span>' . __('隐藏内容', 'zib_language') . '</span></div>';
        }
        ...
        ' . $verification_input . '
            <botton type="button" tabindex="3" class="but c-green mr6 ' . ($user_id ? 'bbs-posts-submit' : 'signin-loader') . '" action="bbs_posts_draft" name="submit">' . __('保存', 'zib_language') . '</botton>
            <botton type="button" tabindex="4" class="but jb-blue ' . ($user_id ? 'bbs-posts-submit' : 'signin-loader') . '" action="bbs_posts_save" name="submit">' . __('发布', 'zib_language') . '</botton>
        </div>';
    ...
    public static function allow_view_set($id = 0, $class = 'zib-widget mb10-sm mb20')
    {
        if (!get_current_user_id()) {
            ...
            return '<div class="allow-view-drop signin-loader"><div class="flex ac jsb drop-btn ' . $class . '">
                        <div class="flex ac">' . zib_get_svg('view', null, 'mr6') . __('阅读权限', 'zib_language') . '</div>
                        <div class="flex ac"><span class="em09 muted-2-color">' . __('公开', 'zib_language') . '</span><i class="ml6 fa fa-angle-right em12"></i></div>
                    </div></div>';
        }
        ...
        $in_type      = '';
        $in_trpe_text = __('公开', 'zib_language');
        if ($id) {
        ...
        $aciton  = '<div class="muted-color pointer text-right mb10" ' . $ok_attr . '>' . __('完成', 'zib_language') . '</div>';
        //显示按钮
        $btn_attr = $wp_is_mobile ? 'data-drawer="show" drawer-title="' . esc_attr__('阅读权限设置', 'zib_language') . '" drawer-selector=".allow-view-drawer"' : 'data-toggle-class="open" data-target=".allow-view-drop"';
        ...
        if ($wp_is_mobile) {
            ...
            $dropdown = '<div class="dropdown-menu fluid" style="margin-bottom: -4px;"><div class="padding-10 flex jsb">' . __('设置阅读权限', 'zib_language') . $aciton . '</div><div class="mini-scrollbar padding-w10 select-drop-box">' . $con . '</div></div>';
    ...
    public static function plate_follow_pay_set_content($plate_id)
    {
        ...
        <div class="flex ac">' . sprintf(__('付费%s', 'zib_language'), $zib_bbs->plate_follow_name) . '</div>
        ...
        if ($points_s) {
            $pay_modo_input .= '<div class="flex ac jsb padding-h10">
                <div class="flex ac">' . __('支付类型', 'zib_language') . '</div>
                <div class="but-average radius em09">
                    <span data-for="follow_pay[pay_modo]" data-value="0" class="but p2-10 pointer' . ($in['pay_modo'] !== 'points' ? ' active' : '') . '">' . __('现金支付', 'zib_language') . '</span>
                    <span data-for="follow_pay[pay_modo]" data-value="points" class="but p2-10 pointer' . ($in['pay_modo'] === 'points' ? ' active' : '') . '">' . __('积分支付', 'zib_language') . '</span>
                </div>
            </div>';
        }
        ...
        if ($pay_price_limit_min) {
            $pay_price_limit_attr .= ' limit-min="' . $pay_price_limit_min . '" warning-min="' . esc_attr(sprintf(__('最低可设置：%s', 'zib_language'), $pay_price_limit_min)) . '"';
        }
        if ($pay_price_limit_max) {
            $pay_price_limit_attr .= ' limit-max="' . $pay_price_limit_max . '" warning-max="' . esc_attr(sprintf(__('最高可设置：%s', 'zib_language'), $pay_price_limit_max)) . '"';
        }
        if ($vip_input_s && $vip_1_s) {
            $vip_pay_price_input .= '<div class="relative mt6">
        <div class="flex ab">
            <div class="muted-color mb6 flex0">' . zibpay_get_vip_icon(1, 'em12 mr6', false) . esc_html(_pz('pay_user_vip_1_name')) . __('价格', 'zib_language') . '</div><input limit-min="0"' . $pay_price_limit_attr . ' type="number" name="follow_pay[vip_1_price]" value="' . $in['vip_1_price'] . '" style="padding: 0;" class="line-form-input em2x key-color text-right">
        ...
        if ($vip_input_s && $vip_2_s) {
            $vip_pay_price_input .= '<div class="relative mt6">
        <div class="flex ab">
            <div class="muted-color mb6 flex0">' . zibpay_get_vip_icon(2, 'em12 mr6', false) . esc_html(_pz('pay_user_vip_2_name')) . __('价格', 'zib_language') . '</div><input limit-min="0"' . $pay_price_limit_attr . ' type="number" name="follow_pay[vip_2_price]" value="' . $in['vip_2_price'] . '" style="padding: 0;" class="line-form-input em2x key-color text-right">
            <i class="line-form-line"></i>
        </div>
    </div>';
        }
        if ($vip_pay_price_input) {
            $vip_pay_price_input .= '<div class="px12 mt6 muted-color">' . __('会员价不能高于普通价，为0则为会员免费', 'zib_language') . '</div>';
        }
        //设置金额
    ...
    <div class="relative">
        <div class="flex ab">
            <div class="muted-color mb6 flex0">' . $money_icon . __('设置价格', 'zib_language') . '</div><input' . $pay_price_limit_attr . ' type="number" name="follow_pay[pay_price]" value="' . $in['pay_price'] . '" style="padding: 0;" class="line-form-input em2x key-color text-right">
            <i class="line-form-line"></i>
        </div>
        ...
        if ($vip_input_s && $vip_1_s) {
            $vip_pay_price_input .= '<div class="relative mt6">
            <div class="flex ab">
                <div class="muted-color mb6 flex0">' . zibpay_get_vip_icon(1, 'em12 mr6', false) . esc_html(_pz('pay_user_vip_1_name')) . __('积分', 'zib_language') . '</div><input limit-min="0" ' . $pay_price_limit_attr . ' type="number" name="follow_pay[vip_1_points]" value="' . $in['vip_1_points'] . '" style="padding: 0;" class="line-form-input em2x key-color text-right">
                <i class="line-form-line"></i>
            </div>
        </div>';
        }
        if ($vip_input_s && $vip_2_s) {
            $vip_pay_price_input .= '<div class="relative mt6">
            <div class="flex ab">
                <div class="muted-color mb6 flex0">' . zibpay_get_vip_icon(2, 'em12 mr6', false) . esc_html(_pz('pay_user_vip_2_name')) . __('积分', 'zib_language') . '</div><input limit-min="0" ' . $pay_price_limit_attr . ' min="0" type="number" name="follow_pay[vip_2_points]" value="' . $in['vip_2_points'] . '" style="padding: 0;" class="line-form-input em2x key-color text-right">
                <i class="line-form-line"></i>
            </div>
        </div>';
        }
        ...
        <div class="flex ab">
            <div class="muted-color mb6 flex0">' . zib_get_svg('points-color', null, 'mr6 em12') . __('设置积分', 'zib_language') . '</div><input type="number"' . $pay_price_limit_attr . ' name="follow_pay[points_price]" value="' . $in['points_price'] . '" style="padding: 0;" class="line-form-input em2x key-color text-right">
            <i class="line-form-line"></i>
        </div>
    </div>' . $vip_pay_price_input . '</div>';
        }
        ...
        if (_pz('pay_income_s')) {
            ...
            if ($income_ratio) {
                $income_desc .= '<div class="c-blue px12 mt6">' . sprintf(__('您已参与创作分成，此收益将与您分成，您可以进入%s查看您的分成明细', 'zib_language'), '<a rel="nofollow" target="_blank" class="c-blue-2" href="' . esc_url(zib_get_user_center_url('income')) . '">' . __('用户中心-创作分成', 'zib_language') . '</a>') . '</div>';
            }
        }
    ...
    public static function plate_allow_view_set_content($id = 0, $class = 'mt20', $show_follow_pay = false)
    {
        ...
        $allow_view_types = array(
            ...
            'follow' => sprintf(__('%1$s此%2$s后可查看', 'zib_language'), $zib_bbs->plate_follow_name, $zib_bbs->plate_name),
        );
        ...
        $type_html = '<div class="flex hh">' . $type_lists_html . '<input type="hidden" name="allow_view" value="' . $in_type . '"></div>';
        //汇总
        $con = $type_html;
    ...
    public static function allow_view_set_content($id = 0)
    {
        ...
        return '<div class="flex hh">' . $lists . '</div><div class="">' . $controller . '</div><input type="hidden" name="allow_view" value="' . $in_type . '">';
    }
    ...
    public static function pay_option($post_id = 0)
    {
        ...
        if ($vip_input_s && $vip_1_s) {
            $vip_pay_price_input .= '<div class="relative mt6">
            <div class="flex ab">
                <div class="muted-color mb6 flex0">' . zibpay_get_vip_icon(1, 'em12 mr6', false) . esc_html(_pz('pay_user_vip_1_name')) . __('价格', 'zib_language') . '</div><input limit-min="0"' . $pay_price_limit_attr . ' type="number" name="posts_zibpay[vip_1_price]" value="' . $in['vip_1_price'] . '" style="padding: 0;" class="line-form-input em2x key-color text-right">
                <i class="line-form-line"></i>
            </div>
        </div>';
        }
        if ($vip_input_s && $vip_2_s) {
            $vip_pay_price_input .= '<div class="relative mt6">
            <div class="flex ab">
                <div class="muted-color mb6 flex0">' . zibpay_get_vip_icon(2, 'em12 mr6', false) . esc_html(_pz('pay_user_vip_2_name')) . __('价格', 'zib_language') . '</div><input limit-min="0"' . $pay_price_limit_attr . ' type="number" name="posts_zibpay[vip_2_price]" value="' . $in['vip_2_price'] . '" style="padding: 0;" class="line-form-input em2x key-color text-right">
                <i class="line-form-line"></i>
            </div>
        </div>';
        }
        ...
        $html = '<div class="mb20"><div class="relative">
                    <div class="flex ab">
                    <div class="muted-color mb6 flex0">' . zib_get_svg('money-color-2', null, 'mr6 em12') . __('设置价格', 'zib_language') . '</div><input' . $pay_price_limit_attr . ' type="number" name="posts_zibpay[pay_price]" value="' . $in['pay_price'] . '" style="padding: 0;" class="line-form-input em2x key-color text-right">
                    <i class="line-form-line"></i>
                </div>
                </div>' . $vip_pay_price_input . '</div>';
    ...
    public static function points_option($post_id = 0)
    {
        ...
        if ($vip_input_s && $vip_1_s) {
            $vip_pay_price_input .= '<div class="relative mt6">
        <div class="flex ab">
            <div class="muted-color mb6 flex0">' . zibpay_get_vip_icon(1, 'em12 mr6', false) . esc_html(_pz('pay_user_vip_1_name')) . __('积分', 'zib_language') . '</div><input limit-min="0"' . $pay_price_limit_attr . ' type="number" name="posts_zibpay[vip_1_points]" value="' . $in['vip_1_points'] . '" style="padding: 0;" class="line-form-input em2x key-color text-right">
            <i class="line-form-line"></i>
        </div>
    </div>';
        }
        if ($vip_input_s && $vip_2_s) {
            $vip_pay_price_input .= '<div class="relative mt6">
        <div class="flex ab">
            <div class="muted-color mb6 flex0">' . zibpay_get_vip_icon(2, 'em12 mr6', false) . esc_html(_pz('pay_user_vip_2_name')) . __('积分', 'zib_language') . '</div><input limit-min="0"' . $pay_price_limit_attr . ' type="number" name="posts_zibpay[vip_2_points]" value="' . $in['vip_2_points'] . '" style="padding: 0;" class="line-form-input em2x key-color text-right">
            <i class="line-form-line"></i>
        </div>
    </div>';
        }
        ...
        $html = '<div class="mb20">
                <div class="relative">
                    <div class="flex ab">
                        <div class="muted-color mb6 flex0">' . zib_get_svg('points-color', null, 'mr6 em12') . __('设置积分', 'zib_language') . '</div><input' . $pay_price_limit_attr . ' type="number" name="posts_zibpay[points_price]" value="' . $in['points_price'] . '" style="padding: 0;" class="line-form-input em2x key-color text-right">
                        <i class="line-form-line"></i>
                    </div>
                </div>' . $vip_pay_price_input . '</div>';
    ...
    public static function pay_public_option($post_id = 0)
    {
        ...
        if (_pz('pay_income_s')) {
            ...
            if ($income_ratio) {
                $desc .= '<div class="c-blue px12 mt6">' . sprintf(__('您已参与创作分成，本文获得的收益将与您分成，您可以进入%s查看您的分成比例及分成详情', 'zib_language'), '<a rel="nofollow" target="_blank" class="c-blue-2" href="' . esc_url(zib_get_user_center_url('income')) . '">' . __('用户中心-创作分成', 'zib_language') . '</a>') . '</div>';
            }
        }
        ...
        if ($hide_type_s) {
            ...
            <label class="muted-color flex ac jsb" style="font-weight: normal; ">
                <input class="hide"' . ($pay_hide_part ? ' checked="checked"' : '') . ' type="checkbox" name="pay_hide_part">' . __('只隐藏部分内容', 'zib_language') . '<div class="form-switch flex0"></div>
            </label>
            <div class="muted-3-color px12 mt3">' . sprintf(__('默认会隐藏%s全部内容，开启后则只会隐藏部分内容，请在内容中添加【隐藏内容-付费可见】内容', 'zib_language'), $name) . '</div>
        </div>';
        }

        return $hide_type_input . '<div class="relative">
                    <textarea class="form-control" name="posts_zibpay[pay_doc]" placeholder="' . esc_attr__('请输入内容摘要', 'zib_language') . '" rows="3">' . $pay_doc . '</textarea>
    ...
    public static function allow_view_roles_lists($post_id = 0)
    {
        ...
        if ($vip_count) {
            if (count($vip_count) < 2) {
                $vip_roles_lists = '<div><label class="badg p2-10 mr10 pointer"><input' . ($in['vip'] ? ' checked="checked"' : '') . ' type="radio" name="allow_view_roles[vip]" value="1">' . zibpay_get_vip_icon(1, 'mr6 ml6 em12') . esc_html(_pz('pay_user_vip_1_name')) . __('可查看', 'zib_language') . '</label></div>';
            } else {
                $vip_roles_lists = '<div><label class="badg p2-10 mr10 pointer"><input type="radio"' . (checked($in['vip'], 1, false)) . ' name="allow_view_roles[vip]" value="1">' . zibpay_get_vip_icon(1, 'mr6 ml6 em12') . esc_html(_pz('pay_user_vip_1_name')) . __('及以上会员可查看', 'zib_language') . '</label></div>';
                $vip_roles_lists .= '<div><label class="badg p2-10 mr10 pointer"><input type="radio"' . (checked($in['vip'], 2, false)) . ' name="allow_view_roles[vip]" value="2">' . zibpay_get_vip_icon(2, 'mr6 ml6 em12') . esc_html(_pz('pay_user_vip_2_name')) . __('可查看', 'zib_language') . '</label></div>';
            }
            $vip_roles_lists = '<div><label class="badg p2-10 mr10 pointer"><input type="radio"' . (!$in['vip'] ? ' checked="checked"' : '') . ' name="allow_view_roles[vip]" value=""><span class="ml6">' . __('不限制会员', 'zib_language') . '</span></label></div>' . $vip_roles_lists;
            $vip_roles_lists = '<div class="mb20"><div class="muted-color mb6">' . __('会员限制', 'zib_language') . '</div><div class="">' . $vip_roles_lists . '</div><div class="muted-3-color em09">' . __('允许查看的会员组', 'zib_language') . '</div></div>';
        }
        ...
        if (_pz('user_level_s', true)) {
            ...
            $level_roles_lists = $level_roles_lists ? '<div><label class="badg p2-10 mr10 pointer"><input type="radio"' . (!$in['level'] ? ' checked="checked"' : '') . ' name="allow_view_roles[level]" value=""><span class="ml6">' . __('不限制等级', 'zib_language') . '</span></label></div>' . $level_roles_lists : '';
            $level_roles_lists = $level_roles_lists ? '<div class="mb20"><div class="muted-color mb6">' . __('等级限制', 'zib_language') . '</div><div class="">' . $level_roles_lists . '</div><div class="muted-3-color em09">' . __('用户的等级达到所选等级时允许查看', 'zib_language') . '</div></div>' : '';
        }

        //认证用户
        $auth_roles_lists = '';
        if (_pz('user_auth_s', true)) {
            $auth_roles_lists = '<label class="muted-color flex ac jsb" style="font-weight: normal; "><input class="hide"' . ($in['auth'] ? ' checked="checked"' : '') . ' type="checkbox" name="allow_view_roles[auth]">' . __('允许认证用户查看', 'zib_language') . '<div class="form-switch flex0"></div></label>';
        }
    ...
    public static function vote_set($id = 0, $class = 'zib-widget mb20')
    {
        if (!get_current_user_id()) {
            ...
            return '<div class="vote-set signin-loader">
                        <div class="flex ac jsb drop-btn ' . $class . '">
                            <div class="flex ac"><i class="fa fa-bar-chart mr6"></i>' . __('投票', 'zib_language') . '</div>
                            <i class="ml6 fa fa-angle-right em12"></i>
                        </div>
                    </div>';
        }
        ...
        if (!$can_edit) {
            return '<div class="vote-set">
                        <div class="flex ac jsb ' . $class . '">
                            <div class="flex ac"><i class="fa fa-bar-chart mr6"></i>' . __('投票', 'zib_language') . '</div>
                            <div><span class="c-red badg badg-sm">' . __('投票已开始，已无法修改', 'zib_language') . '</span></div>
                        </div>
                    </div>';
        }

        //投票标题
        $title_html = $vote_ing ? '<div class="c-red badg mt10 em09" style="width: 100%;"><i class="fa fa-fw fa-info-circle mr6"></i>' . __('投票已开始，请谨慎修改投票选项', 'zib_language') . '</div>' : '';
        $title_html .= '<div class="relative mb10"><input type="text" class="line-form-input" name="vote[title]" placeholder="' . esc_attr__('请输入投票标题', 'zib_language') . '" value="' . esc_attr($opt['title']) . '"><i class="line-form-line"></i></div>';

        //投票选项
        $option_html = '';
        foreach ($opt['options'] as $k) {
            $option_html .= '<div class="cloneable-item vote-opt-item form-right-icon mb6"><input type="input" class="form-control" name="vote[options][]" value="' . esc_attr($k) . '" placeholder="' . esc_attr__('请输入选项内容', 'zib_language') . '"><a href="javascript:;" class="abs-right muted-color cloneable-remove">' . zib_get_svg('close') . '</a></div>';
        }
        $option_html = '<div class="cloneable vote-options" data-max="' . (_pz('bbs_vote_max') ?: 8) . '" data-min="2">' . $option_html . '</div>';

        $option_html .= '<botton type="button" class="cloneable-add but block c-blue">' . zib_get_svg('add') . __('添加选项', 'zib_language') . '</botton>';

        $option_html = '<div class="mb10"><div class="em09 muted-2-color mb6">' . __('投票选项', 'zib_language') . '</div>' . $option_html . '</div>';

        //投票类型
        $type_html  = '';
        $types_args = array(
            'single'   => __('单选', 'zib_language'),
            'multiple' => __('多选', 'zib_language'),
            'pk'       => __('双选PK', 'zib_language'),
        );
        ...
        $type_html = '<div class="mb10"><div class="em09 muted-2-color mb6">' . __('投票类型', 'zib_language') . '</div>' . $type_html . '</div>';
        ...
        $time_limit_html = '<div class=""><div class="em09 muted-2-color mb6">' . __('投票有效期', 'zib_language') . '</div>' . $time_limit_html . '</div>';

        if ($opt['time']) {
            $time_limit_html .= '<div class="mt20 flex ac jsb muted-color"><div class="em09 muted-2-color">' . __('投票开始时间', 'zib_language') . '</div>' . esc_attr($opt['time']) . '</div>';
        }
    ...
    public static function status($in_args = array(), $class = 'zib-widget mb10')
    {
        ...
        $lists       = '';
        $status_args = array(
            'pending' => __('待审核', 'zib_language'),
            'draft'   => __('草稿', 'zib_language'),
            'publish' => __('发布', 'zib_language'),
        );
        ...
        $html = '<div class="type-drop ' . $class . '">
                    <div class="flex ac jsb">
                        <div class="flex ac c-red"><i class="fa fa-thumb-tack mr10"></i>' . __('设置状态', 'zib_language') . '</div>
                        <div class="em09">
                        ' . $lists . '
                        </div>
                    </div>
    ...
    public static function type_select($id = 0, $class = 'zib-widget mb10-sm mb20')
    {
        ...
        $html = '<div class="type-drop ' . $class . '">
                    <div class="flex ac jsb">
                        <div class="flex ac"><i class="fa fa-delicious mr6"></i>' . __('发布类型', 'zib_language') . '</div>
                        <div class="but-average radius em09">
                        ' . $lists . '
                        </div>
                    </div>
    ...
    public static function post_types($id)
    {
        ...
        $default = array('' => __('标准', 'zib_language'));
        $types   = array();

        if (zib_bbs_current_user_can('posts_type_question', $id)) {
            $types['question'] = __('提问', 'zib_language');
        }
    ...
    public static function tag_select($in = array(), $class = 'zib-widget mb10-sm mb20')
    {
        ...
        if (!get_current_user_id()) {
            ...
            return '<div class="topic-drop signin-loader">
                        <div class="flex ac jsb drop-btn ' . $class . '">
                            <div class="flex ac"><icon class="mr6">' . zib_get_svg('tags') . '</icon>' . $name . '</div><div class="flex ac"><span name="plate" class="flex ac em09 muted-2-color">' . __('请选择', 'zib_language') . '</span><i class="ml6 fa fa-angle-right em12"></i></div>
                        </div>
                    </div>';
        }
        ...
        $btn_attr = $wp_is_mobile ? 'data-drawer="show" drawer-title="' . esc_attr(sprintf(__('选择%s', 'zib_language'), $name)) . '" drawer-selector=".tag-drawer"' : 'data-toggle-class="open" data-target=".tag-drop"';
        ...
        $aciton .= '<div class="muted-color pointer" ' . ($wp_is_mobile ? 'data-drawer="hide" drawer-selector=".tag-drawer"' : 'data-toggle-class="open" data-target=".tag-drop"') . '>' . __('完成', 'zib_language') . '</div>';
        $aciton .= zib_bbs_get_term_edit_link('forum_tag', false, 'focus-color', zib_get_svg('add', null, 'icon mr3') . sprintf(__('创建%s', 'zib_language'), $name), 'a');
        $aciton .= '</div>';
    ...
    public static function tag_select_lists($in_id = 0)
    {
        ...
        if ($tab_key === 'search' && isset($_REQUEST['s'])) {
            if ($all_count) {
                zib_send_json_success(array('data' => $lists, 'remind' => __('搜索到以下内容', 'zib_language')));
            } else {
                zib_send_json_success(array('data' => zib_get_null('', 20, 'null-search.svg', '', 0, 120), 'remind' => __('未找到相关内容', 'zib_language')));
            }
            exit;
        }
    ...
    public static function plate_select($in_args = array(), $class = 'zib-widget mb10-sm mb20')
    {
        ...
        if ($posts_id && !zib_bbs_current_user_can('posts_plate_move', $posts_id)) {
            ...
            $lists_box = zib_get_null(sprintf(__('抱歉！您暂无修改%s的权限', 'zib_language'), $zib_bbs->plate_name), 10, 'null-cap.svg', 'flex1', 160);
            $lists_box .= $roles_lists ? '<div class="text-center mb10 em09"><div class="mb10 muted-2-color">' . __('成为以下用户组可拥有此权限', 'zib_language') . '</div>' . $roles_lists . '</div>' : '';
        } else {
            $aciton = '<div class="flex ac jsb padding-10">';
            $aciton .= '<div data-for="topic" data-value="null" class="muted-2-color">' . zib_get_svg('plate-fill', null, 'icon mr3') . sprintf(__('请选择%s', 'zib_language'), $zib_bbs->plate_name) . '</div>';
            $aciton .= zib_bbs_get_plate_add_link(false, 'focus-color', zib_get_svg('add', null, 'icon mr3') . sprintf(__('创建新%s', 'zib_language'), $zib_bbs->plate_name), 'a');
            $aciton .= '</div>';

            $lists_box = $aciton . self::plate_select_lists_tab($in_plate, 'padding-w10 posts-plate-select');
        }

        $btn_attr  = $wp_is_mobile ? 'data-drawer="show" drawer-title="' . esc_attr(sprintf(__('选择%s', 'zib_language'), $zib_bbs->plate_name)) . '" drawer-selector=".plate-drawer"' : 'data-toggle-class="open" data-target=".plate-drop"';
        $add_limit = '';
        if ($in_plate && !zib_bbs_current_user_can('select_plate', $in_plate, $posts_id)) {
            $add_limit = '<div data-controller="plate" data-condition="==" data-value="' . $in_plate . '" class="mt20 c-yellow em09">' . sprintf(__('抱歉！您暂时没有在此%s发布的权限，请切换到其它%s', 'zib_language'), $zib_bbs->plate_name, $zib_bbs->plate_name) . '</div>';
        }
    ...
    public static function select_search($search_ajax_url)
    {
        ...
        $search_html .= '<input type="text" name="s" class="form-control search-input" tabindex="1" value="" placeholder="' . esc_attr__('请输入关键词以搜索', 'zib_language') . '" autocomplete="off">';
        $search_html .= '<div class="search-icon abs-right">' . $search_icon . '</div>';
        $search_html .= '</div>';
        $search_html .= '<div class="mt10 mb10 muted-3-color separator search-remind em09">' . __('请输入关键词以搜索内容', 'zib_language') . '</div>';
    ...
    public static function plate_select_lists_tab($in_plate_id = 0, $class = '', $con_class = '')
    {
        ...
        $ajax_tabs   = array(
            ...
            array(
                'name'     => sprintf(__('已%s', 'zib_language'), $zib_bbs->plate_follow_name),
        ...
        if ($plate_query && $plate_query->have_posts()) {
            foreach ($plate_query->posts as $posts) {
                ...
                if ($can_select_plate) {
                    ...
                    $info = '';
                    $info .= '<span class="mr10">' . sprintf(__('%s:%s', 'zib_language'), $zib_bbs->posts_name, $posts_count) . '</span>';
                    $info .= '<span class="mr10">' . sprintf(__('互动:%s', 'zib_language'), $all_reply_count) . '</span>';
                    $info .= '<span class="">' . sprintf(__('热度:%s', 'zib_language'), $views_count) . '</span>';

                } else {
                    $info = '<span class="c-red">' . __('暂无选择权限', 'zib_language') . '</span>';
                }
    ...
    public static function topic_select($in = 0, $class = 'zib-widget mb10-sm mb20')
    {
        ...
        if (!get_current_user_id()) {
            ...
            return '<div class="topic-drop signin-loader">
                        <div class="flex ac jsb drop-btn ' . $class . '">
                            <div class="flex ac">' . $in_html . '</div><div class="flex ac"><span name="plate" class="flex ac em09 muted-2-color">' . __('请选择', 'zib_language') . '</span><i class="ml6 fa fa-angle-right em12"></i></div>
                        </div>
                    </div>';
        }
        ...
        $aciton = '<div class="flex ac jsb padding-10">';
        $aciton .= '<div' . $drawer_attr . ' data-for="topic" data-value="null" class="pointer but p2-10"><i class="fa fa-ban mr6"></i>' . sprintf(__('不添加%s', 'zib_language'), $zib_bbs->topic_name) . '</div>';
        $aciton .= zib_bbs_get_term_edit_link('forum_topic', false, 'focus-color', zib_get_svg('add', null, 'icon mr3') . sprintf(__('创建新%s', 'zib_language'), $zib_bbs->topic_name), 'a');
        $aciton .= '</div>';
        $btn_attr = $wp_is_mobile ? 'data-drawer="show" drawer-title="' . esc_attr(sprintf(__('选择%s', 'zib_language'), $zib_bbs->topic_name)) . '" drawer-selector=".topic-drawer"' : 'data-target=".topic-drop" data-toggle-class="open"';
    ...
    public static function topping($posts_id = 0)
    {
        ...
        $header  = zib_get_modal_colorful_header('jb-blue', zib_get_svg('topping'), __('设置置顶', 'zib_language'));
        $topping = get_post_meta($posts_id, 'topping', true);

        $con = '';
        $con .= '<div class="em12 mb20">' . sprintf(__('为该%s设置置顶', 'zib_language'), $posts_name) . '</div>';

        $con .= '<div class="muted-color em09 mb10">' . __('请选择置顶模式', 'zib_language') . '</div>';
        $con .= self::topping_radio($topping);
        $con .= '<div class="muted-3-color px12">' . sprintf(__('选择全局置顶则会在所有%s置顶显示', 'zib_language'), $plate_name) . '</div>';
        ...
        $footer .= $hidden_html;
        $footer .= '<button class="but jb-blue padding-lg wp-ajax-submit"><i class="fa fa-check" aria-hidden="true"></i>' . __('确认提交', 'zib_language') . '</button>';
        $footer .= '</div>';
    ...
    public static function plate_move($posts_id = 0)
    {
        ...
        $header   = zib_get_modal_colorful_header('jb-blue', zib_get_svg('plate-fill'), sprintf(__('切换%s', 'zib_language'), $plate_name));

        $con = '';
        $con .= '<div class="muted-3-color mb6">' . sprintf(__('请选择新%s', 'zib_language'), $plate_name) . '</div>';
        $con .= '<div class="">';
    ...
    public static function add_limit_modal($type, $id, $show_plate_follow_pay = false)
    {
        ...
        if ('plate_cat' === $type) {
            ...
            $header_title = sprintf(__('设置%s创建权限', 'zib_language'), $zib_bbs->plate_name);
            $title        = $term->name;
            $type_name .= __('分类', 'zib_language');
            $desc = sprintf(__('为此%s单独%s，只有此处设置的的用户组才能在此%s创建%s', 'zib_language'), $type_name, $header_title, $type_name, $zib_bbs->plate_name);
        } else {
            $plate = get_post($id);
            if (!isset($plate->post_title)) {
                return;
            }
            $header_title = __('设置发布权限', 'zib_language');
            $title        = $plate->post_title;
            $desc         = sprintf(__('为此%s单独%s，只有此处设置的的用户组才能在此%s发布%s', 'zib_language'), $type_name, $header_title, $type_name, $zib_bbs->posts_name);
        }
        ...
        $con = '';
        $con .= '<div class="mb10 font-bold">' . sprintf(__('为%s[%s]%s', 'zib_language'), $type_name, $title, $header_title) . '</div>';
        $con .= '<div class="em09 muted-2-color mb20">' . $desc . '</div>';
        $con .= '<div class="muted-color em09 mb10">' . __('请选择限制模式', 'zib_language') . '</div>';
        $con .= $add_limit_html;
    ...
    public static function posts_plate_select($in_id = 0, $class = 'form-control', $name = 'plate_id', $null_name = null)
    {
        ...
        $null_name   = $null_name ?: sprintf(__('全部%s', 'zib_language'), $plate_name);
        $option_html = '';
        ...
        if (!empty($plate_query->posts)) {
            ...
            if ($plate_query->found_posts > 200) {
                $option_html .= '<option value="" disabled="disabled">' . sprintf(__('共计%d个%s，仅显示200个', 'zib_language'), $plate_query->found_posts, $plate_name) . '</option>';
            }
        }
    ...
    public static function plate_cat($plate_cat_id = 0)
    {
        ...
        global $zib_bbs;
        $name = sprintf(__('%s分类', 'zib_language'), $zib_bbs->plate_name);
    ...
    //term添加、修改通用
    public static function term($term_id = 0, $taxonomy = '', $name = null)
    {
        if (null === $name) {
            $name = __('分类', 'zib_language');
        }
        //前缀
        $prefix             = $term_id ? __('编辑', 'zib_language') : __('创建', 'zib_language');
        $title              = $prefix . $name;
        $slug_v             = '';
        $title_v            = '';
        $content_v          = '';
        ...
        if (($term_id && !zib_bbs_current_user_can($taxonomy . '_edit', $term_id))) {
            return $header . '<div class="touch">' . zib_bbs_get_nocan_info(get_current_user_id(), $taxonomy . '_edit', sprintf(__('无法编辑此%s', 'zib_language'), $name), 20, 0, 200) . '</div>';
        }
        //新建权限判断
        if ((!$term_id && !zib_bbs_current_user_can($taxonomy . '_add'))) {
            return $header . '<div class="touch">' . zib_bbs_get_nocan_info(get_current_user_id(), $taxonomy . '_add', sprintf(__('无法创建%s', 'zib_language'), $name), 20, 0, 200) . '</div>';
        }
        ...
        if ('plate_cat' === $taxonomy && zib_bbs_current_user_can('plate_cat_set_add_limit', $term_id)) {
            ...
            $add_limit_html = $add_limit_html ? '<div class="mb20"><div class="em09 muted-2-color mb6">' . sprintf(__('%s创建权限', 'zib_language'), $zib_bbs->plate_name) . '</div>' . $add_limit_html . '<div class="px12 muted-3-color mb6">' . sprintf(__('为此%s单独设置发布权限，只有此处设置的的用户组才能在此%s发布内容', 'zib_language'), $name, $name) . '</div></div>' : '';
        }
        $con .= $add_limit_html;
        ...
        if (zib_bbs_current_user_can('edit_url_slug') || (!$term_id && zib_bbs_current_user_can('add_url_slug'))) {
            $con .= '<div class="">';
            $con .= '<div class="em09 muted-2-color mb6">' . sprintf(__('%sURL别名', 'zib_language'), $name) . '</div>';
            $con .= '<input type="text" class="form-control" name="slug" tabindex="3" value="' . $slug_v . '" placeholder="' . esc_attr__('请输入URL别名', 'zib_language') . '">';
            $con .= '<div class="px12 muted-3-color mt6">' . __('URL别名是显示在URL网址中的后缀内容，自定义别名可以让网址更加美观，更加有利于SEO，但不建议修改，只能使用小写字母、数字', 'zib_language') . '</div>';
            $con .= '</div>';
        }
    ...
    public static function plate($plate_id, $cat_id = 0)
    {
        ...
        if ($plate_id && !zib_bbs_current_user_can('plate_edit', $plate_id)) {
            return self::edit_header(sprintf(__('编辑%s', 'zib_language'), $name), 'plate-fill') . '<div class="touch">' . zib_bbs_get_nocan_info(get_current_user_id(), 'plate_edit', sprintf(__('无法编辑此%s', 'zib_language'), $name), 20, 0, 220) . '</div>';
        }

        //新建版块权限判断
        if ((!$plate_id && !zib_bbs_current_user_can('plate_add'))) {
            return self::edit_header(sprintf(__('创建%s', 'zib_language'), $name), 'plate-fill') . '<div class="touch">' . zib_bbs_get_nocan_info(get_current_user_id(), 'plate_add', sprintf(__('无法创建%s', 'zib_language'), $name), 20, 0, 220) . '</div>';
        }
        ...
        $prefix = $plate_id ? __('修改', 'zib_language') : __('创建', 'zib_language');
        $title  = $prefix . $name;
        ...
        if (!$cat_opt_html) {
            $cat_opt_html = '<span class="badg c-red mr6 px12 radio-null">' . sprintf(__('暂无可选择的%s分类', 'zib_language'), $name) . '</span>';
        }

        $cat_add_link = zib_bbs_get_plate_cat_add_link(0, 'px12 but c-blue p2-10', zib_get_svg('add') . __('创建新分类', 'zib_language'), 'a', true); //添加分类的按钮
        $cat_html     = '<div class="form-radio mb20 form-but-radio">';
        $cat_html .= '<div class="flex ac jsb mb6"><div class="em09 muted-2-color">' . sprintf(__('%s分类', 'zib_language'), $name) . '</div>' . $cat_add_link . '</div>';
        ...
        if ($plate_type_obj) {
            ...
            $type_html .= '<div class="em09 muted-2-color mb6">' . sprintf(__('%s类型', 'zib_language'), $name) . '</div>';
        ...
        $moderator_html = '';
        if ($plate_id) {
            $moderator_btn     = zib_bbs_get_edit_plate_moderator_link($plate_id, 'but mr10', sprintf(__('管理%s', 'zib_language'), $zib_bbs->plate_moderator_name) . '<i class="fa fa-angle-right ml6"></i>');
            $moderator_add_btn = zib_bbs_get_add_plate_moderator_link($plate_id, 'but', sprintf(__('添加%s', 'zib_language'), $zib_bbs->plate_moderator_name) . '<i class="fa fa-angle-right ml6"></i>');
            if ($moderator_btn) {
        ...
        if (zib_bbs_current_user_can('plate_set_add_limit', $plate_id)) {
            ...
            $add_limit_html = $add_limit_html ? '<div class="mt20"><div class="em09 muted-2-color mb6">' . __('发布权限', 'zib_language') . '</div>' . $add_limit_html . '<div class="px12 muted-3-color mb6">' . sprintf(__('为此%s单独设置发布权限，只有此处设置的的用户组才能在此%s发布内容', 'zib_language'), $name, $name) . '</div></div>' : '';
        }
        ...
        if ((!$plate_id && zib_bbs_current_user_can('plate_set_allow_view')) || ($plate_id && zib_bbs_current_user_can('plate_set_allow_view', $plate_id))) {
            ...
            $allow_view_set = $allow_view_set ? '<div class="mt20 dependency-box"><div class="em09 muted-2-color mb6">' . __('查看权限', 'zib_language') . '</div>' . $allow_view_set . '<div class="px12 muted-3-color mb6">' . sprintf(__('为此%s设置阅读限制，只有满足条件的用户才能查看此版块的%s内容', 'zib_language'), $name, $zib_bbs->posts_name) . '</div></div>' : '';
        }
        ...
        if (zib_bbs_current_user_can('plate_set_follow_pay', $plate_id)) {
            ...
            $follow_pay_set = $follow_pay_set ? '<div class="mt20"><div class="em09 muted-2-color mb6">' . sprintf(__('付费%s', 'zib_language'), $zib_bbs->plate_follow_name) . '</div><div class="muted-box padding-h6">' . $follow_pay_set . '<div class="px12 muted-2-color mb6 mt6">' . sprintf(__('启用付费%s后，请在发布权限、查看权限配置中选择【%s%s】选项', 'zib_language'), $zib_bbs->plate_follow_name, $zib_bbs->plate_follow_name, $zib_bbs->plate_name) . '</div></div></div>' : '';
        }
    ...
    public static function input_add_limit($type = 'posts', $id = 0, $show_plate_follow_pay = false)
    {
        ...
        if ($options) {
            foreach ($options as $key => $name) {
                $roles_lists = '<div class="muted-2-color">' . __('不单独设置，保持系统默认', 'zib_language') . '</div>';
                if ($key) {
                    $roles_lists = zib_bbs_get_cap_roles_lists($type . '_add_limit_' . $key);
                    $roles_lists = $roles_lists ? '<div class="em09 muted-2-color">' . sprintf(__('以下用户组可%s', 'zib_language'), ('posts' === $type ? sprintf(__('发布%s', 'zib_language'), $zib_bbs->posts_name) : sprintf(__('新建%s', 'zib_language'), $zib_bbs->plate_name))) . '</div>' . $roles_lists : '<div class="c-yellow">' . __('仅管理员可发布', 'zib_language') . '</div>';
                }
        ...
        if ('posts' === $type) {
            ...
            $desc .= '<div class="muted-2-color" data-controller="add_limit" data-condition="==" data-value="follow"' . ($in == 'follow' ? '' : ' style="display: none;"') . ' >' . sprintf(__('%1$s%2$s后可发布%3$s', 'zib_language'), $zib_bbs->plate_follow_name, $zib_bbs->plate_name, $zib_bbs->posts_name) . '</div>';
        ...
        return '<div class="dependency-box"><div>' . $input . '</div><div class="muted-box padding-6 radius4">' . $desc . '</div></div>';
    }
    ...
    public static function input_image($name, $val = '')
    {
        ...
        $img_html .= '<div class="em09 muted-2-color mb6">' . sprintf(__('%s图像', 'zib_language'), $name) . '</div>';
        $img_html .= '<label class="pointer">';
        $img_html .= '<div class="preview preview-square">'; //正方形
        $img_html .= '<img class="fit-cover" src="' . $val . '" alt="' . esc_attr__('添加图片', 'zib_language') . '">';
        $img_html .= '</div>';
    ...
    public static function input_title($name, $val = '', $tabindex = 1)
    {
        $con = '';
        $con .= '<div class="mb20">';
        $con .= '<div class="em09 muted-2-color mb6">' . sprintf(__('%s标题', 'zib_language'), $name) . '</div>';
        $con .= '<input type="text" class="form-control" name="title" tabindex="' . $tabindex . '" value="' . $val . '" placeholder="' . esc_attr__('请输入标题', 'zib_language') . '">';
        $con .= '</div>';
        return $con;
    }
    ...
    public static function input_desc($name, $val = '', $tabindex = 2)
    {
        $con = '';
        $con .= '<div class="mb20">';
        $con .= '<div class="em09 muted-2-color mb6">' . sprintf(__('%s简介', 'zib_language'), $name) . '</div>';
        $con .= '<textarea class="form-control" name="desc" tabindex="' . $tabindex . '" placeholder="' . esc_attr__('请输入简介', 'zib_language') . '" rows="2">' . $val . '</textarea>';
        $con .= '</div>';
        return $con;
    }
    ...
    public static function edit_footer($acton = '', $hidden = array(), $attr = '', $_wpnonce = 'save_bbs')
    {
        ...
        $footer .= '<button class="but jb-blue padding-lg" zibupload="submit"' . $attr . '><i class="fa fa-check" aria-hidden="true"></i>' . __('确认提交', 'zib_language') . '</button>';
        $footer .= $hidden_html;
        $footer .= '</div>';
    ...
    public static function posts_delete($post)
    {
        ...
        $desc .= '<div class="c-red mb20">' . sprintf(__('当前%s下，共有%d次%s，确认要删除吗？', 'zib_language'), $posts_name, $all_comment_count, $comment_name) . '</div>';
        //如果不是自己删除，则显示删除后通知作者
        if (get_current_user_id() != $post->post_author) {
            $desc .= '<div class="mb20 dependency-box">
            <div class=""><label class="flex jsb ac" style="font-weight: normal;"><input class="hide" name="msg_s" type="checkbox" checked="checked"><div class="flex1 mr20"><div class="muted-2-color mb3">' . __('通知作者', 'zib_language') . '</div></div><div class="form-switch flex0"></div></label></div>
            <div class="" data-controller="msg_s" data-condition="!=" data-value=""><textarea class="form-control" name="msg" tabindex="1" placeholder="' . esc_attr__('请填写删除原因', 'zib_language') . '" rows="2"></textarea></div>
        </div>';
        }
    ...
    public static function posts_comment_close($post)
    {
        ...
        $text          = $comments_open ? __('关闭评论', 'zib_language') : __('开启评论', 'zib_language');

        $header = zib_get_modal_colorful_header($comments_open ? 'jb-yellow' : 'jb-blue', zib_get_svg('comment'), $text);
        $con    = '<div class="mb20">' . ($comments_open ? sprintf(__('确认要关闭此%s的评论功能？', 'zib_language'), $posts_name) . '<br>' . __('关闭后，将无法发布新的评论，不影响已有的评论', 'zib_language') : sprintf(__('确认要开启此%s的评论功能？', 'zib_language'), $posts_name) . '<br>' . sprintf(__('开启后，将可以在此%s发布新的评论', 'zib_language'), $posts_name)) . '</div>';
        ...
        $footer = '<div class="mt20 but-average">';
        $footer .= $hidden_html;
        $footer .= '<button class="but padding-lg" type="button" data-dismiss="modal" href="javascript;">' . __('取消', 'zib_language') . '</button>';
        $footer .= '<button class="but c-blue wp-ajax-submit padding-lg">' . zib_get_svg('check-circle') . __('确认', 'zib_language') . '</button>';
        $footer .= '</div>';
    ...
    public static function audit($post)
    {
        ...
        if ('pending' === $post->post_status) {
            $header = zib_get_modal_colorful_header('jb-blue', zib_get_svg('approve'), sprintf(__('审核%s', 'zib_language'), $name));
            $con    = '
    <div class="dependency-box">
        ...
        <div class="mb20">
            <div class="flex ac">
                ...
                <span data-for="method" data-value="approve" class="flex-auto hollow-radio pointer active padding-lg c-blue mr6"><i class="fa fa-check mr10" aria-hidden="true"></i>' . __('批准', 'zib_language') . '</span>
                <span data-for="method" data-value="reject" class="flex-auto hollow-radio pointer padding-lg c-red ml6"><i class="fa fa-times mr10" aria-hidden="true"></i>' . __('驳回', 'zib_language') . '</span>
            </div>
        </div>
        <div data-controller="method" data-condition="==" data-value="approve">
            <div class="muted-box muted-2-color">' . sprintf(__('确认将此内容切换为%s状态？', 'zib_language'), '<span class="badg badg-sm c-blue">' . __('已发布', 'zib_language') . '</span>') . '</div>
        </div>
        <div data-controller="method" data-condition="!=" data-value="approve" style="display: none;">
            <div><textarea class="form-control" name="msg" tabindex="1" placeholder="' . esc_attr__('请填写驳回原因或修改建议', 'zib_language') . '" rows="2"></textarea></div>
        </div>
    </div>
        ';
    ...
    <div class="dependency-box">
        ...
        ';
            ...
            $header = zib_get_modal_colorful_header('jb-yellow', zib_get_svg('approve'), sprintf(__('审核%s', 'zib_language'), $name));
            $con    = '<div class="mb20">' . sprintf(__('确认将此%s[%s]的状态切换为%s状态？', 'zib_language'), $name, $title, '<span class="badg badg-sm c-yellow">' . __('待审核', 'zib_language') . '</span>') . '</div>';

            //添加给作者通知的选项，及通知内容
            if (get_current_user_id() != $post->post_author) {
                $con .= '<div class="mb20 dependency-box">
                    <div class=""><label class="flex jsb ac" style="font-weight: normal;"><input class="hide" name="msg_s" type="checkbox" checked="checked"><div class="flex1 mr20"><div class="muted-2-color mb3">' . __('通知作者', 'zib_language') . '</div></div><div class="form-switch flex0"></div></label></div>
                    <div class="" data-controller="msg_s" data-condition="!=" data-value=""><textarea class="form-control" name="msg" tabindex="1" placeholder="' . esc_attr__('请填写审核说明', 'zib_language') . '" rows="2"></textarea></div>
                </div>';
            }
        }
        ...
        $footer .= $hidden_html;
        $footer .= '<button class="but padding-lg" type="button" data-dismiss="modal" href="javascript;">' . __('取消', 'zib_language') . '</button>';
        $footer .= '<button class="but c-blue wp-ajax-submit padding-lg">' . zib_get_svg('approve') . __('确认', 'zib_language') . '</button>';
        $footer .= '</div>';
    ...
    public static function plate_delete($post)
    {
        ...
        $desc              = '<div class="c-red mb20">' . sprintf(__('当前%s下，共有%d个%s和%d次%s，确认要删除吗？', 'zib_language'), $name, $all_posts_count, $posts_name, $all_comment_count, $comment_name) . '</div>';

        if ($all_posts_count) {
            $desc .= '<div class="mt20 dependency-box">';
            $desc .= '<div class="form-radio mb10 form-but-radio">';
            $desc .= '<div class="em09 muted-2-color mb6">' . sprintf(__('删除后如何操作该%s下的%s？', 'zib_language'), $name, $posts_name) . '</div>';
            $desc .= '<label><input type="radio" name="posts_action" value="trash" checked="checked"><span class="p2-10 mr6 but but-radio">' . __('移至回收站', 'zib_language') . '</span></label>';
            $desc .= '<label><input type="radio" name="posts_action" value="move"><span class="p2-10 mr6 but but-radio">' . sprintf(__('移到新的%s', 'zib_language'), $name) . '</span></label>';
            $desc .= '</div>';

            $desc .= '<div data-controller="posts_action" data-condition="==" data-value="move" style="display: none;">';
            $desc .= '<div class="muted-3-color em09 mb6 ml3">' . sprintf(__('请选择新%s', 'zib_language'), $name) . '</div>';
    ...
    public static function term_delete($term)
    {
        ...
        $desc  = '<div class="c-red mb20">' . sprintf(__('当前%s下，共有%d个%s，删除后不可恢复！', 'zib_language'), $name, $count, $posts_name) . '<br>' . __('确认要删除吗？', 'zib_language') . '</div>';
    ...
    public static function delete($id, $name, $acton, $title = '', $desc = '')
    {
        ...
        $header = zib_get_modal_colorful_header('jb-red', '<i class="fa fa-exclamation-triangle" aria-hidden="true"></i>', sprintf(__('确认删除此%s？', 'zib_language'), $name));

        $title = $title ? '<b>（' . $title . '）</b>' : '';
        $con   = '';
        $con .= '<div class="em12 mb10">' . sprintf(__('您正在删除%s%s', 'zib_language'), $name, $title) . '</div>';
        $con .= $desc;
        ...
        $footer = '<div class="mt20 but-average">';
        $footer .= $hidden_html;
        $footer .= '<button type="button" data-dismiss="modal" href="javascript:;" class="but padding-lg">' . __('取消', 'zib_language') . '</button>';
        $footer .= '<button class="but c-red wp-ajax-submit padding-lg"><i class="fa fa-trash-o" aria-hidden="true"></i>' . __('确认删除', 'zib_language') . '</button>';
        $footer .= '</div>';
