...
function zib_bbs_get_taxonomy_name($taxonomy = '')
{
    ...
    $taxonomy_names = array(
        ...
        'plate_cat'   => $zib_bbs->plate_name . __('分类', 'zib_language'),
    );
...
function zib_bbs_get_term_header($taxonomy = '', $term_id = 0, $class = '')
{
    ...
    $count_mate .= '<item class="mate-posts"><div class="em09 opacity5 mb6">' . sprintf(__('%s数', 'zib_language'), $p_name) . '</div><div class="em14"> ' . $all_posts_count . '</div></item>';
    $count_mate .= '<item class="mate-views"><div class="em09 opacity5 mb6">' . __('阅读量', 'zib_language') . '</div><div class="em14"> ' . $views_count . '</div></item>';
    $count_mate      = '<div class="count-mates text-center flex0 ml10' . ($thumb ? ' hide-sm' : '') . '">' . $count_mate . '</div>';
    $count_mate_text = $thumb ? '<div class="px12 mt10 show-sm"><item class="mate-posts mr10">' . $p_name . ' ' . $all_posts_count . '</item><item class="mate-reply mr10">' . sprintf(__('阅读 %s', 'zib_language'), $views_count) . '</item></div>' : '';
...
function zib_bbs_forum_topic_header_excerpt_filter($excerpt, $term)
{
    ...
    $btns .= zib_bbs_get_posts_add_page_link(array('topic_id' => $term_id), 'but c-white mt6', zib_get_svg('add') . sprintf(__('发布%s', 'zib_language'), $zib_bbs->posts_name));
...
function zib_bbs_plate_cat_header_excerpt_filter($excerpt, $term)
{
    ...
    if ($moderator_avatar) {
        $moderator_link = zib_bbs_get_cat_moderator_modal_link($term_id, $class, $moderator_avatar . sprintf(__('%1$s名%2$s', 'zib_language'), count($moderator), $zib_bbs->cat_moderator_name) . '<i class="ml6 fa fa-angle-right em12" style="margin-right: 0;"></i>');
    } else {
        $moderator_link = zib_bbs_get_add_cat_moderator_link($term_id, $class, sprintf(__('添加%s', 'zib_language'), $zib_bbs->cat_moderator_name) . '<i class="ml6 fa fa-angle-right em12" style="margin-right: 0;"></i>');
    }
    $btns .= $moderator_link;
    $btns .= zib_bbs_get_plate_add_link($term_id, $class, zib_get_svg('add') . sprintf(__('创建%s', 'zib_language'), $zib_bbs->plate_name));
    $excerpt .= $btns ? '<div class="mt6 em09 moderator-btns">' . $btns . '</div>' : '';
    return $excerpt;
}
...
function zib_bbs_term_tab_content()
{
    ...
    if (have_posts()) {
        ...
        $lists .= zib_get_ajax_null(__('内容空空如也', 'zib_language'), 100);
    }
...
function zib_bbs_get_term_search_btn($term, $class = '')
{
    ...
    $args = array(
        ...
        'type'        => 'forum',
        'placeholder' => __('在', 'zib_language') . zib_bbs_get_taxonomy_name($term->taxonomy) . '[' . $title . ']中搜索' . $zib_bbs->posts_name,
    );
...
function zib_bbs_get_term_more_dropdown($term_id, $taxonomy, $class = '', $con_class = '', $con = '')
{
    ...
    $add = zib_bbs_get_term_edit_link($taxonomy, 0, '', zib_get_svg('add', null, 'icon mr6 fa-fw') . sprintf(__('创建新%s', 'zib_language'), $name), 'a');
    $action .= $add ? '<li>' . $add . '</li>' : '';

    $edit = zib_bbs_get_term_edit_link($taxonomy, $term_id, '', zib_get_svg('set', null, 'icon mr6 fa-fw') . sprintf(__('编辑此%s', 'zib_language'), $name), 'a');
    ...
    $del = zib_bbs_get_term_modal_delete_link($taxonomy, $term_id, 'c-red', '<i class="fa fa-trash-o mr6 fa-fw"></i>' . sprintf(__('删除此%s', 'zib_language'), $name), 'a');
...
function zib_bbs_taxonomy_names_filter($taxonomy_names)
{
    ...
    $taxonomy_names['plate_cat']   = __('分类', 'zib_language');
