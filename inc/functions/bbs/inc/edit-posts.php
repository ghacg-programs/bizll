    ...
    if ($user_id) {
        ...
        if ($is_edit) {
            ...
            if (!zib_bbs_current_user_can('posts_edit', $is_edit)) {
                ...
                echo zib_get_null(__('抱歉！您暂无编辑此内容的权限', 'zib_language'), 60, 'null-cap.svg', '');
                echo $roles_lists ? '<div class="text-center"><div class="mb20 muted-2-color">' . __('成为以下用户组可编辑此内容', 'zib_language') . '</div>' . $roles_lists . '</div>' : '';
                echo '</div>';
            }
