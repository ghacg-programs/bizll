 ...
 * @LastEditTime : 2026-06-17 19:48:56
...
function zib_bbs_get_respond()
{
    ...
    if (!$user_id) {
        ...
        $html .= '<div class="mb20 muted-3-color">' . __('请登录后发表评论', 'zib_language') . '</div>';
        $html .= '<p>';
        $html .= '<a href="javascript:;" class="signin-loader but c-blue padding-lg"><i class="fa fa-fw fa-sign-in mr10" aria-hidden="true"></i>' . __('登录', 'zib_language') . '</a>';
        $html .= !zib_is_close_signup() ? '<a href="javascript:;" class="signup-loader ml10 but c-yellow padding-lg"><i data-class="icon mr10" data-viewbox="0 0 1024 1024" data-svg="signup" aria-hidden="true"></i>' . __('注册', 'zib_language') . '</a>' : '';
        $html .= '</p>';
    ...
    if (!zib_bbs_current_user_can('comment_add')) {
        return '<div id="respond" class="zib-widget' . $comments_open_class . '"><form id="commentform">' . zib_bbs_get_nocan_info($user_id, 'comment_add', __('暂时无法评论', 'zib_language'), 10, 200) . '</form></div>';
    }
    ...
    $btns_r = '<a class="but c-red mr6" id="cancel-comment-reply-link" href="javascript:;">' . __('取消', 'zib_language') . '</a>';
    $btns_r .= '<button class="but c-blue pw-1em input-expand-submit comment-send" name="submit" id="submit" tabindex="2">' . __('提交回复', 'zib_language') . '</button>';
...
function zib_bbs_get_respond_btns()
{
    ...
    if (_pz('bbs_comment_link', true)) {
        $html .= zib_get_input_expand_but('link');
    }
...
function zib_bbs_get_comment_title()
{
    ...
    $order = '<a rel="nofollow" class="but comment-orderby' . (!$corderby || 'comment_date_gmt' == $corderby ? ' b-theme' : '') . '" href="' . esc_url(add_query_arg('corderby', 'comment_date_gmt', $url)) . '">' . __('最新', 'zib_language') . '</a>';
    $order .= '<a rel="nofollow" class="but comment-orderby' . ('comment_like' == $corderby ? ' b-theme' : '') . '" href="' . esc_url(add_query_arg('corderby', 'comment_like', $url)) . '">' . __('最热', 'zib_language') . '</a>';
 ...
 * @return {*}
 */
function zib_bbs_get_comment_adopt_link($comment, $class = 'but but-sm c-cyan', $con = null, $tag = 'span')
{
    if (null === $con) {
        $con = __('采纳此回答', 'zib_language');
    }
...
function zib_bbs_get_comment_adopted_badeg($comment_id, $class = '')
{
    ...
    return '<span class="badge-question jb-cyan' . $class . '" title="' . esc_attr__('已采纳该回答', 'zib_language') . '" data-toggle="tooltip"><i class="fa fa-check-circle"></i><text>' . __('已采纳', 'zib_language') . '</text></span>';
}

//获取将回答设置为已采纳的确认模态框
function zib_bbs_get_answer_adopt_modal($comment_id)
{
    $header = zib_get_modal_colorful_header('jb-cyan', '<i class="fa fa-check-circle em12"></i>', __('采纳该回答', 'zib_language'));

    $form = '<form>';
    $form .= '<div class="mb10"><b>' . __('如果此回答对您有用，请采纳此回答', 'zib_language') . '</b></div>';
    $form .= '<div class="mb20"><textarea class="form-control" name="desc" tabindex="1" placeholder="' . esc_attr__('给该回答作者留言', 'zib_language') . '" rows="2"></textarea></div>';
    $form .= '<div class="mt20 but-average">
                    ...
                    <button class="but c-blue wp-ajax-submit answer-adopt-submit"><i class="fa fa-check" aria-hidden="true"></i>' . __('确认采纳', 'zib_language') . '</button>
                </div>';
    $form .= '</form>';
...
function zib_bbs_get_comment_hot_badge($id, $class = '')
{
    ...
    $html      = '<span class="img-badge top badge-status comment-hot' . $class . '"><img ' . $lazy_attr . 'src="' . ZIB_TEMPLATE_DIRECTORY_URI . '/img/hot-comment.svg" alt="' . esc_attr__('热门评论', 'zib_language') . '"></span>';
    return $html;
}
...
function zib_bbs_comment_action_set_hot($lists, $comment)
{
    ...
    if (zib_bbs_current_user_can('comment_set_hot', $comment)) {
        ...
        $text   = !$is_hot ? __('设为神评', 'zib_language') : __('取消神评', 'zib_language'); //参数
