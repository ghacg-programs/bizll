 ...
 * @LastEditTime : 2026-06-09 13:17:18
...
function zib_bbs_home_tab_content()
{
    ...
    //输出编辑器全局变量，快速发布依赖
    if (_pz('bbs_home_tab_quick_posts_s', true)) {
...
function zib_bbs_get_home_tab_content_plate($html = '', $option = array())
{
    ...
    if (!$html) {
        $html = zib_get_null(__('暂时没有版块', 'zib_language'), 60, 'null.svg', '', 0, 170);
...
function zib_bbs_get_home_tab_content_follow($html = '', $option = array())
{
    ...
    if (!$user_id) {
        $html .= zib_get_null(__('登录后继续查看', 'zib_language'), 20, 'null-user.svg', '', 0, 220) . zib_get_user_singin_page_box('mb20', '');
    ...
    if (!$lists) {
        $lists = zib_get_ajax_null(__('暂无内容', 'zib_language'), 60, 'null-love.svg');
        if (1 == $page) {
