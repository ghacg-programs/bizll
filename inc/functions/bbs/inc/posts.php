 ...
 * @LastEditTime : 2026-06-29 18:19:28
...
{
    $options = array(
        '0' => __('不置顶', 'zib_language'),
        '1' => __('置顶', 'zib_language'),
        '2' => __('超级置顶', 'zib_language'),
        '3' => __('全局置顶', 'zib_language'),
    );
    return $options;
}
...
function zib_bbs_get_posts_mini_list($class = 'ajax-item', $show_topping = false)
{
    ...
    if ($last_reply) {
        $time = '<span class="icon-circle" title="' . sprintf(__('最后回复：%s', 'zib_language'), $last_reply) . '">' . zib_get_time_ago($last_reply) . __('回复', 'zib_language') . '</span>';
    } else {

        $time = '<span class="icon-circle" title="' . sprintf(__('发布时间：%s', 'zib_language'), $get_the_time) . '">' . $get_the_time_ago . __('发布', 'zib_language') . '</span>';
    }
...
function zib_bbs_get_posts_not_allow_view($post = null, $hide_pay = false)
{
    ...
    if ($not_allow_view && $hide_pay && in_array($data['type'], array('pay', 'points'))) {
        $not_allow_view = '<div class="hide-post mt6"><div class=""><i class="fa fa-unlock-alt mr6"></i>' . __('此处内容已隐藏', 'zib_language') . '</div></div>';
    }
...
function zib_bbs_get_posts_pay_box($posts_id)
{
    ...
    if (_pz('pay_show_paycount', true) && $cuont_volume) {
        $cuont = '<badge class="img-badge hot jb-green px12 mt6">' . sprintf(__('已售 %s', 'zib_language'), $cuont_volume) . '</badge>';
    }
    $price           = zibpay_get_show_price($pay_mate, $posts_id, 'c-red'); //价格
    $vip_price       = zibpay_get_posts_vip_price($pay_mate);
    $pay_doc         = !empty($pay_mate['pay_doc']) ? $pay_mate['pay_doc'] : sprintf(__('此内容为%s，请付费后查看', 'zib_language'), zibpay_get_pay_type_name($pay_mate['pay_type'])); //简介
    $pay_button      = !$user_id ? '<a href="javascript:;" class="but jb-blue signin-loader padding-lg"><i class="fa fa-sign-in mr10" aria-hidden="true"></i>' . __('登录购买', 'zib_language') . '</a>' : zibpay_get_pay_form_but($pay_mate, $posts_id); //购买按钮
...
function zib_bbs_get_allow_view_data($post = null)
{
    ...
    if (is_super_admin() || zib_bbs_user_is_forum_admin()) {
        //管理员可查看
        $data['allow_reason'] = __('您是尊贵的管理员，可查看所有内容', 'zib_language');
    }

    if ($user_id && $post->post_author == $user_id && !$data['allow_reason']) {
        //作者就是自己
        $data['allow_reason'] = __('您是内容作者，可查看此内容', 'zib_language');
    }

    //分区版主和版主
    $moderator_badge = zib_bbs_get_user_moderator_badge($user_id, $post);
    if (!$data['allow_reason'] && $moderator_badge) {
        $data['allow_reason'] = sprintf(__('您是%s，可查看此内容', 'zib_language'), $moderator_badge);
    }

    $sign_btns = '<p><a href="javascript:;" class="signin-loader but jb-blue padding-lg"><i class="fa fa-fw fa-sign-in" aria-hidden="true"></i>' . __('登录', 'zib_language') . '</a>' . (!zib_is_close_signup() ? '<a href="javascript:;" class="signup-loader ml10 but jb-yellow padding-lg">' . zib_get_svg('signup') . __('注册', 'zib_language') . '</a>' : '') . '</p>';
    $con       = '';
    $title     = '';

    switch ($allow_view) {
        case 'signin':
            $data['html'] = '<p class="separator muted-3-color em09">' . __('登录后可查看', 'zib_language') . '</p>';
            if (!$user_id) {
                $title = __('内容已隐藏，请登录后查看', 'zib_language');
                $con   = '<div class="text-center em09 mt20"><p class="separator muted-3-color mb20">' . __('登录后继续查看', 'zib_language') . '</p>' . $sign_btns . '</div>';
            } elseif (!$data['allow_reason']) {
                $data['allow_reason'] = __('您已登录，可查看此内容', 'zib_language');
            }
            break;

        case 'follow':
            $data['html'] = '<p class="separator muted-3-color em09">' . sprintf(__('%s后可查看', 'zib_language'), $zib_bbs->plate_follow_name) . '</p>';
            if (!zib_bbs_is_followed_plate($posts_id, $user_id)) {
                $title = sprintf(__('内容已隐藏，%s后可查看', 'zib_language'), $zib_bbs->plate_follow_name);

                if (!$user_id) {
                    $con = '<div class="text-center em09 mt20"><p class="separator muted-3-color mb20">' . __('请先登录', 'zib_language') . '</p>' . $sign_btns . '</div>';
                } else {
                    $follow_btn = zib_bbs_get_all_plate_follow_btn($posts_id, 'but hollow c-red padding-lg', zib_get_svg('add') . ' ' . sprintf(__('立即%s', 'zib_language'), $zib_bbs->plate_follow_name));
                    $con        = '<div class="text-center em09 mt20"><p class="separator muted-3-color mb20">' . sprintf(__('%s后继续查看', 'zib_language'), $zib_bbs->plate_follow_name) . '</p>' . $follow_btn . '</div>';
                }

            } elseif (!$data['allow_reason']) {
                $data['allow_reason'] = sprintf(__('您已%s，可查看此内容', 'zib_language'), $zib_bbs->plate_follow_name);
            }
            break;
        ...
        case 'points':
            ...
            if (!empty($pay_mate['pay_type']) && 'no' !== $pay_mate['pay_type']) {
                ...
                $data['html']     = '<p class="separator muted-3-color em09">' . sprintf(__('付费内容，需%s后可查看', 'zib_language'), ('points' === $allow_view ? __('支付积分', 'zib_language') : __('购买', 'zib_language'))) . '</p>';
                $data['btn_icon'] = zibpay_get_post_mini_badge($pay_mate);

                if (!$paid) {
                    $pay_hide_part = zib_get_post_meta($post->ID, 'pay_hide_part', true);
                    $title         = ($pay_hide_part ? __('部分', 'zib_language') : '') . __('内容已隐藏', 'zib_language');
                    $con = '<div class="text-center em09 mt20"><p class="separator muted-3-color mb20">' . __('登录后继续查看', 'zib_language') . '</p>' . $sign_btns . '</div>';
                    ...
                    switch ($paid_type) {
                        case 'free':
                            $data['allow_reason'] = __('此内容可免费查看', 'zib_language');
                            break;
                        case 'vip1_free':
                        case 'vip2_free':
                            $data['allow_reason'] = sprintf(__('您是尊贵的%s，可免费查看此内容', 'zib_language'), zibpay_get_vip_icon($paid['vip_level'], 'em12') . _pz('pay_user_vip_' . $paid['vip_level'] . '_name'));
                            break;
                        default:
                            $data['allow_reason'] = __('您已购买此付费内容', 'zib_language');
                    }
                }
            }
            break;
        ...
        case 'comment':
            $data['html'] = '<p class="separator muted-3-color em09">' . __('评论后可查看', 'zib_language') . '</p>';
            if (!$user_id) {
                $title = __('内容已隐藏，请评论后查看', 'zib_language');
                $con   = '<div class="text-center em09 mt20"><p class="separator muted-3-color mb20">' . __('登录后继续评论', 'zib_language') . '</p>' . $sign_btns . '</div>';
                ...
                $con = '<div class="text-center em09 mt20" reply-show="true" reload-hash="#article"><p class="separator muted-3-color mb20">' . __('评论后继续查看', 'zib_language') . '</p><p><a rel="nofollow" href="' . $comment_href . '" class="but jb-blue padding-lg">' . zib_get_svg('comment') . __('去评论', 'zib_language') . '</a></p></div>';
            } elseif (!$data['allow_reason']) {
                $data['allow_reason'] = __('您已参与评论，可查看此内容', 'zib_language');
            }
        ...
        case 'roles':
            ...
            if (isset($allow_roles['vip'])) {
                if (1 == $allow_roles['vip']) {
                    $vip = zibpay_get_vip_icon(1, 'mr6 em12') . _pz('pay_user_vip_1_name') . (_pz('pay_user_vip_2_s', true) ? __('及以上会员', 'zib_language') : '');
                }
            ...
            if (!empty($allow_roles['level'])) {
                $level = zib_get_level_badge($allow_roles['level'], 'mr6 em12') . __('及更高等级', 'zib_language');
            }
            if (!empty($allow_roles['auth'])) {
                $auth = zib_get_svg('user-auth', null, 'mr6 em12') . __('认证用户', 'zib_language');
            }

            $data['html'] = '<div class="text-center em09">';
            $data['html'] .= '<p class="separator muted-3-color">' . __('以下用户组可查看', 'zib_language') . '</p>';
            ...
            if (!$user_id) {
                ...
                $con .= '<p class="separator muted-3-color mb20">' . __('以下用户组可查看', 'zib_language') . '</p>';
                $con .= $roles;
                $con .= '<p class="separator muted-3-color mb20 mt20">' . __('登录后查看我的权限', 'zib_language') . '</p>' . $sign_btns . '';
                $con .= '</div>';
            } else {
                $is_allow = false;
                if (!empty($allow_roles['vip'])) {
                    ...
                    if ($my_vip && $my_vip >= $allow_roles['vip']) {
                        if (!$data['allow_reason']) {
                            $data['allow_reason'] = sprintf(__('您是尊贵的%s，可查看此内容', 'zib_language'), zibpay_get_vip_icon($my_vip, 'em12') . _pz('pay_user_vip_' . $my_vip . '_name'));
                        }
                        $is_allow = true;
                    } else {
                ...
                if (!empty($allow_roles['level'])) {
                    ...
                    if ($my_level && $my_level >= $allow_roles['level']) {
                        if (!$data['allow_reason']) {
                            $data['allow_reason'] = sprintf(__('您的等级为%s，可查看此内容', 'zib_language'), zib_get_level_badge($my_level, 'mr6 em12'));
                        }
                        $is_allow = true;
                    } else {
                ...
                if (!empty($allow_roles['auth'])) {
                    ...
                    if ($my_auth) {
                        if (!$data['allow_reason']) {
                            $data['allow_reason'] = sprintf(__('您已是%s认证用户，可查看此内容', 'zib_language'), $my_auth);
                        }
                        $is_allow = true;
                    } else {
    ...
    if ($con || $title) {
        ...
        if ($title) {
            $title = sprintf(__('该%s%s', 'zib_language'), ('plate' === $post->post_type ? $zib_bbs->plate_name : $zib_bbs->posts_name), $title);
        }
...
function zib_bbs_get_posts_author_card_box($post, $class = '')
{
    ...
    $author_desc = $time_html . '<span class="icon-spot">' . sprintf(__('%s次阅读', 'zib_language'), $views) . '</span>'; //查看数量
 ...
 * @return {*}
 */
function zib_bbs_get_posts_add_page_link($args = array(), $class = 'but c-blue', $con = null)
{
    if (null === $con) {
        $con = __('发布', 'zib_language');
    }
 ...
 * @return {*}
 */
function zib_bbs_get_posts_edit_page_link($id = 0, $class = 'but c-blue', $con = null)
{
    if (null === $con) {
        $con = __('发布', 'zib_language');
    }
 ...
 * @return {*}
 */
function zib_bbs_get_posts_essence_set_link($posts_id = 0, $class = '', $cancel_class = '', $con = null, $cancel_con = null)
{
    if (null === $con) {
        $con = __('设为精华', 'zib_language');
    }
    if (null === $cancel_con) {
        $cancel_con = __('取消精华', 'zib_language');
    }
 ...
 * @return {*}
 */
function zib_bbs_get_posts_topping_set_link($posts_id = 0, $class = '', $con = null, $tag = 'a')
{
    if (null === $con) {
        $con = __('设置置顶', 'zib_language');
    }
 ...
 * @return {*}
 */
function zib_bbs_get_posts_comment_close_link($posts_id = 0, $class = '', $con = null, $open_con = null, $tag = 'a')
{
    if (null === $con) {
        $con = __('关闭评论', 'zib_language');
    }
    if (null === $open_con) {
        $open_con = __('开启评论', 'zib_language');
    }
 ...
 * @return {*}
 */
function zib_bbs_get_posts_allow_view_set_link($posts_id = 0, $class = '', $con = null, $tag = 'a')
{
    if (null === $con) {
        $con = __('设置阅读权限', 'zib_language');
    }
 ...
 */
function zib_bbs_get_posts_audit_link($posts_id = 0, $class = '', $con = null, $revoke_con = null, $tag = 'a')
{
    if (null === $con) {
        $con = __('审核内容', 'zib_language');
    }
    if (null === $revoke_con) {
        $revoke_con = __('审核驳回', 'zib_language');
    }
 ...
 * @return {*} zib_get_refresh_modal_link()
 */
function zib_bbs_get_posts_delete_link($posts_id = 0, $class = '', $con = null, $revoke_con = null, $tag = 'a')
{
    if (null === $con) {
 ...
 * @return {*} zib_get_refresh_modal_link()
 */
function zib_bbs_get_posts_plate_move_link($posts_id = 0, $class = '', $con = null, $tag = 'a')
{
    if (null === $con) {
        $con = '<i class="fa fa-trash-o fa-fw"></i>' . __('移动版块', 'zib_language');
    }
...
function zib_bbs_get_posts_action_metas($posts_id = 0, $main_class = 'action-meta')
{
    ...
    //评分按钮
    $score       = get_post_meta($posts_id, 'score', true); //评分
    $score       = $score ? _cut_count($score) : __('加分', 'zib_language');
    ...
    //回复(评论)按钮
    $reply        = get_comments_number($posts_id); //回复
    $reply        = $reply ? _cut_count($reply) : __('回复', 'zib_language');
 ...
 * @return {*}
 */
function zib_bbs_get_posts_favorite_btn($posts_id, $class = '', $text = null, $ok_text = null, $icon = true, $count = true)
{
    if (null === $text) {
        $text = __('收藏', 'zib_language');
    }
    if (null === $ok_text) {
        $ok_text = __('已收藏', 'zib_language');
    }
...
function zib_bbs_get_essence_badge($class = 'jb-red mr3', $posts_id = 0)
{
    ...
    $html = '<badge class="badge-essence' . $class . '" title="' . esc_attr__('精华', 'zib_language') . '" data-toggle="tooltip">' . __('精', 'zib_language') . '</badge>';
    return $html;
}
...
function zib_bbs_get_question_badge($class = 'mr3', $post = null)
{
    ...
    $question_status_array = array(
        false => __('提问', 'zib_language'),
        1     => __('已解决', 'zib_language'),
        2     => __('已解决', 'zib_language'),
    );

    if ($question_status) {
        $title = __('提问已解决', 'zib_language');
        $con   = '<icon><i class="fa fa-check-circle"></i></icon><text>' . _pz('bbs_question_ok_badge_name', '已解决') . '</text>';
    } else {
        $title = __('提问', 'zib_language');
...
function zib_bbs_get_topping_badge($class = 'jb-blue mr3', $posts_id = 0)
{
    ...
    $title  = isset($titles[$meta]) ? $titles[$meta] : __('置顶', 'zib_language');
...
function zib_bbs_get_vote($posts_id = 0, $class = '', $user_id = 0)
{
    ...
    if ('multiple' == $vote_opt['type']) {
        //多选
        if (!$user_voted) {
            $lists .= '<botton type="button" class="but c-blue vote-submit block radius" style="display: none;"><i class="fa fa-fw fa-check"></i>' . __('提交', 'zib_language') . '</botton>';
        }
    }

    //标题
    $icon  = '<i class="fa fa-bar-chart"></i>';
    $title = $vote_opt['title'] ? $vote_opt['title'] : ('px' == $vote_opt['type'] ? __('参与投票', 'zib_language') : __('参与PK投票', 'zib_language'));
    ...
    $time = __('长期有效', 'zib_language');
    if ($vote_opt['time_limit']) {
        //如果有时间限制
        $ended_text     = __('已结束', 'zib_language');
        $remaining_time = zib_get_time_remaining(strtotime($vote_opt['time'] . ' +' . $vote_opt['time_limit'] . ' day'), $ended_text);
        if ($ended_text == $remaining_time) {
            $is_allow = false;
        } else {
            $remaining_time .= __('结束', 'zib_language');
        }
        $time = $remaining_time;
    }
    $time = '<span class="icon-spot">' . $time . '</span>';

    $user_count = $vote_data['users'] ? '<span class="vote-user-count">' . sprintf(__('%d人已参与', 'zib_language'), count($vote_data['users'])) . '</span>' : '';
    if ($user_voted) {
        $user_count .= '<span class="icon-spot">' . __('您已投票', 'zib_language') . '</span>';
    } elseif ($is_allow) {
        $user_count .= '<span class="vote-start ' . ($user_count ? 'icon-spot' : '') . '">' . __('点击选项以投票', 'zib_language') . '</span>';
    }
