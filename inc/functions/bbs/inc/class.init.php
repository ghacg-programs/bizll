...
class zib_bbs
{
        ...
        if ($post_type === 'forum_post') {
            if ($column_name === 'plate_id') {
                ...
                <div class="inline-edit-col">
                    <label>
                        <span class="title">' . __('板块', 'zib_language') . '</span>
                        ' . zib_bbs_edit::posts_plate_select(0, '', 'plate_id', __('—不更改—', 'zib_language')) . '
                    </label>
                </div>';
            }
        }
    ...
    public function views_comments($views_o)
    {
        ...
        $views['post']       = '<a' . $post_attr . ' href="' . add_query_arg('post_type', 'post') . '">' . __('文章评论', 'zib_language') . '<span class="count">（<span class="trash-count">' . $post_count . '</span>）</span></a>';
    ...
    public function template_redirect()
    {
        ...
        if ($forum_post_edit) {
            ...
            if ($is_edit) {
                $new = sprintf(__('编辑%s', 'zib_language'), $this->posts_name);
            } else {
                $new = sprintf(__('新建%s', 'zib_language'), $this->posts_name);
            }
    ...
    public function register_post_statuses()
    {
        ...
        register_post_status(
            'closed',
            array(
                'label'                     => _x('关闭', 'post', 'zib_language'),
                'label_count'               => _nx_noop('已关闭 <span class="count">(%s)</span>', '已关闭 <span class="count">(%s)</span>', 'post', 'zib_language'),
                'public'                    => true,
    ...
    public function register_post_type()
    {
        ...
        register_post_type(
            ...
            array(
                'labels'              => array(
                    ...
                    'all_items'     => sprintf(__('所有%s', 'zib_language'), $this->plate_name),
                    'add_new'       => sprintf(__('添加%s', 'zib_language'), $this->plate_name),
                    'add_new_item'  => sprintf(__('创建新的%s', 'zib_language'), $this->plate_name),
                    'edit'          => __('编辑', 'zib_language'),
                    'edit_item'     => sprintf(__('编辑%s', 'zib_language'), $this->plate_name),
                    'new_item'      => sprintf(__('新%s', 'zib_language'), $this->plate_name),
                    'view'          => sprintf(__('查看%s', 'zib_language'), $this->plate_name),
                    'view_item'     => sprintf(__('查看%s', 'zib_language'), $this->plate_name),
                ),
                'supports'            => array(
                    'title',
                    'excerpt',
        ...
        $taxonomy_args = [
            'labels'            => [
                'name'              => __($this->forum_name . $this->plate_name . '分类', 'zib_language'),
                'singular_name'     => __($this->forum_name . $this->plate_name . '分类', 'zib_language'),
                'search_items'      => __('搜索' . $this->plate_name . '分类', 'zib_language'),
                'all_items'         => __('所有' . $this->plate_name . '分类', 'zib_language'),
                'parent_item'       => __('父' . $this->plate_name . '分类', 'zib_language'),
                'parent_item_colon' => __('父' . $this->plate_name . '分类' . ':', 'zib_language'),
                'edit_item'         => __('编辑' . $this->plate_name . '分类', 'zib_language'),
                'update_item'       => __('更新' . $this->plate_name . '分类', 'zib_language'),
                'add_new_item'      => __('添加新' . $this->plate_name . '分类', 'zib_language'),
                'new_item_name'     => __('新' . $this->plate_name . '分类' . '名称', 'zib_language'),
                'menu_name'         => __('分类', 'zib_language'),
            ],
            'description'       => sprintf(__('%1$s%2$s分类', 'zib_language'), $this->forum_name, $this->plate_name),
        ...
        register_post_type(
            ...
            array(
                'labels'              => array(
                    ...
                    'all_items'     => sprintf(__('所有%s', 'zib_language'), $this->posts_name),
                    'add_new'       => sprintf(__('添加%s', 'zib_language'), $this->posts_name),
                    'add_new_item'  => sprintf(__('创建新的%s', 'zib_language'), $this->posts_name),
                    'edit'          => sprintf(__('编辑%s', 'zib_language'), $this->posts_name),
                    'edit_item'     => sprintf(__('编辑%s', 'zib_language'), $this->posts_name),
                    'new_item'      => sprintf(__('新%s', 'zib_language'), $this->posts_name),
                    'view'          => sprintf(__('查看%s', 'zib_language'), $this->posts_name),
                    'view_item'     => sprintf(__('[%1$s]查看%2$s', 'zib_language'), $this->forum_name, $this->posts_name),
                ),
                'supports'            => array(
                    'title',
                    'editor',
        ...
        $taxonomy_args = [
            'labels'            => [
                'name'              => __($this->forum_name . $this->topic_name, 'zib_language'),
                'singular_name'     => __($this->forum_name . $this->topic_name, 'zib_language'),
                'search_items'      => __('搜索' . $this->topic_name, 'zib_language'),
                'all_items'         => __('所有' . $this->topic_name, 'zib_language'),
                'parent_item'       => __('父' . $this->topic_name, 'zib_language'),
                'parent_item_colon' => __('父' . $this->topic_name . ':', 'zib_language'),
                'edit_item'         => __('编辑' . $this->topic_name, 'zib_language'),
                'update_item'       => __('更新' . $this->topic_name, 'zib_language'),
                'add_new_item'      => __('添加新' . $this->topic_name, 'zib_language'),
                'new_item_name'     => __('新' . $this->topic_name . '名称', 'zib_language'),
                'menu_name'         => __($this->topic_name, 'zib_language'),
            ],
            'capabilities'      => array(
                'manage_terms' => 'manage_categories',
                'edit_terms'   => 'manage_categories',
        ...
        $taxonomy_args = [
            'labels'            => [
                'name'              => __($this->forum_name . $this->tag_name, 'zib_language'),
                'singular_name'     => __($this->forum_name . $this->tag_name, 'zib_language'),
    ...
    public function plate_columns($columns)
    {
        ...
        $columns['all_count'] = '<a href="' . add_query_arg(array('orderby' => 'posts_count', 'order' => $order)) . '"><span>' . $this->posts_name . ($order_by == 'posts_count' ? $o_icon : '') . '</span></a> · <a href="' . add_query_arg(array('orderby' => 'views', 'order' => $order)) . '"><span>' . __('阅读', 'zib_language') . ($order_by == 'views' ? $o_icon : '') . '</span></a> · <a href="' . add_query_arg(array('orderby' => 'follow_count', 'order' => $order)) . '"><span>' . $this->plate_follow_name . ($order_by == 'follow_count' ? $o_icon : '') . '</span></a> · <a href="' . add_query_arg(array('orderby' => 'reply_count', 'order' => $order)) . '"><span>' . $this->comment_name . ($order_by == 'reply_count' ? $o_icon : '') . '</span>';

        $columns['last_time'] = '<a href="' . add_query_arg(array('orderby' => 'last_post', 'order' => $order)) . '"><span>' . __('最后发布', 'zib_language') . ($order_by == 'last_post' ? $o_icon : '') . '</span></a> · <a href="' . add_query_arg(array('orderby' => 'last_reply', 'order' => $order)) . '"><span>' . sprintf(__('最后%s', 'zib_language'), $this->comment_name) . ($order_by == 'last_reply' ? $o_icon : '') . '</span></a>';
    ...
    public function plate_custom_column($column_name, $plate_id)
    {
        switch ($column_name) {
            ...
            case 'all_count':
                echo '<div style="font-size: 12px;">' . $this->posts_name . zib_bbs_get_plate_posts_count($plate_id) . ' · ' . __('热度', 'zib_language') . zib_bbs_get_plate_views_cut_count($plate_id) . ' · ' . $this->plate_follow_name . zib_bbs_get_plate_follow_cut_count($plate_id) . ' · ' . $this->comment_name . zib_bbs_get_plate_reply_cut_count($plate_id) . '</div>';
                break;
            case 'last_time':
    ...
    public function posts_columns($columns)
    {
        ...
        $add_columns['all_count'] = '<a href="' . add_query_arg(array('orderby' => 'views', 'order' => $order)) . '"><span>' . __('阅读', 'zib_language') . ($order_by == 'views' ? $o_icon : '') . '</span></a>·<a href="' . add_query_arg(array('orderby' => 'score', 'order' => $order)) . '"><span>' . __('评分', 'zib_language') . ($order_by == 'score' ? $o_icon : '') . '</span></a>·<a href="' . add_query_arg(array('orderby' => 'favorite_count', 'order' => $order)) . '"><span>' . __('收藏', 'zib_language') . ($order_by == 'favorite_count' ? $o_icon : '') . '</span></a></a>';
    ...
    public function posts_custom_column($column_name, $posts_id)
    {
        switch ($column_name) {
            case 'plate_id':
                ...
                if (!$plate_id) {
                    echo '<span style="color:#ee4545;">' . sprintf(__('未选择%s', 'zib_language'), $this->plate_name) . '</span>';
                    break;
                }

                $plate = get_post($plate_id);
                if (empty($plate->ID)) {
                    echo '<span style="color:#ee4545;">' . sprintf(__('%s不存在或者已删除', 'zib_language'), $this->plate_name) . '</span>';
                    break;
                }
                $trash = '';
                if ('trash' == $plate->post_status) {
                    $trash = '<span style="color:#ee4545;">[' . __('回收站', 'zib_language') . ']</span>';
                }
                ...
                $html .= '<div style="font-size: 12px;opacity: .8;">' . $this->posts_name . zib_bbs_get_plate_posts_count($plate_id) . ' · ' . __('热度', 'zib_language') . $views_count . ' · ' . $this->plate_follow_name . $follow_count . ' · ' . $this->comment_name . $reply_count . '</div>';
                $html .= '<div class="row-actions"><span class="view"><a href="' . $permalink . '">' . __('查看', 'zib_language') . '</a></span> | <span class="view"><a href="' . $edit . '">' . __('编辑', 'zib_language') . '</a></span></div>';
                $html .= '';
                $html .= '';
            ...
            case 'all_count':
                ...
                echo '<div style="font-size: 12px;">' . __('阅读', 'zib_language') . $views . ' · ' . __('评分', 'zib_language') . $score . ' · ' . __('收藏', 'zib_language') . $favorite_count . '</div>';
                break;
        }
    }
