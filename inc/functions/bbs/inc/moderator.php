...
function zib_bbs_get_add_moderator_link($type = 'plate', $id = 0, $class = '', $con = '')
{
    ...
    if (!$con) {
        global $zib_bbs;
        $con = zib_get_svg('add') . sprintf(__('添加%s', 'zib_language'), ('cat' === $type ? $zib_bbs->cat_moderator_name : $zib_bbs->plate_moderator_name));
    }
    $url_var = array(
...
function zib_bbs_get_moderator_add_user_link($type = 'plate', $user_id = 0, $id = 0, $class = '', $con = '')
{
    ...
    if (!$con) {
        global $zib_bbs;
        $con = sprintf(__('设为%s', 'zib_language'), ('cat' === $type ? $zib_bbs->cat_moderator_name : $zib_bbs->plate_moderator_name));
    }
...
function zib_bbs_get_edit_moderator_link($type = 'plate', $id = 0, $class = '', $con = '')
{
    ...
    if (!$con) {
        global $zib_bbs;
        $con = sprintf(__('管理%s', 'zib_language'), ('cat' === $type ? $zib_bbs->cat_moderator_name : $zib_bbs->plate_moderator_name));
    }
...
function zib_bbs_get_moderator_modal_link($type = 'plate', $id = 0, $class = '', $con = '')
{
    ...
    if (!$con) {
        global $zib_bbs;
        $con = sprintf(__('查看%s', 'zib_language'), ('cat' === $type ? $zib_bbs->cat_moderator_name : $zib_bbs->plate_moderator_name));
    }
 ...
 * @return {*}
 */
function zib_bbs_get_remove_moderator_link($type = 'plate', $user_id = 0, $id = 0, $class = '', $con = null)
{
    if (null === $con) {
        $con = __('移出', 'zib_language');
    }
    //权限判断
    $cap = 'cat' === $type ? 'cat_moderator_edit' : 'moderator_edit';
    if (!zib_bbs_current_user_can($cap, $id)) {
...
function zib_bbs_get_apply_moderator_link($plate = null, $class = '', $con = '')
{
    ...
    if (!$con) {
        global $zib_bbs;
        $con = sprintf(__('申请%s', 'zib_language'), $zib_bbs->plate_moderator_name);
    }
...
{
    global $zib_bbs;
    $name = sprintf(__('%s管理员', 'zib_language'), $zib_bbs->forum_name);
...
function zib_bbs_get_user_identity_badge($user_id = 0, $class = 'mr6')
{
    ...
    if (zib_bbs_user_is_forum_admin($user_id) && !is_super_admin($user_id)) {
        $badge .= '<span class="badg badg-sm c-red ' . $class . '">' . sprintf(__('%s管理员', 'zib_language'), $zib_bbs->forum_name) . '</span>';
    }
    $cat_moderator = zib_bbs_get_user_cat_moderator_ids($user_id);
    if ($cat_moderator) {
        $badge .= '<span data-toggle="tooltip" title="' . esc_attr(sprintf(__('%d个分区的%s', 'zib_language'), count($cat_moderator), $zib_bbs->cat_moderator_name)) . '" class="badg badg-sm c-green-2 moderator-bagd ' . $class . '">' . $zib_bbs->cat_moderator_name . '</span>';
    }
    $plate_author = zib_bbs_get_user_plate_author_ids($user_id);
    if ($plate_author) {
        $badge .= '<span data-toggle="tooltip" title="' . esc_attr(sprintf(__('%d个%s的%s', 'zib_language'), count($plate_author), $zib_bbs->plate_name, $zib_bbs->plate_author_name)) . '" class="badg badg-sm c-blue moderator-bagd ' . $class . '">' . $zib_bbs->plate_author_name . '</span>';
    }
    $plate_moderator = zib_bbs_get_user_plate_moderator_ids($user_id);
    if ($plate_moderator) {
        $badge .= '<span data-toggle="tooltip" title="' . esc_attr(sprintf(__('%d个%s的%s', 'zib_language'), count($plate_moderator), $zib_bbs->plate_name, $zib_bbs->plate_moderator_name)) . '" class="badg badg-sm c-blue-2 moderator-bagd ' . $class . '">' . $zib_bbs->plate_moderator_name . '</span>';
    }
    return $badge;
}
...
function zib_bbs_get_moderator_add_modal($id = '', $type = 'plate')
{
    ...
    if ('cat' === $type) {
        ...
        $header = '<div class="mb10 touch">' . zib_get_svg('plate-fill', null, 'mr6 c-green-2') . '<b>' . sprintf(__('添加%s', 'zib_language'), $moderator_name) . '</b><button class="close" data-dismiss="modal">' . zib_get_svg('close', null, 'ic-close') . '</button></div>';
    } else {
        $plate = get_post($id);
        if (empty($plate->ID) || 'plate' !== $plate->post_type) {
            return false;
        ...
        $header         = '<div class="mb10 touch">' . zib_get_svg('plate-fill', null, 'mr6 c-blue-2') . '<b>' . sprintf(__('添加%s', 'zib_language'), $moderator_name) . '</b><button class="close" data-dismiss="modal">' . zib_get_svg('close', null, 'ic-close') . '</button></div>';
    }
    ...
    $search .= '<input type="text" name="s" class="form-control search-input" tabindex="1" value="" placeholder="' . esc_attr__('请输入关键词以搜索用户', 'zib_language') . '" autocomplete="off">';
    $search .= '<div class="search-icon abs-right">' . zib_get_svg('search') . '</div>';
    $search .= '</div>';
    $search .= '<div class="mt20 mb10 muted-3-color separator search-remind em09">' . __('请输入关键词以搜索用户', 'zib_language') . '</div>';
    ...
    $html = '';
    $html .= '<div class="muted-2-color mb20">' . sprintf(__('为%s[%s]添加新的%s', 'zib_language'), $type_name, '<span class="focus-color">' . $edit_link . '</span>', $moderator_name) . '</div>';
    $html .= $search;
    return $header . $html;
}
...
function zib_bbs_get_moderator_add_user_modal($type = 'plate', $plate = null, $user_id = 0)
{
    ...
    if ('cat' === $type) {
        ...
        $title          = $cat->name;
        $moderator_name = $zib_bbs->cat_moderator_name;
        $header         = zib_get_modal_colorful_header('c-green-2', zib_get_svg('plate-fill'), sprintf(__('添加%s', 'zib_language'), $moderator_name));
    } else {
        $moderator_name = $zib_bbs->plate_moderator_name;
        $plate          = get_post($plate);
        $title          = esc_attr($plate->post_title);
        $ID             = $plate->ID;
        $header         = zib_get_modal_colorful_header('c-blue-2', zib_get_svg('plate-fill'), sprintf(__('添加%s', 'zib_language'), $moderator_name));
    }
    ...
    $form = '<form>';
    $form .= '<div class="mb10"><b>' . sprintf(__('将用户[%1$s]设置为%2$s[%3$s]的%4$s', 'zib_language'), $user_display_name, $type_name, $title, $moderator_name) . '</b></div>';
    $form .= '<div class="mb20"><textarea class="form-control" name="desc" tabindex="1" placeholder="' . esc_attr__('给用户留言', 'zib_language') . '" rows="2"></textarea></div>';
    $form .= '<div>
                    ...
                    <div class="mt20 modal-buts but-average">
                        <button type="button" data-dismiss="modal" href="javascript:;" class="but">' . __('取消', 'zib_language') . '</button>
                        <button class="but c-blue wp-ajax-submit"><i class="fa fa-check" aria-hidden="true"></i>' . __('确认添加', 'zib_language') . '</button>
                    </div>
                </div>';
    $form .= '</form>';
...
function zib_bbs_get_moderator_remove_modal($type, $obj, $user_id = 0)
{
    ...
    $form = '<form>';
    $form .= '<div class="mb10"><b>' . sprintf(__('您正在删除用户[%1$s]在%2$s[%3$s]的%4$s身份', 'zib_language'), $user_display_name, $type_name, $title, $type_name) . '</b></div>';
    $form .= '<div class="c-red mb6">' . __('确认要移出吗？', 'zib_language') . '</div>';
    $form .= '<div class="mb20"><textarea class="form-control" name="desc" tabindex="1" placeholder="' . esc_attr__('请输入删除移出原因', 'zib_language') . '" rows="2"></textarea></div>';
    $form .= '<div class="mt20 but-average">
                    ...
                    <button type="button" data-dismiss="modal" href="javascript:;" class="but">' . __('取消', 'zib_language') . '</button>
                    <button class="but c-red wp-ajax-submit"><i class="fa fa-trash-o" aria-hidden="true"></i>' . __('确认移出', 'zib_language') . '</button>
                </div>';
    $form .= '</form>';
...
function zib_bbs_moderator_add_user($type, $obj, $user_id = 0, $desc = '')
{
    ...
    $msg_con           = '';
    $msg_con .= sprintf(__('您好！%s！', 'zib_language'), $user_display_name) . '<br />';
    $msg_con .= sprintf(__('您已成为%1$s[%2$s]的%3$s', 'zib_language'), $type_name, '<a class="focus-color" href="' . esc_url($permalink) . '">' . $title . '</a>', $moderator_name) . '<br>';

    $msg_con .= sprintf(__('操作用户：%s', 'zib_language'), zib_get_user_name_link($operator_id)) . '<br>';
    $msg_con .= sprintf(__('操作时间：%s', 'zib_language'), current_time('Y-m-d H:i:s')) . '<br>';
    $msg_con .= '<br>';
    $msg_con .= $desc . '<br>';
    $msg_con .= '<a target="_blank" style="margin-top: 20px;" class="but jb-blue padding-lg" href="' . esc_url($permalink) . '">' . sprintf(__('查看此%s', 'zib_language'), $type_name) . '</a>';

    $title    = sprintf(__('您已成为%1$s[%2$s]的%3$s', 'zib_language'), $type_name, $title, $moderator_name);
    $msg_args = array(
        'send_user'    => 'admin',
...
function zib_bbs_get_moderator_modal($type, $id, $is_edit = false)
{
    ...
    $moderator_name = $zib_bbs->plate_moderator_name;
    $type_name      = $zib_bbs->plate_name;
    $header_prefix  = $is_edit ? __('管理', 'zib_language') : __('查看', 'zib_language'); //标题前缀
    ...
    if ('cat' === $type) {
        ...
        $btn    = !$is_edit ? zib_bbs_get_apply_moderator_link($id, 'mt10 but jb-blue padding-lg block', sprintf(__('申请成为%s', 'zib_language'), $moderator_name)) : $btn;
    }
    ...
    if (!$lists) {
        $header = zib_get_modal_colorful_header('jb-yellow', zib_get_svg('plate-fill'), sprintf(__('暂无%s', 'zib_language'), $moderator_name));
        $lists  = '<div class="c-yellow box-body text-center">' . sprintf(__('此%s暂时没有%s', 'zib_language'), $type_name, $moderator_name) . '</div>';
    } else {
...
function zib_bbs_moderator_remove_user($type = 'plate', $obj = null, $user_id = 0, $desc = '')
{
    ...
    $msg_con           = '';
    $msg_con .= sprintf(__('您好！%s！', 'zib_language'), $user_display_name) . '<br />';
    $msg_con .= sprintf(__('您在%1$s[%2$s]的%3$s身份已移出', 'zib_language'), $type_name, '<a class="focus-color" href="' . esc_url($permalink) . '">' . $title . '</a>', $moderator_name) . '<br>';

    $msg_con .= sprintf(__('操作用户：%s', 'zib_language'), zib_get_user_name_link($operator_id)) . '<br>';
    $msg_con .= sprintf(__('操作时间：%s', 'zib_language'), current_time('Y-m-d H:i:s')) . '<br>';
    $msg_con .= '<br>';
    $msg_con .= ($desc ?: '') . '<br>';
    $msg_con .= __('如有疑问，请与管理员联系', 'zib_language') . '<br>';

    $title    = sprintf(__('您在%1$s[%2$s]的%3$s身份已移出', 'zib_language'), $type_name, $title, $moderator_name);
    $msg_args = array(
        'send_user'    => 'admin',
        'receive_user' => $user_id,
        'type'         => 'system',
...
function zib_bbs_get_apply_moderator_process_modal($user_id)
{
    ...
    $header         = zib_get_modal_colorful_header('jb-blue', zib_get_svg('plate-fill'), sprintf(__('处理%s申请', 'zib_language'), $moderator_name));
    ...
    if (!empty($processing->meta['plate_id'])) {
        ...
        $info = '';
        $info .= '<div class="mb10">
                    <div class="author-set-left">' . __('申请用户', 'zib_language') . '</div>
                    <div class="author-set-right mt6">' . zib_get_user_name("id=$user_id") . '</div>
                </div>';
        $info .= '<div class="mb10">
                    <div class="author-set-left">' . sprintf(__('申请%s', 'zib_language'), $plate_name) . '</div>
                    <div class="author-set-right mt6"><a class="focus-color" href="' . get_permalink($plate) . '">' . $plate->post_title . '</a></div>
                </div>';
        $info .= '<div class="mb10">
                    <div class="author-set-left">' . __('申请时间', 'zib_language') . '</div>
                    <div class="author-set-right mt6">' . $time . '</div>
                </div>';
        $info .= '<div class="mb10">
                    <div class="author-set-left">' . __('申请说明', 'zib_language') . '</div>
        ...
        $html .= '<form>';
        $html .= '<div class="mb20">
                        <div class="author-set-left">' . __('处理方式', 'zib_language') . '</div>
                        <div class="author-set-right mt6">
                            <span><label class="badg p2-10 pointer c-blue mr6"><input type="radio" name="process" value="1"><span class="ml6">' . __('批准申请', 'zib_language') . '</span></label></span>
                            <span><label class="badg p2-10 pointer c-red"><input type="radio" name="process" value="2"><span class="ml6">' . __('拒绝申请', 'zib_language') . '</span></label></span>
                        </div>
                    </div>';

        $html .= '<div class="mb20"><div class="author-set-left">' . __('处理留言', 'zib_language') . '</div><div class="author-set-right mt6"><textarea class="form-control" name="desc" placeholder="' . esc_attr__('请输入处理留言', 'zib_language') . '" rows="2"></textarea></div></div>';
        $html .= '<a href="javascript:;" class="but jb-blue padding-lg block wp-ajax-submit"><i class="fa fa-check" aria-hidden="true"></i>' . __('确认提交', 'zib_language') . '</a>';
        ...
        $html = '<div class="em12 text-center c-yellow" style="padding: 30px 0;">' . __('当前申请已处理完成', 'zib_language') . '</div>';
...
function zib_bbs_get_apply_moderator_modal($plate_id = 0)
{
    ...
    if ($roles_lists) {
        $roles_lists_html = '<div class="title-theme font-bold mb6">' . __('申请要求', 'zib_language') . '</div>';
        $roles_lists_html .= '<div class="mb10 em09 muted-2-color">' . __('以下用户组可申请成为版主', 'zib_language') . '</div>';
    ...
    if ($desc) {
        $desc_html = '<div class="title-theme font-bold mb10">' . __('申请说明', 'zib_language') . '</div>';
    ...
    $header_text  = sprintf(__('申请成为%s', 'zib_language'), $name);
    if (!$can) {
        $header_class = 'jb-yellow';
        $header_text  = __('暂时不能申请', 'zib_language');
    }
    ...
    if ($can) {
        ...
        if (!empty($plate->ID)) {
            //指定版块，则显示已选择版块
            $plate_select .= '<div class="c-blue font-bold mb10">' . sprintf(__('您正在申请%1$s[%2$s]的%3$s', 'zib_language'), $zib_bbs->plate_name, $plate->post_title, $name) . '</div>';
            ...
            if ($_plate_select) {
                ...
                $plate_select .= '<div class="em09 muted-2-color mb6">' . sprintf(__('请选择申请的%s', 'zib_language'), $zib_bbs->plate_name) . '</div>';
        ...
        if ($plate_select) {
            ...
            $form .= $plate_select;
            $form .= '<div class="mb20">';
            $form .= '<div class="em09 muted-2-color mb6">' . __('请输入申请理由及说明', 'zib_language') . '</div>';
            $form .= '<textarea class="form-control" name="desc" placeholder="' . esc_attr__('请输入申请理由及说明', 'zib_language') . '" rows="2"></textarea>';
            $form .= '</div>';
            $form .= '<input type="hidden" name="action" value="apply_moderator">';
            $form .= wp_nonce_field('apply_moderator', '_wpnonce', false, false); //安全效验
            $form .= '<a href="javascript:;" class="but jb-blue padding-lg block wp-ajax-submit"><i class="fa fa-check" aria-hidden="true"></i>' . __('提交申请', 'zib_language') . '</a>';
            $form .= '</form>';
        } else {
            $form = '<h5 class="c-red mb20">' . sprintf(__('抱歉！暂时没有您可以申请%1$s的%2$s', 'zib_language'), $name, $zib_bbs->plate_name) . '</h5>';
        }
        ...
        $con = '<h5 class="c-red mb20">' . sprintf(__('抱歉！您暂时无法申请成为%s', 'zib_language'), $name) . '</h5>';
        $con .= $roles_lists_html;
        $con .= $desc_html;
        $footer = '<div class="modal-buts but-average"><a type="button" data-dismiss="modal" class="but" href="javascript:;">' . __('取消', 'zib_language') . '</a></div>';
    }
...
function zib_get_apply_moderator_plate_select($in_id = 0, $class = 'form-control')
{
    ...
    return $option_html ? '<div class="form-select"><select class="' . $class . '" name="plate_id"><option value="">' . sprintf(__('请选择%s', 'zib_language'), $plate_name) . '</option>' . $option_html . '</select></div>' : '';
}
...
function zib_bbs_apply_moderator_create($plate, $desc = '')
{
    ...
    $msg_con         = '';
    $msg_con .= sprintf(__('用户：%s，正在申请成为版主', 'zib_language'), zib_get_user_name_link($user_id, 'focus-color')) . '<br>';
    $msg_con .= sprintf(__('申请%1$s：%2$s', 'zib_language'), $plate_name, '<a class="focus-color" href="' . esc_url(get_permalink($plate)) . '">' . esc_html($plate->post_title) . '</a>') . '<br>';
    $msg_con .= sprintf(__('该%1$s由[%2$s]创建于%3$s，', 'zib_language'), $plate_name, zib_get_user_name_link($plate->post_author, 'focus-color'), get_the_time('Y-m-d H:i:s', $plate));
    $msg_con .= $moderator_count
        ? sprintf(__('目前已有%1$s名%2$s', 'zib_language'), $moderator_count, $moderator_name) . '<br /><br />'
        : sprintf(__('目前还没有%1$s', 'zib_language'), $moderator_name) . '<br /><br />';

    $msg_con .= __('申请说明：', 'zib_language') . '<br />' . $desc . '<br>';
    $msg_con .= sprintf(__('申请时间：%s', 'zib_language'), current_time('Y-m-d H:i:s')) . '<br>';
    $msg_con .= '<br>';

    $msg_con .= __('您可以点击下方按钮快速处理此申请', 'zib_language') . '<br>';

    $motel_btn = zib_bbs_get_apply_moderator_process_link($user_id, 'but jb-blue padding-lg', __('立即处理', 'zib_language'));
    $mail_url  = zib_bbs_get_apply_moderator_process_url();
    $mail_btn  = '<a target="_blank" style="margin-top: 20px;" class="but jb-blue padding-lg" href="' . esc_url($mail_url) . '">' . __('立即处理', 'zib_language') . '</a>' . '<br>';

    $title    = sprintf(__('%1$s[%2$s]有新的%3$s申请待处理-用户：%4$s', 'zib_language'), $plate_name, zib_str_cut($plate->post_title, 0, 8), $moderator_name, $userdata->display_name);
    $msg_args = array(
        'send_user'    => $user_id,
        'receive_user' => 'admin',
...
function zib_bbs_apply_moderator_process($user_id, $process, $desc = '')
{
    ...
    if (1 == $process) {
        //批准审核
        $msg_title   = sprintf(__('%1$s申请已通过，恭喜您成为%2$s[%3$s]的%4$s', 'zib_language'), $moderator_name, $plate_name, $plate->post_title, $moderator_name);
        $status_text = __('已通过', 'zib_language');
        //执行添加版主
        zib_bbs_add_moderator('plate', $plate_id, $user_id);
    } else {
        //拒绝审核
        $status_text = __('被拒绝', 'zib_language');
        $msg_title   = sprintf(__('您的%1$s申请被拒绝', 'zib_language'), $moderator_name);
    }

    $user_display_name = $userdata->display_name;
    $current_user_id   = get_current_user_id();
    $msg_con           = '';
    $msg_con .= sprintf(__('您好！%1$s', 'zib_language'), $user_display_name) . '<br />' . sprintf(__('您于%1$s 发起的%2$s申请%3$s', 'zib_language'), $time, $moderator_name, $status_text) . '<br>';
    $msg_con .= sprintf(__('申请%1$s：%2$s', 'zib_language'), $plate_name, '<a class="focus-color" href="' . esc_url($permalink) . '">' . esc_html($title) . '</a>') . '<br />';
    $msg_con .= sprintf(__('申请时间：%s', 'zib_language'), $time) . '<br>';
    $msg_con .= sprintf(__('处理时间：%s', 'zib_language'), current_time('Y-m-d H:i:s')) . '<br>';
    $msg_con .= sprintf(__('处理人：%s', 'zib_language'), zib_get_user_name_link($current_user_id)) . '<br>';
    $msg_con .= '<br>';
    $msg_con .= $desc . '<br>';
    $msg_con .= __('如有疑问请与管理员联系', 'zib_language');
