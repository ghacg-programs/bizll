...
function zib_bbs_get_author_filter($type = 'plate', $orderby = 'date', $status = 'publish')
{
    ...
    $orderby_array['plate'] = array(
        'date'         => __('最新创建', 'zib_language'),
        'last_post'    => __('最新发布', 'zib_language'),
        'last_reply'   => __('最新回复', 'zib_language'),
        'posts_count'  => sprintf(__('最多%s', 'zib_language'), $zib_bbs->posts_name),
        'reply_count'  => __('最多回复', 'zib_language'),
        'views'        => __('最高热度', 'zib_language'),
    ...
    if ('plate' === $type) {
        ...
        $status_array = array(
            'publish'   => __('创建', 'zib_language') . '<count class="ml3 em09">' . $this_count . '</count>',
            'moderator' => __('管理', 'zib_language') . '<count class="ml3 em09">' . zib_bbs_get_user_moderator_plate_count($author_id) . '</count>',
            'follow'    => sprintf(__('已%s', 'zib_language'), $zib_bbs->plate_follow_name) . '<count class="ml3 em09">' . $follow_count . '</count>',
        );
        ...
        $status_array        = array(
            'post'       => __('文章', 'zib_language') . '<count class="ml3 em09">' . $favorite_post_count . '</count>',
            'forum_post' => $zib_bbs->posts_name . '<count class="ml3 em09">' . $favorite_count . '</count>',
        );
    } else {
        $status_array = array(
            'publish' => __('发布', 'zib_language') . '<count class="ml3 em09">' . $this_count . '</count>',
        );
    }

    if ('forum_post' === $type && (($current_id && $current_id == $author_id) || is_super_admin())) {
        $status_array['pending'] = __('待审核', 'zib_language') . '<count class="ml3 em09">' . zib_get_user_post_count($author_id, 'pending', $type) . '</count>';
        $status_array['draft']   = __('草稿', 'zib_language') . '<count class="ml3 em09">' . zib_get_user_post_count($author_id, 'draft', $type) . '</count>';
