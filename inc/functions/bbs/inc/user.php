...
function zib_bbs_get_user_posts_lists($user_id = 0, $paged = 1, $orderby = 'date', $post_status = 'publish')
{
    ...
    if ('favorite' === $post_status) {
        ...
        if ($follow) {
            ...
            return zib_get_ajax_null(__('暂无收藏内容', 'zib_language'));
        }
    }
...
function zib_bbs_integral_add_options_filter($options)
{
    ...
    $bbs = array(
        'bbs_posts_new'   => array(sprintf(__('发布%s', 'zib_language'), $zib_bbs->posts_name), 10, sprintf(__('发布优质%s并审核通过', 'zib_language'), $zib_bbs->posts_name), __('论坛', 'zib_language')),
        'bbs_posts_hot'   => array(sprintf(__('%s成为热门', 'zib_language'), $zib_bbs->posts_name), 10, sprintf(__('发布%s成为热门', 'zib_language'), $zib_bbs->posts_name), __('论坛', 'zib_language')),
        'bbs_score_extra' => array(sprintf(__('%s被加分', 'zib_language'), $zib_bbs->posts_name), 10, sprintf(__('发布的%s获得加分，每篇%s最多加5次', 'zib_language'), $zib_bbs->posts_name, $zib_bbs->posts_name), __('论坛', 'zib_language')),
        'bbs_essence'     => array(sprintf(__('%s评为精华', 'zib_language'), $zib_bbs->posts_name), 10, sprintf(__('发布%s被评为精华内容', 'zib_language'), $zib_bbs->posts_name), __('论坛', 'zib_language')),
        'bbs_plate_new'   => array(sprintf(__('创建%s', 'zib_language'), $zib_bbs->plate_name), 10, sprintf(__('创建%s块并审核通过', 'zib_language'), $zib_bbs->plate_name), __('论坛', 'zib_language')),
        'bbs_plate_hot'   => array(sprintf(__('%s成为热门', 'zib_language'), $zib_bbs->plate_name), 10, sprintf(__('创建的%s成为热门版块', 'zib_language'), $zib_bbs->plate_name), __('论坛', 'zib_language')),
    );
