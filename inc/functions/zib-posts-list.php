 ...
 * @LastEditTime : 2026-06-17 19:03:52
...
function zib_posts_list($args = array(), $new_query = false, $echo = true)
{
    ...
    if (!$html) {
        $html = zib_get_ajax_null(__('暂无内容', 'zib_language'), '100', 'null-post.svg');
    }
...
function zib_get_posts_list_pay_tags($post)
{
    ...
    if (!empty($posts_pay['pay_type']) && $posts_pay['pay_type'] != 'no') {
        ...
        if (($pay_modo === 'points' && !$points_price) || ($pay_modo !== 'points' && !$pay_price)) {
            return '<a rel="nofollow" href="' . $get_permalink . '#posts-pay" class="meta-pay but jb-yellow">' . __('免费资源', 'zib_language') . '</a>';
        }

        //限制购买
        $pay_limit = !empty($posts_pay['pay_limit']) ? (int) $posts_pay['pay_limit'] : 0;
        if ($pay_limit > 0 && ($pay_user_vip_1_s || $pay_user_vip_2_s)) {
            return '<a rel="nofollow" href="' . $get_permalink . '#posts-pay" data-toggle="tooltip" title="' . esc_attr($order_type_name) . '" class="meta-pay but jb-vip' . $pay_limit . '">' . zibpay_get_vip_icon($pay_limit, '') . ' ' . __('会员专属', 'zib_language') . '</a>';
        }
...
function zib_get_posts_list_pay_badge($post)
{
    ...
    if (!empty($posts_pay['pay_type']) && $posts_pay['pay_type'] != 'no') {
        ...
        if (($pay_modo === 'points' && !$points_price) || ($pay_modo !== 'points' && !$pay_price)) {
            ...
            return '<item class="meta-pay badg badg-sm mr6 c-yellow"  data-toggle="tooltip" title="' . esc_attr($order_type_name) . '">' . $order_type_icon . __('免费', 'zib_language') . '</item>';
        }

        //限制购买
        $pay_limit = !empty($posts_pay['pay_limit']) ? (int) $posts_pay['pay_limit'] : 0;
        if ($pay_limit > 0 && ($pay_user_vip_1_s || $pay_user_vip_2_s)) {
            return '<item class="meta-pay badg badg-sm mr6 jb-vip' . $pay_limit . '"  data-toggle="tooltip" title="' . esc_attr($order_type_name) . '">' . zibpay_get_vip_icon($pay_limit, 'mr3') . __('会员专属', 'zib_language') . '</item>';
        }
...
function zib_get_posts_thumb_graphic($class = 'item-thumbnail')
{
    ...
    $sticky = '';
    if (zib_is_sticky()) {
        $sticky = '<badge class="img-badge left jb-red">' . __('置顶', 'zib_language') . '</badge>';
    }
...
function zib_paging($ajax = true, $echo = true)
{
    ...
    if ($ajax) {
        //ias自动加载
        $nex = _pz('ajax_trigger', __('加载更多', 'zib_language'));
        ...
        if ($max_page > 8) {
            ...
            $array[] = '<a class="pag-jump page-numbers" href="javascript:;"><input autocomplete="off" max="' . $max_page . '" current="' . $paged . '" base="' . $base . '" type="' . ($wp_is_mobile ? 'number' : 'text') . '" class="form-control jump-input" name="pag-go"><span class="hi de-sm mr6 jump-text">' . __('跳转', 'zib_language') . '</span><i class="jump-icon fa fa-angle-double-right em12"></i></a>';
        }
...
function zib_hot_posts($args = array(), $echo = false)
{
    ...
    while ($new_query->have_posts()) {
        ...
        if ($orderby == 'favorite') {
            $_meta = get_post_favorite_count('', __('人收藏', 'zib_language'));
        } elseif ($orderby == 'like') {
            $_meta = get_post_like_count('', __('人点赞', 'zib_language'));
        } elseif ($orderby == 'comment_count') {
            $_meta = get_post_comment_count('', __('条讨论', 'zib_language'));
        }
        if (!$_meta) {
            $_meta = get_post_view_count('', __('人已阅读', 'zib_language'));
        }
        //排第一的文章
        if ($posts_i == 1) {
