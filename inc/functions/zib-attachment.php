...
{
    if (empty($_FILES)) {
        return array('error' => 1, '_FILES' => '', 'msg' => __('上传信息错误，请重新选择文件', 'zib_language'));
    }
...
{
    echo '<p class="c-yellow">' . sprintf(__('如需上传大文件或经常上传失败，推荐开启%s功能', 'zib_language'), '<a target="_blank" href="' . zib_get_admin_csf_url('扩展增强/系统工具') . '">【' . __('大文件分片上传', 'zib_language') . '】</a>') . '</p>';
}
class zib_file_chunk
{
    ...
    {
        echo '<p>' . sprintf(__('您可以在主题设置->权限管理中设置上传大小、上传格式限制%s', 'zib_language'), '<a target="_blank" href="' . zib_get_admin_csf_url('功能权限/上传权限') . '">【' . __('立即设置', 'zib_language') . '】</a>') . '<br>' . __('如需上传超大文件，建议在前台编辑器中上传，支持断点续传、并发上传，速度更快，更稳定', 'zib_language') . '<br>' . __('如果您使用的OSS/COS等第三方云储存，上传大文件会很容易超时失败', 'zib_language') . '</p>';
    }
