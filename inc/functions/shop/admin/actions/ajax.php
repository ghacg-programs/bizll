...
function zib_shop_ajax_admin_shipping_submit()
{
    ...
    if ($order_meta_data['shipping_type'] === 'auto') {
        ...
        if (!$delivery_content) {
            zib_send_json_error(__('请填写发货内容', 'zib_language'));
        }
        ...
        if ($delivery_type != 'no_express' && (!$express_number || !$express_company_name)) {
            zib_send_json_error(__('请填写完整的物流信息', 'zib_language'));
        }
...
function zib_shop_ajax_admin_batch_shipping_submit()
{
    ...
    if (empty($order_ids)) {
        zib_send_json_error(__('请选择要发货的订单', 'zib_language'));
    }
    ...
    foreach ($order_ids as $order_id) {
        ...
        if ($order_meta_data['shipping_type'] === 'auto') {
            if (!$delivery_content) {
                $error_count++;
                $error_reason[] = __('自动发货失败订单未填写发货内容', 'zib_language');
                continue;
            }
            ...
            if ($delivery_type != 'no_express' && (!$express_number || !$express_company_name)) {
                $error_reason[] = __('快递发货失败订单未填写物流信息', 'zib_language');
                $error_count++;
                continue;
            }
    ...
    $msg          = '';
    $title        = __('批量发货成功', 'zib_language');
    $ys           = 'success';
    //成功数量
    if ($success_count > 0) {
        $msg .= sprintf(__('%s个订单发货成功', 'zib_language'), $success_count);
    }
    //失败数量
    if ($error_count > 0) {
        $title = $success_count > 0 ? __('批量发货部分失败', 'zib_language') : __('批量发货失败', 'zib_language');
        $ys    = $success_count > 0 ? 'warning' : 'error';

        $msg .= ($success_count > 0 ? '，' : '') . sprintf(__('%s个订单发货失败', 'zib_language'), $error_count);
    }
    //失败原因
    if (!empty($error_reason)) {
        $msg .= '，' . __('失败原因：', 'zib_language') . implode('、', $error_reason);
    }
...
function zib_shop_ajax_admin_after_sale_handle_submit()
{
    ...
    if (!$is_agreed && !$author_remark) {
        zib_send_json_error(__('请填写拒绝原因等信息', 'zib_language'));
    }

    if ($is_agreed && !$refund_channel && !$is_points && in_array($after_sale_type, ['refund', 'insured_price'])) {
        zib_send_json_error(__('请选择退款到用户的渠道', 'zib_language'));
    }
    ...
    if ($is_agreed) {
        switch ($after_sale_type) {
            case 'refund':
                ...
                $send_msg                      = __('已同意退款', 'zib_language');
                break;
            case 'refund_return':
                //退货退款
                $handle_data['return_address'] = $return_address;
                $send_msg                      = __('已同意退货退款', 'zib_language');
                break;
            case 'replacement':
                //换货
                $handle_data['return_address'] = $return_address;
                $send_msg                      = __('已同意换货', 'zib_language');
                break;
            case 'warranty':
                //保修
                $handle_data['return_address'] = $return_address;
                $send_msg                      = __('已同意保修', 'zib_language');
                break;
            case 'insured_price':
                //保价
                $handle_data['refund_channel'] = $refund_channel;
                $send_msg                      = __('已同意保价', 'zib_language');
                break;
        }
    } else {
        $send_msg = __('已拒绝此售后申请', 'zib_language');
    }
...
function zib_shop_ajax_admin_after_sale_refund_return_handle()
{
    ...
    zib_send_json_success(__('退货退款成功', 'zib_language'));
}
