...
function zib_shop_get_dis_header($dis_id)
{
    ...
    switch ($discount_type) {
        case 'reduction':
            $discount_text = sprintf(__('立减%s', 'zib_language'), $dis_data['reduction_amount']);
            break;
        case 'discount':
            $discount_text = sprintf(__('%s折', 'zib_language'), $dis_data['discount_amount']);
            break;
    }
    ...
    if ((float) $dis_data['price_limit'] > 0) {
        ...
        if ($discount_scope === 'item') {
            $text = __('单价', 'zib_language');
        } elseif ($discount_scope === 'product') {
            $text = __('商品', 'zib_language');
        } elseif ($discount_scope === 'author') {
            $text = __('店铺', 'zib_language');
        } elseif ($discount_scope === 'order') {
            $text = __('跨店', 'zib_language');
        }

        $limit[] = '<span class="badg badg-sm c-yellow">' . sprintf(__('%1$s满%2$s可用', 'zib_language'), $text, $dis_data['price_limit']) . '</span>';
    }
    ...
    if ($dis_data['user_limit']) {
        switch ($dis_data['user_limit']) {
            case 'vip':
                $limit[] = '<span class="badg badg-sm c-yellow">' . __('VIP可用', 'zib_language') . '</span>';
                break;
            case 'vip_2':
                $limit[] = '<span class="badg badg-sm c-yellow">' . __('VIP2可用', 'zib_language') . '</span>';
                break;
            case 'auth':
                $limit[] = '<span class="badg badg-sm c-yellow">' . __('认证用户可用', 'zib_language') . '</span>';
                break;
        }
    }
    ...
    if (!empty($dis_data['time_limit']) && !empty($dis_data['time_limit_config'])) {
        ...
        if (!empty($dis_data['time_limit_config']['countdown'])) {
            ...
            $time_countdown = '<div class="flex ac mt6 em12"><span class="mr3 opacity5 px12">' . __('活动仅剩', 'zib_language') . '</span><span class="em12 badg badg-sm c-red" int-second="auto" data-countdown="' . $countdown_end_time . '" data-over-text="' . esc_attr__('已结束', 'zib_language') . '">' . __('00分00秒', 'zib_language') . '</span></div>';
        } else {
            ...
            $limit[] = '<div class="badg badg-sm c-yellow-2">' . sprintf(__('活动时间：%1$s%2$s', 'zib_language'), $start_h, $end_h) . '</div>';
        }
    }
    ...
    if (!empty($dis_data['gift_config'][0])) {
        ...
        $gift = $gift_text ? '<div class="mt3"><span class="muted-2-color">' . __('赠送', 'zib_language') . '</span>' . $gift_text . '</div>' : '';
    }
    ...
    if (!empty($dis_data['discount_error'])) {
        ...
        switch ($dis_data['discount_error']) {
            case 'time_limit_start':
                $error = $dis_data['discount_error_msg'] . '，' . sprintf(__('开始时间：%s', 'zib_language'), $dis_data['start_time']);
                break;

            case 'time_limit_end':
                $error = $dis_data['discount_error_msg'] . '，' . sprintf(__('结束时间：%s', 'zib_language'), $dis_data['end_time']);
                break;

            case 'config_error':
                if (is_super_admin()) {
                    $error = '<span data-toggle="tooltip" data-placement="top" title="' . esc_attr__('当前优惠活动配置错误，请检查配置！(此内容仅管理员可见)', 'zib_language') . '">' . sprintf(__('配置错误：%s', 'zib_language'), $dis_data['discount_error_msg']) . '</span>';
                }
                break;
        }
    ...
    $html .= '<div class="shop-cat-filter-orderby shop-cat-filter-child" win-ajax-replace="orderby"><span class="opacity5">' . __('排序', 'zib_language') . '</span>' . $orderby_lists . '</div>';
