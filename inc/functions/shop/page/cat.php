...
function zib_shop_get_cat_breadcrumbs($cat_id)
{
    ...
    if (_pz('shop_breadcrumbs_home', true)) {
        $breadcrumbs .= '<li><a href="' . home_url() . '">' . __('首页', 'zib_language') . '</a></li>';
    }
...
function zib_shop_get_cat_filter($cat_id)
{
    ...
    if (!empty($child_terms) && !is_wp_error($child_terms)) {
        ...
        foreach ($sibling_terms as $_term) {
            if ($_term->term_id == $cat_id) {
                ...
                $tab_cat .= '<li><a ajax-replace="true" route="1" route-title="' . esc_attr($_term->name) . '-' . __('商城', 'zib_language') . $delimiter_blog_name . '" class="ajax-next" href="' . get_term_link($_term->term_id) . '">' . $_term->name . '</a></li>';
            }
        }
        $tab_cat .= '</div>';

        $tab_cat .= '<div class="shop-cat-filter-child">';
        $tab_cat .= '<span class="focus-color">' . __('全部', 'zib_language') . '</span>';
        foreach ($child_terms as $_term) {
            $tab_cat .= '<a ajax-replace="true" route="1" route-title="' . esc_attr($_term->name) . '-' . __('商城', 'zib_language') . $delimiter_blog_name . '" class="ajax-next" href="' . get_term_link($_term->term_id) . '">' . $_term->name . '</a>';
        }
        $tab_cat .= '</div>';
    } else {
        //有父级分类
        if (!empty($parent_terms) && !is_wp_error($parent_terms)) {
            ...
            $tab_cat .= '<div class="shop-cat-filter-child">';
            $tab_cat .= '<a ajax-replace="true" route="1" route-title="' . esc_attr($_term->name) . '-' . __('商城', 'zib_language') . $delimiter_blog_name . '" class="ajax-next" href="' . get_term_link($parent_id) . '">' . __('全部', 'zib_language') . '</a>';
            foreach ($sibling_terms as $_term) {
                if ($_term->term_id == $cat_id) {
    ...
    $tab_cat .= '<div class="shop-cat-filter-orderby shop-cat-filter-child"><span class="opacity5">' . __('排序', 'zib_language') . '</span>' . $orderby_lists . '</div>';
