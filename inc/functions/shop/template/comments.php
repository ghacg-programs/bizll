...
function zib_shop_comment_header()
{
    ...
    $order = '<a rel="nofollow" class="but comment-orderby' . (!$corderby || 'comment_date_gmt' == $corderby ? ' b-theme' : '') . '" href="' . esc_url(add_query_arg('corderby', 'comment_date_gmt', $this_url)) . '">' . __('最新', 'zib_language') . '</a>';
    $order .= '<a rel="nofollow" class="but comment-orderby' . ('comment_like' == $corderby ? ' b-theme' : '') . '" href="' . esc_url(add_query_arg('corderby', 'comment_like', $this_url)) . '">' . __('最热', 'zib_language') . '</a>';
    ...
    $type_filter_args = array(
        ...
        array(
            'name'  => __('有图', 'zib_language'),
            'type'  => 'image',
            'count' => $c_has_image,
        ),
        array(
            'name'  => __('好评', 'zib_language'),
            'type'  => 'good',
            'count' => $c_good,
        ),
        array(
            'name'  => __('中/差评', 'zib_language'),
            'type'  => 'bad',
            'count' => $c_bad,
        ),
    );
    ...
    if ($score_data && !empty($score_data['average'])) {
        ...
        if ($score_average) {
            $score_average_html = '<div class="">
                <div class="em09 muted-2-color">' . __('综合评分', 'zib_language') . '</div>
        ...
        $item_name_args = [
            ...
            'service'  => __('服务', 'zib_language'),
            'shipping' => __('物流', 'zib_language'),
        ];
...
{
    $comment_name = __('评价', 'zib_language');
    ...
    if (have_comments()) {
        ...
        echo zib_get_null(sprintf(__('暂无%s内容', 'zib_language'), $comment_name), 55, 'null.svg', 'comment comment-null');
...
function zib_shop_comment_author_reply_list($comment, $args, $depth)
{
    ...
    $html .= '<div class="mb10 comment-content" id="comment-content-' . $comment_id . '"><b class="">' . __('商家回复：', 'zib_language') . '</b>' . $con . '</div>';
    $html .= '</div>';
    $html .= '</li>';
    $html .= '</ul>';
...
function zib_shop_comment_list($comment, $args, $depth)
{
    ...
    if (!$con) {
        $con = '<div class="comment-null muted-2-color">' . (empty($score_data['is_auto']) ? __('用户未填写评价内容', 'zib_language') : __('系统默认好评', 'zib_language')) . '</div>';
    }
    ...
    if ($score_image) {
        ...
        foreach ($score_image as $img_id) {
            ...
            if (!empty($img_src[0])) {
                ...
                $score_image_html .= '<span><img ' . $lazy_attr . ' alt="' . esc_attr__('评价图片', 'zib_language') . '"></span>';
            }
        }
