 ...
 * @LastEditTime : 2026-06-23 23:46:22
 */
...
(function () {
    function shop__(key) {
        ...
        if (typeof zib__ === 'function') {
            return zib__(key);
        }
        return key;
    }

    function shop_sprintf(key) {
    ...
    $.fn.shopScore = function () {
        return this.each(function () {
            ...
            function updateScoreText(score) {
                var text = '';
                if (score <= 1) {
                    text = shop__('shop_rating_very_bad');
                } else if (score <= 2) {
                    text = shop__('shop_rating_bad');
                } else if (score <= 3) {
                    text = shop__('shop_rating_normal');
                } else if (score <= 4) {
                    text = shop__('shop_rating_good');
                } else {
                    text = shop__('shop_rating_very_good');
                }
    ...
    var VueTemplate = {
                    ...
                    <div class="product-info flex jsb xx flex1">
                        ...
                        <div class="cart-col-price product-price-box flex jsb ac">
                            <div class="price-box" :class="${this.data_prefix}pay_modo == 'points' ? 'c-yellow' : 'c-red'">
                                ...
                                <div class="badg badg-sm c-yellow" v-if="${this.data_prefix}prices.total_discount">${shop__('shop_save_prefix')} {{ ${this.data_prefix}prices.total_discount }}</div>
                            </div>
                        </div>
                ...
                <div class="product-opts-box mini-scrollbar scroll-y max-vh5 mb10">
                    <div v-if="${this.data_prefix}product_options.length > 0">
                        <div class="p-d-mb" v-for="(opt,opt_index) in ${this.data_prefix}product_options" :key="'opt-' + opt_index">
                            ...
                            <div class="pointer muted-2-color ml10 em09" @click="switchOptView(opt)" v-if="optHasImg(opt)"><svg class="icon mr3" aria-hidden="true"><use xlink:href="#icon-img-lists"></use></svg>{{ opt.view_mode === \'img\' ? \'${shop__('shop_view_list')}\' :  \'${shop__('shop_view_image')}\'  }}</div>
                            </div>
                    ...
                    <div class="padding-h10 flex ac jsb border-top mt10">
                        <div class="product-opt-title">${shop__('shop_quantity')}</div>
        ...
        remarkModalBox: function () {
            return `
                <div class="border-title touch"><div class="flex jc"><b>${shop__('shop_remark')}</b></div></div>
                <div class="remark-modal-body">
                    <div class="remark-modal-content mb20" style="min-height: 150px;">
                        <textarea class="form-control" style="min-height: 120px;" v-model.trim="${this.data_prefix}remark" placeholder="${shop__('shop_remark_placeholder')}" @keyup.ctrl.enter="remarkModalSubmit"></textarea>
                    </div>
                </div>
                <botton class="but jb-blue padding-lg btn-block" @click.prevent="remarkModalSubmit">${shop__('shop_confirm')}</botton>
            `;
        },
        ...
        createUserRequiredModal: function () {
            ...
            var modal_body = `
            <div class="border-title touch"><div class="flex jc"><b>${shop__('shop_fill_required')}</b></div></div>
            ...
            <botton class="but jb-blue padding-lg btn-block" @click.prevent="userRequiredModalSubmit">${shop__('shop_confirm')}</botton>
            `;
        ...
        userAddressModalBox: function () {
            return `
            <div class="border-title touch"><div class="flex jc"><b>{{ is_edit ? '${shop__('shop_address_edit')}' : '${shop__('shop_address_add')}' }}</b></div></div>
                <div class="address-form">
                    <!-- 收货人姓名 -->
                    <div class="form-group">
                        <div class="input-group">
                            <div class="input-group-addon"><i class="fa fa-user"></i></div>
                            <input type="text" class="form-control" v-model="form.name" placeholder="${shop__('shop_address_name_ph')}" maxlength="20" />
                        </div>
                    ...
                    <div class="form-group">
                        <div class="input-group">
                            ...
                            <input type="tel" class="form-control" v-model="form.phone" placeholder="${shop__('shop_address_phone_ph')}" maxlength="30" />
                        </div>
                    ...
                    <div class="form-group">
                        <div class="region-select flex" v-if="$.isEmptyObject(regions)">
                            <div class="flex1 mr10">
                                <input type="text" class="form-control" v-model="form.province" placeholder="${shop__('shop_address_province_ph')}" />
                            </div>
                            <div class="flex1 mr10">
                                <input type="text" class="form-control" v-model="form.city" placeholder="${shop__('shop_address_city_ph')}"/>
                            </div>
                            <div class="flex1">
                                <input type="text" class="form-control" v-model="form.county" placeholder="${shop__('shop_address_county_ph')}"/>
                            </div>
                        </div>
                        <div class="region-select flex" v-else>
                            <div class="form-select flex1">
                                <select class="form-control" v-model="form.province" @change="provinceChange">
                                    <option value="">${shop__('shop_select_province')}</option>
                            ...
                            <div class="form-select flex1 ml10">
                                <select class="form-control" v-model="form.city" @change="cityChange" ref="regionCitySelect">
                                    <option value="">${shop__('shop_select_city')}</option>
                            ...
                            <div class="form-select flex1 ml10" v-if="!$.isEmptyObject(counties)">
                                <select class="form-control" v-model="form.county" ref="regionCountySelect">
                                    <option value="">${shop__('shop_select_county')}</option>
                    ...
                    <div class="form-group">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" v-model="form.address" placeholder="${shop__('shop_address_detail_ph')}" maxlength="100" />
                        </div>
                    ...
                    <div class="form-group">
                        <div class="address-tags">
                            <div class="em09 muted-2-color mb10">${shop__('shop_address_tag_label')}</div>
                            <div class="tag-list">
                                ...
                                <span v-if="!showCustomInput" class="badg p2-10 pointer mb6" :class="form.tag === customTag ? 'active' : ''" @click="showCustomTagInput">{{ customTag || '${shop__('shop_custom')}' }}</span>
                                <span v-else class="tag-item custom-tag">
                                    <input type="text" class="badg-form badg p2-10 mb6" v-model="customTag" @blur="setCustomTag" @keyup.enter="setCustomTag" ref="customTagInput" maxlength="5" placeholder="${shop__('shop_address_tag_custom_ph')}" />
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="muted-box padding-10 mb10">
                        <label class="muted-color flex ac jsb" style="font-weight: normal; margin: 0">
                            ...
                            ${shop__('shop_address_set_default_checkbox')}
            ...
            <div class="modal-buts but-average">
                <button type="button" class="but" data-dismiss="modal">${shop__('shop_cancel')}</button>
                <button type="button" class="but c-blue" @click="editSaveAddress" ref="editSaveAddressBtn">${shop__('shop_save')}</button>
            </div>
            `;
        },
...
<div class="border-title touch">
    <div class="flex jc"><b>${shop__('shop_address_title')}</b></div>
</div>
<div class="user-address-list-modal">
    <div class="address-list mini-scrollbar scroll-y max-vh5 mb10">
        ...
        <div class="address-item muted-box padding-10 mb10 pointer" v-for="(item, index) in addresses" :key="'address-' + index" :class="selectedAddressId === item.id ? 'active' : ''" @click="selectAddress(item)">
            ...
            <div class="address-actions flex ac jsb">
                <span class="default-badge badg badg-sm c-red" v-if="item.is_default">${shop__('shop_address_default')}</span>
                <button class="default-badge but but-sm c-blue flex ac" v-else @click.prevent.stop="setDefaultAddress" :item-id="item.id">${shop__('shop_address_set_default')}</button>
                <div class="action-btns px12">
                    <button class="but p2-10" @click.prevent.stop="editAddress(item)">${shop__('shop_address_edit_btn')}</button>
                    <button class="but c-red p2-10 ml6" @click.prevent.stop="deleteAddress(item.id)" v-if="!item.is_default">${shop__('shop_address_delete_btn')}</button>
                </div>
            </div>
        </div>
        ...
        <div class="address-empty flex jc mt20 mb20" v-if="!addresses.length">
            <div class="empty-icon c-yellow text-center">
                ...
                <div class="empty-text">${shop__('shop_address_empty')}</div>
                <button type="button" class="mt20 but hollow c-blue padding-lg" @click="addNewAddress">${shop__('shop_address_add_now')}</button>
            </div>
        </div>
    </div>
    <div class="modal-buts but-average">
        <button type="button" class="but c-blue" @click="addNewAddress">${shop__('shop_address_add_new')}</button>
    </div>
</div>
    ...
    function createVueComponents() {
        return {
            ...
            stockAllText: function (stock_all) {
                ...
                if (stock_all === 0) {
                    return shop__('shop_out_of_stock');
                }

                return shop_sprintf('shop_stock', stock_all);
            },
            ...
            paramsModal: function (params) {
                ...
                return this.newModal('<div class="mini-scrollbar scroll-y max-vh5" style="min-height: 20vh;">' + html + '</div>', shop__('shop_product_params'));
            },
            ...
            serviceModal: function (service) {
                ...
                return this.newModal('<div class="mini-scrollbar scroll-y max-vh5" style="min-height: 20vh;">' + html + '</div>', shop__('shop_product_service'));
            },
            discountHitModal: function (item_data, discount_data, product_data) {
                ...
                <div class="flex muted-box">
                    <div class="text-ellipsis flex1 mr20">${shop__('shop_total_discount')}</div>
                    <div class="desc c-red">-${total_discount_amount}</div>
                </div>
                `;

                return this.newModal('<div class="mini-scrollbar scroll-y max-vh5">' + html + '</div>', shop__('shop_discount_detail'));
            },
                ...
                $.each(gift_data, function (index, item) {
                    switch (item.gift_type) {
                        ...
                        case 'vip_2':
                            ...
                            break;
                        case 'auth':
                            text.push(shop__('shop_gift_auth'));
                            break;
                        case 'level_integral':
                            text.push(shop__('shop_gift_exp'));
                            break;
                        case 'points':
                            text.push(shop__('shop_gift_points'));
                            break;
                        case 'product':
                            text.push(shop__('shop_gift_product'));
                            break;
                        case 'other':
            ...
            giftModal: function (gift_data) {
                ...
                $.each(gift_data, function (index, item) {
                    switch (item.gift_type) {
                        case 'vip_1':
                            var time_text = item.vip_time === 'Permanent' ? shop__('shop_permanent') : item.vip_time + shop__('shop_day_unit');
                        ...
                        case 'auth':
                            ...
                            gift_cards_html += '<div class="gift-card-item flex mt6"><div class="gift-name">' + shop__('shop_gift_auth') + '</div><div class="muted-3-color"> ' + desc + '</div></div>';
                            break;
                        case 'level_integral':
                            gift_cards_html += '<div class="gift-card-item flex mt6"><div class="gift-name">' + shop__('shop_gift_exp') + '</div><div class="muted-2-color"> ' + item.level_integral + '</div></div>';
                            break;
                        case 'points':
                            gift_cards_html += '<div class="gift-card-item flex mt6"><div class="gift-name">' + shop__('shop_gift_points') + '</div><div class="muted-2-color"> ' + item.points + '</div></div>';
                            break;
                        case 'product':
                            gift_cards_html += '<div class="gift-card-item flex mt6"><div class="gift-name">' + shop__('shop_gift_product') + '</div><div class="muted-2-color"> ' + item.product_id + '</div></div>';
                            break;
                        case 'other':
                ...
                return this.newModal(discount_data_html, shop__('shop_gift_detail'));
            },
            discountModal: function (discount_data, hit_data, valid_show) {
                ...
                $.each(_discount_data, function (index, item) {
                    ...
                    switch (discount_type) {
                        case 'reduction':
                            left_main = '<div class="discount-amount"><div class="price-text">' + item.reduction_amount + '</div><div class="price-type px12">' + shop__('shop_discount_reduce') + '</div></div>';
                            break;
                        case 'discount':
                            left_main = '<div class="discount-amount"><span class="price-text em14">' + getDecimalHtml(item.discount_amount) + '</span><span class="price-type px12 ml6">' + shop__('shop_discount_fold_unit') + '</span></div>';
                            break;
                        case 'gift':
                            break;
                    }
                    ...
                    if (Number(item.price_limit) > 0) {
                        ...
                        var _text = '';
                        if (discount_scope === 'item') {
                            _text = shop__('shop_limit_single');
                        } else if (discount_scope === 'product') {
                            _text = shop__('shop_limit_product');
                        } else if (discount_scope === 'author') {
                            _text = shop__('shop_limit_store');
                        } else if (discount_scope === 'order') {
                            _text = shop__('shop_limit_cross');
                        }

                        limit.push('<span class="badg badg-sm">' + shop_sprintf('shop_limit_full', _text, item.price_limit) + '</span>');
                    }
                    ...
                    if (item.user_limit) {
                        switch (item.user_limit) {
                            case 'vip':
                                limit.push('<span class="badg badg-sm">' + shop__('shop_vip_available') + '</span>');
                                break;
                            case 'vip_2':
                                limit.push('<span class="badg badg-sm">' + shop__('shop_vip2_available') + '</span>');
                                break;
                            case 'auth':
                                limit.push('<span class="badg badg-sm">' + shop__('shop_auth_user_available') + '</span>');
                                break;
                        }
                    }
                    ...
                    if (item.time_limit && item.time_limit_config) {
                        if (item.time_limit_config.show_html) {
                            ...
                            if (item.time_limit_config.countdown) {
                                right_main_h += '<div class="flex ac px12 mt3"><span class="mr3 opacity8 px12">' + shop__('shop_activity_remaining') + '</span><span class="badg badg-sm c-red" int-second="auto" data-countdown="' + item.time_limit_config.end + '" data-over-text="' + shop__('shop_countdown_ended') + '"></div>';
                            } else {
                                ...
                                var start_h = _start ? _start + (_end ? ' - ' : shop__('shop_time_start')) : '';
                                var end_h = _end ? _end + (_start ? '' : shop__('shop_time_end')) : '';
                ...
                gift_cards = gift_cards ? '<div class="discount-card-gift">' + (discount_cards ? '<div class="title-theme mb10 mt20">' + shop__('shop_gift_section') + '</div>' : '') + gift_cards + '</div>' : '';

                var discount_data_html = '<div class="mini-scrollbar scroll-y max-vh5">' + discount_cards + gift_cards + '</div>';

                return this.newModal(discount_data_html, shop__('shop_discount_info'));
            },
    ...
    function VShopConfirmModal(data) {
        ...
        self.ajaxget = function (data) {
            ...
            var local_storage_key = 'confirm_modal_html';
            if ($modal.length) {
                data.no_html = true;
            } else {
                var localData = localStorage.getItem(local_storage_key);
                if (localData) {
                    var parse_data = JSON.parse(localData);
                    if (parse_data[_win.ver]) {
                        data.no_html = true;
                        _html = parse_data[_win.ver];
                    }
                }
            }
            ...
            $.ajax({
                ...
                success: function (n) {
                    ...
                    if (n.html) {
                        localStorage.setItem(local_storage_key, JSON.stringify({[_win.ver]: n.html}));
                    }
        ...
        self.confirmModal = function (html, vue_data) {
            ...
            if (!$modal.length) {
                $body.createModal({
                    ...
                    id: id,
                    title: shop__('shop_confirm_order'),
                });
    ...
    function getChinaRegions() {
        ...
        // 先尝试从本地存储获取
        var local_storage_key = 'china_regions';
        var localData = localStorage.getItem(local_storage_key);
        if (localData) {
            try {
                var data = JSON.parse(localData);
                if (data[_win.ver]) {
                    return data[_win.ver];
                }
            } catch (e) {
                // 如果解析失败，从远程获取
            }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                // 保存到本地存储
                localStorage.setItem(local_storage_key, JSON.stringify({ [_win.ver]: newData }));
                regions = newData;
            },
    ...
    //地址管理VUE
    function VShopAddress(manual_address) {
        ...
        var app_data = $.extend(
            {
                ...
                submitting: false,
                tag_list: [shop__('shop_tag_home'), shop__('shop_tag_company'), shop__('shop_tag_school')],
                ...
                deleteAddress(address_id) {
                    if (!confirm(shop__('shop_delete_address'))) {
                        return;
                    }
                    ...
                    $.ajax({
                        ...
                        success(response) {
                            if (!response.error) {
                                notyf(shop__('shop_delete_success'), 'success');
                                //移出列表
                                var new_addresses = _this.addresses.filter((addr) => addr.id !== address_id);
                                _this.updateAddresses(new_addresses);
                            } else {
                                notyf(response.msg || shop__('shop_delete_failed'), 'danger');
                            }
                        },
                        error() {
                            notyf(shop__('shop_network_error'), 'danger');
                        },
                    });
                },
                ...
                validateForm() {
                    ...
                    if (!this.form.name) {
                        this.errors.name = shop__('shop_enter_name');
                    }

                    if (!this.form.phone) {
                        this.errors.phone = shop__('shop_enter_phone');
                    } else if (!/^(?:\d{5,}|\+\d{6,}|[\+]?\d{2,}[\-\s]\d{2,}[0-9|\-|\s]{0,15})$/.test(this.form.phone)) {
                        this.errors.phone = shop__('shop_phone_invalid');
                    }

                    if (!this.form.province || !this.form.city || (!this.form.county && !$.isEmptyObject(this.counties))) {
                        this.errors.region = shop__('shop_region_incomplete');
                    }

                    if (!this.form.address) {
                        this.errors.address = shop__('shop_enter_address');
                    }
    ...
    function VShopModifyAddress(selector) {
        ...
        var app_data = $.extend(
            createVueComponents(),
            {
                ...
                mounted: function () {
                    var _this = this;
                    _this.address_data = VShopAddress(config.config.manual_address);
            ...
            createVueComponents(),
            {
                ...
                userRequiredModalSubmit: function () {
                    ...
                    $.each(user_required, function (index, item) {
                        ...
                        if (!item.value) {
                            item.error = shop_sprintf('shop_enter_field', item.name);
                    ...
                    if (error.length > 0) {
                        return notyf(shop_sprintf('shop_enter_field', error.join(',')), 'warning');
                    }
                ...
                submitOrder: function (e) {
                    ...
                    if (_this.shipping_has_auto && !_this.user_data.email && _this.email_fill === 'required') {
                        return notyf(shop__('shop_enter_email'), 'warning');
                    }
                    ...
                    if (_this.shipping_has_express) {
                        if (!_this.user_data.address_data['name'] || !_this.user_data.address_data['phone'] || !_this.user_data.address_data['address']) {
                            return notyf(shop__('shop_select_address'), 'warning');
                        }
                    }
                    ...
                    if (user_required_error.length > 0) {
                        return notyf(shop_sprintf('shop_fill_fields', user_required_error.join(',')), 'warning');
                    }
        ...
        var app_data = $.extend(
            createVueComponents(),
            {
                    ...
                    if (_this_modal_data.product_options.length > 0 && _this_modal_data.options_active.length !== _this_modal_data.product_options.length) {
                        _this_modal_data.product_options.forEach(function (product_option, product_option_index) {
                            if (!_this_modal_data.options_active[product_option_index]) {
                                return notyf(shop_sprintf('shop_select_field', product_option.name), 'warning'), false;
                            }
                        });
                        return;
                    }
                    ...
                    _this.CartOptsEach(function (author_id, product_id, opt_data, opt_key_str) {
                        ...
                        if (product_options.length > 0) {
                            ...
                            if (opt_data.options_active_name === '') {
                                opt_data.options_active_name = shop__('shop_select_option'); //初始化
                            } else {
                                //去除最后一个分隔符
                ...
                _ajaxUpdataCart: function () {
                    ...
                    $.ajax({
                        ...
                        error: function (response) {
                            console.error(shop__('shop_update_cart_failed'), response);
                        },
                        success: function (response) {
                            if (response.error) {
                                console.error(response.msg || shop__('shop_update_cart_failed'), response);
                            }
                            if (response.count > 0) {
                ...
                removeChecked: function () {
                    ...
                    //系统二次确认
                    if (!confirm(shop__('shop_remove_selected'))) {
                        return;
                    }
                ...
                removeItem: function (author_id, product_id, opt_key, is_confirm) {
                    ...
                    if (is_confirm) {
                        //系统二次确认
                        if (!confirm(shop__('shop_remove_item'))) {
                            return;
                        }
                    }
                ...
                checkItem: function (opt_data) {
                    if (opt_data.stock_all == 0) {
                        opt_data.checked = false;
                        return notyf(shop__('shop_stock_insufficient'), 'warning');
                    }

                    if (opt_data.selected_count == 0) {
                        opt_data.checked = false;
                        return notyf(shop__('shop_select_quantity'), 'warning');
                    }
                    ...
                    if (opt_data.checked) {
                        ...
                        if (this.isOptError(opt_data.product_id, opt_data)) {
                            opt_data.checked = false;
                            return notyf(shop__('shop_select_option'), 'warning');
                        }
                ...
                _syncTotal: function () {
                    ...
                    _this.CartOptsEach(function (author_id, product_id, opt_data) {
                        ...
                        if (!opt_data.checked) {
                            ...
                            if (_this.isOptError(product_id, opt_data)) {
                                ...
                                is_edit || debounce_notyf(shop_sprintf('shop_select_product_option', _this.product_data[product_id].product_title), 'warning');
                            } else {
                    ...
                    $.each(each_product_count, function (product_id, product_count) {
                        ...
                        if (_this.isStockLow(product_id, product_count)) {
                            ...
                            is_edit || debounce_notyf(shop_sprintf('shop_product_stock_low', _this.product_data[product_id].title), 'warning');
                        }

                        //判断限购
                        if (_this.isLimitBuy(product_id, product_count)) {
                            is_can_pay = false;
                            var msg = shop_sprintf('shop_product_limit_pieces', _this.product_data[product_id].title, _this.product_data[product_id].limit_buy.limit);
                            if (_this.product_data[product_id].limit_buy.limit == 0) {
                                msg = shop_sprintf('shop_product_limit_none', _this.product_data[product_id].title);
                            }
                    ...
                    if (all_checked_price_count > 0 && all_checked_points_count > 0) {
                        ...
                        is_edit || debounce_notyf(shop__('shop_mixed_pay_mode'), 'error');
                    }
                ...
                countChange: function (opt_data, no_sync) {
                    if (opt_data.stock_all >= 0 && opt_data.selected_count > opt_data.stock_all) {
                        ...
                        return notyf(shop__('shop_stock_low'), 'warning');
                    }
                    ...
                    if (this.isLimitBuy(opt_data.product_id, opt_data.selected_count)) {
                        ...
                        var msg = shop_sprintf('shop_limit_buy', this.product_data[opt_data.product_id].limit_buy.limit);
                        if (this.product_data[opt_data.product_id].limit_buy.limit == 0) {
                            msg = shop__('shop_limit_cannot_buy');
                        }
            ...
            createVueComponents(),
            {
                ...
                countMaxChange: function () {
                    ...
                    if (this.limit_buy.is_limit && max > this.limit_buy.limit) {
                        var msg = shop_sprintf('shop_limit_buy', this.limit_buy.limit);
        ...
        var VueDirective = {
            ...
            'limit-buy': function (ctx) {
                var updateLimitBuy = function (el, value) {
                    ...
                    el.classList.remove('hide');
                    var text = shop_sprintf('shop_limit_label', value.limit);
                    if (value.limit == 0) {
                        text = shop__('shop_limit_purchased');
                    }
                ...
                $(ctx.el).on('click', function () {
                    ...
                    function getLimitItem(val, name) {
                        val = Number(val);
                        var text = shop_sprintf('shop_limit_pieces', val);
                        if (val === -1) {
                            text = shop__('shop_limit_no_limit');
                        }
                        if (val == 0) {
                            text = shop__('shop_limit_unavailable');
                        }
                    ...
                    var lists = '';
                    var content = '';
                    var title = '<div class="border-title touch"><div class="flex jc"><b>' + shop__('shop_limit_rules_title') + '</b></div></div>';
                    ...
                    if (limit.limit_all || limit.bought_count) {
                        lists += '<div class="muted-box mt10 c-yellow text-center">' + shop_sprintf('shop_limit_bought', limit.bought_count, limit.limit ? shop_sprintf('shop_limit_can_buy_more', limit.limit) : shop__('shop_limit_cannot_buy_more')) + '</div>';
                    } else {
                        lists += '<div class="muted-box mt10 c-yellow text-center">' + shop_sprintf('shop_limit_can_buy', limit.limit) + '</div>';
                    }
                ...
                //添加鼠标
                $(ctx.el).css('cursor', 'help').attr('title', shop__('shop_limit_rules_hint'));
            ...
            spinner: function (ctx) {
                ...
                var input_number_modal = function (config, callback) {
                    ...
                    var _max_attr = '';
                    if (config.min !== -1) {
                        _min_attr = ' limit-min="' + config.min + '" warning-min="' + shop__('shop_qty_min_warning') + '"';
                    }
                    if (config.max !== -1) {
                        if (config.max <= 3) {
                            return;
                        }
                        _dasc = shop_sprintf('shop_max_purchase', config.max);
                        _max_attr = ' limit-max="' + config.max + '" warning-max="' + shop__('shop_qty_max_warning') + '"';
                    }

                    _dasc = _dasc ? '<div class="muted-2-color mt6">' + _dasc + '</div>' : '';

                    var modal_title = '<div class="flex jc"><b>' + shop__('shop_input_quantity') + '</b></div>';
                    var modal_content = '<form><div style="mb20"><div class="mt20 relative"><input type="number" class="form-control" name="val"' + _min_attr + _max_attr + '></div>' + _dasc + ' </div><botton type="submit" class="mt20 but jb-blue padding-lg btn-block">' + shop__('shop_confirm') + '</botton></form>';
                    ...
                    $modal.find('form').on('submit', function (e) {
                        ...
                        if (config.max !== -1 && val > config.max) {
                            return notyf(shop_sprintf('shop_max_qty_exceed', config.max), 'warning'), false;
                        }
                        if (config.min !== -1 && val < config.min) {
                            return notyf(shop_sprintf('shop_min_qty_below', config.min), 'warning'), false;
                        }
    ...
    $body.on('click', '.expand-toggle', function () {
        ...
        var collapse_text = $this.attr('collapse-text') || shop__('shop_collapse_info');
        var expand_text = $this.attr('expand-text') || shop__('shop_expand_info');
