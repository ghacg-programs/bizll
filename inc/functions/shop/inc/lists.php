...
function zib_shop_get_product_list_graphic($args = [], $post = null)
{
    ...
    if ($show_sales !== 'off') {
        ...
        $sales_count = $sales_count ? '<badge class="img-badge jb-red px12">' . sprintf(__('已售%s', 'zib_language'), $sales_count) . '</badge>' : '';
    }
...
function zib_shop_get_search_product()
{
    ...
    if ($lists) {
        ...
        $lists = zib_get_ajax_null(__('未找到相关商品', 'zib_language'), '75', 'null-search.svg');
    }
...
function zib_shop_get_orderby_lists($orderby = '', $order = '', $current_url = '')
{
    ...
    if ($orderby == 'zibpay_price') {
        ...
        $lists .= '<a rel="nofollow" ajax-replace="true" class="ajax-next focus-color" href="' . esc_url($href) . '">' . __('售价', 'zib_language') . $_icon . '</a>';
    } else {
        $lists .= '<a rel="nofollow" ajax-replace="true" class="ajax-next" href="' . esc_url(add_query_arg(array('orderby' => 'zibpay_price'), $current_url)) . '">' . __('售价', 'zib_language') . '</a>';
    }
