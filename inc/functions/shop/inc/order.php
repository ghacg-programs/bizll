            ...
            foreach ($products_items[$posts_item->ID] as $item_key => $item_val) {
                ...
                if (isset($product_options[0])) {
                    ...
                    if ((!$options_active || count($options_active) !== count($product_options))) {
                        ...
                        $error_data[] = [
                            ...
                            'error_msg'          => sprintf(__('商品[%s]选项错误，请重新选择商品选项', 'zib_language'), $product_data[$posts_item->ID]['title']),
                ...
                if (($item_stock_all === 0 || $item_stock_all < $item_count) && $item_stock_all !== -1) {
                    $error_data[] = [
                        'product_id'         => $posts_item->ID,
                        'options_active_str' => $item_key,
                        'error_msg'          => sprintf(__('商品[%1$s][%2$s]库存不足', 'zib_language'), $product_data[$posts_item->ID]['title'], $options_active_name),
            ...
            foreach ($opt_items as $item_key => $opt_item_data) {
                ...
                if ($item_product_data['stock_type'] !== 'opts') {
                    ...
                    if (($product_stock_all === 0 || $product_stock_all < $item_product_data['count']) && $product_stock_all !== -1) {
                        $error_data[] = [
                            ...
                            'error_msg'  => sprintf(__('商品[%s]库存不足', 'zib_language'), $item_product_data['title']),
                ...
                if (!empty($item_product_data['limit_buy']['is_limit']) && $item_product_data['limit_buy']['limit'] < $item_product_data['count']) {
                    ...
                    if ($item_product_data['limit_buy']['limit'] == 0) {
                        $_msg = __('限购，无法购买', 'zib_language');
                        if (!empty($item_product_data['limit_buy']['limit_all'])) {
                            $_msg = sprintf(__('限购%1$s件，您已下单%2$s件，无法再购买', 'zib_language'), $item_product_data['limit_buy']['limit_all'], $item_product_data['limit_buy']['bought_count']);
                        }
                    } else {
                        $_msg = sprintf(__('限购%s件', 'zib_language'), $item_product_data['limit_buy']['limit']);
                    }

                    $error_data[] = [
                        'product_id'  => $item_product_data['product_id'],
                        'error_msg'   => sprintf(__('商品[%1$s]%2$s', 'zib_language'), $item_product_data['title'], $_msg),
...
//获取加入购物车链接
function zib_shop_get_order_add_cart_link($order, $class = '', $text = null)
{
    if (null === $text) {
        $text = __('加入购物车', 'zib_language');
    }
    $class              = $class ? ' ' . $class : '';
...
//获取查看订单的自动发货内容详情的模态框
function zib_shop_get_order_delivery_content_link(array $order, $class = '', $text = null)
{
    if (null === $text) {
        $text = __('查看内容', 'zib_language');
    }
    $class = $class ? ' ' . $class : '';
...
//获取撤销修改收货地址链接
function zib_shop_get_order_modify_address_cancel_link($msg_id, $class = '', $text = null)
{
    if (null === $text) {
        $text = __('撤销修改', 'zib_language');
    }
    $class = $class ? ' ' . $class : '';

    return '<button type="button" class="but c-blue wp-ajax-submit ' . $class . '" form-data=\'' . json_encode(['action' => 'modify_address_cancel', 'msg_id' => $msg_id]) . '\' data-confirm="' . esc_attr__('确认取消修改收货地址申请？', 'zib_language') . '">' . $text . '</button>';
}
...
function zib_shop_get_order_thumb($order, $class = '')
{
    ...
    $alt        = __('商品主图', 'zib_language') . zib_get_delimiter_blog_name();
