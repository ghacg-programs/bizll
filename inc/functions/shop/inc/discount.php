...
function zib_shop_get_discount_policy($data_or_id)
{
    ...
    if (empty($discount_config['discount_type'])) {
        ...
        $data['discount_error_msg'] = __('配置无效', 'zib_language');
        return $data;
    }
    ...
    switch ($data['discount_type']) {
        case 'reduction':
            ...
            if ($data['reduction_amount'] <= 0) {
                $data['is_valid']           = false;
                $data['discount_error']     = 'config_error';
                $data['discount_error_msg'] = __('立减金额必须大于0', 'zib_language');
        ...
        case 'discount':
            ...
            if ($data['discount_amount'] < 0.01 || $data['discount_amount'] > 9.99) {
                $data['is_valid']           = false;
                $data['discount_error']     = 'config_error';
                $data['discount_error_msg'] = __('折扣必须在0.01-9.99之间', 'zib_language');
                return $data;
            }
            break;
        ...
        case 'gift':
            ...
            if (empty($data['gift_config'][0])) {
                $data['is_valid']           = false;
                $data['discount_error']     = 'config_error';
                $data['discount_error_msg'] = __('赠品配置无效', 'zib_language');
                return $data;
            }
    ...
    if (!empty($discount_config['time_limit'])) {
        ...
        if ($start_timestamp && $current_time < $start_timestamp) {
            ...
            $data['discount_error_msg'] = __('活动未开始', 'zib_language');
            return $data;
        }
        ...
        if ($end_timestamp && $current_time > $end_timestamp) {
            ...
            $data['end_time']           = $end;
            $data['discount_error_msg'] = __('活动已结束', 'zib_language');
            return $data;
        }
            ...
            case 'vip_1':
                if (_pz('pay_user_vip_1_s', true) && (is_numeric($item['vip_time']) || $item['vip_time'] == 'Permanent' || $item['vip_time'] == 'permanent')) {
                    $valid_gift_config[] = [
                        ...
                        'gift_type'  => $gift_type,
                        'desc'       => (($item['vip_time'] == 'Permanent' || $item['vip_time'] == 'permanent') ? __('永久', 'zib_language') : sprintf(__('%s天', 'zib_language'), $item['vip_time'])) . _pz('pay_user_vip_1_name', 'VIP'),
                    ];
                    $has_vip1 = true;
                }
                break;
            case 'vip_2':
                if (_pz('pay_user_vip_2_s', true) && (is_numeric($item['vip_time']) || $item['vip_time'] == 'Permanent' || $item['vip_time'] == 'permanent')) {
                    $valid_gift_config[] = [
                        ...
                        'vip_time'   => $item['vip_time'],
                        'gift_type'  => $gift_type,
                        'desc'       => (($item['vip_time'] == 'Permanent' || $item['vip_time'] == 'permanent') ? __('永久', 'zib_language') : sprintf(__('%s天', 'zib_language'), $item['vip_time'])) . _pz('pay_user_vip_2_name', 'VIP2'),
                    ];
                    $has_vip2 = true;
                }
                break;
            case 'auth':
                if (_pz('user_auth_s', true) && !empty($item['auth_info']['name'])) {
                    $valid_gift_config[] = [
                        ...
                        'desc'      => $item['auth_info']['name'] . __('认证', 'zib_language'),
                    ];
                }
                break;
            case 'level_integral':
                if (_pz('user_level_s', true) && (int) $item['level_integral'] > 0) {
                    $valid_gift_config[] = [
                        ...
                        'desc'           => (int) $item['level_integral'] . __('经验值', 'zib_language'),
                    ];
                }
                break;
            case 'points':
                if (_pz('points_s', true) && (int) $item['points'] > 0) {
                    $valid_gift_config[] = [
                        ...
                        'desc'      => (int) $item['points'] . __('积分', 'zib_language'),
                    ];
                }
                break;
            case 'product':
...
function zib_shop_get_gift_type_name($gift_data)
{
    switch ($gift_data['gift_type']) {
        ...
        case 'vip_2':
            ...
            break;
        case 'auth':
            return __('认证资格', 'zib_language');
            break;
        case 'level_integral':
            return __('经验值', 'zib_language');
            break;
        case 'points':
            return __('积分', 'zib_language');
            break;
        case 'product':
            return __('商品', 'zib_language');
            break;
        case 'other':
