 ...
 * @LastEditTime : 2026-06-16 13:05:33
 * @Project      : Zibll子比主题
...
function zib_shop_get_auto_delivery_content($auto_delivery)
{
    ...
    //添加额外发货内容
    $extra_delivery_content = $auto_delivery['extra_delivery_content'] ?? '';
    if ($extra_delivery_content) {
        $delivery_html .= '<div class="muted-box mt20" style="white-space:pre-wrap;">' . $extra_delivery_content . '</div>';
    }
...
function zib_shop_get_default_express_traces(array $order)
{
    ...
    if (empty($traces)) {
        ...
        if ($pay_time) {
            $traces[] = array(
                'time'    => $pay_time,
                'context' => __('用户完成支付', 'zib_language'),
            );
        }
        ...
        if ($delivery_time && $shipping_status >= 1) {
            $traces[] = array(
                'time'    => $delivery_time,
                'context' => __('商家已发货', 'zib_language') . ($delivery_type == 'no_express' ? __('（商家选择无需物流发货）', 'zib_language') : '') . ($delivery_remark ? sprintf(__('，商家备注：%s', 'zib_language'), $delivery_remark) : ''),
                'state'   => __('已发货', 'zib_language'),
            );
        }
        ...
        if ($receive_time && $shipping_status >= 2) {
            $traces[] = array(
                'time'    => $receive_time,
                'context' => __('用户已确认收货', 'zib_language'),
                'state'   => __('已收货', 'zib_language'),
            );
        }
...
function zib_shop_get_auto_delivery_card_pass_content($auto_delivery_config)
{
    ...
    foreach ($db_data as $card_pass_data) {
        ...
        if ($type == 'invit_code') {
            $delivery_html .= '<div class="flex ac" style="font-weight: bold; display: flex; align-items: center;margin: 5px;"><span>' . __('邀请码：', 'zib_language') . ' </span><span class="badg c-blue" style="color: #2997f7; background: rgba(41, 151, 247, 0.1); border-radius: 4px; padding: .3em .6em;" data-clipboard-tag="' . esc_attr__('邀请码', 'zib_language') . '" data-clipboard-text="' . esc_attr($card_pass_data['password']) . '">' . $card_pass_data['password'] . '</span></div>';
        } else {
            $card_name = __('卡号', 'zib_language');
            $pass_name = __('卡密', 'zib_language');
            if (in_array($card_pass_data['type'], array('vip_coupon', 'coupon'))) {
                $pass_name = __('优惠码', 'zib_language');
            }
            ...
            $delivery_html .= $card_pass_data['card'] ? '<div class="flex ac" style="font-weight: bold; display: flex; align-items: center;margin: 5px;"><span>' . $card_name . '： </span><span class="badg c-blue" style="color: #2997f7; background: rgba(41, 151, 247, 0.1); border-radius: 4px; padding: .3em .6em;" data-clipboard-tag="' . esc_attr__('卡号', 'zib_language') . '" data-clipboard-text="' . esc_attr($card_pass_data['card']) . '">' . $card_pass_data['card'] . '</span></div>' : '';
            $delivery_html .= '<div class="flex ac" style="font-weight: bold; display: flex; align-items: center;margin: 5px;"><span>' . $pass_name . '： </span><span class="badg c-blue" style="color: #2997f7; background: rgba(41, 151, 247, 0.1); border-radius: 4px; padding: .3em .6em;" data-clipboard-tag="' . esc_attr__('卡密', 'zib_language') . '" data-clipboard-text="' . esc_attr($card_pass_data['password']) . '">' . $card_pass_data['password'] . '</span></div>';
        }
    }
...
function zib_shop_manual_shipping($order, $shipping_data)
{
    ...
    if ($order_meta_data['shipping_type'] === 'auto') {
        ...
        if (!empty($shipping_data['delivery_remark'])) {
            $delivery_html .= '<div class="muted-2-color">' . sprintf(__('商家备注：%s', 'zib_language'), $shipping_data['delivery_remark']) . '</div>';
        }
    ...
    if (!$delivery_remark && $old_express_number) {
        $delivery_remark = __('商家更新发货快递信息', 'zib_language');
    }
            ...
            switch ($gift['gift_type']) {
                ...
                case 'vip_2':
                    ...
                    $data             = array(
                        ...
                        'type'      => __('购买商品赠送', 'zib_language'), //中文说明
