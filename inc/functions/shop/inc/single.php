...
function zib_shop_single_mobile_comment_drawer()
{
    ...
    if ($comments) {
        foreach ($comments as $comment) {
            ...
            $user_name = __('匿名用户', 'zib_language');
            if ($user_id) {
            ...
            if ($score_image) {
                foreach ($score_image as $img_id) {
                    ...
                    if (!empty($img_src[0])) {
                        ...
                        $score_image_html = '<div class="ml20 featured-comment-img"><img ' . $lazy_attr . ' alt="' . esc_attr__('评价图片', 'zib_language') . '">' . $count . '</div>';
                        break;
                    }
                }
            }
    ...
    $box = '<div data-drawer="show" drawer-title="' . esc_attr(sprintf(__('商品评价 · %s', 'zib_language'), $comment_count)) . '" drawer-selector=".comment-drawer" class="zib-widget mb10">
                <div class="flex ac jsb">
                    <div><b class="">' . __('商品评价', 'zib_language') . '</b>' . ($comment_count ? '<span class="ml6 em09 muted-2-color">' . $comment_count . '</span>' : '') . '</div>
...
function zib_shop_single_content()
{
    ...
    if (zib_shop_single_comment_is_show_tab()) {
        ...
        $tabs_args['comment'] = [
            'title'         => __('评价', 'zib_language') . $comment_count,
    ...
    if ($tabs_args) {
        ...
        $tab_nav     = '<div class="mb10 mt6 muted-3-color separator">' . __('商品详情', 'zib_language') . '</div>';
    ...
    if (_pz('shop_single_related_s', true)) {
        ...
        $title = $related_config['title'] ?? __('相关推荐', 'zib_language');
...
function zib_shop_single_content_article()
{
    ...
    if (!$content) {
        $content = '<div class="text-center" style="padding:60px 0;"><img class="no-imgbox" style="width:280px;opacity: .7;" src="' . ZIB_TEMPLATE_DIRECTORY_URI . '/img/null.svg"><p style="margin-top:60px;" class="em09 muted-3-color separator">' . __('暂无详情介绍', 'zib_language') . '</p></div>';
    }
...
function zib_shop_get_product_detail($post)
{
    ...
    <div class="flex ab" :class="pay_modo == \'points\' ? \'c-yellow\' : \'c-red\'">
        ...
        <div class="text-left muted-2-color original-price-box flex" v-if="prices.total_price > prices.total_discount_price">
            <div class="px12-sm">' . __('优惠前：', 'zib_language') . '</div>
    ...
    <div class="sales-count muted-color muted-2-color px12-sm" v-if="sales_count">' . __('已售', 'zib_language') . '{{ sales_count }}</div>
</div>
<div v-else class="product-price-box flex ac jsb muted-box important-price-box" :class="important_discount.important_class">
        <div class="flex mr20 shrink0 xx important-price-left">
            <div class="flex ab">
                <div class="price-box mr6">
                    ...
                    <div class="px12-sm">' . __('原价:', 'zib_language') . '</div>
                    <div class="original-price px12-sm">
                        <span class="pay-mark" v-html="show_mark"></span>
                        <span v-price="prices.total_price"></span>
                    </div>
            ...
            <div class="flex ac">
                <div class="sales-count opacity8 px12-sm mt6" v-if="sales_count">' . __('已售', 'zib_language') . '{{ sales_count }}</div>
            </div>
       </div>
     <div class="flex ac jsb xx important-price-right overflow-hidden">
        ...
        <div class="flex jc mt6 hh" v-if="important_discount.time_limit_config && important_discount.time_limit_config.countdown">
            <span class="mr3 opacity8 px12">' . __('倒计时', 'zib_language') . '</span><span class="c-white badg badg-sm" int-second="auto" :data-countdown="important_discount.time_limit_config.end" data-over-text="' . esc_attr__('已结束', 'zib_language') . '">' . __('00分00秒', 'zib_language') . '</span>
        </div>
...
';
    ...
    <div class="p-d-mb" v-for="(opt,opt_index) in product_options" :key="\'opt-\' + opt_index">
        <div class="product-opt-title-box flex ac jsb">
            ...
            <div class="pointer muted-2-color ml10 em09" @click="switchOptView(opt)" v-if="optHasImg(opt)"><svg class="icon mr3" aria-hidden="true"><use xlink:href="#icon-img-lists"></use></svg>{{ opt.view_mode === \'img\' ? \'' . esc_js(__('列表', 'zib_language')) . '\' :  \'' . esc_js(__('图片', 'zib_language')) . '\'  }}</div>
        </div>
    ...
    <div class="flex ac">
        ...
        <div class="selected-title muted-color flex1 mr6" v-effect="$el.textContent = \'' . esc_js(__('已选：', 'zib_language')) . '\'+getOptionsActiveName(options_active)"></div>
    ...
    <div v-spinner="selected_count" @change="countChange" @max-change="countMaxChange" min="1" :max="spinnerMax()" @min-change="notyf(\'' . esc_js(__('最少选择一件', 'zib_language')) . '\', \'warning\')"></div>
    <div class="ml10 flex">
    ...
    <div class="flex jsb ac">
        <div class="but-group shop-paybtn-group">
            <button class="but c-yellow shop-add-cart-btn" @click.prevent="cartBtnClick" ref="addCartBtn">' . __('加入购物车', 'zib_language') . '</button>
            <button class="but jb-red shop-buy-btn" @click.prevent="orderBtnClick">' . __('立即购买', 'zib_language') . '</button>
        </div>
...
function zib_shop_get_breadcrumbs()
{
    ...
    if (_pz('shop_breadcrumbs_shop_home', true)) {
        ...
        if ($home_url) {
            $home_name = _pz('shop_breadcrumbs_shop_home_name') ?: __('商城', 'zib_language');
...
function zib_shop_single_footer_tabbar($btn)
{
    ...
    $cart_button = '<a class="tabbar-item tabbar-cart" href="' . $url . '">' . $icon . '<text>' . __('购物车', 'zib_language') . '</text><badge cart-count="' . $count . '">' . ($count ?: '') . '</badge></a>';

    $pay = '
    <div class="but-group shop-paybtn-group">
            <button class="but jb-yellow-2" data-onclick=".shop-add-cart-btn">' . __('加入购物车', 'zib_language') . '</button>
            <button class="but jb-red" data-onclick=".shop-buy-btn">' . __('立即购买', 'zib_language') . '</button>
        </div>';
    ...
    if ($shop_author_show) {
        ...
        $author_btn    = '<a class="tabbar-item" href="' . $author_url . '"><icon>' . $author_avatar . '</icon><text>' . __('店铺', 'zib_language') . '</text></a>';
...
function zib_shop_single_frontend_set_input_array($input_array, $post_id)
{
    ...
    $page_input[] = array(
        ...
        'type'     => 'radio',
        'inline'   => true,
        'name'     => __('页面布局', 'zib_language'),
        'value'    => zib_shop_get_product_config($post_id, 'content_layout'),
        'question' => __('商品详情页宽度填充方式', 'zib_language'),
        'options'  => array(
            ''     => __('默认', 'zib_language'),
    ...
    $page_input[] = array(
        ...
        'type'     => 'radio',
        'inline'   => true,
        'name'     => __('详情背景盒子', 'zib_language'),
        'value'    => zib_shop_get_product_config($post_id, 'content_show_bg'),
        'question' => __('商品详情页的内容部分是否显示背景盒子，常用图片作为商品介绍，推荐隐藏，如果选择了带侧边栏布局则建议开启', 'zib_language'),
        'options'  => array(
            ''    => __('默认', 'zib_language'),
