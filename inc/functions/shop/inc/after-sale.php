...
//获取用户申请售后链接
function zib_shop_get_order_after_sale_link(array $order, $class = '', $text = null)
{
    $class = $class ? ' ' . $class : '';
    ...
    if (null === $text) {
        if (in_array($after_sale_status, [1, 2])) {
            $text = __('售后详情', 'zib_language');
            $class .= ' c-yellow';
        } else {
            $text = __('申请售后', 'zib_language');
        }
    }
...
//获取取消售后的链接
function zib_shop_get_order_after_sale_cancel_link(array $order, $class = '', $text = null)
{
    $class = $class ? ' ' . $class : '';
    ...
    return '<a class="wp-ajax-submit' . $class . '" form-data="' . esc_attr(json_encode($form_data)) . '" href="javascript:;" data-confirm="' . esc_attr__('确定取消此售后申请？', 'zib_language') . '">' . $text . '</a>';
}

//获取售后退货快递的链接，此函数不做判断，需要调用者自行判断
function zib_shop_get_order_after_sale_return_express_link(array $order, $class = '', $text = null)
{
    if (null === $text) {
        $text = __('填写快递信息', 'zib_language');
    }
...
//获取售后记录链接
function zib_shop_get_order_after_sale_record_link(array $order, $class = '', $text = null)
{
    if (null === $text) {
        $text = __('售后记录', 'zib_language');
    }
...
function zib_shop_get_after_sale_type_reason($type = '')
{
    $reason_default = [
        'refund'        => [
            ['t' => __('与商家协商一致', 'zib_language')],
            ['t' => __('质量问题', 'zib_language')],
            ['t' => __('商品不喜欢', 'zib_language')],
            ['t' => __('物流问题', 'zib_language')],
            ['t' => __('退运费', 'zib_language')],
            ['t' => __('其他原因', 'zib_language')],
        ],
        'refund_return' => [
            ['t' => __('不想要了', 'zib_language')],
            ['t' => __('与商家协商一致', 'zib_language')],
            ['t' => __('质量问题', 'zib_language')],
            ['t' => __('商品不喜欢', 'zib_language')],
            ['t' => __('物流问题', 'zib_language')],
...
function zib_shop_after_sale_to_end(array $order, array $after_sale_data)
{
    ...
    if ($after_sale_status == 3 && in_array($after_sale_data['type'], ['refund', 'insured_price', 'refund_return'])) {
        ...
        $type_name = [
            'refund'        => __('商品退款', 'zib_language'),
            'refund_return' => __('商品退货退款', 'zib_language'),
            'insured_price' => __('商品保价', 'zib_language'),
        ];
        //退款渠道
        ...
        if ($refund_channel === 'balance' && $after_sale_data['price'] > 0 && !$is_points) {
            ...
            $user_balance_args = array(
                ...
                'desc'      => __('商品[', 'zib_language') . $product_title . ']' . str_replace('商品', '', $type_name[$after_sale_data['type']]), //说明
            );
...
function zib_shop_get_after_sale_record_timeline($after_sale_data, $_mark = '')
{
    ...
    $type_names = [
        'refund'        => '<span class="badg badg-sm c-red">' . __('仅退款', 'zib_language') . '</span>',
        'refund_return' => '<span class="badg badg-sm c-yellow">' . __('退货退款', 'zib_language') . '</span>',
        'replacement'   => '<span class="badg badg-sm c-purple">' . __('换货', 'zib_language') . '</span>',
        'warranty'      => '<span class="badg badg-sm c-green-2">' . __('保修', 'zib_language') . '</span>',
        'insured_price' => '<span class="badg badg-sm c-yellow-2">' . __('保价', 'zib_language') . '</span>',
    ];
    $type_name = $type_names[$type];
    $content   = '';
    if ($after_sale_data['price'] ?? 0) {
        $content .= '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('申请金额', 'zib_language') . '</div><div class="flex0 c-red">' . $_mark . $after_sale_data['price'] . '</div></div>';
    }
    if ($after_sale_data['reason'] ?? '') {
        $content .= '<div class="flex jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('申请原因', 'zib_language') . '</div><div class="ml20">' . esc_attr($after_sale_data['reason']) . '</div></div>';
    }
    if ($after_sale_data['remark'] ?? '') {
        $content .= '<div class="flex jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('用户备注', 'zib_language') . '</div><div class="ml20">' . esc_attr($after_sale_data['remark']) . '</div></div>';
    }

    $timeline[] = [
        'time'    => $after_sale_data['user_apply_time'],
        'title'   => __('用户发起申请', 'zib_language') . $type_name,
        'content' => $content,
    ];
    ...
    if (!empty($after_sale_data['author_handle_time'])) {
        ...
        $agreed_text   = $author_agreed ? '<span class="c-green">' . __('商家同意', 'zib_language') . '</span>' : '<span class="c-red">' . __('商家拒绝', 'zib_language') . '</span>';
        $content       = '';
    ...
    if ($status <= 1) {
        $timeline[] = [
            'time'  => '',
            'title' => __('等待商家处理', 'zib_language'),
        ];
        return $timeline;
    } elseif ($status == 2) {
        $timeline[] = [
            'time'  => '',
            'title' => __('等待用户发货', 'zib_language'),
        ];
    } else {
    }
}
...
function zib_shop_get_after_sale_record_lists($order_id)
{
    ...
    $record_html  = '';
    $status_names = array(
        '1' => '<span class="badg badg-sm c-yellow">' . __('待处理', 'zib_language') . '</span>',
        '2' => '<span class="badg badg-sm c-blue">' . __('处理中', 'zib_language') . '</span>',
        '3' => '<span class="badg badg-sm c-green">' . __('已完成', 'zib_language') . '</span>',
        '4' => '<span class="badg badg-sm">' . __('已取消', 'zib_language') . '</span>',
        '5' => '<span class="badg badg-sm c-red">' . __('已驳回', 'zib_language') . '</span>',
    );
    ...
    foreach ($after_sale_record as $time => $record) {
        ...
        $record_item .= '<div class="flex ac"><b class="mr10">' . ($type_name ?: __('售后', 'zib_language')) . '</b>' . $status_names[$status] . '</div>';
        $record_item .= '<div class="muted-2-color">' . $user_apply_time . '</div>';
        $record_item .= '</div>';

        if ($price) {
            $record_item .= '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('申请金额', 'zib_language') . '</div><div class="flex0 c-red">' . $_mark . $price . '</div></div>';
        }

        if ($reason) {
            $record_item .= '<div class="flex jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('申请原因', 'zib_language') . '</div><div class="ml20">' . esc_attr($reason) . '</div></div>';
        }

        if ($remark) {
            $record_item .= '<div class="flex jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('用户备注', 'zib_language') . '</div><div class="ml20">' . esc_attr($remark) . '</div></div>';
        }

        if ($author_agreed !== -1) {
            $agreed_text = $author_agreed ? '<span class="c-green">' . sprintf(__('商家同意%s', 'zib_language'), $type_name) . '</span>' : '<span class="c-red">' . __('商家拒绝', 'zib_language') . '</span>';

            $record_item .= '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('处理结果', 'zib_language') . '</div><div class="flex0">' . $agreed_text . '</div></div>';

            if ($author_handle_time) {
                $record_item .= '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('处理时间', 'zib_language') . '</div><div class="flex0">' . $author_handle_time . '</div></div>';
            }

            if ($author_remark) {
                $record_item .= '<div class="flex jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('商家备注', 'zib_language') . '</div><div class="ml20">' . esc_attr($author_remark) . '</div></div>';
            }
        }
        ...
        if ($status == 3) {
            ...
            if ($refund_channel) {
                ...
                $record_item .= '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('退款到', 'zib_language') . '</div><div class="flex0 c-blue-2">' . $refund_channel_name . '</div></div>';
            }
        }

        if (!empty($record['refund_return_author_remark'])) {
            $record_item .= '<div class="flex jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('商家退货备注', 'zib_language') . '</div><div class="ml20">' . esc_attr($record['refund_return_author_remark']) . '</div></div>';
        }

        if ($end_time && in_array($status, [3, 4, 5]) && $end_time !== $author_handle_time) {
            $record_item .= '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('完成时间', 'zib_language') . '</div><div class="flex0">' . $end_time . '</div></div>';
        }
