 ...
 * @LastEditTime : 2026-06-24 10:40:48
* @Project : Zibll子比主题
...
function zib_shop_user_center_page_sidebar_order($con)
{
    ...
    $con .= '<div class="zib-widget padding-6 mb10-sm"><div class="padding-6 flex ac jsb"><div class="ml3">' . __('我的订单', 'zib_language') . '</div><div><a class="muted-2-color em09" href="javascript:;" data-onclick="[data-target=\'#user-tab-order\'],[data-target=\'#user-order-tab-all\']">' . __('全部', 'zib_language') . '<i class="ml6 fa fa-angle-right em12"></i></a></div></div><div class="flex ac jsb text-center padding-6 padding-w10 user-order-tab-btn-box">' . $buttons_html . '</div></div>';
    return $con;
}
...
function zib_shop_user_center_page_sidebar_button_1_args_cart($buttons)
{
    $cart_buttons = array(array(
        ...
        'icon' => '',
        'tab'  => '',
        'html' => '<item class="icon-but-cart"><a href="' . zib_shop_get_cart_url() . '"><div class="em16">' . zib_get_svg('cart-color') . '</div><div class="px12 muted-color mt3">' . __('购物车', 'zib_language') . '</div></a></item>',
    ));
...
function zib_shop_user_order_tabs($tabs)
{
    ...
    $tabs['wait-shipped'] = array(
        'tab'   => 'wait-shipped',
        'title' => __('待发货', 'zib_language'),
        'count' => zib_shop_get_user_order_count('wait-shipped'),
    );
    $tabs['wait-receive'] = array(
        'tab'   => 'wait-receive',
        'title' => __('待收货', 'zib_language'),
        'count' => zib_shop_get_user_order_count('wait-receive'),
    );
    $tabs['wait-evaluate'] = array(
        'tab'   => 'wait-evaluate',
        'title' => __('待评价', 'zib_language'),
        'count' => zib_shop_get_user_order_count('wait-evaluate'),
    );
    $tabs['after-sale'] = array(
        'tab'   => 'after-sale',
        'title' => __('售后/退款', 'zib_language'),
        'count' => zib_shop_get_user_order_count('after-sale'),
    );
...
function zib_shop_user_order_list_card($html, $order_type, $order)
{
    ...
    if ($post) {
        ...
        $_title = '<span class="font-bold">' . ($order_data['product_title'] ?? __('商品已下架', 'zib_language')) . '</span>';
    }
    ...
    $_refund_price_box    = $_refund_price ? '<span class="muted-2-color em09 ml10">' . __('退款', 'zib_language') . '<b class="muted-color ml3">' . $_mark . zib_floatval_round($_refund_price) . '</b></span>' : '';
    $_pay_price_type_name = $status == '1' || $status == -2 ? __('实付', 'zib_language') : __('应付', 'zib_language');
    ...
    if ($status == '0') {
        ...
        $_status_name   = '<span class="c-red">' . __('待支付', 'zib_language') . ' <span class="c-yellow px12 badg badg-sm" int-second="1" max-unit="minute" data-over-text="' . esc_attr__('交易已关闭', 'zib_language') . '" data-countdown="' . $time_remaining . '"><span class="minute"></span>:<span class="second"></span></span></span>';
    }

    if ($status == -2) {
        $after_sale_type       = $order_data['after_sale_data']['type'] ?? '';
        $after_sale_type_names = [
            'refund'        => __('已退款', 'zib_language'),
            'refund_return' => __('已退货退款', 'zib_language'),
        ];
    ...
    if ($status == '1') {
        ...
        if ($shipping_status == '0') {
            $_status_name = '<span class="c-yellow">' . __('待发货', 'zib_language') . '</span>';
        } elseif ($shipping_status == '1') {
            $_status_name  = '<span class="c-green">' . __('待收货', 'zib_language') . '</span>';
            ...
            if (isset($express_data['state'])) {
                ...
                $delivery_type_name = $shipping_data['delivery_type'] == 'no_express' ? '<span class="ml6">' . __('商家选择无需物流发货', 'zib_language') . '</span>' : '';
                $delivery_remark    = $shipping_data['delivery_remark'] ? '<span class="ml10">' . $shipping_data['delivery_remark'] . '</span>' : '';

                $_express = '<div class="text-ellipsis muted-color"><i class="fa fa-truck mr6 fa-fw"></i><b class="mr6">' . __('已发货', 'zib_language') . '</b>' . $delivery_type_name . '<span class="ml10 muted-2-color">' . $shipping_data['delivery_time'] . '</span>' . $delivery_remark . '</div>';
            }
            ...
            $_status_name = '<span class="c-blue">' . __('交易完成', 'zib_language') . '</span>';
        }
    }
    ...
    if (in_array($after_sale_status, [1, 2])) {
        ...
        if ($after_sale_status !== 4) {
            $_status_name = '<span class="c-yellow">' . __('售后中', 'zib_language') . '</span>';
            if ($after_sale_status === 1) {
                $after_sale_status_html = '<span class="ml10 muted-color">' . __('等待商家处理', 'zib_language') . '</span><span class="ml10 muted-2-color">' . $after_sale_data['user_apply_time'] . '</span>';
                ...
                if ($progress === 1) {
                    if ($return_express_over_time) {
                        ...
                        $time_countdown = '<span class="ml10 em09 muted-2-color">' . sprintf(__('剩余发货时间：%s', 'zib_language'), '<span class="c-yellow badg badg-sm" int-second="1" data-over-text="' . esc_attr__('1秒', 'zib_language') . '" data-countdown="' . $time_remaining . '"></span>') . '</span>';
                    } else {
                        $time_countdown = '';
                    }

                    $after_sale_status_html = '<span class="ml10 muted-color">' . __('等待您发货', 'zib_language') . '</span>' . $time_countdown;
                } elseif ($progress === 2) {
                    $after_sale_status_html = '<span class="ml10 muted-color">' . __('您已发货，等待商家处理', 'zib_language') . '</span><span class="ml10 muted-2-color">' . $after_sale_data['user_return_time'] . '</span>';
                } elseif ($progress === 3) {
                    $after_sale_status_html = '<span class="ml10 muted-color">' . __('商家已发货，等待您收货', 'zib_language') . '</span><span class="ml10 muted-2-color">' . $after_sale_data['user_apply_time'] . '</span>';
                }
            }

            $_after_sale = '<div class="text-ellipsis muted-color"><i class="fa-fw fa fa-heart-o"></i><b class="ml6">' . __('售后中', 'zib_language') . '</b>' . $type_name . $after_sale_status_html . '</div>';
...
function zib_shop_user_order_details_footer($order)
{
    ...
    $author_contact = zib_shop_get_author_contact_link($order['post_author'], 'but', zib_get_svg('manual-service') . '<span class="muted-2-color ">' . __('客服', 'zib_language') . '</span>');
...
function zib_shop_get_user_order_btns($order)
{
    ...
    // 根据订单状态添加按钮
    if ($status == 0) {
        $btns[] = zibpay_get_order_close_link($order, 'but ml6', __('关闭订单', 'zib_language'));
        $btns[] = zibpay_get_order_pay_link($order, 'but c-red ml6', __('立即支付', 'zib_language'));
    } elseif ($status == 1) {
        ...
        if ($delivery_type === 'express') {
            $btns[] = zib_shop_get_order_express_link($order, 'but ml6', __('查看物流', 'zib_language'));
        }
        $btns[] = zib_shop_get_order_comment_link($order, 'but ml6 c-green', __('评价', 'zib_language'));
        $btns[] = zib_shop_get_order_receive_confirm_link($order, 'but ml6 c-blue', __('确认收货', 'zib_language'));
    } elseif ($status == -2) {
        $btns[] = zib_shop_get_order_after_sale_record_link($order, 'but ml6 c-red', __('售后详情', 'zib_language'));
    }
    ...
    // 如果按钮数量不足，添加"再次购买"按钮
    if ($btn_count < 3) {
        $product_link = zib_shop_get_product_link($order['post_id'], 'but ml6 c-blue-2', __('再次购买', 'zib_language'));
        if ($product_link) {
    ...
    if ($btn_count < 3 && $status != 0) {
        $cart_link = zib_shop_get_order_add_cart_link($order, 'but ml6', __('加入购物车', 'zib_language'));
        if ($cart_link) {
...
function zib_shop_user_order_details_info($order)
{
    ...
    $order_data           = zibpay::get_meta($order['id'], 'order_data');
    $status               = $order['status'];
    $_pay_price_type_name = $status == 1 || $status == -2 ? __('实付款', 'zib_language') : __('应付款', 'zib_language');
    $is_points            = $order_data['pay_modo'] === 'points';
    $_mark                = '<span class="pay-mark px12">' . ($is_points ? zibpay_get_points_mark() : zibpay_get_pay_mark()) . '</span>';
    ...
    $discount_lists       = '';
    if ($total_discount) {
        $discount_lists = '<div class="flex0 mr10 muted-2-color">' . __('优惠金额', 'zib_language') . '</div><div class="flex0">- <span>' . $_mark . zib_floatval_round($total_discount) . '</span>' . '<i class="fa fa-angle-right em12 ml6"></i></div>';
    ...
    if (!empty($order_data['gift_data'])) {
        ...
        $gift_lists = '<div class="flex0 mr10 muted-2-color">' . __('赠品', 'zib_language') . '</div><div class="ml20 overflow-hidden flex jab ac"><div class="text-ellipsis">' . $gift_lists . '<i class="fa fa-angle-right em12 ml6"></i></div></div>';
    ...
    $info_lists .= '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('商品总价', 'zib_language') . '</div><div class="flex0">' . $total_price_info . '</div></div>';
    $info_lists .= $discount_lists; //优惠金额+优惠明细
    $info_lists .= $gift_lists; //赠品
    $info_lists .= $shipping_type === 'express' ? '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('运费', 'zib_language') . '</div><div class="flex0">' . $shipping_fee . '</div></div>' : ''; //运费
    ...
    //退款金额
    if ($refund_price > 0) {
        $refund_lists = '<div class="flex0 mr10 muted-2-color">' . __('售后退款', 'zib_language') . '</div><div class="flex0 c-red">- <span>' . $_mark . zib_floatval_round($refund_price) . '</span>' . '<i class="fa fa-angle-right em12 ml6"></i></div>';
    ...
    $info_lists .= '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('订单编号', 'zib_language') . '</div><div class="flex0"><span>' . $order['order_num'] . '</span><a href="javascript:;" class="copy-text icon-spot" data-clipboard-tag="' . esc_attr__('订单号', 'zib_language') . '" data-clipboard-text="' . $order['order_num'] . '">' . __('复制', 'zib_language') . '</a></div></div>';
    $info_lists .= $order_remark ? '<div class="flex jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('订单备注', 'zib_language') . '</div><div class="text-right ml20">' . esc_attr($order_remark) . '</div></div>' : ''; //订单备注
    ...
    $info_lists .= '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('创建时间', 'zib_language') . '</div><div class="flex0">' . $order['create_time'] . '</div></div>';
    //已经支付
    if ($status == 1) {
        $info_lists .= (float) $order_data['prices']['pay_price'] > 0 ? '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('支付方式', 'zib_language') . '</div><div class="flex0">' . zibpay_get_order_pay_detail_lists($order) . '</div></div>' : '';
        $info_lists .= '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('支付单号', 'zib_language') . '</div><div class="ml20 overflow-hidden flex ac flex"><div class="text-ellipsis">' . $order['pay_num'] . '</div><a href="javascript:;" class="flex flex0 ac copy-text icon-spot" data-clipboard-tag="' . esc_attr__('支付单号', 'zib_language') . '" data-clipboard-text="' . $order['pay_num'] . '">' . __('复制', 'zib_language') . '</a></div></div>';
        $info_lists .= '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('支付时间', 'zib_language') . '</div><div class="flex0">' . $order['pay_time'] . '</div></div>';
        $shipping_status = zib_shop_get_order_shipping_status($order['id']);
        if ($shipping_status > 0) {
            //已发货
            ...
            $info_lists .= $delivery_time ? '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('发货时间', 'zib_language') . '</div><div class="flex0">' . $delivery_time . '</div></div>' : '';
            $info_lists .= $delivery_remark ? '<div class="flex jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('发货备注', 'zib_language') . '</div><div class="text-right ml20">' . esc_attr($delivery_remark) . '</div></div>' : '';
            $info_lists .= $receive_time ? '<div class="flex ac jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('收货时间', 'zib_language') . '</div><div class="flex0">' . $receive_time . '</div></div>' : '';
            $info_lists .= $receive_remark ? '<div class="flex jsb padding-h6"><div class="flex0 mr10 muted-2-color">' . __('收货备注', 'zib_language') . '</div><div class="text-right ml20">' . esc_attr($receive_remark) . '</div></div>' : '';
        }
    }
...
function zib_shop_user_order_details_consignee_box($order)
{
    ...
    if ($shipping_type === 'auto') {
        $delivery_type_name = zib_shop_get_delivery_type_name($delivery_type) ?: __('虚拟商品', 'zib_language');
        $emali              = $consignee['email'] ?? '';

        $express_html = '<div class="timeline-box ' . ($shipping_status > 0 ? '' : ' zib-widget mb10') . '">
            <div class="timeline-content">
                <div class="timeline-time"><span class="mr6">' . __('自动发货', 'zib_language') . '</span><span class="badg badg-sm">' . $delivery_type_name . '</span></div>
                ' . ($shipping_status == 0 && $status == 1 ? '<div class="c-yellow em09">' . __('自动发货失败，等待商家处理', 'zib_language') . '</div>' : '') . '
            </div>
        ...
        if ($shipping_type !== 'manual') {
            ...
            $address_html = '';
            if ($address) {
                $modify_address_button = zib_shop_get_order_modify_address_link($order, 'flex0 ml20 em09 muted-color', __('修改', 'zib_language') . '<i class="fa fa-angle-right em12 ml6"></i>'); //修改地址的按钮
        ...
        if ($status == 1) {
            if ($shipping_status > 0) {
                ...
                if ($delivery_type === 'no_express') {
                    //发货备注
                    $delivery_remark      = $order_data['shipping_data']['delivery_remark'] ?? '';
                    $delivery_remark_html = $delivery_remark ? '<div class="em09 muted-2-color mt3">' . $delivery_remark . '</div>' : '<div class="em09 mt3 muted-2-color">' . __('商家选择无需物流发货', 'zib_language') . '</div>';

                    $express_html = '<div class="flex muted-color mb10">
                    <div class="icon-header mr10"><i class="fa-fw fa fa-truck"></i></div>
                    <div class="">
                        <div><b class="font-bold mr6">' . __('已发货', 'zib_language') . '</b>' . ($delivery_remark ? '<span class="em09 muted-color">' . __('商家选择无需物流发货', 'zib_language') . '</span>' : '') . '</div>
                    ...
                    <div class="icon-header mr10"><i class="fa fa-truck fa-fw"></i></div>
                    <div class="">
                        <div><b class="font-bold">' . __('已发货', 'zib_language') . '</b></div>
                        ' . $context . '
                    </div>
                </div>';
                    }
                    ...
                    $delivery_remark = $order_data['shipping_data']['delivery_remark'] ?? '';
                    $delivery_remark = $delivery_remark ? '<div class="em09 muted-2-color mt3">' . sprintf(__('发货备注：%s', 'zib_language'), $delivery_remark) . '</div>' : '';
                    $delivery_time   = $order_data['shipping_data']['delivery_time'] ?? '';

                    $express_html = '<div class="flex muted-color">
                    <div class="icon-header mr10"><i class="fa-fw fa fa-truck"></i></div>
                    <div class="">
                        <div><b class="focus-color mr10">' . __('已发货', 'zib_language') . '</b><span class="em09 opacity8">' . $delivery_time . '</span></div>
                        ' . $delivery_remark . '
                    </div>
                </div>';
                }
                ...
                $express_html = '<div class="flex muted-color">
                    <div class="icon-header mr10"><i class="fa-fw fa fa-truck"></i></div>
                    <div class="focus-color font-bold">' . __('等待商家发货', 'zib_language') . '</div>
                </div>';
            }
        }
    }
...
function zib_shop_user_order_details_header_title($order)
{
    ...
    if ($status == 0) {
        ...
        if ($time_remaining == 'over') {
            ...
            $time_remaining = zib_time_to_c($time_remaining);
            $time_countdown = '<div class="mt6 em09 muted-2-color">' . sprintf(__('剩余%s自动关闭订单', 'zib_language'), '<span class="c-yellow badg badg-sm" int-second="1" data-over-text="' . esc_attr__('1秒', 'zib_language') . '" data-countdown="' . $time_remaining . '"></span>') . '</div>';
        }
    }

    $order_data = zibpay::get_meta($order['id'], 'order_data');
    if ($status == -1) {
        $status_text = '<div class="font-bold">' . __('交易关闭', 'zib_language') . '</div>';
        $status_text .= !empty($order_data['close_reason']) ? '<div class="mt6 px12 muted-2-color">' . $order_data['close_reason'] . '</div>' : '';
    } elseif ($status == 0) {
        $status_text = '<div class="c-yellow mb6 font-bold">' . zib_get_svg('time') . ' ' . __('待支付', 'zib_language') . '</div>' . $time_countdown;
    } elseif ($status == 1) {
        //发货状态
        $shipping_status = zib_shop_get_order_shipping_status($order['id']);
        if ($shipping_status == 0) {
            $status_text = '<div class="font-bold">' . __('下单成功', 'zib_language') . '</div><span class="mt6 px12 muted-2-color font-normal">' . __('等待商家发货', 'zib_language') . '</span>';
        } elseif ($shipping_status == 1) {
            $express_state_text = '';
            ...
            $status_text       = '<div class="font-bold">' . zib_get_svg('time') . ' ' . __('待收货', 'zib_language') . $express_state_text . '</div>';
            ...
            if ($receipt_over_time && $receipt_over_time != 'over') {
                ...
                $status_text .= '<div class="mt6 em09 muted-2-color">' . sprintf(__('剩余%s自动确认收货', 'zib_language'), '<span class="c-yellow badg badg-sm" int-second="1" data-over-text="' . esc_attr__('1秒', 'zib_language') . '" data-countdown="' . $receipt_over_time_time_remaining . '"></span>') . '</div>';
            }
        } else {
            $status_text = '<b class="inflex ac"><i class="fa fa-check-circle-o mr6 em12"></i>' . __('交易完成', 'zib_language') . '</b>';
            ...
            if (in_array($after_sale_status, [1, 2])) {
                ...
                if ($after_sale_status !== 4) {
                    if ($after_sale_status === 1) {
                        $status_text = '<div class="font-bold c-yellow">' . zib_get_svg('time') . ' ' . __('售后中', 'zib_language') . '</div><span class="mt6 px12 muted-2-color font-normal">' . __('等待商家处理', 'zib_language') . '</span>';
                    } elseif ($after_sale_status === 2) {
                        $progress = zib_shop_get_order_after_sale_progress($after_sale_data);
                        if ($progress === 1) {
                            if ($return_express_over_time) {
                        ...
                        $status_text = '<div class="font-bold c-yellow">' . zib_get_svg('time') . ' ' . __('售后中', 'zib_language') . '</div><div class="mt6 px12 muted-2-color font-normal">' . __('等待您发货', 'zib_language') . '</div>' . $_after_sale;
                    }
                }
            }
        }
        ...
        $after_sale_type_name = $after_sale_type_names[$after_sale_type] ?? __('已退款', 'zib_language');
...
function zib_shop_user_order_product_box($order, $class = 'mb10')
{
    ...
    if ($post) {
        ...
        $_img   = zib_get_svg('order-color', null, 'fit-cover muted-box');
        $_title = '<span class="text-ellipsis mb6 font-bold">' . ($order_data['product_title'] ?? __('商品已下架', 'zib_language')) . '</span>';
    }
