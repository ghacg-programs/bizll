...
function zib_shop_get_star_badge($score, $class = 'inflex ac')
{
    ...
    return '<div class="stars-badge-box ' . $class . '" data-toggle="tooltip" data-placement="top" title="' . sprintf(__('%s分', 'zib_language'), $score) . '">' . $star_html . '</div>';
}
...
{
    $types['product'] = __('商品', 'zib_language');
    return $types;
}
...
function zib_shop_get_express_modal(array $data)
{
    ...
    if ($express_number) {
        $express_box .= '<div class="flex ac jsb express-info-header border-bottom padding-h10">
                ...
                <div class="flex0">
                    <a href="javascript:void(0)" class="muted-2-color" data-clipboard-text="' . $express_number . '" data-clipboard-tag="' . esc_attr__('单号', 'zib_language') . '">' . __('复制', 'zib_language') . '</a>
                </div>
            </div>';
    }
    ...
    if ($_i >= $_expand_count) {
        $express_timeline .= '<div class="timeline-content pointer expand-toggle" closest-selector=".timeline-box" expand-count="' . $_expand_count . '" expand-text="' . esc_attr__('查看全部物流信息', 'zib_language') . '" collapse-text="' . esc_attr__('收起更多物流信息', 'zib_language') . '" >
                <div class="muted-2-color em09"><span class="btn-text">' . __('查看全部物流信息', 'zib_language') . '</span><i class="ml6 fa fa-angle-down"></i></div>
            </div>';
    }
    ...
    if ($address_data) {
        $consignee_box = '<div class="flex muted-color">
                ...
                <div class="">
                    <div class=""><span class="mr6 muted-2-color">' . __('送至', 'zib_language') . '</span><b class="">' . $address_data['city'] . $address_data['address'] . '</b></div>
                    <div class=""><span class="mr6">' . $address_data['name'] . '</span><span class="muted-2-color">' . $address_data['phone'] . '</span></div>
                </div>
            </div>';
    }

    $header = '<div class="border-title touch"><div class="flex jc"><b>' . __('物流信息', 'zib_language') . '</b></div></div><button class="close abs-close" data-dismiss="modal"><svg class="ic-close" aria-hidden="true"><use xlink:href="#icon-close"></use></svg></button>';
