...
function zib_shop_get_payment_data($pay_type = 'price')
{
    ...
    if ($pay_type === 'points') {
        ...
        $data['points_pay_link']    = zibpay_get_points_pay_link('but c-yellow', __('购买积分', 'zib_language'));
        return $data;
    }
    ...
    if ($user_id && in_array('balance', $pay_methods_keys)) {
        ...
        //余额充值的链接
        $balance_charge_link = zibpay_get_balance_charge_link('but c-yellow', __('充值', 'zib_language'));
    ...
    if (!$data['pay_methods']) {
        $data['error_msg'] = is_super_admin() ? '<a href="' . zib_get_admin_csf_url('支付付费/收款接口') . '" class="but c-red btn-block">' . __('请先在主题设置中配置收款方式及收款接口', 'zib_language') . '</a>' : '<span class="badg px12 c-yellow-2">' . __('暂时无法购买，请与客服联系', 'zib_language') . '</span>';
    }
...
function zib_shop_pay_success_modal_notice_footer()
{
    ...
    foreach ($orders['orders'] as $order) {
        ...
        if ($shipping_type === 'auto') {
            ...
            if ($shipping_status == 0) {
                $auto_delivery_notice_lists .= zib_shop_get_author_contact_link($order['user_id'], '', '<div class="c-yellow muted-box mt6 padding-10">' . __('您购买的商品自动发货失败，点击此处与客服联系', 'zib_language') . '</div>');
            } else {
    ...
    if ($auto_delivery_notice_lists) {
        $modal_html = '<div class="mb10"><div class="flex jc c-green"><b>' . __('请查收您购买的商品', 'zib_language') . '</b></div></div>
                        <div class="max-vh5 scroll-y mini-scrollbar">' . $auto_delivery_notice_lists . ' </div>
                        <botton type="button" class="mt10 but jb-green padding-lg btn-block auto-delivery-notice-received">' . __('我知道了', 'zib_language') . '</botton>';
    } else {
        $modal_html = '
        <div class="flex ac mb20 padding-h15">
            <div class="flex1 mr10">
                <div class="font-bold em12 c-green">' . __('等待商家发货', 'zib_language') . '</div>
                <div class="muted-color em09 mt6">' . __('商品正在准备中，将尽快为您发货', 'zib_language') . '</div>
            </div>
            <div class="em2x"><span class="badg cir c-green"><i class="fa fa-truck"></i></span></div>
        </div>
        <div class="flex ac">
            <div class="flex-auto"><a href="javascript:;" class="but hollow padding-lg btn-block" data-dismiss="modal">' . __('继续逛逛', 'zib_language') . '</a></div>
            ' . ($is_logged_in ? '<div class="flex-auto ml6"><a href="' . zib_get_user_center_url('order') . '" class="but c-blue padding-lg btn-block hollow view-order-link">' . __('查看订单', 'zib_language') . '</a></div>' : '') . '
        </div>';
    }
        ...
        <script type="text/javascript">
            $(document).ready(function(){
                ...
                $(\'.auto-delivery-notice-received\').click(function(){
                    //弹出二次确认
                    if(confirm(\'' . esc_js(__('请确认已保存收货内容！', 'zib_language') . ($is_logged_in ? __('如需再次查看，您可在用户中心->我的订单查看', 'zib_language') : __('关闭此弹窗后，内容将不再显示！', 'zib_language'))) . '\')){
