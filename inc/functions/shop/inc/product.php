 ...
 * @return {*}
 */
function zib_shop_get_product_favorite_btn($posts_id, $class = 'but', $text = null, $ok_text = null, $icon = true, $count = false)
{
    if (null === $text) {
...
function zib_shop_get_product_limit_buy_config($product_id)
{
    ...
    $config['key_names'] = [
        'all' => __('普通用户', 'zib_language'),
    ];

    if (_pz('pay_user_vip_1_s', true)) {
        $config['key_names']['vip_1'] = _pz('pay_user_vip_1_name', __('黄金会员', 'zib_language'));
    }
    if (_pz('pay_user_vip_2_s', true)) {
        $config['key_names']['vip_2'] = _pz('pay_user_vip_2_name', __('钻石会员', 'zib_language'));
    }
    if (_pz('user_auth_s', true)) {
        $config['key_names']['auth'] = __('认证用户', 'zib_language');
    }
