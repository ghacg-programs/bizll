 ...
 * @LastEditTime : 2026-06-24 00:21:10
...
function zib_shop_ajax_cart_add()
{
    ...
    if (!$product_id) {
        zib_send_json_error(['msg' => __('商品ID不能为空', 'zib_language'), 'code' => 'no_product_id']);
    }

    $product = get_post($product_id);
    if (!$product || $product->post_type !== 'shop_product') {
        zib_send_json_error(['msg' => __('商品不存在', 'zib_language'), 'code' => 'no_product']);
    }

    //判断是否可以加入购物车
    $can_add = zib_shop_can_add_cart($product, $options_key);
    if (!$can_add) {
        zib_send_json_error(['msg' => __('商品不能加入购物车', 'zib_language'), 'code' => 'no_product']);
    }
...
function zib_shop_ajax_update_cart()
{
    ...
    return zib_send_json_success(['msg' => __('已更新购物车', 'zib_language'), 'cart_items' => $items, 'count' => $count]);
}
...
function zib_shop_ajax_shop_confirm_modal()
{
    ...
    if (empty($products)) {
        zib_send_json_error(['code' => 'products_error', 'msg' => __('未选择商品', 'zib_language')]);
    }

    //获取订单确认数据
    $confirm_data = zib_shop_get_confirm_data($products);
    if (!$confirm_data) {
        zib_send_json_error(['code' => 'products_error', 'msg' => __('未选择商品，或商品不存在', 'zib_language')]);
    }
    ...
    if (!$user_id) {
        foreach ($confirm_data['product_data'] as $product_data) {
            if (!$product_data['guest_buy']) {
                zib_send_json_error(['code' => 'login_error', 'msg' => __('请先登录', 'zib_language')]);
            }
        }
    }

    if ($confirm_data['is_mix']) {
        zib_send_json_error(['code' => 'mix_error', 'msg' => __('积分商品和现金商品不能同时支付，请重新选择', 'zib_language')]);
    }

    //商品参数基本判断：库存，限购
    if (!empty($confirm_data['error_data'])) {
        foreach ($confirm_data['error_data'] as $error_data) {
            $error_data['msg']  = $error_data['error_msg'] ?? __('商品参数错误', 'zib_language');
    ...
    if (empty($_POST['no_html'])) {
        $v_confirm_html = zib_shop_v_confirm_modal_template();
    }
...
function zib_shop_ajax_submit_order()
{
    ...
    //判断是否混合支付
    if ($confirm_data['is_mix']) {
        zib_send_json_error(['code' => 'mix_error', 'msg' => __('积分商品和现金商品不能同时支付，请返回购物车重新选择', 'zib_language')]);
    }
    ...
    if ($confirm_data['shipping_has_express']) {
        if (empty($_POST['address_data']['name']) || empty($_POST['address_data']['phone']) || empty($_POST['address_data']['address'])) {
            zib_send_json_error(['code' => 'address_error', 'msg' => __('请先设置收货地址', 'zib_language')]);
        }
    }
    ...
    if ($confirm_data['shipping_has_auto'] && $confirm_data['email_fill'] !== 'off') {
        if (empty($_POST['user_email']) && $confirm_data['email_fill'] === 'required') {
            zib_send_json_error(['code' => 'email_error', 'msg' => __('请输入邮箱', 'zib_language'), 'email_fill' => $confirm_data['email_fill']]);
        }

        //邮箱格式判断
        if (!empty($_POST['user_email']) && !filter_var($_POST['user_email'], FILTER_VALIDATE_EMAIL)) {
            zib_send_json_error(['code' => 'email_error', 'msg' => __('邮箱格式错误', 'zib_language'), 'email_fill' => $confirm_data['email_fill']]);
        }
    } 
    ...
    if (round((float) $_post_price, 2) !== round((float) $payment_price, 2)) {
        zib_send_json_error(['code' => 'price_error', 'msg' => __('订单金额发生变化，请重新提交', 'zib_language')]);
    }

    if (!$is_points && (float) $payment_price > 0) {
        //支付方式选择
        if (!$payment_method) {
            zib_send_json_error(['code' => 'payment_method_error', 'msg' => __('请选择支付方式', 'zib_language')]);
        }

        //支付方式合法判断
        $pay_methods = zib_shop_get_payment_methods();
        if (!isset($pay_methods[$payment_method])) {
            zib_send_json_error(['code' => 'payment_method_error', 'msg' => __('支付方式错误', 'zib_language')]);
        }
    }
    ...
    if (!$zibpay_payment) {
        zib_send_json_error(['code' => 'add_payment_error', 'msg' => __('支付数据创建失败', 'zib_language')]);
    }
            ...
            foreach ($opt_items as $opt_key => $item_data_item) {
                ...
                if ($item_data_item['user_required']) {
                    ...
                    if ($user_required_error) {
                        zib_send_json_error(['code' => 'user_required_error', 'msg' => sprintf(__('请填写%s', 'zib_language'), implode(',', $user_required_error))]);
                    }
                }
                //必填项目结束
                ...
                if ($add_order_data) {
                    ...
                    zib_send_json_error(['code' => 'add_order_error', 'msg' => __('订单创建失败', 'zib_language')]);
                }
            }
        }
    }
...
function zib_shop_ajax_save_user_address()
{
    ...
    if (empty($address)) {
        zib_send_json_error(__('地址数据不能为空', 'zib_language'));
    }
    ...
    if (empty($address['name'])) {
        zib_send_json_error(__('请输入收货人姓名', 'zib_language'));
    }

    if (empty($address['phone'])) {
        zib_send_json_error(__('请输入手机号码', 'zib_language'));
    }

    if (empty($address['province']) || empty($address['city'])) {
        zib_send_json_error(__('地区信息不完整', 'zib_language'));
    }

    if (empty($address['address'])) {
        zib_send_json_error(__('请输入详细地址', 'zib_language'));
    }

    // 保存地址
    $result = zib_shop_save_user_address($address, $user_id, $is_author);

    zib_send_json_success(['msg' => __('保存成功', 'zib_language'), 'data' => $result]);
}
...
function zib_shop_ajax_delete_user_address()
{
    ...
    if (empty($address_id)) {
        zib_send_json_error(__('地址ID不能为空', 'zib_language'));
    }

    $result = zib_shop_delete_user_address($address_id, $user_id, $is_author);
    zib_send_json_success(['msg' => __('删除成功', 'zib_language'), 'data' => $result]);
}
...
function zib_shop_ajax_comment_modal()
{
    ...
    if (!$order_id) {
        zib_ajax_notice_modal('danger', __('订单ID不能为空', 'zib_language'));
    }

    $comment_status = zib_shop_get_order_comment_status($order_id);
    if ($comment_status == 1) {
        zib_ajax_notice_modal('danger', __('订单已评价', 'zib_language'));
    }
    if ($comment_status == 2) {
        zib_ajax_notice_modal('danger', __('当前订单存在售后，无法评价', 'zib_language'));
    }

    //评分组件
    $score_html = '';
    $score_html .= '<div class="mb10 shop-score-box flex ac">';
    $score_html .= '<div class="em09 muted-2-color score-title">' . __('商品评分', 'zib_language') . '</div><div class="score-icon em16 mr10 ml20" style=" height: 30px; "></div><div class="score-text em09 muted-2-color"></div>';
    ...
    if (zib_shop_get_order_delivery_type($order_id) === 'express') {
        $score_html .= '<div class="mb10 shop-score-box flex ac">';
        $score_html .= '<div class="em09 muted-2-color score-title">' . __('物流服务', 'zib_language') . '</div><div class="score-icon em16 mr10 ml20" style=" height: 30px; "></div><div class="score-text em09 muted-2-color"></div>';
        $score_html .= '<input class="score-input" type="hidden" name="score[shipping]" value="0">';
        $score_html .= '</div>';
    }
    $score_html .= '<div class="mb10 shop-score-box flex ac">';
    $score_html .= '<div class="em09 muted-2-color score-title">' . __('服务态度', 'zib_language') . '</div><div class="score-icon em16 mr10 ml20" style=" height: 30px;"></div><div class="score-text em09 muted-2-color"></div>';
    ...
    if ($img_num > 0) {
        ...
        $img_html .= '<div class="em09 muted-2-color mb6">' . __('图片', 'zib_language') . '</div>';
    ...
    $con .= '<div class="mb20">';
    $con .= '<textarea class="form-control" name="comment" tabindex="2" placeholder="' . esc_attr(_pz('shop_comment_placeholder', __('请输入评价内容', 'zib_language'))) . '" rows="4" autoheight="true" maxheight="188"></textarea>';
    $con .= '</div>';
    $con .= $img_html;
    ...
    $footer = '<div class="but-average modal-buts"><button type="button" data-dismiss="modal" class="but">' . __('取消', 'zib_language') . '</button>';
    $footer .= '<button class="but c-blue" zibupload="submit" zibupload-nomust="true"><i class="fa fa-check" aria-hidden="true"></i>' . __('确认提交', 'zib_language') . '</button>';
    $footer .= $hidden_html;
    $footer .= '</div>';

    $title     = '<b>' . __('评价订单', 'zib_language') . '</b>';
    $over_time = zib_shop_get_order_comment_over_time($order_id);
    if ($over_time) {
        $time_remaining = zib_time_to_c($over_time);
        $title .= '<span class="px12 muted-2-color ml6"><span int-second="1" data-over-text="' . esc_attr__('交易已关闭', 'zib_language') . '" data-countdown="' . $time_remaining . '"></span>' . __('后自动好评', 'zib_language') . '</span>';
    }
...
function zib_shop_order_comment()
{
    ...
    $order_id = isset($_REQUEST['order_id']) ? (int) $_REQUEST['order_id'] : 0;
    if (!$order_id) {
        zib_send_json_error(['code' => 'order_id_error', 'msg' => __('订单ID不能为空', 'zib_language')]);
    }

    $order = zibpay::get_order($order_id);
    if (!$order || $order['user_id'] != $user_id || !$order['post_id'] || $order['order_type'] != zib_shop_get_order_type()) {
        zib_send_json_error(['code' => 'order_error', 'msg' => __('订单数据异常，请重试', 'zib_language')]);
    }

    //判断当前商品是否开启了评论
    $product_id = $order['post_id'];
    if (!zib_shop_product_is_open_comment($product_id)) {
        zib_send_json_error(['code' => 'product_error', 'msg' => __('商品未开启评论', 'zib_language')]);
    }

    //判断订单是否已评价
    $comment_status = zib_shop_get_order_comment_status($order_id);
    if ($comment_status == -1) {
        zib_send_json_error(['code' => 'comment_error', 'msg' => __('商品评价功能已关闭', 'zib_language')]);
    }
    if ($comment_status == 1) {
        zib_send_json_error(['code' => 'comment_error', 'msg' => __('订单已评价，请勿重复评价', 'zib_language')]);
    }
    if ($comment_status == 2) {
        zib_send_json_error(['code' => 'comment_error', 'msg' => __('当前订单存在售后，无法评价', 'zib_language')]);
    }
    ...
    if (zib_new_strlen($comment) < 4) {
        //    zib_send_json_error(['code' => 'comment_error', 'msg' => __('评价内容过短', 'zib_language')]);
    }
    ...
    if ($score_product < 1 || $score_service < 1) {
        zib_send_json_error(['code' => 'score_error', 'msg' => __('请对商品进行评分', 'zib_language')]);
    }

    if (isset($score['shipping'])) {
        $score_shipping = $score['shipping'] ?? 0;
        if ($score_shipping < 1) {
            zib_send_json_error(['code' => 'score_error', 'msg' => __('请对物流服务进行评分', 'zib_language')]);
        }
    }
    ...
    $comment_handle['msg']    = __('评价成功', 'zib_language');
...
function zib_shop_ajax_shipping_express_data()
{
    ...
    if ($order['order_type'] != zib_shop_get_order_type()) {
        zib_send_json_error(__('当前订单类型不支持查询物流信息', 'zib_language'));
    }
...
function zib_shop_ajax_order_receive_confirm_modal()
{
    ...
    $order    = zibpay::get_order($order_id);
    if (!$order) {
        zib_ajax_modal_error(__('订单不存在', 'zib_language'));
    }

    //判断订单类型
    if ($order['order_type'] != zib_shop_get_order_type()) {
        zib_ajax_modal_error(__('当前订单类型不支持确认收货', 'zib_language'));
    }

    //判断权限：自己或管理员
    if ($order['user_id'] != get_current_user_id() && !current_user_can('administrator')) {
        zib_ajax_modal_error(__('权限不足', 'zib_language'));
    }

    //判断订单状态
    if ($order['status'] != 1) {
        zib_ajax_modal_error(__('订单状态不支持确认收货', 'zib_language'));
    }

    //判断是否已确认收货
    if (zib_shop_get_order_shipping_status($order_id) == 2) {
        zib_ajax_modal_error(__('订单已确认收货', 'zib_language'));
    }

    $header = zib_get_modal_colorful_header('jb-yellow', '<i class="fa fa-check-circle-o" aria-hidden="true"></i>', __('确认收到货了吗？', 'zib_language'));

    $html = '';
    $html .= '<div class="text-center c-yellow" style="padding: 20px">';
    $html .= __('为了保障您的权益，请收到商品后确认无误后，再确认收货', 'zib_language');
    $html .= '</div>';
    ...
    $html .= '<div class="mt20 but-average">
        ...
        <button class="but jb-yellow padding-lg wp-ajax-submit"><i class="fa fa-check" aria-hidden="true"></i>' . __('确认收货', 'zib_language') . '</button>
    </div>';
...
function zib_shop_ajax_order_receive_confirm()
{
    ...
    //判断订单类型
    if ($order['order_type'] != zib_shop_get_order_type()) {
        zib_send_json_error(__('当前订单类型不支持确认收货', 'zib_language'));
    }
    ...
    //判断订单状态
    if ($order['status'] != 1) {
        zib_send_json_error(__('订单状态不支持确认收货', 'zib_language'));
    }

    //判断是否已确认收货
    if (zib_shop_get_order_shipping_status($order_id) == 2) {
        zib_send_json_success(['reload' => true, 'msg' => __('已确认收货', 'zib_language')]);
    }

    //更新订单状态
    zib_shop_order_receive_confirm($order_id, 'user');

    zib_send_json_success(['reload' => true, 'goto' => zib_get_user_center_url('order', 'wait-evaluate'), 'msg' => __('确认收货成功', 'zib_language')]);
}
...
function zib_shop_ajax_order_express_modal()
{
    ...
    //权限判断
    if ($order['user_id'] != get_current_user_id() && !current_user_can('administrator')) {
        zib_ajax_modal_error(__('无权限操作', 'zib_language'));
    }
    ...
    if ($shipping_type !== 'express') {
        zib_ajax_modal_error(__('当前订单不是快递发货订单，无法查询物流信息', 'zib_language'));
    }
...
function zib_shop_ajax_order_modify_address_modal()
{
    ...
    //必须为已支付
    if ($order['status'] != '1') {
        zib_ajax_modal_error(__('当前订单不是已支付订单，无法修改收货地址', 'zib_language'));
    }

    //必须为快递发货
    if ($shipping_type !== 'express') {
        zib_ajax_modal_error(__('当前订单不是快递发货订单，无法修改收货地址', 'zib_language'));
    }

    //已发货的不显示
    $shipping_status = zib_shop_get_order_shipping_status($order['id']);
    if ($shipping_status > 0) {
        zib_ajax_modal_error(__('当前订单已发货，无法修改收货地址', 'zib_language'));
    }

    //有售后不显示
    $after_sale_status = zib_shop_get_order_after_sale_status($order['id']);
    if (in_array($after_sale_status, [1, 2])) {
        zib_ajax_modal_error(__('当前订单有售后，无法修改收货地址', 'zib_language'));
    }

    $header = '<div class="border-title touch"><div class="flex jc"><b>' . __('修改收货地址', 'zib_language') . '</b></div></div><button class="close abs-close" data-dismiss="modal"><svg class="ic-close" aria-hidden="true"><use xlink:href="#icon-close"></use></svg></button>';
    ...
    if ($address) {
        $html .= '<div class="address-box zib-widget flex at relative">
                    ...
                    <span class="badg badg-sm c-blue abs right-top">' . __('当前地址', 'zib_language') . '</span>
                </div>';
    }
    ...
    if ($modifying_data) {
        ...
        <span class="badg badg-sm c-yellow abs right-top">' . __('新地址', 'zib_language') . '</span>
    </div>';
        $html .= '<div class="muted-box muted-2-color mb20 c-yellow">' . __('你提交的地址修改申请正在处理中，请耐心等待', 'zib_language') . '</div>';
        $service_btn = zib_shop_get_author_contact_link($order['post_author'], 'but c-yellow', __('联系客服', 'zib_language'));
        //撤销修改
        $cancel_btn = zib_shop_get_order_modify_address_cancel_link($modifying_data['id'], 'but c-blue', __('撤销修改', 'zib_language'));
        ...
        <div class="address-box-body" v-else>
            <div class="flex muted-color jsb ac pointer" @click="showAddressModal">
                <div class="icon-header mr20 shrink0">
                    ...
                    <span class="">' . __('选择新的收货地址', 'zib_language') . '</span>
                </div>
    ...
    <div class="muted-box muted-2-color mb20">' . __('如需要修改收货地址，请选择新的收货地址，修改收货地址需商家确认，如有疑问请联系商家', 'zib_language') . '</div>
';

    $service_btn = zib_shop_get_author_contact_link($order['post_author'], 'but c-yellow', __('联系客服', 'zib_language'));
    $submit_btn  = '<div class="modal-buts but-average">' . $service_btn . '<button type="submit" class="but c-blue wp-ajax-submit">' . __('确认提交', 'zib_language') . '</button></div>';
    ...
    $config = [
        ...
        'config'             => [
            'lang'           => get_locale(), //当前语言
            'manual_address' => (_pz('shop_manual_address') || get_locale() !== 'zh_CN') ? true : false, //是否手动填写地址
        ],
    ];
...
function zib_shop_ajax_order_modify_address()
{
    ...
    if (empty($new_address['id'])) {
        zib_send_json_error(['msg' => __('请选择新的收货地址', 'zib_language'), 'new_address' => $new_address]);
    }
    ...
    //判断订单状态
    if ($order['status'] != '1') {
        zib_send_json_error(__('当前订单不是已支付订单，无法修改收货地址', 'zib_language'));
    }

    //判断订单类型
    if ($order['order_type'] != zib_shop_get_order_type()) {
        zib_send_json_error(__('当前订单类型不支持修改收货地址', 'zib_language'));
    }
    ...
    if ($shipping_type !== 'express') {
        zib_send_json_error(__('当前订单不是快递发货订单，无法修改收货地址', 'zib_language'));
    }

    //已发货的不显示
    $shipping_status = zib_shop_get_order_shipping_status($order['id']);
    if ($shipping_status > 0) {
        zib_send_json_error(__('当前订单已发货，无法修改收货地址', 'zib_language'));
    }

    //判断订单是否有售后
    $after_sale_status = zib_shop_get_order_after_sale_status($order['id']);
    if (in_array($after_sale_status, [1, 2])) {
        zib_send_json_error(__('当前订单有售后，无法修改收货地址', 'zib_language'));
    }
    ...
    if ($old_address['name'] == $new_address['name'] && $old_address['phone'] == $new_address['phone'] && $old_address['county'] == $new_address['county'] && $old_address['city'] == $new_address['city'] && $old_address['address'] == $new_address['address']) {
        zib_send_json_error(__('新旧地址相同，无需修改', 'zib_language'));
    }
    ...
    //创建消息通知
    $title   = __('用户申请修改订单收货地址，请及时处理', 'zib_language');
    $msg_con = sprintf(__('用户%1$s，正在申请修改订单收货地址，请及时处理', 'zib_language'), zib_get_user_name_link($order['user_id'])) . '<br>';
    $msg_con .= sprintf(__('订单号：%s', 'zib_language'), $order['order_num']) . '<br>';
    $msg_con .= sprintf(__('商品：%s', 'zib_language'), '<a href="' . get_the_permalink($product_id) . '">' . $post_title . (!$options_active_name ? '' : '[' . $options_active_name . ']') . '</a>') . '<br>';

    $msg_con .= sprintf(__('新地址：%s', 'zib_language'), $new_address['name'] . ' ' . $new_address['phone'] . ' ' . $new_address['county'] . $new_address['city'] . $new_address['address']) . '<br>';
    $msg_con .= sprintf(__('提交时间：%s', 'zib_language'), current_time('Y-m-d H:i:s')) . '<br><br>';
    $msg_con .= __('您可以点击下方按钮快速处理此申请', 'zib_language') . '<br>';
    $msg_con .= zib_shop_get_modify_address_apply_link(null, 'but jb-blue padding-lg', __('立即处理', 'zib_language'));
    $msg_args = array(
    ...
    if (!$add_msg) {
        zib_send_json_error(__('提交失败，请稍候再试', 'zib_language'));
    }

    zib_send_json_success(['hide_modal' => true, 'msg' => __('地址修改信息已提交，等待商家处理', 'zib_language')]);
}
...
function zib_shop_ajax_modify_address_apply_modal()
{
    ...
    if (!$msg) {
        zib_ajax_modal_error(__('没有待处理的地址修改申请', 'zib_language'));
    }
    ...
    foreach ($msg as $item) {
        ...
        if ($old_address) {
            ...
            <div class="flex at jsb mb10">
                <div class="mr20 muted-2-color">' . __('旧地址', 'zib_language') . '</div>
        ...
        $_item_html .= '
        <div class="flex at jsb mb10">
            <div class="mr20 muted-2-color">' . __('新地址', 'zib_language') . '</div>
            <div class="text-right">
        ...
        $_item_html .= '
        <div class="flex at jsb mb10">
            <div class="mr20 muted-2-color">' . __('处理方式', 'zib_language') . '</div>
            <div class="text-right">
                <div><label class="badg p2-10 pointer mr6"><input type="radio" checked="checked" name="data[' . $item['id'] . '][status]" value="1"> ' . __('批准', 'zib_language') . '</label><label class="badg p2-10 pointer"><input type="radio" name="data[' . $item['id'] . '][status]" value="2"> ' . __('驳回', 'zib_language') . '</label></div>
            </div>
        </div>

        <input type="text" class="form-control" name="data[' . $item['id'] . '][msg]" tabindex="1" value="" placeholder="' . esc_attr__('点击输入处理备注或驳回原因', 'zib_language') . '">
        ';
    ...
    $html .= '<div class="modal-buts but-average"><button type="submit" class="but c-blue wp-ajax-submit" data-confirm="' . esc_attr__('确认处理所有地址修改申请？', 'zib_language') . '">' . __('确认处理', 'zib_language') . '</button></div>';
    $header = '<div class="border-title touch"><div class="flex jc"><b>' . __('处理地址修改申请', 'zib_language') . '</b></div></div><button class="close abs-close" data-dismiss="modal"><svg class="ic-close" aria-hidden="true"><use xlink:href="#icon-close"></use></svg></button>';
...
function zib_shop_ajax_modify_address_apply()
{
    ...
    if (empty($data)) {
        zib_send_json_error(__('处理数据不能为空', 'zib_language'));
    }
    ...
    foreach ($data as $key => $item) {
        ...
        if (!$msg_data) {
            $error[] = sprintf(__('编号为%1$s的地址修改申请不存在或用户已撤销此申请', 'zib_language'), $msg_id);
            continue;
        }

        if ($msg_data['post_author'] != $user_id && !$is_super_admin) {
            $error[] = sprintf(__('编号为%1$s的地址修改申请无权限处理', 'zib_language'), $msg_id);
            continue;
        }
        ...
        if (!$order) {
            $error[] = sprintf(__('编号为%1$s的订单不存在', 'zib_language'), $msg_id);
            continue;
        }

        //处理订单收货地址
        $new_title = __('商家已驳回您的订单地址修改申请', 'zib_language');
        if ($status == 1) {
            //同意
            $new_title                                    = __('订单地址已修改完成', 'zib_language');
        ...
        if (!$update_msg) {
            $error[] = __('更新当前消息状态失败', 'zib_language');
            continue;
        }
        ...
        //发送新消息
        $new_msg_con = sprintf(__('商家已%1$s您的订单地址修改申请', 'zib_language'), ($status == 1 ? __('同意', 'zib_language') : __('驳回', 'zib_language'))) . '<br>';
        $new_msg_con .= sprintf(__('订单号：%s', 'zib_language'), $order['order_num']) . '<br>';
        $new_msg_con .= sprintf(__('商品：%s', 'zib_language'), '<a href="' . get_the_permalink($product_id) . '">' . $post_title . (!$options_active_name ? '' : '[' . $options_active_name . ']') . '</a>') . '<br>';
        $new_msg_con .= sprintf(__('新地址：%s', 'zib_language'), $new_address['name'] . ' ' . $new_address['phone'] . ' ' . $new_address['county'] . $new_address['city'] . $new_address['address']) . '<br>';
        $new_msg_con .= $msg ? sprintf(__('处理备注：%s', 'zib_language'), $msg) . '<br>' : '';
        $new_msg_con .= sprintf(__('处理时间：%s', 'zib_language'), current_time('Y-m-d H:i:s')) . '<br><br>';
        $new_msg_args = array(
    ...
    zib_send_json_success(['msg' => __('处理成功', 'zib_language'), 'hide_modal' => true]);
}
...
function zib_shop_ajax_modify_address_cancel()
{
    ...
    if (!$msg) {
        zib_send_json_error(__('撤销失败，请稍候再试', 'zib_language'));
    }

    //判断状态，如果状态为1，则说明已经处理了
    if ($msg['status'] == 1) {
        zib_send_json_error(['msg' => __('当前申请商家已处理，如有疑问请联系商家', 'zib_language'), 'hide_modal' => true]);
    }
    ...
    zib_send_json_success(['msg' => __('撤销成功', 'zib_language'), 'hide_modal' => true]);
}
...
function zib_shop_ajax_order_delivery_content_modal()
{
    ...
    $content .= '<div class="flex ac jsb mb10"><div class="mr20 muted-2-color">' . __('发货时间', 'zib_language') . '</div><div class="muted-color">' . $delivery_time . '</div></div>';
    $content .= '<div class="flex ac jsb mb20"><div class="mr20 muted-2-color">' . __('商品类型', 'zib_language') . '</div><div class="muted-color"><span class="badg badg-sm ml6 c-green">' . ($delivery_type_name ?: $delivery_type) . '</span></div></div>';

    $header = '<div class="border-title touch"><div class="flex jc"><b>' . __('发货信息', 'zib_language') . '</b></div></div><button class="close abs-close" data-dismiss="modal"><svg class="ic-close" aria-hidden="true"><use xlink:href="#icon-close"></use></svg></button>';
    $footer = '<div class="modal-buts but-average"><a type="button" data-dismiss="modal" class="but" href="javascript:;">' . __('确认', 'zib_language') . '</a></div>';
...
function zib_shop_ajax_order_discount_modal()
{
    ...
    $header = '<div class="border-title touch"><div class="flex jc"><b>' . __('订单优惠明细', 'zib_language') . '</b></div></div><button class="close abs-close" data-dismiss="modal"><svg class="ic-close" aria-hidden="true"><use xlink:href="#icon-close"></use></svg></button>';
...
function zib_shop_ajax_order_gift_modal()
{
    ...
    foreach ($discount_hit as $discount) {
        ...
        foreach ($gift_data as $gift) {
            switch ($gift['gift_type']) {
                case 'vip_1':
                    $time_text = $gift['vip_time'] === 'Permanent' ? __('永久', 'zib_language') : sprintf(__('%s天', 'zib_language'), $gift['vip_time']);
                ...
                case 'auth':
                    ...
                    $gift_cards_html .= '<div class="gift-card-item flex mt6"><div class="gift-name">' . __('认证资格', 'zib_language') . '</div><div class="muted-3-color"> ' . $auth_desc . '</div></div>';
                    break;
                case 'level_integral':
                    $gift_cards_html .= '<div class="gift-card-item flex mt6"><div class="gift-name">' . __('经验值', 'zib_language') . '</div><div class="muted-2-color"> ' . $gift['level_integral'] . '</div></div>';
                    break;
                case 'points':
                    $gift_cards_html .= '<div class="gift-card-item flex mt6"><div class="gift-name">' . __('积分', 'zib_language') . '</div><div class="muted-2-color"> ' . $gift['points'] . '</div></div>';
                    break;
                case 'product':
                    $gift_cards_html .= '<div class="gift-card-item flex mt6"><div class="gift-name">' . __('商品', 'zib_language') . '</div><div class="muted-2-color"> ' . $gift['product_id'] . '</div></div>';
                    break;
                case 'other':
    ...
    $header = '<div class="border-title touch"><div class="flex jc"><b>' . __('订单赠品明细', 'zib_language') . '</b></div></div><button class="close abs-close" data-dismiss="modal"><svg class="ic-close" aria-hidden="true"><use xlink:href="#icon-close"></use></svg></button>';
...
function zib_shop_ajax_order_after_sale_modal()
{
    ...
    if ($shipping_status == 0) {
        $shipping_status_name = '<span class="badg badg-sm c-yellow">' . __('待发货', 'zib_language') . '</span>';
        $product_time         = '<div class="flex ac jsb mt10"><div class="flex0 mr10 muted-2-color">' . __('付款时间', 'zib_language') . '</div><div class="flex0"><span>' . $order['pay_time'] . '</span></span></div></div>';
    }

    if ($shipping_status == 1) {
        $shipping_status_name = '<span class="badg badg-sm c-yellow">' . __('待收货', 'zib_language') . '</span>';
        $product_time         = '<div class="flex ac jsb mt10"><div class="flex0 mr10 muted-2-color">' . __('发货时间', 'zib_language') . '</div><div class="flex0"><span>' . $order_data['shipping_data']['delivery_time'] . '</span></span></div></div>';
    }

    if ($shipping_status == 2) {
        $shipping_status_name = '<span class="badg badg-sm c-green">' . __('已完成', 'zib_language') . '</span>';
        $product_time         = '<div class="flex ac jsb mt10"><div class="flex0 mr10 muted-2-color">' . __('收货时间', 'zib_language') . '</div><div class="flex0"><span>' . $order_data['shipping_data']['receive_time'] . '</span></span></div></div>';
    }

    $product_box = '<div class="muted-box mb10 padding-10">' . zib_shop_user_order_product_box($order, '') . '
    <div class="flex ac jsb mt10"><div class="flex0 mr10 muted-2-color">' . __('付款明细', 'zib_language') . '</div><div class="flex0"><span>' . $total_price . '</span></span></div></div>
    <div class="flex ac jsb mt10"><div class="flex0 mr10 muted-2-color">' . __('订单状态', 'zib_language') . '</div><div class="flex0"><span>' . $shipping_status_name . '</span></span></div></div>
    ' . $product_time . '
    </div>';
    ...
    if (!$after_sale_status || $after_sale_status > 2) {
        ...
        if ($after_sale_record) {
            ...
            $_after_sale_record_link_text = '<div class="flex0 mr10 muted-2-color">' . __('售后记录', 'zib_language') . '</div><div class="flex0"><span>' . sprintf(__('已有%s条记录', 'zib_language'), $after_sale_record_count) . '<i class="fa fa-angle-right em12 ml6"></i></span></div>';
        ...
        foreach ($can_apply as $apply) {
            if (in_array($apply, ['refund', 'refund_return'])) {
                ...
                $type_con .= '<div class="form-but-radio mb10" data-controller="type" data-condition="==" data-value="' . $apply . '" style="display: none;"><div class="mb6 muted-2-color em09">' . __('请选择选择原因', 'zib_language') . '</div><div class="flex1">' . $reason_radio . '</div></div>';
            }
        ...
        if ($shipping_status > 0) {
            ...
            $type_con .= '<div class="mb10 relative" data-controller="type" data-condition="any" data-value="refund,insured_price" style="display: none;"><div class="flex ab"><div class="muted-color mb6 flex0">' . __('请输入退款金额', 'zib_language') . '</div><input limit-min="0" warning-min="' . esc_attr(sprintf(__('最低可设置：%s', 'zib_language'), '1$')) . '" limit-max="' . $limit_max . '" warning-max="' . esc_attr(sprintf(__('最高可设置：%s', 'zib_language'), '1$')) . '" type="number" name="price" value="" style="padding: 0;" class="line-form-input em16 key-color text-right"><i class="line-form-line"></i></div></div>';
        } else {
        ...
        $collection_text_img = '';
        if ($weixin) {
            $collection_text_img .= '<img class="img-icon ml6" src="' . $weixin . '" alt="' . esc_attr__('微信收款', 'zib_language') . '">';
        }
        if ($alipay) {
            $collection_text_img .= '<img class="img-icon ml6" src="' . $alipay . '" alt="' . esc_attr__('支付宝收款', 'zib_language') . '">';
        }

        if (!$collection_text_img) {
            $collection_text_img = '<span class="c-red">' . __('请先配置收款信息', 'zib_language') . '</span>';
        }

        $collection_text = '<div class="flex0 muted-color">' . __('收款信息', 'zib_language') . '</div><div class="flex ac"><span>' . $collection_text_img . '<i class="fa fa-angle-right em12 ml10"></i></span></div>';
        ...
        //申请备注
        $type_con .= '<div class="mb10"><div class="mb6 muted-2-color em09">' . __('售后留言', 'zib_language') . '</div><textarea class="form-control" name="remark" tabindex="2" placeholder="' . esc_attr__('请输入售后理由、说明等需要告知商家的信息', 'zib_language') . '" rows="2"></textarea><div class="px12 c-yellow mt3" data-controller="type" data-condition="==" data-value="replacement" style="display: none;">' . __('请详细描述您需要更换的商品选项及参数', 'zib_language') . '</div></div>';
        ...
        //联系客服按钮加替提交按钮
        $service_btn = zib_shop_get_author_contact_link($order['post_author'], 'but c-yellow', __('联系客服', 'zib_language'));
        $submit_btn  = '<div class="modal-buts but-average">' . $service_btn . '<button type="submit" class="but c-blue wp-ajax-submit">' . __('提交申请', 'zib_language') . '</button></div>';
        //添加隐藏参数
        ...
        if (!$type_radio) {
            zib_ajax_modal_error(__('当前订单不支持申请售后', 'zib_language'));
        }

        $type_radio = '<div class="form-but-radio mb10"><div class="mb6 muted-2-color em09">' . __('请选择售后类型', 'zib_language') . '</div><div class="flex1">' . $type_radio . '</div></div>';
        ...
        $header = '<div class="border-title touch"><div class="flex jc"><b>' . __('申请售后', 'zib_language') . '</b></div></div><button class="close abs-close" data-dismiss="modal"><svg class="ic-close" aria-hidden="true"><use xlink:href="#icon-close"></use></svg></button>';
        $html   = $header . $content;
        echo $html;
        exit;
    }
    ...
    if ($after_sale_status === 1) {
        ...
        $content .= '<div class="muted-2-color mb20">' . sprintf(__('您的%s申请已提交，正在等待商家处理', 'zib_language'), $type_name) . '</div>';

        // 申请详情
        $content .= '<div class="muted-box padding-10 text-left mb20">';
        $content .= '<div class="flex ac jsb"><span class="muted-2-color">' . __('售后类型：', 'zib_language') . '</span><span class="c-blue">' . $type_name . '</span></div>';
        if ($price) {
            $content .= '<div class="flex ac jsb mt10"><span class="muted-2-color flex0 mr10">' . __('退款金额：', 'zib_language') . '</span><span class="c-red">' . $price . '</span></div>';
        }
        if ($reason) {
            $content .= '<div class="flex ac jsb mt10"><span class="muted-2-color flex0 mr10">' . __('申请原因：', 'zib_language') . '</span><span>' . $reason . '</span></div>';
        }
        if ($remark) {
            $content .= '<div class="flex jsb mt10"><span class="muted-2-color flex0 mr10">' . __('申请备注：', 'zib_language') . '</span><span>' . $remark . '</span></div>';
        }
        $content .= '<div class="flex ac jsb mt10"><span class="muted-2-color flex0 mr10">' . __('申请时间：', 'zib_language') . '</span><span>' . $user_time . '</span></div>';
        $content .= '</div>';

        // 底部按钮
        $service_btn = zib_shop_get_author_contact_link($order['post_author'], 'but c-yellow', __('联系商家', 'zib_language'));
        $cancel_btn  = zib_shop_get_order_after_sale_cancel_link($order, 'but c-blue', __('取消申请', 'zib_language')) ?: '<button type="button" class="but c-blue" data-dismiss="modal">' . __('关闭', 'zib_language') . '</button>';
        $content .= '<div class="mt20 modal-buts but-average">' . $service_btn . $cancel_btn . '</div>';
        $content .= '</div>';

        $header = '<div class="border-title touch"><div class="flex jc"><b>' . __('售后申请详情', 'zib_language') . '</b></div></div><button class="close abs-close" data-dismiss="modal"><svg class="ic-close" aria-hidden="true"><use xlink:href="#icon-close"></use></svg></button>';

        $header = zib_get_modal_colorful_header('jb-blue', '<i class="fa fa-check-circle-o"></i>', __('售后申请已提交', 'zib_language'));
    ...
    if ($after_sale_status === 2) {
        ...
        $info_box .= '<div class="flex ac jsb"><span class="muted-2-color">' . __('售后类型：', 'zib_language') . '</span><span class="c-blue">' . $type_name . '</span></div>';

        // 退款金额（如果有）
        if ($price) {
            $info_box .= '<div class="flex ac jsb mt10"><span class="muted-2-color flex0 mr10">' . __('退款金额：', 'zib_language') . '</span><span class="c-red">' . $price . '</span></div>';
        }

        // 申请原因和备注
        if ($reason) {
            $info_box .= '<div class="flex ac jsb mt10"><span class="muted-2-color flex0 mr10">' . __('申请原因：', 'zib_language') . '</span><span>' . $reason . '</span></div>';
        }

        $info_box .= '<div class="flex ac jsb mt10"><span class="muted-2-color flex0 mr10">' . __('申请时间：', 'zib_language') . '</span><span>' . $user_time . '</span></div>';

        if ($remark) {
            $info_box .= '<div class="flex jsb mt10"><span class="muted-2-color flex0 mr10">' . __('申请备注：', 'zib_language') . '</span><span>' . $remark . '</span></div>';
        }

        if (!empty($after_sale_data['author_remark'])) {
            $info_box .= '<div class="flex jsb mt10"><span class="muted-2-color flex0 mr10">' . __('商家备注：', 'zib_language') . '</span><span>' . $after_sale_data['author_remark'] . '</span></div>';
        }

        $info_box .= '<div class="flex ac jsb mt10"><span class="muted-2-color flex0 mr10">' . __('商家同意时间：', 'zib_language') . '</span><span>' . ($after_sale_data['author_handle_time'] ?? '') . '</span></div>';
        ...
        if ($progress == 1 && !empty($after_sale_data['return_address'])) {
            ...
            $info_box .= '<div class="flex jsb mt10"><span class="muted-2-color flex0 mr10">' . __('退货地址：', 'zib_language') . '</span>
                            <div class="flex1 text-right">
                                <div class="mb6">' . $address['name'] . ' ' . $address['phone'] . '<a href="javascript:;" class="c-blue icon-spot" data-clipboard-tag="' . esc_attr__('地址信息', 'zib_language') . '" data-clipboard-text="' . $clipboard_text . '">' . __('复制', 'zib_language') . '</a></div>
        ...
        if ($progress == 1) {
            ...
            $express_company_option = '<option value="">' . __('请选择', 'zib_language') . '</option>';
            foreach ($express_companies as $express_company) {
                $express_company_option .= '<option value="' . $express_company . '">' . $express_company . '</option>';
            }

            $input_box = '
            <div class="mb10 muted-2-color em09"><i class="fa fa-truck mr6"></i>' . __('请填写退货快递信息', 'zib_language') . '</div>
            <div class="muted-box padding-10 mb10" style="padding: 0 10px;">
                <div class="relative padding-h10">
                    <div class="flex ac">
                        <div class="muted-color flex0 mr10">' . __('快递单号', 'zib_language') . '</div><input type="text" name="express_number" value="" style="padding: 0;" class="line-form-input key-color text-right">
                        <i class="line-form-line"></i>
                    </div>
                </div>
                <div class="relative padding-h10">
                    <div class="flex ac">
                        <div class="muted-color flex0 mr10">' . __('快递公司', 'zib_language') . '</div>
                ...
                <div class="relative padding-h10">
                    <div class="flex ac">
                        <div class="muted-color flex0 mr10">' . __('发货备注', 'zib_language') . '</div><input type="text" name="return_remark" value="" style="padding: 0;" class="line-form-input key-color text-right">
                        <i class="line-form-line"></i>
                    </div>
                </div>
            </div>';
        ...
        if ($progress == 2) {
            ...
            if (isset($express_data['state'])) {
                ...
                $context              = '<div class="text-ellipsis em09 muted-2-color mt3">' . $after_sale_data['user_return_time'] . __('已发货', 'zib_language') . ($return_remark ? sprintf(__('，备注：%s', 'zib_language'), $return_remark) : '') . '</div>';
                $_express             = '<div class="flex muted-color mb10">
                                            <div class="icon-header mr10"><i class="fa fa-truck fa-fw"></i></div>
                                            <div class="">
        ...
        // 根据进度显示不同的按钮
        if ($progress == 1) {
            $express_btn = '<a href="javascript:;" class="but c-blue wp-ajax-submit" data-confirm="' . esc_attr__('请确认发货', 'zib_language') . '">' . __('确认发货', 'zib_language') . '</a>'; //退货快递信息
            //取消售后
            $cancel_btn = zib_shop_get_order_after_sale_cancel_link($order, 'but c-red', __('撤销售后', 'zib_language'));
            $footer .= '<div class="mt20 modal-buts but-average">' . $service_btn . $cancel_btn . $express_btn . '</div>';
            $header_text = '<div class="font-bold c-yellow"><i class="fa fa-truck mr6"></i>' . __('等待您发货', 'zib_language') . '</div>';
            if ($return_express_over_time) {
                $time_remaining = zib_time_to_c($return_express_over_time);

                $time_countdown = '<div class="mt6 px12 muted-2-color">' . sprintf(__('剩余%s自动关闭此售后', 'zib_language'), '<span class="badg badg-sm" int-second="1" data-over-text="' . esc_attr__('1秒', 'zib_language') . '" data-countdown="' . $time_remaining . '"></span>') . '</div>';
            ...
            $record_btn = zib_shop_get_order_after_sale_return_express_link($order, 'but c-blue', __('查看物流信息', 'zib_language'));
            $footer .= '<div class="mt20 modal-buts but-average">' . $service_btn . $record_btn . '</div>';

            $header = zib_get_modal_colorful_header('jb-blue', '<i class="fa fa-refresh fa-spin"></i>', __('等待商家处理', 'zib_language'));
        } elseif ($progress == 3) {
            $confirm_btn = '<a href="javascript:;" class="but c-blue wp-ajax-submit" form-data=\'' . json_encode(['action' => 'order_after_sale_confirm', 'order_id' => $order['id']]) . '\' data-confirm="' . esc_attr__('确认已收到货物？', 'zib_language') . '">' . __('确认收货', 'zib_language') . '</a>';
            $record_btn  = zib_shop_get_order_after_sale_record_link($order, 'but c-blue', __('售后记录', 'zib_language'));
            $footer .= '<div class="mt20 modal-buts but-average">' . $service_btn . $confirm_btn . $record_btn . '</div>';

            $header = zib_get_modal_colorful_header('jb-green', '<i class="fa fa-check-circle"></i>', __('商家已处理完成', 'zib_language'));
        }
...
function zib_shop_ajax_order_after_sale_return_express()
{
    ...
    $after_sale_status = zib_shop_get_order_after_sale_status($order['id']);
    if ($after_sale_status !== 2) {
        zib_send_json_error(__('订单状态变化，请刷新页面后重试', 'zib_language'));
    }
    ...
    if (!$express_number) {
        zib_send_json_error(__('请输入快递单号', 'zib_language'));
    }

    if (!$express_company_name) {
        zib_send_json_error(__('请选择快递公司', 'zib_language'));
    }
    ...
    zib_send_json_success(['order_id' => $order_id, 'hide_modal' => true, 'msg' => __('退货快递信息已提交，等待商家处理', 'zib_language')]);
}
...
function zib_shop_ajax_order_after_sale_apply()
{
    ...
    //判断订单类型
    if ($order['order_type'] != zib_shop_get_order_type()) {
        zib_send_json_error(__('当前订单类型不支持申请售后', 'zib_language'));
    }
    ...
    //判断订单状态
    if ($order['status'] != 1) {
        zib_send_json_error(__('订单状态不支持申请售后', 'zib_language'));
    }

    //判断售后状态
    $after_sale_status = zib_shop_get_order_after_sale_status($order['id']);
    if ($after_sale_status && in_array($after_sale_status, [1, 2])) {
        zib_send_json_error(__('当前订单已申请售后，请勿重复申请', 'zib_language'));
    }
    ...
    if (!$type) {
        zib_send_json_error(__('请选择售后类型', 'zib_language'));
    }
    ...
    switch ($type) {
        case 'refund': //退款
            if ($limit_max && !$price) {
                zib_send_json_error(sprintf(__('请输入退款金额%s', 'zib_language'), $limit_max));
            }
            if ($price > $limit_max) {
                zib_send_json_error(sprintf(__('退款金额不能大于订单总金额：%s', 'zib_language'), $limit_max));
            }
        ...
        case 'replacement': //换货
            if (!$remark) {
                zib_send_json_error(__('请输入售后留言，详细描述您需要更换的商品选项及参数', 'zib_language'));
            }
            break;
        case 'warranty': //保修
            break;
        case 'insured_price': //保价
            if ($limit_max && !$price) {
                zib_send_json_error(__('请输入保价退款金额', 'zib_language'));
            }
            if ($price > $limit_max) {
                zib_send_json_error(sprintf(__('保价金额不能大于订单总金额：%s', 'zib_language'), $limit_max));
            }
            break;
        default:
            zib_send_json_error(__('请选择售后类型', 'zib_language'));
    ...
    zib_send_json_success(['order_id' => $order['id'], 'hide_modal' => true, 'msg' => __('售后申请已提交，等待商家处理', 'zib_language')]);
}
...
function zib_shop_ajax_order_after_sale_cancel()
{
    ...
    if (in_array($after_sale_status, [3, 4, 5])) {
        zib_send_json_error(__('当前申请售后已处理完成，无法取消', 'zib_language'));
    }
    ...
    zib_send_json_success(['order_id' => $order['id'], 'hide_modal' => true, 'msg' => __('售后申请已取消', 'zib_language')]);
}
...
function zib_shop_ajax_order_after_sale_record_modal()
{
    ...
    if (!$record_html) {
        zib_ajax_modal_error(__('当前订单没有售后记录', 'zib_language'));
    }

    $header  = '<div class="border-title touch"><div class="flex jc"><b>' . __('售后记录', 'zib_language') . '</b></div></div><button class="close abs-close" data-dismiss="modal"><svg class="ic-close" aria-hidden="true"><use xlink:href="#icon-close"></use></svg></button>';
...
function zib_shop_ajax_favorite_product()
{
    ...
    if (in_array($id, $is_follow)) {
        ...
        $type      = 'cancel';
        $active    = false;
        $text      = __('收藏', 'zib_language');
    } else {
        //关注
        $type   = 'add';
        $text   = __('已收藏', 'zib_language');
        $active = true;
...
function zib_shop_ajax_author_contact_modal()
{
    ...
    if ($msg_s) {
        $name     = !empty($options['msg_name']) ? $options['msg_name'] : __('站内客服', 'zib_language');
    ...
    $header = zib_get_modal_colorful_header('jb-blue', $_svg, __('联系客服', 'zib_language'));
