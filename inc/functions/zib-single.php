...
function zib_get_breadcrumbs()
{
    ...
    if ($categorys) {
		...
		<li><a href="' . get_bloginfo('url') . '"><i class="fa fa-map-marker"></i> ' . (!_pz('breadcrumbs_home_text', true) ? get_bloginfo('name') : __('首页', 'zib_language')) . '</a></li><li>
		' . get_category_parents($category->term_id, true, ' </li><li> ') . (!_pz('breadcrumbs_single_text', true) ? get_the_title() : __('正文', 'zib_language')) . '</li></ul>';
    ...
    if (!$_thumb && _pz('article_video_cover')) {
        ...
        if ($video) {
            ...
            if ($episode_array && is_array($episode_array)) {
                foreach ($episode_array as $episode) {
                    if (!empty($episode['url'])) {
                        $episode_index++;
                        $episode_title = $episode['title'] ? $episode['title'] : sprintf(__('第%d集', 'zib_language'), $episode_index);
            ...
            if ($episode_lists) {
                ...
                $episode_title = $episode_title ? $episode_title : __('第1集', 'zib_language');
                ...
                $title = '<span class="badg badg-sm b-theme mr6"><i class="fa fa-play-circle mr3" aria-hidden="true"></i>' . sprintf(__('共%s集', 'zib_language'), $episode_index) . '</span>' . $get_title;
            }
...
//获取文章时间的显示
function zib_get_post_time_tooltip($post = null, $source = 'modified')
{
    if (!is_object($post)) {
        $post = get_post($post);
    }
    ...
    if ($source == 'date') {
        ...
        $time_html = '<span data-toggle="tooltip" data-placement="bottom" title="' . esc_attr(date_i18n(__('Y年m月d日 H:i', 'zib_language'), strtotime($time)) . __('发布', 'zib_language')) . '">' . zib_get_time_ago($time) . __('发布', 'zib_language') . '</span>';
        return $time_html;
    }

    $modified_time = get_the_modified_time('Y-m-d H:i:s', $post);
    if (isset($post->post_status) && $post->post_status !== 'publish') {
        return '<span data-toggle="tooltip" data-placement="bottom" title="' . esc_attr(date_i18n(__('Y年m月d日 H:i', 'zib_language'), strtotime($modified_time)) . __('更新', 'zib_language')) . '">' . zib_get_time_ago($modified_time) . __('更新', 'zib_language') . '</span>';
    }

    $time = get_the_time('Y-m-d H:i:s', $post);
    if (strtotime($modified_time) > strtotime($time)) {
        //更新时间大于发布时间，显示更新时间
        $time_html = '<span data-toggle="tooltip" data-placement="bottom" title="' . esc_attr(date_i18n(__('Y年m月d日 H:i', 'zib_language'), strtotime($time)) . __('发布', 'zib_language')) . '">' . zib_get_time_ago($modified_time) . __('更新', 'zib_language') . '</span>';
    } else {
        $time_html = '<span data-toggle="tooltip" data-placement="bottom" title="' . esc_attr(date_i18n(__('Y年m月d日 H:i', 'zib_language'), strtotime($time)) . __('发布', 'zib_language')) . '">' . zib_get_time_ago($time) . __('发布', 'zib_language') . '</span>';
    }
