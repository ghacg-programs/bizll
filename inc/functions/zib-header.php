...
function zib_header()
{
    ...
    echo '<div class="flex jsb ac mb20">
            <div class="navbar-logo">' . zib_get_adaptive_theme_img(_pz('logo_src'), _pz('logo_src_dark'), '', '', false) . '</div>
...
function zib_menu_button($layout = 1)
{
    ...
    if (_pz('nav_newposts')) {
        ...
        if ($nav_newposts_btns) {
            ...
            $new_add_btns = zib_get_new_add_btns($nav_newposts_btns, $show_class, (_pz('nav_newposts_text') ?: __('发布', 'zib_language')));
            $button .= $new_add_btns;
        }
    }
    ...
    if (_pz('nav_pay_vip', true) && !zib_is_close_sign() && (_pz('pay_user_vip_1_s', true) || _pz('pay_user_vip_2_s', true))) {
        ...
        if ($user_id) {
            if (!zib_get_user_vip_level($user_id)) {
                $vip_button = '<a class="pay-vip but jb-red radius payvip-icon ml10" href="javascript:;">' . zib_get_svg('vip_1', '0 0 1024 1024', 'em12 mr3') . __('开通会员', 'zib_language') . '</a>';
                $button .= '<span class="hover-show inline-block">' . $vip_button . $hover_show . '</span>';
            }
        } else {
            $vip_button = '<a class="signin-loader but jb-red radius payvip-icon ml10" href="javascript:;">' . zib_get_svg('vip_1', '0 0 1024 1024', 'em12 mr3') . __('开通会员', 'zib_language') . '</a>';
            $button .= '<span class="hover-show inline-block">' . $vip_button . $hover_show . '</span>';
        }
    }
    ...
    if (!zib_is_close_sign(true)) {
        ...
        if (2 == $layout) {
            $sign_but = '<li><a href="javascript:;" class="signin-loader">' . __('登录', 'zib_language') . '</a></li>';
            $sign_but .= !zib_is_close_signup() ? '<li><a href="javascript:;" class="signup-loader">' . __('注册', 'zib_language') . '</a></li>' : '';
...
function zib_header_user_box()
{
    ...
    if ($user_id) {
        ...
        $href .= '<a rel="nofollow" href="' . zib_get_user_center_url() . '" ><div class="badg mb6 toggle-radius c-blue">' . zib_get_svg('user', '50 0 924 924') . '</div><div class="c-blue">' . __('用户中心', 'zib_language') . '</div></a>';
        $href .= '<a rel="nofollow" href="' . zib_get_user_center_url('order') . '" ><div class="badg mb6 toggle-radius c-purple">' . zib_get_svg('handbag') . '</div><div class="c-purple">' . __('我的订单', 'zib_language') . '</div></a>';

        $user_newposts = _pz('nav_user_newposts_btn', 'post');
        if ($user_newposts) {
            $new_add_btns = zib_get_new_add_btns([$user_newposts], 'start-new-posts', '<div class="badg mb6 toggle-radius c-green"><i class="fa fa-fw fa-pencil-square-o"></i></div><div class="c-green">' . _pz('nav_user_newposts_btn_text', __('发布文章', 'zib_language')) . '</div>');
            $href .= $new_add_btns;
        }

        $href .= '<a href="javascript:;" data-toggle="modal" data-target="#modal_signout" ><div class="badg mb6 toggle-radius c-red">' . zib_get_svg('signout') . '</div><div class="c-red">' . __('退出登录', 'zib_language') . '</div></a>';
        $href .= '</div>';

        if (is_super_admin()) {
            $href .= '<div class="flex jsa header-user-href">';
            $href .= '<a rel="nofollow" target="_blank" href="' . zib_get_admin_csf_url() . '"><div class="badg mb6 toggle-radius c-yellow">' . zib_get_svg('theme') . '</div><div class="c-yellow">' . __('主题设置', 'zib_language') . '</div></a>';
            $href .= '<a rel="nofollow" target="_blank" href="' . zib_get_customize_widgets_url() . '" ><div class="badg mb6 toggle-radius c-yellow"><i class="fa fa-pie-chart"></i></div><div class="c-yellow">' . __('模块布局', 'zib_language') . '</div></a>';
            $href .= '<a rel="nofollow" target="_blank" href="' . admin_url('admin.php?page=zibpay_page#/') . '" ><div class="badg mb6 toggle-radius c-yellow">' . zib_get_svg('handbag') . '</div><div class="c-yellow">' . __('商城中心', 'zib_language') . '</div></a>';
            $href .= '<a rel="nofollow" target="_blank" href="' . admin_url() . '" ><div class="badg mb6 toggle-radius c-yellow">' . zib_get_svg('set') . '</div><div class="c-yellow">' . __('后台管理', 'zib_language') . '</div></a>';
            $href .= '</div>';
        }
        ...
        if (_pz('checkin_s') && _pz('checkin_header_user_show')) {
            ...
            $checkin_btn = zib_get_user_checkin_btn('but block ' . $class, '<i class="fa fa-calendar-check-o"></i> ' . $text, '<i class="fa fa-calendar-check-o"></i> ' . __('今日已签到', 'zib_language'));
        }
        ...
        $href = ((_pz('pay_user_vip_1_s', true) || _pz('pay_user_vip_2_s', true)) && _pz('nav_user_pay_vip', true)) ? '<div><a class="em09 signin-loader but jb-red radius4 payvip-icon btn-block mt10" href="javascript:;">' . zib_get_svg('vip_1', '0 0 1024 1024', 'em12 mr10') . __('开通会员 尊享会员权益', 'zib_language') . '</a></div>' : '';
        ...
        if ($ocial_login) {
            $con .= '<p class="social-separator separator muted-3-color em09 mt10">' . __('快速登录', 'zib_language') . '</p>';
...
function zib_get_header_payvip_icon($user_id = 0)
{
    ...
    if ($vip_level) {
        ...
        $button = '<a class="pay-vip but radius4 payvip-icon btn-block" href="javascript:;">' . zib_get_svg('vip_1', '0 0 1024 1024', 'em12 mr6') . __('开通会员 尊享会员权益', 'zib_language') . '</a>';
        return $button;
    }
    return '';
}
...
function zib_get_menu_search()
{
    ...
    echo '<input type="text" class="form-control search-input focus-show" name="s" placeholder="' . esc_attr__('搜索内容', 'zib_language') . '">';
...
function zib_menu_items($location = 'topmenu', $echo = true)
{
    ...
    if (!$menu && is_super_admin()) {
        $menu = '<ul class="nav navbar-nav"><li><a href="' . admin_url('nav-menus.php') . '" class="loaderbt">' . __('添加导航菜单', 'zib_language') . '</a></li></ul>';
    }
    if ($echo) {
        echo $menu;
    } else {
...
function zib_navbar_header()
{
    ...
    . zib_get_adaptive_theme_img(_pz('logo_src'), _pz('logo_src_dark'), $t, '', false) . '
			</a>';
...
function zib_nav_mobile($location = 'mobilemenu')
{
    ...
    if (!$wp_nav_menu && is_super_admin()) {
        $wp_nav_menu = '<ul class="mobile-menus theme-box"><li><a href="' . admin_url('nav-menus.php') . '" class="loaderbt">' . __('添加导航菜单', 'zib_language') . '</a></li></ul>';
    }
    $menu .= $wp_nav_menu;
