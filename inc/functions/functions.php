...
function zib_new_slider($args, $echo = true)
{
    ...
    $slides                      = '';
    $seo_alt                     = _pz('hometitle') ? _pz('hometitle') : __('幻灯片', 'zib_language') . $zib_get_delimiter_blog_name;
    $_i                          = 0;
    foreach ($args['slides'] as $slide) {
        $lazy_src = ZIB_TEMPLATE_DIRECTORY_URI . '/img/thumbnail-lg.svg';
        $s_class  = isset($slide['class']) ? $slide['class'] : '';
...
function zib_get_user_badges($user_id)
{
    ...
    $html = '';
    $html .= '<a class="but c-blue tag-posts" data-toggle="tooltip" title="' . esc_attr(sprintf(__('共%s篇文章', 'zib_language'), $post_n)) . '" href="' . zib_get_user_home_url($user_id) . '">' . zib_get_svg('post') . $post_n . '</a>';
    $html .= '<a class="but c-green tag-comment" data-toggle="tooltip" title="' . esc_attr(sprintf(__('共%s条评论', 'zib_language'), $com_n)) . '" href="' . zib_get_user_home_url($user_id, array('tab' => 'comment')) . '">' . zib_get_svg('comment') . $com_n . '</a>';

    if ($followed_n) {
        $html .= '<a class="but c-yellow tag-follow" data-toggle="tooltip" title="' . esc_attr(sprintf(__('共%s个粉丝', 'zib_language'), $followed_n)) . '" href="' . zib_get_user_home_url($user_id, array('tab' => 'follow')) . '"><i class="fa fa-heart em09"></i>' . $followed_n . '</a>';
    } else {
        if ($like_n) {
            $html .= '<span class="badg c-yellow tag-like" data-toggle="tooltip" title="' . esc_attr(sprintf(__('获得%s个点赞', 'zib_language'), $like_n)) . '">' . zib_get_svg('like') . $like_n . '</span>';
        }
    }
    $html .= '<span class="badg c-red tag-view" data-toggle="tooltip" title="' . esc_attr(sprintf(__('人气值 %s', 'zib_language'), $view_n)) . '">' . zib_get_svg('hot') . $view_n . '</span>';
...
function zib_posts_prevnext()
{
    ...
    if (!empty($prev_post)):
        ...
        $prev_title = __('无更多文章', 'zib_language');
    ...
    if (!empty($next_post)):
        ...
        $next_title = __('无更多文章', 'zib_language');
        ...
        <nav class="article-nav">
            <div class="main-bg box-body radius8 main-shadow">
                <a <?php echo $prev_link; ?>>
                    <p class="muted-2-color"><i class="fa fa-angle-left em12"></i><i
                            class="fa fa-angle-left em12 mr6"></i><?php echo esc_html__('上一篇', 'zib_language'); ?></p>
            ...
            <div class="main-bg box-body radius8 main-shadow">
                <a <?php echo $next_link; ?>>
                    <p class="muted-2-color"><?php echo esc_html__('下一篇', 'zib_language'); ?><i class="fa fa-angle-right em12 ml6"></i><i
...
function zib_posts_related($related_title = null, $limit = 6, $orderby = 'views')
{
    if (null === $related_title) {
        $related_title = __('相关阅读', 'zib_language');
    }
    global $post;
    ...
    if (!$posts_lits) {
        echo '<li>' . esc_html__('暂无相关文章', 'zib_language') . '</li>';
    } else {
        echo $posts_lits;
    }
    ...
    if (!empty($tags[0])) {
        ...
        if (!empty($tags_s[0])) {
            foreach ($tags_s as $tag_id) {
                ...
                $tag = get_tag($tag_id);
                $html .= '<a href="' . $url . '"' . ($ajax_replace ? ' ajax-replace="true"' : '') . ' title="' . esc_attr__('查看此标签更多文章', 'zib_language') . '" class="' . $class . '">' . $before . $tag->name . $after . '</a>';
                if ($count && $count == $ii) {
                    break;
                }
            }
...
function zib_get_topics_tags($pid = '', $class = 'but', $before = '', $after = '', $count = 0)
{
    ...
    if (!empty($category[0])) {
        ...
        foreach ($category as $category1) {
            ...
            $cat .= '<a class="' . $class . ' ' . $cls[$ii - 1] . '" title="' . esc_attr__('查看此专题更多文章', 'zib_language') . '" href="' . get_category_link($category1->term_id) . '">' . $before . $category1->name . $after . '</a>';
            if ($count && $ii == $count) {
                break;
            }
        }
...
function zib_get_cat_tags($class = 'but', $before = '', $after = '', $count = 0)
{
    ...
    if (!empty($category[0])) {
        ...
        foreach ($category as $category1) {
            ...
            $cat .= '<a class="' . $class . ' ' . $cls[$ii - 1] . '" title="' . esc_attr__('查看更多分类文章', 'zib_language') . '" href="' . get_category_link($category1->term_id) . '">' . $before . $category1->cat_name . $after . '</a>';
            if ($count && $ii == $count) {
                break;
            }
        }
...
function zib_get_posts_meta($post = null)
{
    ...
    if (comments_open($post) && !_pz('close_comments')) {
        ...
        $meta .= '<item class="meta-comm"><a rel="nofollow" data-toggle="tooltip" title="' . esc_attr__('去评论', 'zib_language') . '" href="' . $comment_href . '">' . zib_get_svg('comment') . get_comments_number($post_id) . '</a></item>';
    }
...
function zib_get_post_status_badge($class = '', $post = null)
{
    ...
    $html      = '<span class="img-badge top badge-status' . $class . '"><img ' . $lazy_attr . 'src="' . ZIB_TEMPLATE_DIRECTORY_URI . '/img/' . $status_img_name[$post_status] . '.svg" alt="' . esc_attr(sprintf(__('状态：%s', 'zib_language'), $post_status)) . '"></span>';
    return $html;
}
    ...
    if (_pz('social') && function_exists('xh_social_loginbar')) {
        ...
        foreach ($args as $arg) {
            ...
            if ($href) {
                ...
                $buttons .= '<a rel="nofollow" title="' . esc_attr(sprintf(__('%s登录', 'zib_language'), $name)) . '" href="' . esc_url($href) . '" class="social-login-item ' . $_class . '">' . $icon . ($b_c ? sprintf(__('%s登录', 'zib_language'), $name) : '') . '</a>';
            }
        }
    }
    if ($echo && $buttons) {
        echo '<p class="social-separator separator muted-3-color em09">' . esc_html__('社交账号登录', 'zib_language') . '</p>';
...
function zib_get_social_type_name($type)
{
    ...
    return isset($type_name[$type]['name']) ? $type_name[$type]['name'] : __('第三方', 'zib_language');
}
...
function zib_get_social_type_data()
{
    ...
    $args['dingtalk'] = array(
        'name'  => __('钉钉', 'zib_language'),
        'type'  => 'dingtalk',
        'class' => 'c-blue',
        'icon'  => 'zibsvg-dingtalk',
    );
    $args['huawei'] = array(
        'name'  => __('华为', 'zib_language'),
        'type'  => 'huawei',
        'class' => 'c-blue',
        'icon'  => 'zibsvg-huawei',
    );
    $args['xiaomi'] = array(
        'name'  => __('小米', 'zib_language'),
        'type'  => 'xiaomi',
        'class' => 'c-blue',
        'icon'  => 'zibsvg-xiaomi',
    );
...
function zib_submit_links_modal($args = array())
{
    ...
    if ($args['sign'] && !get_current_user_id()) {
        ...
        $input .= '<div class="mb20 muted-3-color">' . esc_html__('请先登录', 'zib_language') . '</div>';
        $input .= '<p>';
        $input .= '<a href="javascript:;" class="signin-loader but c-blue padding-lg"><i class="fa fa-fw fa-sign-in mr10" aria-hidden="true"></i>' . esc_html__('登录', 'zib_language') . '</a>';
        $input .= !zib_is_close_signup() ? '<a href="javascript:;" class="signup-loader ml10 but c-yellow padding-lg"><i data-class="icon mr10" data-viewbox="0 0 1024 1024" data-svg="signup" aria-hidden="true"></i>' . esc_html__('注册', 'zib_language') . '</a>' : '';
        $input .= '</p>';
        ...
        $cats_options = $cats_options ? '<div class="col-sm-12 mb10">
            <div class="em09 muted-2-color mb6">' . esc_html__('网站类别', 'zib_language') . '</div>
        ...
        $input .= '<div class="row gutters-5">
                        <div class="col-sm-6 mb10">
                            <div class="em09 muted-2-color mb6">' . esc_html__('网站名称（必填）', 'zib_language') . '</div>
                            <input type="text" class="form-control" id="link_name" name="link_name" placeholder="' . esc_attr__('请输入网站名称', 'zib_language') . '">
                        </div>
                        <div class="col-sm-6 mb10">
                            <div class="em09 muted-2-color mb6">' . esc_html__('网站地址（必填）', 'zib_language') . '</div>
                            <input type="text" class="form-control" id="link_url" name="link_url" placeholder="https://...">
                        </div>

                    <div class="col-sm-12 mb10">
                        <div class="em09 muted-2-color mb6">' . esc_html__('网站简介', 'zib_language') . '</div>
                        <input type="text" class="form-control" id="link_description" name="link_description" placeholder="' . esc_attr__('一句话介绍网站', 'zib_language') . '">
                    </div>
                     ' . $cats_options . '
                    <div class="col-sm-12 mb10">
                        <div class="em09 muted-2-color mb6">' . esc_html__('LOGO图像', 'zib_language') . '</div>
                        <label class="pointer"><div class="preview preview-square"><img class="fit-cover" src="' . ZIB_TEMPLATE_DIRECTORY_URI . '/img/upload-add.svg" alt="' . esc_attr__('添加图片', 'zib_language') . '"></div>
                            <input class="hide" type="file" zibupload="image_upload" accept="image/gif,image/jpeg,image/jpg,image/png" name="image">
                        </label>
                        <div class="px12 muted-2-color mb6">' . esc_html__('请选择正方形LOGO图像，支持jpg/png/gif格式', 'zib_language') . '</div>
                    </div>
                </div>';
        ...
        $input .= '<div class="text-right edit-footer">
                        <button type="button" zibupload="submit" class="but c-blue padding-lg input-expand-upload" name="submit"><i class="fa fa-check" aria-hidden="true"></i>' . esc_html__('确认提交', 'zib_language') . '</button>
                    </div>';
...
{
    return apply_filters('new_add_btns_options', array('post' => __('文章投稿', 'zib_language')));
}
...
function zib_new_add_btns_post_filter()
{
    if (_pz('post_article_s', true) && !is_page_template('pages/newposts.php')) {
        ...
        return '<a rel="nofollow" class="btn-newadd" href="' . $href . '">' . $icon . '<text>' . _pz('post_article_btn_txte', __('发布文章', 'zib_language')) . '</text></a>';
    }
    return;
}
...
function zib_get_input_expand_but($type = 'smilie', $args_1 = null, $args_2 = null)
{
    ...
    switch ($type) {
        //链接
        case 'link':
            $but      = '<a class="but btn-input-expand input-link mr6" href="javascript:;"><i class="fa fa-fw fa-link"></i><span class="hide-sm">' . esc_html__('链接', 'zib_language') . '</span></a>';
            $dropdown = '<div class="dropdown-link">';
            $dropdown .= '<p>' . esc_html__('请输入链接：', 'zib_language') . '</p>';
            $dropdown .= '<p><textarea rows="2" tabindex="1" class="form-control input-textarea link-textarea" style="height:55px;" placeholder="https://..."></textarea></p>';
            $dropdown .= '<p><input type="text" class="form-control link-text" placeholder="链接文本"></p>';
            $dropdown .= '<div class=""> <label class="muted-box flex ac jsb padding-h10 mb10 pointer" style=" font-weight: normal; "><div class="flex ac">新窗口打开</div><input class="hide" name="target" type="checkbox" value="1"><div class="form-switch flex0"></div></label> </div>';
            $dropdown .= '<div class="text-right"><a type="submit" class="but c-blue pw-1em" href="javascript:;">' . esc_html__('确认', 'zib_language') . '</a></div>';
            $dropdown .= '</div>';
            break;

        case 'quick':
            $btn_title   = __('快捷回复', 'zib_language');
            $quick_id    = $args_2;
            ...
            if (!$sys_lists) {
                $sys_lists = '<div class="text-center muted-2-color" style="margin: 60px 0;">' . esc_html__('系统暂无快捷回复', 'zib_language') . '</div>';
            } else {
                $sys_is_has = true;
            }
            ...
            if ($current_user_id) {
                ...
                if (!$user_lists) {
                    $user_lists = '<div class="quick-reply-myitem-box"><div class="text-center muted-2-color" style="margin: 40px 0 30px;">' . esc_html__('暂无快捷回复', 'zib_language') . '</div></div><div class="padding-10">' . zib_get_user_edit_quick_often_link('but block c-blue', zib_get_svg('add') . __('添加快捷回复', 'zib_language')) . '</div>';
                } else {
                    $user_is_has = true;
                    $user_lists  = '<div class="quick-reply-myitem-box">' . $user_lists . '</div><div class="padding-10">' . zib_get_user_edit_quick_often_link('but block', zib_get_svg('quick-reply') . __('编辑我的快捷回复', 'zib_language')) . '</div>';
                }
            }
            ...
            if ($user_lists) {
                ...
                $dropdown      = '<div class="relative dropdown-quick-often">
                                <ul class="list-inline tab-nav-theme text-center">
                                    <li class="' . ($sys_is_active ? 'active' : '') . '"><a class="ml3 mr3" data-toggle="tab" href="#input_expand_quick_often_sys_' . $quick_id . '">' . esc_html__('系统', 'zib_language') . '</a></li>
                                    <li class="' . (!$sys_is_active ? 'active' : '') . '"><a class="ml3 mr3" data-toggle="tab" href="#input_expand_quick_often_user_' . $quick_id . '">' . esc_html__('我的', 'zib_language') . '</a></li>
                                </ul>
        ...
        case 'smilie':
            $but              = '<a class="but btn-input-expand input-smilie mr6" href="javascript:;"><i class="fa fa-fw fa-smile-o"></i><span class="hide-sm">' . esc_html__('表情', 'zib_language') . '</span></a>';
            ...
            break;
        case 'code':
            $but = '<a class="but btn-input-expand input-code mr6" href="javascript:;"><i class="fa fa-fw fa-code"></i><span class="hide-sm">' . esc_html__('代码', 'zib_language') . '</span></a>';

            $dropdown = '<div class="dropdown-code">';
            $dropdown .= '<p>' . esc_html__('请输入代码：', 'zib_language') . '</p>';
            $dropdown .= '<p><textarea rows="6" tabindex="1" class="form-control input-textarea" placeholder="' . esc_attr__('在此处粘贴或输入代码', 'zib_language') . '"></textarea></p>';
            $dropdown .= '<div class="text-right"><a type="submit" class="but c-blue pw-1em" href="javascript:;">' . esc_html__('确认', 'zib_language') . '</a></div>';
            $dropdown .= '</div>';
        ...
        case 'image':
            ...
            $but = '<a class="but btn-input-expand input-image mr6" href="javascript:;"><i class="fa fa-fw fa-image"></i><span class="hide-sm">' . esc_html__('图片', 'zib_language') . '</span></a>';

            $dropdown = '<div class="tab-content">';

            //第一个tab|输入图片地址
            $dropdown .= '<div class="tab-pane fade in active dropdown-image" id="image-tab-' . $upload_id . '-1">';
            $dropdown .= '<p>' . esc_html__('请填写图片地址：', 'zib_language') . '</p>';
            ...
            if ($upload) {
                $dropdown .= '<a class="but c-yellow mr10 pw-1em" data-toggle="tab" href="#image-tab-' . $upload_id . '-2" data-onclick="#input_' . $upload_id . '_image_upload">' . esc_html__('上传图片', 'zib_language') . '</a>';
            }
            $dropdown .= '<a type="submit" class="but c-blue pw-1em" href="javascript:;">' . esc_html__('确认', 'zib_language') . '</a>';
            $dropdown .= '</div>';
            $dropdown .= '</div>';
            ...
            if ($upload) {
                ...
                $dropdown .= '<p><a class="muted-color" data-toggle="tab" href="#image-tab-' . $upload_id . '-1"><i class="fa fa-angle-left mr6"></i>' . esc_html__('填写图片地址', 'zib_language') . '</a></p>';
                ...
                $from = '<div class="form-upload">
                            ...
                            <div class="text-right">
                                <button type="button" zibupload="submit" auto-submit="true" class="but jb-blue pw-1em input-expand-upload" name="submit">' . esc_html__('确认上传', 'zib_language') . '</button>
...
function zib_graphic_card($args = array(), $echo = false)
{
    ...
    if ($args['video'] && !wp_is_mobile()) {
        ...
        $img      = '<img class="fit-cover' . ($lazy ? ' lazyload' : '') . '" ' . ($lazy ? 'data-src="' . $args['img'] . '" src="' . $lazy_src . '"' : 'src="' . $args['img'] . '"') . ' alt="' . ($args['alt'] ?: __('图片', 'zib_language')) . zib_get_delimiter_blog_name() . '">';
    }
    ...
    $height_scale = $args['height_scale'] ? ' style="--cover-height-scale: ' . (int) $args['height_scale'] . '%;"' : '';
...
function zib_get_ajax_next_paginate($count_all, $page = 1, $ice_perpage = 10, $ajax_url = '', $pag_class = 'text-center theme-pagination ajax-pag', $next_class = 'next-page ajax-next', $nex = '', $query_key = 'paged', $scroll = null, $ajaxpager_target = false)
{
    ...
    $con         = '';
    if ($total_pages > $page) {
        $nex = $nex ? $nex : _pz('ajax_trigger', __('加载更多', 'zib_language'));
        if (!$ajax_url) {
...
function zib_get_paginate_links($args)
{
    ...
    $defaults = array(
        ...
        'current'      => 1, //当前页码
        'page_size'    => 12, //每页几条
        'prev_text'    => '<i class="fa fa-angle-left em12"></i><span class="hide-sm ml6">' . __('上一页', 'zib_language') . '</span>', //上一页按钮文字
        'next_text'    => '<span class="hide-sm mr6">' . __('下一页', 'zib_language') . '</span><i class="fa fa-angle-right em12"></i>', //下一页按钮文字
        'array'        => false,
    ...
    for ($n = 1; $n <= $total_pages; $n++):
        if ($n == $current):
            ...
            if ($n <= $end_size || ($current && $n >= $current - $mid_size && $n <= $current + $mid_size) || $n > $total_pages - $end_size):
                ...
                $dots = true;
            elseif ($dots):
                $page_links[] = '<span class="page-numbers dots">' . __('&hellip;', 'zib_language') . '</span>';
    ...
    if ($total_pages > $jump_size && $link_base) {
        $page_links[] = sprintf($link_sprintf, 'pag-jump page-numbers', 'javascript:;', '<input autocomplete="off" max="' . $total_pages . '" current="' . $current . '" base="' . $link_base . '" type="' . ($wp_is_mobile ? 'number' : 'text') . '" class="form-control jump-input" name="pag-go"><span class="hi de-sm mr6 jump-text">' . esc_html__('跳转', 'zib_language') . '</span><i class="jump-icon fa fa-angle-double-right em12"></i>');
    }
 ...
 * @return {*}
 */
function zib_get_ajax_null($text = null, $margin = '60', $img = 'null.svg')
{
    if (null === $text) {
        $text = __('暂无内容', 'zib_language');
    }
 ...
 * @return {*}
 */
function zib_get_null($text = null, $margin = '60', $img = 'null.svg', $class = '', $width = 280, $height = 0)
{
    if (null === $text) {
        $text = __('暂无内容', 'zib_language');
    }
...
function zibpay_admin_pagenavi($total_count, $number_per_page = 15)
{
    ...
    <div class="tablenav bottom">
        <div class="tablenav-pages">
            <span class="displaying-num"><?php printf(esc_html__('每页 %1$s 共%2$s', 'zib_language'), $number_per_page, $total_count); ?></span>
            <span class="pagination-links">
                <a class="first-page button                                                                                                                                                                                                                                           <?php if (1 == $current_page) {
        echo 'disabled';
    }
    ?>" title="<?php echo esc_attr__('前往第一页', 'zib_language'); ?>" href="<?php echo $first_page_url; ?>">«</a>
                <a class="prev-page button                                                                                                                                                                                                                                     <?php if (1 == $current_page) {
        echo 'disabled';
    }
    ?>" title="<?php echo esc_attr__('前往上一页', 'zib_language'); ?>" href="<?php echo $prev_page_url; ?>">‹</a>
                <span class="paging-input"><?php printf(__('第 %1$s 页，共 %2$s 页', 'zib_language'), $current_page, '<span class="total-pages">' . $total_pages . '</span>'); ?></span>
                <a class="next-page button                                                                                                                                                                                                                                     <?php if ($current_page == $total_pages) {
        echo 'disabled';
    }
    ?>" title="<?php echo esc_attr__('前往下一页', 'zib_language'); ?>" href="<?php echo $next_page_url; ?>">›</a>
                <a class="last-page button                                                                                                                                                                                                                                     <?php if ($current_page == $total_pages) {
        echo 'disabled';
    }
    ?>" title="<?php echo esc_attr__('前往最后一页', 'zib_language'); ?>" href="<?php echo $last_page_url; ?>">»</a>
            </span>
        </div>
        <br class="clear">
    </div>
...
function zib_get_main_tab_nav($type = 'nav', $tabs_options = array(), $id_prefix = 'home', $is_mobile_swiper = true, $active_key_str = 'tab', $route = false, $base_url = '')
{
    ...
    $placeholder_default = '<div class="mb20"><div class="text-center muted-2-color mt20"><i class="loading mr10"></i>' . esc_html__('加载中...', 'zib_language') . '</div></div>';
    ...
    foreach ($tabs_options as $key => $opt) {
        ...
        if ('nav' == $type) {
            ...
            //nav按钮
            $name = $opt['title'] ? $opt['title'] : __('栏目', 'zib_language');
...
function zib_get_cfs_icon($val, $class = '')
{
    ...
    if (!stristr($val, 'fa ') && stristr($val, 'fa-')) {
        return '<i class="fa ' . $val . $class . '" aria-hidden="true"></i>';
    }
...
//获取zibll主题教程的link
function zib_get_zibll_theme_tutorial_link($post_id, $class = '', $text = null)
{
    if (null === $text) {
        $text = __('【查看官网教程】', 'zib_language');
    }
    ...
    return '<a target="_blank" href="' . esc_url('https://www.zibll.com/' . $_url) . '" class="' . $class . '">' . $text . '</a>';
}
