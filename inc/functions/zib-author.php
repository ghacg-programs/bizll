...
{
    $super_admin = is_super_admin($user_id) ? '<span class="badg c-red">' . __('管理员', 'zib_language') . '</span>' : '';
...
function zib_get_author_header_btns($author_id)
{
    ...
    if ($current_id && $current_id == $author_id) {
        //登录用户就是作者
        $btns .= zib_get_user_center_link('but c-blue ml10 pw-1em radius', zib_get_svg('user') . __('用户中心', 'zib_language')); //用户中心
        ...
        if (_pz('private_s', true) && _pz('message_s', true)) {
            $btns .= Zib_Private::get_but($author_id, zib_get_svg('private') . __('私信', 'zib_language'), 'but jb-blue ml10 pw-1em'); //私信
        } else {
...
function zib_get_author_header_metas($author_id)
{
    ...
    $items = $view_n ? '<item><a data-toggle="tooltip" data-original-title="' . esc_attr(sprintf(__('人气值 %s', 'zib_language'), $view_n)) . '">' . zib_get_svg('hot') . $view_n . '</a></item>' : '';
    $items .= $like_n ? '<item><a data-toggle="tooltip" data-original-title="' . esc_attr(sprintf(__('获得%s个点赞', 'zib_language'), $like_n)) . '">' . zib_get_svg('like') . $like_n . '</a></item>' : '';
    $items .= $followed_n ? '<item><a data-toggle="tooltip" data-original-title="' . esc_attr(sprintf(__('共%s个粉丝', 'zib_language'), $followed_n)) . '"><i class="fa fa-heart em09"></i>' . $followed_n . '</a></item>' : '';
        ...
        foreach ($tabs_opt as $opt_k => $opt_v) {
            ...
            switch ($opt_k) {
                case 'post':
                    ...
                    $new_tab    = array(
                        'title'         => __('文章', 'zib_language') . '<count class="opacity8 ml3">' . $post_count . '</count>',
                        'content_class' => '',
                        'route'         => true,
                ...
                case 'favorite':
                    $new_tab = array(
                        'title'         => __('收藏<count class="opacity8 ml3">', 'zib_language') . zib_get_user_favorite_count($author_id) . '</count>',
                        'content_class' => '',
                        'route'         => true,
                        'loader'        => zib_get_author_tab_loader('post'),
                    );
                ...
                case 'follow':
                    $new_tab = array(
                        'title'         => __('粉丝', 'zib_language') . '<count class="opacity8 ml3">' . _cut_count(zib_get_user_follow_count($author_id, 'followed')) . '</count>',
...
function zib_get_author_favorite_filter($author_id, $type = 'post', $orderby = 'post__in')
{
    ...
    $favorite_args[$type]['orderby'] = array_merge(['post__in' => __('收藏时间', 'zib_language')], $favorite_args[$type]['orderby'] ?? []);
    ...
    $orderby_html .= '<a class="but" href="javascript:;" data-toggle="dropdown">' . __('排序', 'zib_language') . '<i class="fa fa-caret-down opacity5 ml6" aria-hidden="true" style="margin-right: 0;"></i></a>';
...
function zib_get_favorite_posts_lists($user_id, $paged = 1, $orderby = 'post__in')
{
    ...
    if (!$lists) {
        $lists = zib_get_ajax_null(__('暂无收藏内容', 'zib_language'), 60, 'null-2.svg');
    }
...
function zib_get_follow_user_list($user_id, $type = 'followed', $paged = 1)
{
    ...
    if ('followed' == $type) {
        $text  = __('粉丝', 'zib_language');
        $meta  = $followed;
        $count = $followed_count;
    } else {
        $text  = __('关注用户', 'zib_language');
        $meta  = $follow;
        $count = $follow_count;
    }
    ...
    if (1 == $paged) {
        $header_but = '<li><a rel="nofollow" ajax-replace="true" class="ajax-next ' . ('followed' == $type ? 'focus-color' : 'muted-color') . '" href="' . add_query_arg(array('type' => 'followed', 'paged' => false), $ajax_url) . '">' . __('粉丝', 'zib_language') . ' ' . $followed_count . '</a></li>';
        $header_but .= '<li><a rel="nofollow" ajax-replace="true" class="ajax-next ' . ('followed' != $type ? 'focus-color' : 'muted-color') . '" href="' . add_query_arg(array('type' => 'follow', 'paged' => false), $ajax_url) . '">' . __('关注', 'zib_language') . ' ' . $follow_count . '</a></li>';
    ...
    if (!$lists) {
        $lists = zib_get_ajax_null(sprintf(__('暂无%s', 'zib_language'), $text), '40', 'null-love.svg');
    }
...
function zib_main_author_tab_content_post()
{
    ...
    if (1 == $page || !_pz('paging_ajax_s', true)) {
        ...
        if (($current_id && $current_id == $author_id) || (is_super_admin())) {
            $status_array['pending'] = __('待审核', 'zib_language');
            $status_array['draft']   = __('草稿', 'zib_language');
        ...
        if ($status_html || zib_get_user_post_count($author_id, 'publish')) {
            ...
            $orderby_html = '<div class="dropdown flex0 pull-right">';
            $orderby_html .= '<a href="javascript:;" class="but" data-toggle="dropdown">' . __('排序', 'zib_language') . '<i class="ml6 fa fa-caret-down opacity5" aria-hidden="true" style="margin-right:0;"></i></a>';
            $orderby_html .= '<ul class="dropdown-menu">' . $orderby_dropdown_but . '</ul>';
            $orderby_html .= '</div>';
        }
    ...
    if (!$posts_list) {
        $posts_list = zib_get_ajax_null(__('暂无文章', 'zib_language'));
        //if ($page == 1) $header = '';
        if (1 == $page && $current_id && $current_id == $author_id) {
            $add = zib_get_write_posts_button('ml10 mr10 mt10 but hollow c-blue padding-lg', __('发布文章', 'zib_language'));
...
function zib_get_user_post_count($user_id, $poststatus = 'publish', $post_type = 'post')
{
    ...
    $db = zibDB::table($wpdb->posts);
    $db->where('post_author', $cuid);
    $db->where('post_type', $post_type);
    if ('all' != $poststatus) {
        $db->where('post_status', $poststatus);
    }
    $count = $db->count();
...
function zib_get_user_card_lists($users_args = array())
{
    ...
    $users_args = zib_query_orderby_filter($users_args['orderby'], $users_args);
...
function zib_get_author_header_dropup_btn()
{
    ...
    $lists = '<li>' . zib_get_user_details_data_link($author_id, '', zib_get_svg('user', null, 'mr6') . __('更多资料', 'zib_language')) . '</li>';
    $lists .= '<li>' . zib_get_user_search_link($author_id, '', '', zib_get_svg('search', null, 'mr6') . __('搜索内容', 'zib_language')) . '</li>';
...
function zib_get_user_card_box($args = array(), $echo = false)
{
    $defaults = array(
        ...
        'show_button'        => true,
        'button_1'           => 'post',
        'button_1_text'      => __('发布文章', 'zib_language'),
        'button_1_class'     => 'jb-pink',
        'button_2'           => 'home',
        'button_2_text'      => __('用户中心', 'zib_language'),
    ...
    if ($args['show_checkin'] && get_current_user_id() == $user_id) {
        $checkin = zib_get_user_checkin_btn('img-badge jb-yellow', '<i class="fa fa-calendar-check-o ml3 mr6"></i>' . __('签到', 'zib_language'), '<i class="ml3 mr6 fa fa-calendar-check-o"></i>' . __('已签到', 'zib_language'));
    }
