...
{
    add_submenu_page('users.php', __('消息管理', 'zib_language'), __('消息管理', 'zib_language'), 'administrator', 'user_messags', 'zibmsg_submenu_page');
}
...
function zibmsg_user_row_actions($actions, $user)
{
    if (is_super_admin()) {
        ...
        $actions['zibmsg'] = '<a href="' . $edit_link . '">' . __('消息管理', 'zib_language') . '</a>';
    }
    return $actions;
}
