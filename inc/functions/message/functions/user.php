 ...
 * @LastEditTime : 2026-07-01 23:13:57
...
function zib_msg_center_page_content()
{
    ...
    $tabs_array['news'] = array(
        'title'         => '<img ' . zib_get_lazy_attr('lazy_other', $img_uri . 'msg-news.svg', 'fit-cover') . ' alt="' . esc_attr__('未读消息', 'zib_language') . '">' . __('未读消息', 'zib_language') . $badge_news,
    ...
    $tabs_array['posts'] = array(
        'title'         => '<img ' . zib_get_lazy_attr('lazy_other', $img_uri . 'msg-comment.svg', 'fit-cover') . ' alt="' . esc_attr__('未读消息', 'zib_language') . '">' . __('文章评论', 'zib_language') . $badge_posts,
        'nav_attr'      => 'data-route="' . zibmsg_get_conter_url('posts') . '"',
        'content_class' => '',
        'loader'        => str_repeat('<div class="border-bottom box-body nopw-sm"><ul class="list-inline relative msg-list"><li><a class="msg-img placeholder"></a></li><li><dl><dt class="placeholder k2" style=" width: 80%; "></dt><dd class="mt10"><i class="placeholder s1"></i><i class="placeholder s1 ml10"></i></dd></dl></li></ul></div>', 5),
    );
    $tabs_array['like'] = array(
        'title'         => '<img ' . zib_get_lazy_attr('lazy_other', $img_uri . 'msg-followed.svg', 'fit-cover') . ' alt="' . esc_attr__('未读消息', 'zib_language') . '">' . __('点赞喜欢', 'zib_language') . $badge_like,
...
function zibmsg_page_main_tab_content_news()
{
    ...
    if ($clear_types) {
        ...
        if ($clear_time > 0) {
            $html .= '<div class="ajax-item abs-center right-top"><i data-toggle="tooltip" title="' . esc_attr(sprintf(__('部分已读消息将会在%d天后自动清理', 'zib_language'), $clear_time)) . '" class="muted-color mt20 mr10 fa fa-question-circle-o"></i></div>';
        }
    }
 ...
 * @return {*}
 */
function zib_get_msg_all_readed($user_id, $cat = '', $class = 'border-bottom box-body notop nopw-sm ajax-item', $a_class = 'but', $text = null)
{
    if (null === $text) {
        $text = __('全部标为已读', 'zib_language');
    }
 ...
 * @return {*}
 */
function zib_get_msg_all_readed_link($user_id, $cat = '', $class = 'but', $text = null)
{
    if (null === $text) {
        $text = __('全部标为已读', 'zib_language');
    }
...
function zib_get_user_news_msg($user_id = '')
{
    ...
    if (!_pz('private_s', true)) {
        $where['type'] = ['operator' => '!=', 'value' => 'private'];
    }
...
function zib_get_user_msg_lists($user_id = '', $where = array(), $ajax_url = '', $cat = 'news')
{
    ...
    if ($count_all && $db_msg) {
        ...
        $lists .= zib_get_ajax_null(__('暂无消息', 'zib_language'));
    }
...
function zib_msg_set_user_page_tabs_array($tabs_array)
{
    ...
    $tabs_array['msg'] = array(
        'title'    => __('消息通知', 'zib_language'),
        'nav_attr' => 'drawer-title="' . __('消息通知', 'zib_language') . '"',
...
function zib_msg_set_user_page_tab_content()
{
    ...
    $but_args[] = array(
        ...
        'neme'    => 'posts',
        'title'   => __('文章评论', 'zib_language'),
        'label'   => __('接收文章、评论、收藏等相关消息', 'zib_language'),
    );
    $but_args[] = array(
        'checked' => in_array('like', $msg_set),
        'neme'    => 'like',
        'title'   => __('点赞喜欢', 'zib_language'),
        'label'   => __('接收点赞、关注等相关消息', 'zib_language'),
    );
    $but_args[] = array(
        'checked' => in_array('system', $msg_set),
        'neme'    => 'system',
        'title'   => __('系统消息', 'zib_language'),
        'label'   => __('接收订单、活动、等系统消息', 'zib_language'),
    );
    ...
    $set .= '<button type="button" zibajax="submit" class="but jb-blue padding-lg mt10" name="submit"><i class="fa fa-check mr10"></i>' . __('确认提交', 'zib_language') . '</button>';
    $set .= '</div>';
...
function zib_msg_center_seo()
{
    ...
    if ($user_id) {
        ...
        $new           = $private_count ? sprintf(__('%d封私信/', 'zib_language'), $private_count) : '';
        $new .= $all_count ? sprintf(__('%d条消息', 'zib_language'), $all_count) . _get_delimiter() : '';
        $new .= $user->display_name . _get_delimiter() . __('消息中心', 'zib_language');
    } else {
        $new = __('消息中心', 'zib_language');
    }
