 ...
 * @LastEditTime : 2026-05-17 17:56:14
...
function zib_ajax_user_message_shield()
{
    ...
    if (!_pz('message_user_set', true)) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('暂未提供此功能', 'zib_language'))));
        exit;
    }

    $user_id = get_current_user_id();
    if (!$user_id) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('登录失效，请刷新页面', 'zib_language'))));
        exit;
    }
    //安全验证
    if (!wp_verify_nonce($_REQUEST['_wpnonce'], 'user_msg_set')) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('安全验证失败，请刷新后再试', 'zib_language'))));
        exit();
    }
    ...
    echo(json_encode(array('msg' => __('设置已保存', 'zib_language'), 'shield' => $shield)));
    exit;
}
...
function zib_ajax_user_msg_content()
{
    ...
    if ($msg && $user_id && in_array($msg->receive_user, (array) $receive_user_args)) {
        ...
        $con = zib_get_ajax_null(__('内容获取出错', 'zib_language'), 70, 'null.svg');
    }
...
function zib_ajax_user_msg_all_readed()
{
    ...
    echo(json_encode(array('error' => 0, 'reload' => 1, 'msg' => __('已全部标为已读', 'zib_language'))));
    exit;
}
...
function zib_ajax_send_private()
{
    ...
    if (!_pz('message_s', true) || !_pz('private_s', true)) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('管理员已关闭此功能', 'zib_language'))));
        exit;
    }

    //验证消息是否为空
    if (empty($_POST['receive'])) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('内容不能为空', 'zib_language'))));
        exit;
    }
    if (!trim(strip_tags($_POST['receive']))) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('内容不合法', 'zib_language'))));
        exit;
    }
    ...
    if ($is_blacklist) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('消息发送失败，对方已将您添加至黑名单', 'zib_language'))));
        exit;
    }
    ...
    if (!$msg) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('操作失败', 'zib_language'))));
        exit;
    }
    $html = '<div class="private-item">' . Zib_Private::get_msg_box($msg, $user_id) . '</div>';
    zib_send_json_success(array('msg' => __('消息已发送', 'zib_language'), 'html' => $html));
    exit;
}
...
function zib_ajax_private_window_modal()
{
    ...
    if (!_pz('message_s', true) || !_pz('private_s', true)) {
        zib_ajax_notice_modal('danger', __('管理员已关闭此功能', 'zib_language'));
    }
    ...
    if (_pz('user_ban_s', true) && zib_user_is_ban($send_user)) {
        zib_ajax_notice_modal('danger', __('您正处于禁封中，暂时无法发送私信', 'zib_language'));
    }
...
{
    if (!_pz('message_s', true) || !_pz('private_s', true)) {
        echo(zib_get_ajax_error_html(array('error' => 1, 'ys' => 'danger', 'msg' => __('暂未提供此功能', 'zib_language'))));
        exit;
    }
    if (!$user_id) {
        $user_id = get_current_user_id();
    ...
    if (!$user_id) {
        echo(zib_get_ajax_error_html(array('error' => 1, 'ys' => 'danger', 'msg' => __('权限不足', 'zib_language'))));
        exit;
    }
    ...
    if (!$lists) {
        $con = zib_get_ajax_null(__('暂无私信消息', 'zib_language'), 70, 'null-order.svg');
    } else {
        $con .= '<div class="col-sm-4 scroll-y mini-scrollbar ajaxpager" id="user_chat_lists" style="max-height: 550px;">' . $lists . '</div>';

        $submit_text = '<i class="fa fa-send-o"></i>' . __('发送', 'zib_language');
        $wondos      = '<div class="private-window post_ajax_trigger"><div class="private-window-header mb10 text-center">' . __('消息内容', 'zib_language') . '</div><div class="private-window-content mb10 opacity5 relative"><div class="abs-center em09 muted-color separator">' . __('请选择用户', 'zib_language') . '</div></div><div class="private-window-footer"><form class="from-private"><p><textarea placeholder="" class="form-control" rows="2" disabled="disabled"></textarea></p><div class="pull-right"><button class="but c-blue" disabled="disabled">' . $submit_text . '</button></div></form></div></div>';
        $wondos .= '<div class="private-window post_ajax_loader" style="display: none;"><div class="private-window-header mb10 text-center"><i class="loading mr10"></i>' . __('加载中...', 'zib_language') . '</div><div class="placeholder mb10 placeholder" style="height: 400px;"></div><div class="private-window-footer"><form class="from-private"><p><textarea placeholder="" class="form-control" rows="2" disabled="disabled"></textarea></p><div class="pull-right"><button class="but c-blue pw-1em" disabled="disabled">' . $submit_text . '</button></div></form></div></div>';
...
function zib_ajax_user_private_lists()
{
    $receive_user = !empty($_REQUEST['receive_user']) ? $_REQUEST['receive_user'] : '';
    if (!$receive_user) {
        echo(zib_get_ajax_error_html(array('error' => 1, 'ys' => 'danger', 'msg' => __('参数传入错误', 'zib_language'))));
        exit;
    }
    //安全验证
    if (!wp_verify_nonce($_REQUEST['_wpnonce'], 'user_private_lists')) {
        echo zib_get_ajax_error_html((array('error' => 1, 'ys' => 'danger', 'msg' => __('安全验证失败，请稍候再试', 'zib_language'))));
        exit;
    }
...
function zib_ajax_private_blacklist()
{
    ...
    if (in_array($receive_user, $private_blacklist)) {
        ...
        echo(json_encode(array('text' => __('加入黑名单', 'zib_language'), 'msg' => __('已从黑名单移除', 'zib_language'))));
        exit;
    } else {
        //添加黑名单
        $private_blacklist[] = $receive_user;
        update_user_meta($user_id, 'private_blacklist', $private_blacklist);
        echo(json_encode(array('text' => __('移除黑名单', 'zib_language'), 'msg' => __('已加入黑名单，不再接受此用户消息', 'zib_language'))));
        exit;
    }
...
function zib_ajax_clear_user_private()
{
    ...
    echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('暂未开放此功能', 'zib_language'))));
    exit;
}
