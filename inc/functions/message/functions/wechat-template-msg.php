...
function zib_wechat_template_send($user_id, $type, $data, $url = '')
{
    ...
    if (!_pz('wechat_template_msg_s')) {
        return ['error' => 1, 'msg' => __('微信公众号模板消息功能未开启', 'zib_language')];
    }

    //获取微信公众号配置
    $wxConfig = get_oauth_config('weixingzh');
    if (!$wxConfig['appid'] || !$wxConfig['appkey']) {
        return ['error' => 1, 'msg' => __('微信公众号配置错误，请检查AppID或AppSecret', 'zib_language')];
    }

    //获取模板ID
    $template_id = zib_get_wechat_template_id($type);
    if (!$template_id) {
        return ['error' => 1, 'msg' => __('未设置该类型消息的模板ID', 'zib_language')];
    }

    //获取用户微信open_id
    $open_id = zib_get_user_wechat_open_id($user_id);
    if (!$open_id) {
        return ['error' => 1, 'msg' => __('用户未绑定微信', 'zib_language')];
    }
    ...
    return ['error' => 0, 'msg' => __('发送成功', 'zib_language'), 'result' => $result, 'open_id' => $open_id, 'template_id' => $template_id, 'data' => $data];
}
