 ...
 * @LastEditTime : 2026-07-02 11:56:17
...
class ZibMsg
{
    ...
    public static function get_content($msg, $type = '')
    {
        ...
        if ('mini' == $type) {
            $con = preg_replace('/(\[img=|\<img)(.*?)(\]|\>)/', __('[图片]', 'zib_language'), $con);
            $con = preg_replace('/\[code]([\s\S]*)\[\/code]/', __('[代码]', 'zib_language'), $con);
            $con = preg_replace_callback('/\[g=(.*?)\]/', function ($matches) {
                return sprintf(__('[%s:%s]', 'zib_language'), __('表情', 'zib_language'), $matches[1]);
            }, $con);
            $con = preg_replace('/\[link=(.*?) target=_blank\](.*?)\[\/link\]/', '$2', $con);
            $con = preg_replace('/\[link=(.*?)\](.*?)\[\/link\]/', '$2', $con);
        } else {
            ...
            $con = preg_replace('/\[link=(.*?) target=_blank\](.*?)\[\/link\]/', '<a href="$1" target="_blank">$2</a>', $con);
            $con = preg_replace('/\[link=(.*?)\](.*?)\[\/link\]/', '<a href="$1">$2</a>', $con);
        }
