...
function zib_search_content()
{
    ...
    if (!$s) {
        echo '<div class="zib-widget search-content">' . zib_get_ajax_null(__('请输入搜索关键词', 'zib_language'), '60', 'null-search.svg', '', '300') . '</div>';
        return;
    }
    ...
    foreach ($tabs_args as $k => $tab) {
        if ($type === $k) {
            ...
            $tab_nav .= '<li><a ajax-replace="true" route="1" route-title="' . esc_attr($s . ' ' . __('搜索', 'zib_language') . $tab['title'] . $delimiter_blog_name) . '" class="ajax-next" href="' . esc_url(add_query_arg('type', $k, $base_url)) . '">' . $tab['title'] . '</a></li>';
        }
    }
    ...
    ' . $tab_content . '
    <div class="post_ajax_loader search-loader" style="display: none;"><div class="mb20"><div class="text-center muted-2-color mt20"><i class="loading mr10"></i>' . __('加载中...', 'zib_language') . '</div></div></div>
    </div>';
    return;
}
...
function zib_get_search_tab_nav_filter()
{
    ...
    $keys = [
        ...
        'forum'   => [
            'plate_id'    => __('版块', 'zib_language'),
            'forum_topic' => __('专题', 'zib_language'),
            'forum_tag'   => __('标签', 'zib_language'),
        ],
        'product' => [
            'shop_cat'      => __('分类', 'zib_language'),
            'shop_tag'      => __('标签', 'zib_language'),
            'shop_discount' => __('优惠', 'zib_language'),
        ],
        'plate'   => [
            'plate_cat' => __('分类', 'zib_language'),
        ],
    ];
    ...
    if (!empty($opts) && !empty($t_keys)) {
        foreach ($t_keys as $k => $name) {
            if (!empty($opts[$k])) {
                $_f_lists = '';
                if ($k === 'plate_id') {
                    $name = $GLOBALS['zib_bbs']->plate_name ?? __('版块', 'zib_language');
...
function zib_get_search_orderby_lists()
{
    ...
    $search_orderby = [
        'post'    => [
            'date'          => __('最新', 'zib_language'),
            'views'         => __('热门', 'zib_language'),
        ...
        'forum'   => [
            ...
            'views'          => __('热门', 'zib_language'),
            'comment_count'  => __('评论', 'zib_language'),
            'favorite_count' => __('收藏', 'zib_language'),
        ...
        'plate'   => [
            'date'         => __('最新', 'zib_language'),
            'views'        => __('热门', 'zib_language'),
            'follow_count' => __('关注', 'zib_language'),
            'posts_count'  => __('帖子数', 'zib_language'),
            'last_reply'   => __('最新回帖', 'zib_language'),
        ],
        'user'    => [
            'name'       => __('昵称', 'zib_language'),
            'registered' => __('注册时间', 'zib_language'),
        ],
    ];
    ...
    $filter_lists = '';
    $filter_lists .= $orderby_lists ? '<div class="flex ac mini-scrollbar scroll-x search-filter-item search-filter-orderby"><span class="opacity5">' . __('排序', 'zib_language') . '</span>' . $orderby_lists . '</div>' : '';
...
function zib_get_search_desc()
{
    ...
    if ($user) {
        ...
        if (!empty($user->display_name)) {
            $user_text = sprintf(__('用户%s', 'zib_language'), '<a class="focus-color" href="' . zib_get_user_home_url($user->ID) . '"><b>' . esc_attr($user->display_name) . '</b></a>');
            $text .= $user_text;
        }
    }
    ...
    if ($cat) {
        ...
        if (stristr($cat, 'plate_')) {
            ...
            if (!empty($get_post->post_title)) {
                ...
                $in_cat_type = $GLOBALS['zib_bbs']->plate_name ?? __('版块', 'zib_language');
    ...
    $text = $text ? sprintf(__('在%s中', 'zib_language'), $text) : '';

    $text .= sprintf(__('搜索 %1$s ，共找到 %2$s 个%3$s', 'zib_language'), '<a href="' . home_url('/?s=' . $s . '&type=' . $type) . '"><b class="search-key focus-color">' . $s . '</b></a>', '<b class="focus-color">' . $search_total . '</b>', zib_get_search_types()[$type]);
    return $text;
}
...
function zib_get_search_content_user()
{
    ...
    if ($lists) {
        ...
        if ($paged < 2) {
            $lists = zib_get_ajax_null(__('未找到相关用户', 'zib_language'), '75', 'null-user.svg');
        }
    }
...
function zib_single_cat_search($cat_id)
{
    ...
    <div class="theme-box zib-widget dosc-search">
        <div class="title-h-left"><b><?php echo __('搜索', 'zib_language') . $cat_obj->cat_name; ?></b></div>
...
$more_cats = array();
    ...
    $args = array(
        ...
        'show_input_cat' => true,
        'show_more_cat'  => true,
        'placeholder'    => __('搜索', 'zib_language') . $cat_obj->cat_name,
        'in_cat'         => $cat_id,
        'more_cats'      => $more_cats,
    );
...
{
    if (!is_super_admin()) {
        zib_ajax_notice_modal('danger', __('没有权限进行此操作！', 'zib_language'));
    }
    ...
    if (!$lists) {
        zib_ajax_notice_modal('warning', '<div class="muted-3-color em09 mb20">' . __('暂无热门搜索关键词', 'zib_language') . '</div><div class="muted-3-color px12">' . sprintf(__('如需修改置顶关键词，请在%s中修改', 'zib_language'), '<a href="' . zib_get_admin_csf_url('全局功能/搜索功能') . '">' . __('主题配置/全局功能/搜索功能', 'zib_language') . '</a>') . '<br>' . __('此弹窗及按钮仅管理员登录时显示', 'zib_language') . '</div>');
    } else {
        $lists = '<div class="mb10 flex ac jsa pb10"><div class="w40 text-center">' . __('关键词', 'zib_language') . '</div><div class="w40 text-center">' . __('搜索次数', 'zib_language') . '</div></div><div class="cloneable">' . $lists . '</div><botton type="button" class="cloneable-add but block c-blue"><svg class="icon" aria-hidden="true"><use xlink:href="#icon-add"></use></svg>' . __('添加关键词', 'zib_language') . '</botton>';
    }

    $form = '<form>
    <div class="mb20 mini-scrollbar scroll-y max-vh7">
        <div class="muted-3-color em09 mb20">' . __('在此处编辑已保存的搜索关键词，或修改搜索次数以重新排序。', 'zib_language') . '<br>' . __('使用后缀', 'zib_language') . '<badg class="badg badg-sm c-yellow">&type=xxx</badg>' . __('可定义搜索类型，可选类型有：post(文章：默认)、user(用户)、plate(板块)、forum(帖子)', 'zib_language') . '<br>' . sprintf(__('如需修改置顶关键词，请在%s中修改', 'zib_language'), '<a href="' . zib_get_admin_csf_url('全局功能/搜索功能') . '">' . __('主题配置/全局功能/搜索功能', 'zib_language') . '</a>') . '<br>' . __('备注：此弹窗及按钮仅管理员登录时显示', 'zib_language') . '</div>
        ' . $lists . '
    </div>
    <button class="but jb-blue padding-lg btn-block wp-ajax-submit"><i class="fa fa-check" aria-hidden="true"></i>' . __('确认保存', 'zib_language') . '</button>
    <input type="hidden" name="action" value="search_keywords_edit">
    ' . zib_nonce_field('search_keywords_edit') . '
    </form>';

    $title = '<div class="border-title touch"><button class="close abs-close" data-dismiss="modal"><svg class="ic-close" aria-hidden="true"><use xlink:href="#icon-close"></use></svg></button><div class="flex jc"><b>' . __('编辑搜索关键词', 'zib_language') . '</b></div></div>';
...
function zib_search_keywords_edit()
{
    ...
    if (!is_super_admin()) {
        zib_send_json_error(__('没有权限进行此操作！', 'zib_language'));
    }
    ...
    zib_send_json_success(['hide_modal' => true, 'msg' => __('保存成功！刷新页面后生效', 'zib_language')]);
}
add_action('wp_ajax_search_keywords_edit', 'zib_search_keywords_edit');

//搜索关键词管理编辑按钮
function zib_get_search_keywords_edit_btn($class = 'pull-right muted-3-color', $con = null)
{
    if (null === $con) {
        $con = '<i data-toggle="tooltip" title="' . esc_attr__('编辑搜索关键词', 'zib_language') . '" class="fa em12 fa-edit"></i>';
    }
    if (!is_super_admin()) {
        return '';
    }
...
function zib_get_search_box($args = array(), $echo = false)
{
    $defaults = array(
        ...
        'popular_limit'  => 0,
        'show_history'   => true,
        'keywords_title' => _pz('search_popular_title', __('热门搜索', 'zib_language')),
        'placeholder'    => _pz('search_placeholder', __('开启精彩搜索', 'zib_language')),
        'show_input_cat' => true,
        'show_posts'     => false,
        'show_type'      => false,
        'in_cat'         => '',
    ...
    if ($args['show_form']) {
        ...
        if ($args['show_input_cat']) {
            ...
            if ($args['in_cat']) {
                ...
                if (!$all_cat) {
                    $all_cat = '<li data-for="trem" data-value="null"><a href="javascript:;" class="">' . __('全部', 'zib_language') . '</a></li>';
            ...
            if ($in_cat_name || $all_cat) {
                $input_cat_name = $in_cat_name ? zib_str_cut($in_cat_name, 0, 6) : __('请选择', 'zib_language');
    ...
    if ($args['show_history']) {
        ...
        if ($keyword_link) {
            ...
            $history_html = '<div class="search-keywords history-search">
                                <p class="muted-color"><span>' . __('历史搜索', 'zib_language') . '</span><a class="pull-right trash-history-search muted-3-color" href="javascript:;"><i class="fa fa-trash-o em12" aria-hidden="true"></i></a></p>
                                <div>' . $keyword_link . '</div>
                            </div>';
        }
    }
    ...
    if ($args['show_posts']) {
        ...
        <p class="muted-color">' . __('热门文章', 'zib_language') . '</p>
...
function zib_get_search_cat($cat_ids = array(), $link_class = '')
{
    ...
    $links = '';
    $links .= '<li data-for="trem" data-value="null"><a href="javascript:;" class="' . $link_class . '">' . __('请选择', 'zib_language') . '</a></li>';
    foreach ($cats as $cat) {
