...
function zib_private_receive_email($msg)
{
    ...
    $title = sprintf(__('收到用户[%s]发来的私信', 'zib_language'), $send_udata->display_name);

    $message = sprintf(__('您好！%s', 'zib_language'), $receive_udata->display_name) . '<br>';
    $message .= __('收到一条新的私信消息', 'zib_language') . '<br />';
    $message .= __('用户：', 'zib_language') . $send_udata->display_name . '<br>';
    ...
    $message .= __('时间：', 'zib_language') . $msg['create_time'] . '<br>';
    $message .= '<br>';

    $message .= __('您可以打开下方链接查看此消息', 'zib_language') . '<br />';
...
function zib_comment_approved_email($comment)
{
    ...
    $title = __('您发布的评论已通过审核', 'zib_language');

    $message = sprintf(__('您好！%s', 'zib_language'), get_comment_author($comment->comment_ID)) . '<br>';
    $message .= sprintf(__('您在[%s]中的评论，已经通过审核', 'zib_language'), '<a class="muted-color" href="' . esc_url($post_tlink) . '">' . $post_title . '</a>') . '<br>';
    ...
    $message .= __('评论时间：', 'zib_language') . $comment->comment_date . '<br>';
    $message .= '<br>';

    $message .= __('您可以打开下方链接查看评论', 'zib_language') . '<br />';
    $message .= '<a target="_blank" style="margin-top: 20px" href="' . esc_url($_link) . '">' . $_link . '</a>' . '<br>';
...
function zib_email_publish_to_pending($post)
{
    ...
    $title = __('您发布的内容已被驳回', 'zib_language');

    $message = sprintf(__('您好！%s', 'zib_language'), $udata->display_name) . '<br>';
    $message .= sprintf(__('您发布的内容[%s]，已被驳回，当前为待审核状态', 'zib_language'), zib_str_cut($post->post_title, 0, 20, '...')) . '<br>';
    ...
    $message .= __('提交时间：', 'zib_language') . get_the_time('Y-m-d H:i:s', $post) . '<br>';
    $message .= __('驳回时间：', 'zib_language') . $post->post_modified . '<br>';
    $message .= $msg ? __('驳回说明：', 'zib_language') . $msg . '<br>' : '';
    $message .= '<br>';

    $message .= __('您可以打开下方链接查看此内容', 'zib_language') . '<br />';
...
function zib_email_pending_to_publish($post)
{
    ...
    $_link = get_permalink($post->ID);
    $title = __('您发布的内容已通过审核', 'zib_language');
    ...
    $message .= '<div class="muted-box" style=" padding:10px 15px;border-radius:8px;background:rgba(141, 141, 141, 0.05); line-height: 1.7;">' . zib_str_cut(trim(strip_tags($post->post_content)), 0, 200, '...') . '</div>';
    $message .= __('提交时间：', 'zib_language') . get_the_time('Y-m-d H:i:s', $post) . '<br>';
    $message .= __('审核时间：', 'zib_language') . get_the_modified_time('Y-m-d H:i:s', $post) . '<br>';
    $message .= '<br>';
...
function zib_links_submit_email_to_admin($data)
{
    ...
    $title = sprintf(__('有新的链接提交待审核：%s', 'zib_language'), $linkdata['link_name']);

    $message = __('网站有新的链接提交：', 'zib_language') . '<br />';
    $message .= __('链接名称：', 'zib_language') . $linkdata['link_name'] . '<br>';
    $message .= __('链接地址：', 'zib_language') . $linkdata['link_url'] . '<br>';
    $message .= __('链接简介：', 'zib_language') . $linkdata['link_description'] . '<br>';
    $message .= __('链接Logo：', 'zib_language') . $linkdata['link_image'] . '<br>';
    $message .= '<br>';

    $message .= __('您可以打开下方地址以审核该链接', 'zib_language') . '<br />';
    $message .= '<a target="_blank" style="margin-top: 20px" href="' . esc_url($_link) . '">' . $_link . '</a>' . '<br>';
    /**发送邮件 */
