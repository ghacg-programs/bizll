...
function zib_admin_frontend_set_botton($float)
{
    if (zib_admin_frontend_set_s()) {
        $float .= '<a href="javascript:;" data-toggle="modal" data-target="#modal_admin_set" title="' . esc_attr__('设置页面参数', 'zib_language') . '" class="admin-set-page float-btn"><i class="fa fa-cog fa-spin" aria-hidden="true"></i><div class="abs-right badg c-red px12 admin-set-info" style="width:187px;">' . __('在此编辑此页面参数', 'zib_language') . '</div></a>';
    }
    return $float;
}
...
function zib_frontend_set_modal()
{
    ...
    $header = '<div class="modal-header"><strong class="modal-title"><i class="fa fa-sliders mr10" aria-hidden="true"></i>' . $title . __('设置', 'zib_language') . '</strong>
    <button class="close" data-dismiss="modal">' . zib_get_svg('close', null, 'ic-close') . '</button>
                </div>';
    $footer = '<div class="modal-footer">
                    <a target="_blank" title="' . esc_attr__('使用可视化布局配置此页面', 'zib_language') . '" data-toggle="tooltip" href="' . zib_get_customize_widgets_url() . '" class="but c-yellow-2 padding-lg"><i class="fa fa-pie-chart" aria-hidden="true"></i>' . __('模块布局', 'zib_language') . '</a>
                    <a target="_blank" href="' . $edit_post_link . '" class="but c-yellow padding-lg"><i class="fa fa-wordpress" aria-hidden="true"></i>' . __('后台编辑', 'zib_language') . '</a>
                    <button class="but jb-blue padding-lg wp-ajax-submit"><i class="fa fa-check" aria-hidden="true"></i>' . __('确认修改', 'zib_language') . '</button>
                </div>';
...
function zib_frontend_set_save_ajax()
{
    ...
    if (empty($_POST['id'])) {
        echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('页面数据出错', 'zib_language'))));
        exit();
    }
    ...
    if ($type === 'tax') {
        ...
        if (!$object_data) {
            echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('未找到可用term数据', 'zib_language'))));
            exit();
        }
    } else {
        $object_data = get_post($_POST['id']);
        if (!$object_data) {
            echo(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => __('未找到可用post数据', 'zib_language'))));
            exit();
        }
    }
    ...
    echo(json_encode(array('msg' => __('保存成功，正在刷新页面', 'zib_language'), 'reload' => true)));
    exit();
}
...
function zib_edit_input_construct($input)
{
    ...
    $Examples[] = array(
        ...
        'options'     => array(
            'enlighter'  => __('默认浅色主题', 'zib_language'),
            'bootstrap4' => __('浅色：Bootstrap', 'zib_language'),
        ),
        'settings'    => array(
            'rows' => 3,
        ),
