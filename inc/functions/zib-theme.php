...
// 注册菜单位置
function zib_register_nav_menus()
{
...
add_action('init', 'zib_register_nav_menus');
...
function zib_php_error_message_filter($msg)
{
    if (strpos($msg, 'Supplied nav_menu_item value missing property')) {
        $msg .= '<br>' . sprintf(__('此问题请参看此教程进行处理：%s', 'zib_language'), '<a href="https://www.zibll.com/forum-post/16833.html">https://www.zibll.com/forum-post/16833.html</a>');
    }
    return $msg;
}
...
add_action('login_init', 'zib_replace_wp_login_is_logged_in');
...
{
    $labels = [
        'name'              => __('专题', 'zib_language'),
        'singular_name'     => __('专题', 'zib_language'),
        'search_items'      => __('搜索专题', 'zib_language'),
        'all_items'         => __('所有专题', 'zib_language'),
        'parent_item'       => __('父专题', 'zib_language'),
        'parent_item_colon' => __('父专题:', 'zib_language'),
        'edit_item'         => __('编辑专题', 'zib_language'),
        'update_item'       => __('更新专题', 'zib_language'),
        'add_new_item'      => __('添加新专题', 'zib_language'),
        'new_item_name'     => __('新专题名称', 'zib_language'),
        'menu_name'         => __('专题', 'zib_language'),
    ];
    $args = [
        'description'       => __('添加文章专题', 'zib_language'),
        'hierarchical'      => true,
        'labels'            => $labels,
        'show_ui'           => true,
        'show_in_menu'      => true,
...
//获取用户id
function zib_get_avatar_user_id($id_or_email)
{
...
//从多种输入解析用户邮箱
function zib_get_avatar_email($id_or_email)
{
    $email = '';
    ...
    if (is_numeric($id_or_email)) {
        ...
        if (!$email && !empty($id_or_email->user_email)) {
            $email = $id_or_email->user_email;
        }
        if (!$email && !empty($id_or_email->comment_author_email)) {
            $email = $id_or_email->comment_author_email;
...
//获取头像缓存键
function zib_get_avatar_cache_key($id_or_email)
{
    $user_id = zib_get_avatar_user_id($id_or_email);
    if ($user_id) {
        return 'u' . $user_id;
    }

    $email = zib_get_avatar_email($id_or_email);
    if ($email) {
        return 'e' . md5($email);
    }
...
//获取外部头像 URL
function zib_get_external_avatar_url($id_or_email, $size = 96)
{

    $source = _pz('avatar_external', '');
    if (!$source) {
        return '';
    }
    $hosts = array(
    ...
    $size       = max(1, min(2048, (int) $size ?: 96));
    $avatar_url = $host . $hash;

    $avatar_url = add_query_arg(rawurlencode_deep(array(
        's' => $size,
        'd' => zib_default_avatar(),
    )), $avatar_url);
...
{
    $cache_key = zib_get_avatar_cache_key($id_or_email);
    $cache     = wp_cache_get($cache_key, 'user_avatar', true);
    if (false === $cache) {
        $avatar = get_avatar($id_or_email, $size, $alt);
        wp_cache_set($cache_key, $avatar, 'user_avatar');
    } else {
        $avatar = $cache;
    }
...
function zib_get_user_avatar_url($id_or_email, $size = 96)
{
    $user_id = zib_get_avatar_user_id($id_or_email);
    if ($user_id) {
        ...
        if ($custom_avatar) {
            //优化百度头像地址
            $custom_avatar = str_replace('tb.himg.baidu.com', 'himg.bdimg.com', $custom_avatar);
            $custom_avatar = preg_replace('/^(https:|http:)/', '', $custom_avatar);

            return $custom_avatar;
        }
    }

    $external_avatar = zib_get_external_avatar_url($id_or_email, $size);
    if ($external_avatar) {
...
function zib_get_avatar($avatar, $id_or_email, $args)
{
    ...
    $alt        = $user_id ? get_the_author_meta('nickname', $user_id) . __('的头像', 'zib_language') . zib_get_delimiter_blog_name() : __('头像', 'zib_language') . zib_get_delimiter_blog_name();
    $avatar_url = zib_get_user_avatar_url($id_or_email, $args['size'] ?? 96);

    $args['size'] = esc_attr($args['size']);
    return '<img alt="' . esc_attr($alt) . '" src="' . esc_url($avatar_url) . '" class="avatar' . ($args['size'] ? ' avatar-' . $args['size'] : '') . ' avatar-id-' . $user_id . '"' . ($args['size'] ? ' height="' . $args['size'] . '" width="' . $args['size'] . '"' : '') . '>';
}
...
function zib_cron_schedules_filter($schedules)
{
    return array_merge($schedules, [
        'monthly' => ['interval' => 2678400, 'display' => __('每月一次', 'zib_language')],
    ]);
}
...
function zib_brush_limit_post($post_data)
{
    ...
    if ($minutes_10) {
        ...
        if ($count > $minutes_10) {
            zib_send_json_error(__('您的操作过于频繁，请10分钟后再试', 'zib_language'));
        }
    }
    ...
    if ($day_1) {
        ...
        if ($count > $day_1) {
            zib_send_json_error(__('您的提交已达今日上限，请明日再试', 'zib_language'));
        }
    }
}
...
function zib_brush_limit_comment($approved, $commentdata)
{
    ...
    if ($minutes_10) {
        ...
        if ($count > $minutes_10) {
            $comment_flood_message = __('您的操作过于频繁，请10分钟后再试', 'zib_language');
    ...
    if ($day_1) {
        ...
        if ($count > $day_1) {
            $comment_flood_message = __('您的提交已达今日上限，请明日再试', 'zib_language');
            return new WP_Error('comment_flood', $comment_flood_message, 429);
        }
    }
...
function zib_brush_limit_register($errors, $sanitized_user_login, $user_email)
{
    ...
    if ($minutes_10) {
        ...
        if ($count > $minutes_10) {
            $errors->add('brush_limit', __('您的操作过于频繁，请10分钟后再试', 'zib_language'));
        }
    }
    ...
    if ($day_1) {
        ...
        if ($count > $day_1) {
            $errors->add('brush_limit', __('您的账号注册已达今日上限，请明日再试', 'zib_language'));
        }
    }
...
function zib_brush_limit_upload($file)
{
    ...
    if ($minutes_10) {
        ...
        if ($count > $minutes_10) {
            $file['error'] = __('您的操作过于频繁，请10分钟后再试', 'zib_language');
            $file['debug'] = array(
                'sql'   => $sql,
                'count' => $count,
                'opt'   => $minutes_10,
    ...
    if ($day_1) {
        ...
        if ($count > $day_1) {
            $file['error'] = __('您的文件上传已达今日上限，请明日再试', 'zib_language');
            $file['debug'] = array(
                'sql'   => $sql,
                'count' => $count,
                'opt'   => $day_1,
...
function _load_scripts()
{
    if (!is_admin()) {
        ...
        wp_enqueue_script('loader_js', $purl . '/js/loader.js', array('jquery'), THEME_VERSION, true);
    }
}
...
function zib_get_adaptive_theme_img($white_src = '', $dark_src = '', $alt = '', $more = '', $lazy = 'auto')
{
    ...
    if ($lazy === 'auto') {
        $lazy = zib_is_lazy('lazy_other');
    }

    $lazy_src = ZIB_TEMPLATE_DIRECTORY_URI . '/img/thumbnail.svg';
    $this_src = zib_get_theme_mode() == 'dark-theme' ? $dark_src : $white_src;
    $img      = '<img ' . ($lazy ? 'src="' . $lazy_src . '" data-' : '') . 'src="' . $this_src . '" white-src="' . $white_src . '" dark-src="' . $dark_src . '" alt="' . $alt . '" ' . $more . (!$lazy ? ' loading="lazy"' : '') . '>';

    //如果启用了$lazy，则添加lazyload的class
    if ($lazy && !strstr($img, 'lazyload')) {
        if (strstr($img, 'class="')) {
            $img = str_replace(' class="', ' class="lazyload ', $img);
        } else {
...
//获取文章评论数量
function get_post_comment_count($before = null, $after = null, $post_id = 0)
{
    if (null === $before) {
        $before = __('评论[', 'zib_language');
    }
    if (null === $after) {
        $after = ']';
    }
...
//获取文章点赞数量
function get_post_like_count($before = null, $after = null, $post_id = 0)
{
    if (null === $before) {
        $before = __('点赞[', 'zib_language');
    }
    if (null === $after) {
        $after = ']';
    }
...
function get_post_favorite_count($before = null, $after = null, $post_id = 0)
{
    if (null === $before) {
        $before = __('收藏[', 'zib_language');
    }
    if (null === $after) {
        $after = ']';
    }
...
function get_post_view_count($before = null, $after = null, $post_id = 0)
{
    if (null === $before) {
        $before = __('阅读[', 'zib_language');
    }
    if (null === $after) {
        $after = ']';
    }
...
function zib_url_to_gourl($url)
{
    if (!$url || !_pz('go_link_s')) {
        return $url;
    }
    ...
    function the_content_img_auto_alt($content)
    {
        ...
        if (!is_null($images)) {
            foreach ($images[1] as $index => $value) {
                ...
                if (empty($is_alt[1])) {
                    //判断没有alt则添加
                    $new_img = str_replace('<img', '<img alt="' . sprintf(__('图片[%d]', 'zib_language'), ($index + 1)) . _get_delimiter() . esc_attr(strip_tags($alt)) . '"', $images[0][$index]);
...
function zib_get_rewards_button($user_ID, $class = 'ml6 but c-blue', $before = '', $after = '')
{
    ...
    $text   = _pz('post_rewards_text', __('赞赏', 'zib_language'));
    $text   = '<text>' . $text . '</text>';
...
// 写文章、投稿按钮
function zib_get_write_posts_button($class = 'but b-theme', $text = null, $before = '', $after = '')
{
    if (null === $text) {
        $text = __('写文章', 'zib_language');
    }
...
//前台管理员编辑按钮
function zib_get_term_admin_edit($title = null, $term = null, $class = 'admin-edit', $before = '', $after = '')
{
    if (null === $title) {
        $title = __('编辑', 'zib_language');
    }
    if (!is_super_admin()) {
        return;
    }
    ...
    $name = '[' . __('编辑', 'zib_language') . ']';
    $link = edit_term_link($name, $bef, $aft, $term, false);
    return $link;
}

//获取文章点赞按钮
function zib_get_post_like($class = '', $pid = '', $text = null, $count = false, $before = '', $after = '')
{
    if (null === $text) {
        $text = __('点赞', 'zib_language');
    }
    if (!_pz('post_like_s')) {
        return;
    }
...
//获取用户关注按钮
function zib_get_user_follow($class = '', $follow_id = '', $text = null, $ok_text = null, $before = '', $after = '')
{
    if (null === $text) {
        $text = '<i class="fa fa-heart-o mr3" aria-hidden="true"></i>' . __('关注', 'zib_language');
    }
    if (null === $ok_text) {
        $ok_text = '<i class="fa fa-heart mr3" aria-hidden="true"></i>' . __('已关注', 'zib_language');
    }
...
//获取文章收藏
function zib_get_post_favorite($class = '', $pid = '', $text = null, $count = false, $icon = '')
{
    if (null === $text) {
...
function get_user_desc($user_id, $yiyan = true)
{
    ...
    if (!$des) {
        $des = _pz('user_desc_std', __('这家伙很懒，什么都没有写...', 'zib_language'));
    }
    return esc_attr($des);
}
...
function get_user_cover_img($user_id)
{
    ...
    $img             = '<img ' . $cover_lazy_attr . ' alt="' . esc_attr__('用户封面', 'zib_language') . '">';
    return $img;
}
...
{
    $plugin_array['precode']   = ZIB_TEMPLATE_DIRECTORY_URI . '/js/editextend.min.js?ver=' . THEME_VERSION . '';
...
add_action('wp_enqueue_editor', function () {
    ...
    // 此时 header 尚未发送，不能直接 echo，需延后到页脚输出。
    $hook = is_admin() ? 'admin_print_footer_scripts' : 'wp_print_footer_scripts';
    add_action($hook, 'zib_echo_mce_global_config_script', 5);
});

function zib_echo_mce_global_config_script()
{
    ...
    echo '<script type="text/javascript">var mce = {
            ...
            hide_btn:"' . (apply_filters('tinymce_hide', false) || is_admin()) . '",
            hide_pay:"' . (apply_filters('tinymce_hide_pay', false) || (is_super_admin() && is_admin())) . '",
            float_toolbar:"' . $float_toolbar . '",
            i18n: ' . wp_json_encode(zib_get_mce_i18n_strings(), JSON_UNESCAPED_UNICODE) . ',
        }</script>';
}
...
{
    $a              = '#commentform';
    $default_notice = '<a class="hidden-text" href="javascript:(scrollTopTo(\'' . $a . '\',-50));"><i class="fa fa-exclamation-circle"></i>&nbsp;&nbsp;' . __('此处内容已隐藏，请评论后刷新页面查看.', 'zib_language') . '</a>';
    extract(shortcode_atts(array('notice' => $default_notice), $atts));
    $_hide = '<div class="hidden-box">' . $notice . '</div>';
    $_show = '<div class="hidden-box show"><div class="hidden-text">' . __('本文隐藏内容', 'zib_language') . '</div>' . do_shortcode($content) . '</div>';

    if (is_super_admin()) {
        //管理员登陆直接显示内容
        return '<div class="hidden-box show"><div class="hidden-text">' . __('本文隐藏内容 - 管理员可见', 'zib_language') . '</div>' . do_shortcode($content) . '</div>';
    } else {
...
function add_shortcode_postsbox($atts, $content = null)
{
    ...
    if (!$con && is_super_admin()) {
        $con = '<div class="hidden-box"><div class="text-center">[postsbox post_id="' . $post_id . '"]</div><div class="hidden-text">' . __('未找到文章，请重新设置短代码文章ID', 'zib_language') . '</div></div>';
    }
    return $con;
}
...
function add_shortcode_hidecontent($atts, $content = null)
{
    ...
    $type_text = array(
        ...
        'payshow'  => __('付费阅读', 'zib_language'),
        'logged'   => __('登录可见', 'zib_language'),
        'password' => __('密码验证', 'zib_language'),
        'vip1'     => _pz('pay_user_vip_1_name') . __('可见', 'zib_language'),
    ...
    if (is_super_admin()) {
        //管理员登陆直接显示内容
        return '<div class="hidden-box show"><div class="hidden-text">' . sprintf(__('[%s]隐藏内容 - 管理员可见', 'zib_language'), $type_text[$type]) . '</div>' . ($content) . '</div>';
    }

    //通过挂钩直接显示
    if (apply_filters('hidecontent_is_show', false, $type)) {
        return '<div class="hidden-box show"><div class="hidden-text">' . __('本文隐藏内容', 'zib_language') . '</div>' . ($content) . '</div>';
    }

    if ($user_id && $user_id == $post->post_author) {
        //作者直接显示内容
        return '<div class="hidden-box show"><div class="hidden-text">' . sprintf(__('[%s]隐藏内容 - 作者可见', 'zib_language'), $type_text[$type]) . '</div>' . ($content) . '</div>';
    }

    switch ($type) {
        case 'reply':
            $a     = '#commentform';
            $_hide = '<div class="hidden-box" reply-show="true" reload-hash="#hidden-box-comment"><a class="hidden-text" href="javascript:(scrollTopTo(\'' . $a . '\',-50));"><i class="fa fa-exclamation-circle"></i>&nbsp;&nbsp;' . __('此处内容已隐藏，请评论后刷新页面查看.', 'zib_language') . '</a></div>';
            $_show = '<div class="hidden-box show" id="hidden-box-comment"><div class="hidden-text">' . __('本文隐藏内容', 'zib_language') . '</div>' . ($content) . '</div>';
        ...
        case 'payshow':
            $a        = '#posts-pay';
            $_hide    = '<div class="hidden-box"><a class="hidden-text" href="javascript:(scrollTopTo(\'' . $a . '\',-50));"><i class="fa fa-exclamation-circle"></i>&nbsp;&nbsp;' . __('此处内容已隐藏，请付费后查看', 'zib_language') . '</a></div>';
            ...
            if ($paid) {
                ...
                if ('free' == $paid['paid_type'] && _pz('pay_free_logged_show') && !$user_id) {
                    return '<div class="hidden-box"><a class="hidden-text signin-loader" href="javascript:;"><i class="fa fa-exclamation-circle"></i>&nbsp;&nbsp;' . __('免费资源，请登录后查看', 'zib_language') . '</a></div>';
                } else {
                    //已经购买，且有效
                    ...
                    if ($paid['paid_type'] == 'paid' && $order_expired_time) {
                        $order_expired_html = '<span class="badg badg-sm c-yellow">' . sprintf(__('%s前可查看', 'zib_language'), $order_expired_time) . '</span>';
                    }

                    return '<div class="hidden-box show"><div class="hidden-text">' . sprintf(__('本文付费阅读内容 - %s', 'zib_language'), $paid_name . $order_expired_html) . '</div>' . ($content) . '</div>';
                }
            } else {
        ...
        case 'logged':
            if ($user_id > 0) {
                return '<div class="hidden-box show"><div class="hidden-text">' . __('本文隐藏内容 - 登录可见', 'zib_language') . '</div>' . ($content) . '</div>';
            } else {
                return '<div class="hidden-box"><a class="hidden-text signin-loader" href="javascript:;"><i class="fa fa-exclamation-circle"></i>&nbsp;&nbsp;' . __('隐藏内容，请登录后查看', 'zib_language') . '</a></div>';
            }
            break;
        case 'vip1':
        case 'vip2':
            ...
            if ($user_id > 0) {
                if (!$vip_level) {
                    return '<div class="hidden-box"><a class="hidden-text pay-vip" vip-level="' . $vip_l . '" href="javascript:;"><i class="fa fa-exclamation-circle"></i>&nbsp;&nbsp;' . sprintf(__('此处内容已隐藏，%s', 'zib_language'), $type_text['vip' . $vip_l]) . '<br><i class="fa fa-diamond"></i>&nbsp;&nbsp;' . __('请开通会员后查看', 'zib_language') . '</a></div>';
                } elseif ($vip_level < $vip_l) {
                    return '<div class="hidden-box"><a class="hidden-text pay-vip" vip-level="' . $vip_l . '" href="javascript:;"><i class="fa fa-exclamation-circle"></i>&nbsp;&nbsp;' . sprintf(__('此处内容已隐藏，%s', 'zib_language'), $type_text['vip' . $vip_l]) . '<br><i class="fa fa-diamond"></i>&nbsp;&nbsp;' . __('请升级会员后查看', 'zib_language') . '</a></div>';
                } else {
                    return '<div class="hidden-box show"><div class="hidden-text">' . sprintf(__('本文隐藏内容 - %s', 'zib_language'), $type_text['vip' . $vip_l]) . '</div>' . ($content) . '</div>';
                }
            } else {
                return '<div class="hidden-box"><a class="hidden-text signin-loader" href="javascript:;"><i class="fa fa-exclamation-circle"></i>&nbsp;&nbsp;' . sprintf(__('此处内容已隐藏，%s', 'zib_language'), $type_text['vip' . $vip_l]) . '<br><i class="fa fa-sign-in"></i>&nbsp;&nbsp;' . __('请登录后查看特权', 'zib_language') . '</a></div>';
            }
            break;
        case 'password':
            $html = '';
            ...
            if ($pas && $pas == $password) {
                $html = '<div class="hidden-box show"><div class="hidden-text">' . __('此处隐藏内容', 'zib_language') . '</div>' . ($content) . '</div>';
            } else {
                ...
                $from .= '<input type="text" name="hidecontent-password" class="form-control" placeholder="' . esc_attr__('请输入密码', 'zib_language') . '">';
                $from .= '<button type="submit" class="but c-blue ml10 flex0">' . __('提交', 'zib_language') . '</button>';
                $from .= '</form>';

                $html .= '<div class="hidden-box flex em09-sm">';
                $html .= $img;
                $html .= '<div class="flex1 flex xx jsb padding-10 nopw-sm noph-sm">';
                $html .= '<dd class="title-theme em09-sm">' . __('隐藏内容，输入密码后查看', 'zib_language') . '</dd>';
                $html .= $desc ? '<div class="muted-color mt6 mb6">' . $desc . '</div>' : '';
                $html .= $pas ? '<div class="px12 c-red">' . __('密码输入错误，请重新输入', 'zib_language') . '</div>' : '';
                $html .= $from;
                $html .= '</div>';
                $html .= '</div>';
            }
...
function ZFuncTime($func, $arr = null)
{
    ...
    $log = array(
        __('数据库查询', 'zib_language') => get_num_queries(),
        __('页面加载', 'zib_language')  => (timer_stop(0, 10) * 1000),
        __('测试函数', 'zib_language')  => $func,
        __('测试参数', 'zib_language')  => $arr,
        __('执行时间', 'zib_language')  => ($time_taken * 1000),
    );
