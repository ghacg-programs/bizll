...
function zib_ajax_invit_code_must_verify()
{
    ...
    zib_send_json_success(array('code' => $invit_code->password, 'msg' => __('邀请码有效，请继续完成注册', 'zib_language')));
}
...
function zib_ajax_invit_code_verify($code = null)
{
    ...
    if (!$code && $invit_code_s === 'must') {
        zib_send_json_error(__('请输入邀请码', 'zib_language'));
    }
    ...
    if (empty($invit_code->id)) {
        zib_send_json_error(__('邀请码无效或已使用', 'zib_language') . ($invit_code_s !== 'must' ? __('，请修改或删除邀请码', 'zib_language') : ''));
    }
            ...
            case 'points':
                ...
                if ($v && _pz('points_s')) {
                    $data = array(
                        ...
                        'type'      => __('邀请奖励', 'zib_language'), //中文说明
                        'desc'      => __('使用邀请码注册奖励', 'zib_language'), //说明
                    );
                ...
                if ($v > 0 && _pz('pay_user_vip_' . $v . '_s')) {
                    ...
                    if ($exp_date) {
                        $data = array(
                            ...
                            'type'      => __('邀请奖励', 'zib_language'), //中文说明
                            'desc'      => __('使用邀请码注册赠送', 'zib_language'), //说明
                        );
