...
function zib_user_add_medal_new_msg($user_id, $medal_args)
{
    ...
    $title   = sprintf(__('恭喜您！获得新的徽章[%s]', 'zib_language'), $medal_args['name']);
    $message = '<div class="c-blue text-center mb20">' . __('恭喜您！获得新的徽章', 'zib_language') . '</div>';
    $message .= '<div class="mb20 medal-card medal-single-card relative" style="max-width: 360px;margin: auto;"><div class="medal-single-bg"><img class="fit-cover" src="' . ZIB_TEMPLATE_DIRECTORY_URI . '/img/medal/medal-background.svg" alt="' . esc_attr__('徽章', 'zib_language') . '"></div><div class="relative"><img class="img-icon medal-icon mt20" src="' . $medal_args['icon'] . '" alt="' . esc_attr(sprintf(__('徽章-%s', 'zib_language'), $medal_args['name'])) . zib_get_delimiter_blog_name() . '"><div class="mt6">' . $medal_args['name'] . '</div><div class="muted-2-color px12 mt6">' . $medal_args['desc'] . '</div><div class="c-blue px12 mt6">' . sprintf(__('%s 获得', 'zib_language'), $medal_args['time']) . '</div></div></div>';
    $message .= '<div class="text-center" style="max-width: 360px;margin: auto;">' . zib_get_user_medal_info_link($user_id, 'mt20 but btn-block radius c-blue-2', zib_get_svg('medal-color') . __('查看我的徽章', 'zib_language')) . '</div>';
    $message .= '';
 ...
 * @return {*}
 */
function zib_get_user_medal_info_link($user_id = 0, $class = '', $con = null)
{
    if (null === $con) {
        $con = __('徽章明细', 'zib_language');
    }
...
function zib_get_single_medal_info_modal($medal_name, $user_id = 0)
{
    ...
    if (!$args) {
        return zib_get_null(__('参数传入错误或没有此徽章', 'zib_language'), '30', 'null-2.svg');
    }
    ...
    if ($is_current) {
        ...
        if ($is_has) {
            $time_html = '<div class="c-blue px12 mt10">' . sprintf(__('%s 获得', 'zib_language'), $is_has['time']) . '</div>';
            $wear_html = '<div class="box-body">' . zib_get_medal_wear_link($medal_name, 'but btn-block radius', '<span class="badg btn-block c-blue">' . __('当前佩戴此徽章', 'zib_language') . '</span>') . '</div>';
        } else {
            $time_html = '<div class="c-yellow px12 mt20">' . __('暂未获得', 'zib_language') . '</div>';
        }
    } elseif ($user_id && $args['get_type'] === 'manually_add') {
        //不是自己，如果拥有收回徽章权限则显示收回徽章的按钮
        $manually  = zib_get_medal_manually_remove_link($user_id, $medal_name, 'but btn-block radius c-red', __('收回此徽章', 'zib_language'));
    ...
    $html = '<button class="close" data-dismiss="modal">' . zib_get_svg('close', null, 'ic-close') . '</button>
                <div class="box-body medal-card mb20 mt20 medal-single-card relative">
                    <div class="medal-single-bg">
                        <img class="fit-cover" src="' . ZIB_TEMPLATE_DIRECTORY_URI . '/img/medal/medal-background.svg" alt="' . esc_attr__('徽章', 'zib_language') . '">
                    </div>
                    <div class="relative">
                        <img ' . $lazy_attr . ' alt="' . esc_attr(sprintf(__('徽章-%s', 'zib_language'), $args['name'])) . zib_get_delimiter_blog_name() . '">
...
function zib_get_medal_wear_link($medal_name, $class = 'but btn-block radius', $is_wear_html = '')
{
    ...
    $name      = !$is_wear ? __('佩戴此徽章', 'zib_language') : __('取消佩戴', 'zib_language');
    $form_data = array(
...
function zib_get_user_medal_info_modal($user_id)
{
    ...
    $display_name = $user_id != $current_id ? $user->display_name : __('我', 'zib_language');
    $avatar_img   = zib_get_avatar_box($user_id, 'avatar-img', false, false);
    $user_name    = '<span class="em09">' . sprintf(__('%s的徽章', 'zib_language'), $display_name) . '</span>';
    ...
    if ($wear_medal) {
        ...
        $user_name .= '<img ' . $lazy_attr . ' data-toggle="tooltip" title="' . esc_attr(sprintf(__('当前佩戴：%s', 'zib_language'), $wear_medal['name'])) . '"  alt="' . esc_attr(sprintf(__('徽章-%s', 'zib_language'), $wear_medal['name'])) . zib_get_delimiter_blog_name() . '">';
    } else {
        $user_name .= '<span class="em09 ml6"><span class="badg-sm badg c-red">' . __('暂无徽章', 'zib_language') . '</span></span>';
    }
    ...
    if (!$medal_info) {
        $medal_info = zib_get_null(__('暂无徽章', 'zib_language'), '30', 'null-2.svg');
    }

    $my           = '';
    $manually_add = zib_get_user_medal_manually_add_link($user_id, 'but c-green', zib_get_svg('add-ring') . __('授予徽章', 'zib_language'));
    if ($user_id != $current_id) {
        $my = zib_get_user_medal_info_link($current_id, 'but c-blue-2', zib_get_svg('medal-color') . __('查看我的徽章', 'zib_language'));
    }
    ...
    if ($medal_details || $current_id == $user_id) {
        ...
        foreach ($medal_cat_args as $k => $v) {
            ...
            foreach ($v as $item_k => $item_v) {
                ...
                <img ' . $lazy_attr . ' alt="' . esc_attr(sprintf(__('徽章-%s', 'zib_language'), $item_k)) . zib_get_delimiter_blog_name() . '">
...
function zib_author_header_identity_filter_medal($desc, $user_id)
{
    ...
    if (!$medal) {
        $medal = zib_get_user_medal_manually_add_link($user_id, 'but', zib_get_svg('medal-color') . __('授予徽章', 'zib_language') . '<i class="fa fa-angle-right" style="margin: 0 0 0 .3em;"></i>');
    }
...
function zib_user_page_header_desc_filter_medal($desc, $user_id)
{
    ...
    if (!$my) {
        $my = zib_get_user_medal_info_link($user_id, 'but', zib_get_svg('medal-color') . __('获取徽章', 'zib_language') . '<i class="fa fa-angle-right" style="margin: 0 0 0 .3em;"></i>');
    }
...
function zib_get_user_medal_show_link($user_id, $class = 'but', $max_icon = 3)
{
    ...
    foreach ($medal_details as $k => $v) {
        ...
        $icon_url  = $v['icon'];
        $lazy_attr = zib_get_lazy_attr('lazy_other', $icon_url, 'img-icon medal-icon', ZIB_TEMPLATE_DIRECTORY_URI . '/img/thumbnail-null.svg');
        $icon_html .= '<img ' . $lazy_attr . ' data-toggle="tooltip" title="' . esc_attr($k) . '" alt="' . esc_attr(sprintf(__('徽章-%s', 'zib_language'), $k)) . zib_get_delimiter_blog_name() . '">';
    }
    return zib_get_user_medal_info_link($user_id, $class, $icon_html . '<span class="ml3">' . sprintf(__('%d枚徽章', 'zib_language'), $count) . '<i class="fa fa-angle-right" style="margin: 0 0 0 .3em;"></i></span>');
}
 ...
 * @return {*}
 */
function zib_get_user_medal_manually_add_link($user_id = 0, $class = '', $con = null)
{
    if (null === $con) {
        $con = __('授予徽章', 'zib_language');
    }
 ...
 * @return {*}
 */
function zib_get_medal_manually_add_link($user_id, $medal_name, $class = 'but btn-block radius', $con = null)
{
    if (null === $con) {
        $con = __('授予', 'zib_language');
    }
 ...
 * @return {*}
 */
function zib_get_medal_manually_remove_link($user_id, $medal_name, $class = 'but btn-block radius', $con = null)
{
    if (null === $con) {
        $con = __('收回', 'zib_language');
    }
    if (!_pz('user_medal_s', true) || !zib_current_user_can('medal_manually_set')) {
        return;
    }
...
function zib_get_user_medal_manually_add_modal($user_id = 0)
{
    ...
    $avatar_img   = zib_get_avatar_box($user_id, 'avatar-img', false, false);
    $user_name    = '<span class="em09">' . sprintf(__('为%s授予新的徽章', 'zib_language'), '<span class="focus-color">' . $display_name . '</span>') . '</span>';
    $header       = zib_get_modal_colorful_header('c-blue', $avatar_img, $user_name);
    ...
    if (!$medal_manually_add_lists) {
        $medal_manually_add_lists = zib_get_null(__('暂无可授予的徽章', 'zib_language'), '30', 'null-2.svg');
    }
