...
function zib_get_user_auth_name($id = 0)
{
    ...
    return isset($auth_info['name']) ? $auth_info['name'] : __('官方认证', 'zib_language');
}
...
//获取申请认证的链接
function zib_get_user_auth_apply_link($class = '', $text = null)
{
    if (null === $text) {
        $text = __('申请认证', 'zib_language');
    }
    if (!_pz('user_auth_s', true)) {
        return;
    }
...
function zib_get_user_auth_info_modal($user_id = 0)
{
    ...
    if (!$auth_badge) {
        return zib_ajax_notice_modal('warning', __('该用户暂未认证', 'zib_language'));
    }

    $icon = zib_get_svg('user-auth');

    $header    = zib_get_modal_colorful_header('c-blue', $icon, __('官方认证', 'zib_language'));
    $auth_info = zib_get_user_meta($user_id, 'auth_info', true);

    $name = isset($auth_info['name']) ? $auth_info['name'] : __('官方认证', 'zib_language');

    $desc = isset($auth_info['desc']) ? '<div class="muted-2-color mt10">' . $auth_info['desc'] . '</div>' : '';
    $time = isset($auth_info['time']) ? '<div class="muted-2-color mt10">' . sprintf(__('认证时间：%s', 'zib_language'), $auth_info['time']) . '</div>' : '';

    $apply = '';
    if (get_current_user_id() != $user_id) {
        $apply = zib_get_user_auth_apply_link('but c-blue padding-lg btn-block mt20', __('我也要申请认证', 'zib_language'));
    }
...
function zib_get_user_auth_apply_modal()
{
    ...
    $icon   = zib_get_svg('user-auth');
    $header = zib_get_modal_colorful_header('c-blue', $icon, __('申请身份认证', 'zib_language'));
...
function zib_get_user_auth_apply_from($user_id = 0)
{
    ...
    if ($apply_record) {
        ...
        $time = $time ? '<div class="muted-2-color mt10">' . sprintf(__('申请时间：%s', 'zib_language'), $time) . '</div>' : '';
        $msg  = $msg ? '<div class="muted-2-color mt10">' . sprintf(__('申请说明：%s', 'zib_language'), $msg) . '</div>' : '';

        $img_html = '';
        if ($img && is_array($img)) {
            foreach ($img as $img_url) {
                $img_html .= '<div class="preview-item"><img alt="' . esc_attr__('身份认证申请', 'zib_language') . '" src="' . $img_url . '"></div>';
            }
        }
        $img_html = $img_html ? '<div class="mt20">' . $img_html . '</div>' : '';

        $html .= '<div class="c-red mb10">' . __('您的申请正在审核中，请耐心等待', 'zib_language') . '</div>';
        $html .= '<div class="">' . $icon . '<b class="ml3">' . $name . '</b></div>';
        $html .= $desc;
        $html .= $msg;
        $html .= $time;
        $html .= $img_html;
        $html .= '<button class="but c-blue padding-lg btn-block mt20" data-dismiss="modal">' . __('确认', 'zib_language') . '</button>';
        return $html;
    } else {
        //没有申请记录，则开始新的申请
        ...
        if (empty($apply_option['s'])) {
            ...
            $html .= '<div class="c-red"><i class="fa mr6 fa-info-circle"></i>' . __('抱歉！暂时不能申请身份认证', 'zib_language') . '</div>';
            $html .= $disable_desc;
            $html .= '<button class="but padding-lg btn-block mt20" data-dismiss="modal">' . __('取消', 'zib_language') . '</button>';
            return $html;
        }
        ...
        if (_pz('user_level_s', true) && !empty($apply_option['limit_level']) && $apply_option['limit_level'] > zib_get_user_level($user_id)) {
            ...
            $disable_desc = '<div class="muted-color mt20">' . sprintf(__('由于您的等级过低，暂时无法申请身份认证，请您升级到%s之后再来申请', 'zib_language'), '<span class="em12">' . $limit_level . '</span>') . '</div>';
            $html .= '<div class="c-red"><i class="fa mr6 fa-info-circle"></i>' . __('抱歉！您暂时不能申请身份认证', 'zib_language') . '</div>';
            $html .= $disable_desc;
            $html .= '<button class="but padding-lg btn-block mt20" data-dismiss="modal">' . __('取消', 'zib_language') . '</button>';
            return $html;
        }
        ...
        $img_html .= '<div class="em09 muted-2-color mb6">' . __('证明材料', 'zib_language') . '</div>';
        ...
        $con = '';
        $con .= '<div class="mb20">';
        $con .= '<div class="em09 muted-2-color mb6">' . __('认证身份', 'zib_language') . '</div>';
        $con .= '<input type="text" class="form-control" name="name" tabindex="1" value="" placeholder="' . esc_attr__('请输入身份名称', 'zib_language') . '">';
        $con .= '</div>';

        $con .= '<div class="mb20">';
        $con .= '<div class="em09 muted-2-color mb6">' . __('身份简介', 'zib_language') . '</div>';
        $con .= '<textarea class="form-control" name="desc" tabindex="2" placeholder="' . esc_attr__('请输入申请的身份简介', 'zib_language') . '" rows="2"></textarea>';
        $con .= '</div>';

        $con .= '<div class="mb20">';
        $con .= '<div class="em09 muted-2-color mb6">' . __('申请说明', 'zib_language') . '</div>';
        $con .= '<textarea class="form-control" name="msg" tabindex="2" placeholder="' . esc_attr__('请输入申请缘由及说明', 'zib_language') . '" rows="2"></textarea>';
        $con .= '</div>';
        $con .= $img_html;
...
function zib_user_auth_apply_process($args)
{
    ...
    $status_text = __('已批准', 'zib_language');
    if ('1' == $args['status']) {
        zib_add_user_auth($args['user_id'], array(
            'name' => $args['name'],
            'desc' => $args['desc'],
        ));
        $msg_title   = __('身份认证通过，恭喜您成为认证用户', 'zib_language');
        $status_text = __('已通过', 'zib_language');
    }
    if (2 == $args['status']) {
        $status_text = __('被拒绝', 'zib_language');
        $msg_title   = __('您的身份认证申请被拒绝', 'zib_language');
    }

    //准备新消息
    $user_display_name = get_userdata($args['user_id'])->display_name;
    $msg_con           = '';
    $msg_con .= sprintf(__('您好！%s', 'zib_language'), $user_display_name) . '<br />' . sprintf(__('您于%s发起的身份认证申请%s', 'zib_language'), date('Y年m月d日 H:i', strtotime($msg_db->create_time)), $status_text) . '<br>';

    $msg_con .= sprintf(__('认证身份：%s', 'zib_language'), $args['name']) . '<br>';
    $msg_con .= sprintf(__('身份简介：%s', 'zib_language'), $args['desc']) . '<br>';
    $msg_con .= sprintf(__('申请时间：%s', 'zib_language'), $msg_db->create_time) . '<br>';
    $msg_con .= sprintf(__('处理时间：%s', 'zib_language'), $msg_db->modified_time) . '<br>';

    $msg_con .= '<br>';
    $msg_con .= $args['msg'] . '<br>';
    $msg_con .= __('如有疑问请与客服联系', 'zib_language');
...
function zib_main_user_tab_content_auth()
{
    ...
    if (zib_is_user_auth($user_id)) {
        ...
        $desc       = !empty($apply_option['desc']) ? '<div class="muted-color mb20">' . $apply_option['desc'] . '</div>' : '';
        $desc .= zib_get_user_auth_apply_link('but jb-blue padding-lg btn-block mt20', __('申请认证', 'zib_language'));
        if (empty($apply_option['s'])) {
            //如果未开启申请功能，则显示禁止申请的提醒
            $disable_desc = !empty($apply_option['disable_desc']) ? '<div class="muted-color mt20">' . $apply_option['disable_desc'] . '</div>' : '';

            $desc = '<div class="c-red"><i class="fa mr6 fa-info-circle"></i>' . __('抱歉！暂时不能申请身份认证', 'zib_language') . '</div>';
            $desc .= $disable_desc;
        }
        if (_pz('user_level_s', true) && !empty($apply_option['limit_level']) && $apply_option['limit_level'] > zib_get_user_level($user_id)) {
            //如果未达到申请要求
            $limit_level  = zib_get_level_badge($apply_option['limit_level']);
            $disable_desc = '<div class="muted-color mt20">' . sprintf(__('由于您的等级过低，暂时无法申请身份认证，请您升级到%s之后再来申请', 'zib_language'), '<span class="em12">' . $limit_level . '</span>') . '</div>';
            $desc         = '<div class="c-red"><i class="fa mr6 fa-info-circle"></i>' . __('抱歉！您暂时不能申请身份认证', 'zib_language') . '</div>';
            $desc .= $disable_desc;
        }

        $con = '<div class="text-left">';
        $con .= '<div class="em12 mb10">' . $icon . '<b class="ml6">' . __('申请认证', 'zib_language') . '</b></div>';
        $con .= $desc;
        $con .= '</div>';
    }
    ...
    $html .= '<div class="em4x">' . $icon . '</div><div class="mt10 em14 padding-w10 font-bold mb40">' . __('官方认证', 'zib_language') . '</div>';
    $html .= $con;
    $html .= '</div>';
    $html .= '</div>';
