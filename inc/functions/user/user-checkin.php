...
function zib_get_user_checkin_details_modal($user_id)
{
    ...
    if ($points_s) {
        ...
        $user_card .= '<a rel="nofollow" class="block flex1 muted-box padding-10" href="' . zib_get_user_center_url('balance') . '">
                        <div class="muted-2-color em09 mb6">' . __('我的积分', 'zib_language') . '</div>
    ...
    if ($user_level_s) {
        ...
        $user_card .= '<a rel="nofollow" class="block flex1 muted-box padding-10 ml6" href="' . zib_get_user_center_url('level') . '">
                        <div class="muted-2-color em09 mb6 flex jsb ac"><div>' . __('我的等级', 'zib_language') . '</div>' . $level_badge . '</div>
                        <div class="em12">' . $integral . '<span class="px12 ml6 muted-2-color">' . __('经验值', 'zib_language') . '<i class="fa fa-angle-right ml6"></i></span></div>
                    </a>';
    }

    if ($user_card) {
        $checkin_all_day = zib_get_user_checkin_all_day($user_id);
        $checkin_all_day = $checkin_all_day ? '<div class="mb6 em09">' . sprintf(__('累计签到%s天', 'zib_language'), '<badge class="c-blue">' . $checkin_all_day . '</badge>') . '</div>' : '';
    ...
    if ($theday_details) {
        $theday_reward = $theday_details['points'] ? __('积分', 'zib_language') . '+' . $theday_details['points'] . ' ' : '';
        $theday_reward .= $theday_details['integral'] ? __('经验值', 'zib_language') . '+' . $theday_details['integral'] : '';
        $checkin_btn = '<div class="em09 c-blue">
                                <div class="mb10"><i class="fa fa-calendar-check-o mr6"></i>' . __('今日已签到', 'zib_language') . '</div>
                                <div class="px12">' . $theday_reward . '</div>
                            </div>';

    } else {
        $checkin_btn = zib_get_user_checkin_btn('but c-blue ml10 p2-10 radius', __('立即签到', 'zib_language'), __('今日已签到', 'zib_language'));
    }
    $checkin_reward_points   = (int) _pz('checkin_reward_points', 0);
    $checkin_reward_integral = (int) _pz('checkin_reward_integral', 0);
    $reward_badge            = $checkin_reward_points && $points_s ? '<badge class=" p2-10 mr6 c-green"  data-toggle="tooltip" title="' . esc_attr(sprintf(__('积分奖励%s', 'zib_language'), $checkin_reward_points)) . '">' . zib_get_svg('points') . '+' . $checkin_reward_points . '</badge>' : '';
    $reward_badge .= $checkin_reward_integral && $user_level_s ? '<badge class=" p2-10 mr6 c-purple"  data-toggle="tooltip" title="' . esc_attr(sprintf(__('经验值奖励%s', 'zib_language'), $checkin_reward_integral)) . '"><i class="fa fa-line-chart"></i>+' . $checkin_reward_integral . '</badge>' : '';

    $card_1 = '<div class="zib-widget flex ac jsb">
                <div class="mr10">
                    <div class="em09 mb10">' . zib_get_svg('gift-color', null, 'mr6 em12') . __('每日签到奖励', 'zib_language') . '</div>
    ...
    $reward_continuous_day_html = $reward_continuous_day > 1 ? '<div class="mb6 em09">' . sprintf(__('已连续签到%s天', 'zib_language'), '<badge class="c-blue">' . $reward_continuous_day . '</badge>') . '</div>' : '';
    ...
    $card_2 = '<div class="mt10">
                <div class="flex ac jsb"><div class="em09 mb10">' . zib_get_svg('gift-color', null, 'mr6 em12') . __('连续签到奖励', 'zib_language') . '</div>' . $reward_continuous_day_html . '</div>
                <div class="flex ac jsb">' . $continuous_lists . '</div>
            </div>';

    $html = zib_get_modal_colorful_header('c-blue', '<i class="fa fa-calendar-check-o"></i>', __('签到领奖励', 'zib_language'));
    $html .= $user_card;
    $html .= $card_1;
    $html .= $card_2;
    return $html;
...
function zib_get_checkin_continuous_mini_card($d, $_points, $_integral, $is_ed = false)
{
    ...
    $_mini_card = '';
    $_mini_card .= $_points && $points_s ? '<div class="badg badg-sm ' . ($is_ed ? 'b' : 'c') . '-green"  data-toggle="tooltip" title="' . esc_attr(sprintf(__('积分奖励%s', 'zib_language'), $_points)) . '">' . zib_get_svg('points') . '<br/>+' . $_points . '</div>' : '';
    $_mini_card .= $_integral && $user_level_s ? '<div class="badg badg-sm ' . ($is_ed ? 'b' : 'c') . '-purple"  data-toggle="tooltip" title="' . esc_attr(sprintf(__('经验值奖励%s', 'zib_language'), $_integral)) . '"><i class="fa fa-line-chart"></i><br/>+' . $_integral . '</div>' : '';

    return '<div class="flex1 text-center" style="flex: 1;">
                <div class="mb6 checkin-mini-box">' . $_mini_card . '</div>
                <div class="muted-2-color smail' . ($is_ed ? ' c-blue' : '') . '">' . ($is_ed ? '<i class="fa fa-check-circle"></i>' : '') . sprintf(__('第%d天', 'zib_language'), $d) . '</div>
            </div>';
}
