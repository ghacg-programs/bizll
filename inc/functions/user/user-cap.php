...
function zib_get_hascaps_roles_lists($roles = array(), $class = 'badg mm3')
{
    ...
    if (!empty($roles['level'])) {
        $lists .= '<span class="' . $class . '">' . zib_get_level_badge($roles['level'], 'mr6 em12') . __('及更高等级', 'zib_language') . '</span>';
    }

    if (!empty($roles['auth'])) {
        $lists .= '<span class="' . $class . '">' . zib_get_svg('user-auth', null, 'mr6 em12') . __('认证用户', 'zib_language') . '</span>';
    }
...
function zib_get_nocan_info($user_id, $capability, $msg = '', $margin = 30, $width = 280, $height = 0)
{
    ...
    if (!$can_info) {
        $null_msg = __('抱歉！权限不足', 'zib_language');
        $null_msg .= $msg ? '，' . $msg : '';
        $can_info = zib_get_null($null_msg, $margin, 'null-cap.svg', '', $width, $height);

        if (!$user_id) {
            //未登录
            $can_info = zib_get_user_singin_page_box('mt20', __('Hi！请先登录', 'zib_language'));
        } else {
            $roles_lists = zib_get_cap_roles_lists($capability);
            $can_info .= $roles_lists ? '<div class="text-center mb20"><div class="em09 muted-2-color mb20">' . __('成为以下用户组可拥有此权限', 'zib_language') . '</div>' . $roles_lists . '</div>' : '';
        }
    }
