 ...
 * @return {*}
 */
function zib_get_user_center_link($class = '', $text = null)
{
    if (null === $text) {
        $text = __('用户中心', 'zib_language');
    }
...
function zib_get_user_search_link($user_id, $type = '', $class = '', $con = '')
{
    ...
    $args = array(
        ...
        'user'        => $user_id,
        'type'        => $type,
        'placeholder' => sprintf(__('在用户[%1$s]中搜索%2$s', 'zib_language'), $name, $type_name),
    );
 ...
 * @return {*}
 */
function zib_get_user_details_data_link($user_id, $class = '', $text = null)
{
    if (null === $text) {
        $text = __('更多资料', 'zib_language');
    }
    if (!$user_id) {
        return;
    }
...
function zib_get_user_singin_page_box($class = 'box-body', $hi = null)
{
    ...
    if ($social_login) {
        $social_login_html = '<div class="social-separator separator muted-3-color em09 mt20 mb20">' . __('社交账号登录', 'zib_language') . '</div><div class="social_loginbar">' . $social_login . '</div>';
    }

    $html = '<div class="text-center ' . $class . '">
                <p class="muted-color box-body em12">' . $hi . '</p>
                <p>
                    <a href="javascript:;" class="signin-loader but jb-blue padding-lg"><i class="fa fa-fw fa-sign-in" aria-hidden="true"></i>' . __('登录', 'zib_language') . '</a>
                    ' . (!zib_is_close_signup() ? '<a href="javascript:;" class="signup-loader ml10 but jb-yellow padding-lg">' . zib_get_svg('signup') . __('注册', 'zib_language') . '</a>' : '') . '
                </p>
                ' . $social_login_html . '
            </div>';
    return $html;
...
function zib_get_user_join_day_desc($user_id = 0, $calss = 'em09 muted-2-color')
{
    ...
    if ($join_day) {
        return '<span class="' . $calss . '">' . sprintf(__('已加入%s%d天', 'zib_language'), $name, $join_day) . '</span>';
    }
}
 ...
 * @return {*}
 */
function zib_get_user_edit_quick_often_link($class = '', $con = null)
{
    if (null === $con) {
        $con = __('添加', 'zib_language');
    }
