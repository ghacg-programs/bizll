...
class ZibMeilisearchHttp
{
    ...
    public function autoError($response)
    {
        ...
        if (is_wp_error($response)) {
            throw new \Exception(sprintf(__('ZibMeilisearchHttp网络请求失败：%s', 'zib_language'), $response->get_error_message()));
            return [];
        }
        ...
        $code_msg = [
            'missing_authorization_header' => __('权限异常：该接口必须填写密钥', 'zib_language'),
            'invalid_api_key'              => __('API密钥错误', 'zib_language'),
        ];
        ...
        if ((int) $response_code > 400) {
            if ($body_code && isset($code_msg[$body_code])) {
                throw new \Exception(sprintf(__('ZibMeilisearchHttp错误：%s', 'zib_language'), $code_msg[$body_code]));
            }

            if (!empty($result['message'])) {
                throw new \Exception(sprintf(__('ZibMeilisearchHttp错误：%s%s', 'zib_language'), $result['message'], ($body_code ? __('，错误代码：', 'zib_language') . $body_code : '')));
            }

            throw new \Exception(sprintf(__('ZibMeilisearchHttp请求错误：%s', 'zib_language'), json_encode($response)));
        }

        if (!is_array($result)) {
            throw new \Exception(__('ZibMeilisearchHttp响应数据异常：响应非json数据', 'zib_language'));
        }
    }
}
