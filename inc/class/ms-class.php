...
class zib_ms
{
    ...
    public function import($paged = 1, $page_size = 500)
    {
        ...
        try {
            ...
            return ['error' => 1, 'msg' => sprintf(__('处理失败，错误信息：%s', 'zib_language'), $e->getMessage())];
        }
    ...
    public function sync_post($post)
    {
        ...
        if (!$post) {
            return ['error' => 1, 'msg' => __('文章不存在', 'zib_language')];
        }
        ...
        try {
            ...
            return ['error' => 1, 'msg' => sprintf(__('同步单个数据失败，错误信息：%s', 'zib_language'), $e->getMessage())];
        }

        return ['error' => 0, 'msg' => sprintf(__('文章数据同步成功，ID： %s', 'zib_language'), $post_data['id'])];
    }
