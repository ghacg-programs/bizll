 ...
 * @LastEditTime : 2026-06-21 00:45:29
* @Project : Zibll子比主题
...
class zib_db
{
    ...
    public function buildOrderBy()
    {
        ...
        foreach ($this->order as $field => $direction) {
            if (!is_string($field) || !$field || !is_string($direction)) {
                continue;
            }

            $safeField = $this->escapeOrderBy($field);
            if ($safeField) {
                $safeDirection = strtoupper(trim($direction)) === 'DESC' ? 'DESC' : 'ASC';
                $parts[]       = "{$safeField} {$safeDirection}";
            }
        }
     ...
     * @return string
     */
    public function escapeOrderBy($field)
    {
        ...
        //严格限制长度
        if ($field === '' || strlen($field) > 128) {
            return '';
        }

        // 数值排序：alias.column+0
        $id = '[a-zA-Z_][a-zA-Z0-9_]{0,63}';
        if (preg_match('/^(' . $id . ')\.(' . $id . ')([\+\-\*\/])([0-9]+)$/', $field, $m)) {
            return '`' . $m[1] . '`.`' . $m[2] . '`' . $m[3] . $m[4];
        }

        // 数值排序：column+0
        if (preg_match('/^(' . $id . ')([\+\-\*\/])([0-9]+)$/', $field, $m)) {
            return '`' . $m[1] . '`' . $m[2] . $m[3];
        }

        // 单列或 alias.column（最多两段，禁止首尾点与空段）
        if (!preg_match('/^' . $id . '(?:\.' . $id . ')?$/', $field)) {
            return '';
        }
