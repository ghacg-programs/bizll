...
class ZibAudit
{
    ...
    public static function request($content, $type = 'text', $sdk = null)
    {
        ...
        if (!$sdk || 'null' == $sdk) {
            return array('error' => 'nosdk', 'msg' => __('未设置内容审核接口', 'zib_language'));
        }
        ...
        if (!$result) {
            return array('error' => 'nosdk', 'msg' => __('未设置内容审核接口或者其它未知错误', 'zib_language'));
        }
    ...
    public static function _return($conclusionType, $msg = "", $data = array())
    {
        if (!$conclusionType || 4 == $conclusionType) {
            return array('error' => '1', 'msg' => __('暂无审核结果', 'zib_language'));
        }

        $conclusion_array = array(
            1 => __('合规', 'zib_language'),
            2 => __('不合规', 'zib_language'),
            3 => __('疑似不合规', 'zib_language'),
        );

        $conclusion = isset($conclusion_array[$conclusionType]) ? $conclusion_array[$conclusionType] : __('审核失败', 'zib_language');
    ...
    public static function baidu($content, $type)
    {
        ...
        if (empty($response->success)) {
            return array('error' => '1', 'msg' => __('百度内容审核接口链接超时，请稍候再试', 'zib_language'));
        }

        $res = $response->json(true);
        if (!empty($res['error_msg'])) {
            return array('error' => '1', 'msg' => sprintf(__('百度内容审核失败：%s', 'zib_language'), $res['error_msg']));
        }
    ...
    public static function baidu_get_token()
    {
        ...
        if (empty($cofig['appkey']) || empty($cofig['secretkey'])) {
            return array('error' => __('百度内容识别：缺少配置参数', 'zib_language'));
        }
        ...
        if (empty($response->success)) {
            return array('error' => __('百度内容审核获取access_token链接超时，请稍候再试', 'zib_language'));
        }
        $res = $response->json(true);
        ...
        return array('error' => sprintf(__('百度内容审核access_token获取失败：%s', 'zib_language'), json_encode($res)));
    }
}
