...
class ZibSMS
{
    ...
    public static function send($to, $code, $sdk = '')
    {
        ...
        if (!empty($result['result']) && empty($result['msg'])) {
            $result['msg'] = __('短信已发送', 'zib_language');
        }
        return $result;
    }
    ...
    public static function huaxinyun_send()
    {
        ...
        if (empty($cofig['username']) || empty($cofig['password']) || empty($cofig['template'])) {
            return array('error' => 1, 'ys' => 'danger', 'msg' => __('华信云短信：缺少配置参数', 'zib_language'));
        }
        ...
        if (empty($response->success)) {
            return array('msg' => __('链接超时，请稍候再试', 'zib_language'));
        }
    ...
    public static function fcykj_send()
    {
        ...
        if (empty($cofig['appid']) || empty($cofig['auth_token']) || empty($cofig['template_id'])) {
            return array('error' => 1, 'ys' => 'danger', 'msg' => __('风吹雨短信：缺少配置参数', 'zib_language'));
        }
    ...
    public static function smsbao_send()
    {
        ...
        if (empty($cofig['userame']) || (empty($cofig['password']) && empty($cofig['template']))) {
            return array('error' => 1, 'ys' => 'danger', 'msg' => __('短信宝：缺少配置参数', 'zib_language'));
        }
        ...
        $statusStr = array(
            '0'  => __('短信发送成功', 'zib_language'),
            '-1' => __('参数不全', 'zib_language'),
            '-2' => __('服务器空间不支持,请确认支持curl或者fsocket，联系您的空间商解决或者更换空间！', 'zib_language'),
            '30' => __('短信宝：密码错误', 'zib_language'),
            '40' => __('短信宝：账号不存在', 'zib_language'),
            '41' => __('短信宝：余额不足', 'zib_language'),
            '42' => __('短信宝：帐户已过期', 'zib_language'),
            '43' => __('短信宝：IP地址限制', 'zib_language'),
            '50' => __('短信宝：内容含有敏感词', 'zib_language'),
            '51' => __('手机号码不正确', 'zib_language'),
        );
        ...
        if ($result == 0) {
            ...
            $toArray['error']  = 1;
            $toArray['msg']    = (isset($statusStr[$result]) ? $statusStr[$result] : __('短信发送失败', 'zib_language')) . ' | ' . __('错误码:', 'zib_language') . $result;
            $toArray['result'] = false;
        }
        return $toArray;
    }
    ...
    public static function tencent_send($to, $code)
    {
        ...
        if (empty($cofig['app_id']) || empty($cofig['app_key']) || empty($cofig['sign_name']) || empty($cofig['template_id'])) {
            return array('error' => 1, 'ys' => 'danger', 'msg' => __('腾讯云短信：缺少配置参数', 'zib_language'));
        }
        // 短信应用 SDK AppID
        $app_id = $cofig['app_id'];
        // 短信应用 SDK AppKey
        ...
        try {
            ...
            if (isset($toArray['result']) && $toArray['result'] == 0) {
                ...
                $toArray['error']  = 1;
                $toArray['msg']    = $toArray['errmsg'] . ' | ' . __('错误码:', 'zib_language') . $toArray['result'];
                $toArray['result'] = false;
            }
            return $toArray;
        } catch (\Exception $e) {
    ...
    public static function ali_verify_send($to, $code, $cofig)
    {
        ...
        if (empty($cofig['keyid']) || empty($cofig['keysecret']) || empty($cofig['sign_name'])) {
            return array('error' => 1, 'ys' => 'danger', 'msg' => __('阿里云短信：缺少配置参数', 'zib_language'));
        }
        //准备参数
