 ...
 * @LastEditTime : 2026-06-15 20:44:58
...
{
    $html = '<div class="custom-svg-box"><div class="csf-icon-title" style=" padding: 8px; ">' . esc_html__('自定义SVG图标代码', 'zib_language') . ' <div class="mt6" style=" font-size: 12px; opacity: .8; font-weight: normal; ">' . esc_html__('插入SVG格式图标代码，请注意代码规范，查看官网教程 ', 'zib_language') . '<a target="_blank" href="https://www.zibll.com/547.html" class="loginbtn">' . esc_html__('【查看官网教程】', 'zib_language') . '</a></div> </div>';
    $html .= '<div class="flex" style="margin:5px;position: relative;">
            <textarea placeholder="' . esc_attr__('请粘贴SVG格式代码，示例：', 'zib_language') . '&lt;svg viewBox=&quot;0 0 1024 1024&quot;&gt;&lt;path d=&quot;...&quot;&gt;&lt;/path&gt;&lt;/svg&gt;" name="icon_html" rows="2" style="height: 108px;" class="flex1"></textarea>
            <div class="flex ac" style="margin:5px;position: absolute;right: 5px;bottom: 5px;">
                <div class="flex jc mr3 custom-svg-display" style="width: 28px;height: 28px;line-height: 28px;font-size: 24px;color: #555;text-align: center;border: 1px solid #ccc;background-color: #f7f7f7;border-radius: 2px;"></div> <a href="#" class="button button-primary custom-svg-submit" data-nonce="70527b7cf6">' . esc_html__('确认插入', 'zib_language') . '</a>
            </div>
        </div>
    ...
    $icons[] = array(
        'title' => __('主题内置SVG图标', 'zib_language'),
        'icons' => array(
            'zibsvg-like',
            'zibsvg-view',
            'zibsvg-comment',
            ...
            'zibsvg-poster-color',
            'zibsvg-copy-color',
            'zibsvg-facebook-color',
            'zibsvg-twitter-color',
            'zibsvg-telegram-color',
            'zibsvg-whatsapp-color',
            'zibsvg-linkedin-color',
            'zibsvg-reddit-color',
            'zibsvg-line-color',
            'zibsvg-vk-color',
            'zibsvg-user-color',
...
function zib_csf_add_custom_wp_enqueue()
{
    ...
    // Script
    wp_enqueue_script('csf_custom_js', get_template_directory_uri() . '/inc/csf-framework/assets/js/main.min.js', array('jquery', 'zib_admin_man'), THEME_VERSION);
}
...
function zib_get_admin_csf_url($tab = '')
{
    ...
    foreach ($tab_array as $tab_i) {
        $tab_array_sanitize[] = sanitize_title(__($tab_i, 'zib_language'));
    }
...
function zib_notice_update()
{
    ...
    if ($version && version_compare($version, $theme_data['Version'], '<')) {
        ...
        $con     = '<div class="notice notice-success is-dismissible">
				<h2 style="color:#fd4c73;"><i class="fa fa-heart fa-fw"></i> ' . esc_html__('恭喜您！Zibll子比主题已更新', 'zib_language') . '</h2>
                ' . $up_desc . '
                <p>' . sprintf(esc_html__('欢迎使用zibll子比主题V%s，使用前请务必先配置好伪静态和固定链接，否则会出现404错误！查看官网教程 ', 'zib_language'), THEME_VERSION) . '<a target="_bank" style="color:#217ff9;" href="https://www.zibll.com/3025.html">' . esc_html__('【查看官网教程】', 'zib_language') . '</a></p>
                <p><a target="_bank" style="color:#217ff9;box-shadow:none !important;" href="https://www.zibll.com/375.html">' . esc_html__('看一下更新了哪些新功能？', 'zib_language') . '</a></p>
                <p>' . esc_html__('更新主题系统会自动刷新缓存，但如果遇到异常，建议手动', 'zib_language') . '<b style="color:#ff321d;">' . esc_html__('清空缓存、刷新CDN', 'zib_language') . '</b>！</p>
                <p>' . sprintf(esc_html__('保存一下%s后，此通知会自动关闭', 'zib_language'), '<a href="' . zib_get_admin_csf_url() . '">' . esc_html__('主题设置', 'zib_language') . '</a>') . '</p>
                <p><a class="button" style="margin: 2px;" href="' . zib_get_admin_csf_url() . '">' . esc_html__('体验新功能', 'zib_language') . '</a><a class="button" style="margin: 2px;" href="' . zib_get_admin_csf_url('文档更新') . '">' . esc_html__('查看主题文档', 'zib_language') . '</a><a target="_blank" class="button" style="margin: 2px;" href="https://www.zibll.com/375.html">' . esc_html__('查看更新日志', 'zib_language') . '</a></p>
			</div>';
        echo $con;
    }
}
...
function zib_notice_new_install()
{
    ...
    if (!$version) {
        ...
        $con = '<div class="notice notice-success is-dismissible">
				<h2 style="color:#fd4c73;"><i class="fa fa-heart fa-fw"></i> ' . esc_html__('感谢您使用子比主题', 'zib_language') . '</h2>
                <p>' . esc_html__('使用zibll子比主题请务必先配置好伪静态和固定链接，否则会出现404错误！查看官网教程 ', 'zib_language') . '<a target="_bank" style="color:#217ff9;" href="https://www.zibll.com/3025.html">' . esc_html__('【查看官网教程】', 'zib_language') . '</a></p>
                <p><a class="button" style="margin: 2px;" href="' . zib_get_admin_csf_url('文档更新') . '">' . esc_html__('查看主题文档', 'zib_language') . '</a><a target="_blank" class="button" style="margin: 2px;" href="https://www.zibll.com/375.html">' . esc_html__('查看更新日志', 'zib_language') . '</a></p>
			</div>';
        echo $con;
    }
}
...
function zib_notice_permalink_structure()
{
    if (!get_option('permalink_structure')) {
        $con = '<div class="notice notice-error is-dismissible">
            <h2 style="color:#f73d3f;"><i class="dashicons-before dashicons-admin-settings"></i> ' . esc_html__('请完成固定链接设置', 'zib_language') . '</h2>
            <p>' . esc_html__('您的网站还未完成固定链接配置，部分页面会出现404错误，请先完成伪静态和固定链接设置', 'zib_language') . '</p>
            <p><a class="button button-primary" style="margin: 2px;" href="' . admin_url('options-permalink.php') . '">' . esc_html__('立即设置', 'zib_language') . '</a><a target="_blank" class="button" style="margin: 2px;" href="https://www.zibll.com/3025.html">' . esc_html__('查看官网教程', 'zib_language') . '</a></p>
        </div>';
        echo $con;
    }
}
