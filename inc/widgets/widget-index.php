 ...
 * @LastEditTime : 2026-05-25 15:47:43
 * @Email         : 770349780@qq.com
 * @Project       : Zibll子比主题
 * @Description   : 一款极其优雅的Wordpress主题|小工具模块
...
function zib_main_register_sidebar()
{
    if (function_exists('register_sidebar')) {
        ...
        $sidebars[] = array(
            'name'        => __('所有页面-顶部全宽度', 'zib_language'),
            'id'          => 'all_top_fluid',
            'description' => __('同时显示在首页、分类页、标签页、搜索页、作者页、用户中心、消息中心等页面的顶部位置（不含论坛、商城的页面）', 'zib_language'),
        );

        $sidebars[] = array(
            'name'        => __('所有页面-底部全宽度', 'zib_language'),
            'id'          => 'all_bottom_fluid',
            'description' => __('同时显示在首页、分类页、标签页、搜索页、作者页、用户中心、消息中心等页面的底部位置（不含论坛、商城的页面）', 'zib_language'),
        );

        $sidebars[] = array(
            'name'        => __('所有页面-侧边栏-顶部位置', 'zib_language'),
            'id'          => 'all_sidebar_top',
            'description' => __('同时显示在首页、分类页、标签页等页面的侧边栏顶部位置（不含论坛、商城的页面）', 'zib_language'),
        );

        $sidebars[] = array(
            'name'        => __('所有页面-侧边栏-底部位置', 'zib_language'),
            'id'          => 'all_sidebar_bottom',
            'description' => __('同时显示在首页、分类页、标签页等页面的侧边栏底部位置（不含论坛、商城的页面）', 'zib_language'),
        );
        ...
        foreach ($pags as $key => $value) {
            foreach ($poss as $poss_key => $poss_value) {
                $sidebars[] = array(
                    ...
                    'id'          => $key . '_' . $poss_key,
                    'description' => sprintf(__('显示在 %1$s 的 %2$s 位置，由于位置较多，建议使用实时预览管理！', 'zib_language'), $value, $poss_value),
                );
            }
        }

        $pags_full = array(
            'author' => __('作者页', 'zib_language'),
        ...
        foreach ($pags_full as $key => $value) {
            $sidebars[] = array(
                'name'        => $value . '-' . __('顶部全宽度', 'zib_language'),
                'id'          => $key . '_top_fluid',
                'description' => sprintf(__('显示在 %1$s 的 内容顶部 位置，由于位置较多，建议使用实时预览管理！', 'zib_language'), $value),
            );
            $sidebars[] = array(
                'name'        => $value . '-' . __('底部全宽度', 'zib_language'),
                'id'          => $key . '_bottom_fluid',
                'description' => sprintf(__('显示在 %1$s 的 内容底部 位置，由于位置较多，建议使用实时预览管理！', 'zib_language'), $value),
            );
            if ($key === 'user') {
                $sidebars[] = array(
                    'name'        => $value . '-' . __('内容区上面', 'zib_language'),
                    'id'          => $key . '_top_content',
                    'description' => sprintf(__('显示在 %1$s 的 内容顶部 位置，由于位置较多，建议使用实时预览管理！', 'zib_language'), $value),
                );
            }
        ...
        $sidebars[] = array(
            'name'        => __('前台投稿-侧边栏顶部', 'zib_language'),
            'id'          => 'newposts_sidebar_top',
            'description' => __('显示在前台投稿页面的侧边栏顶部，由于宽度较小，请勿添加大尺寸模块，同时会在移动端显示', 'zib_language'),
        );
        $sidebars[] = array(
            'name'        => __('前台投稿-侧边栏底部', 'zib_language'),
            'id'          => 'newposts_sidebar_bottom',
            'description' => __('显示在前台投稿页面的侧边栏底部，由于宽度较小，请勿添加大尺寸模块，同时会在移动端显示', 'zib_language'),
        );
...
function zib_main_register_sidebar_2()
{
    $sidebars   = array();
    $sidebars[] = array(
        'name'        => __('底部脚区内部', 'zib_language'),
        'id'          => 'all_footer',
        'description' => __('显示最底部页脚区域内部，由于位置较多，建议使用实时预览管理！', 'zib_language'),
    );
    $sidebars[] = array(
        'name'        => __('移动端-弹出菜单底部', 'zib_language'),
        'id'          => 'mobile_nav_fluid',
        'description' => __('显示在移动端弹出的菜单内部的下方，由于宽度较小，请勿添加大尺寸模块', 'zib_language'),
    );
    zib_register_sidebar($sidebars);
}
    ...
    foreach ($register_pages as $page) {
        ...
        if ($meta_value && is_array($meta_value)) {
            foreach ($meta_value as $key) {
                ...
                register_sidebar(array(
                    'name'          => sprintf(__('[页面:%1$s]-%2$s', 'zib_language'), $name, $args[$key]),
                    'id'            => 'page_' . $key . '_' . $page->ID,
                    'description'   => sprintf(__('显示在[页面：%1$s]的 %2$s 位置', 'zib_language'), $name, $args[$key]),
                    'before_widget' => ($is_preview ? '<div class="customize-preview-widget-box" id="%1$s"><div class="customize-preview-widget-edit">' . esc_html__('编辑', 'zib_language') . '</div>' : '') . '<div class="zib-widget %2$s">',
                    'after_widget'  => ($is_preview ? '</div>' : '') . '</div>',
                    'before_title'  => '<h3>',
                    'after_title'   => '</h3>',
...
function zib_cat_help()
{
    ?>
	<div>
		<?php echo esc_html__('分类限制：', 'zib_language'); ?><a class="cat-help-button" style="font-weight:bold;color: #ff0039;text-decoration:none;background: #ffe8e8;width: 1.5em;line-height: 1.5em;text-align: center;display: inline-block;border-radius: 50%;" href="javascript:;">?</a>
		<div class="cat-help-con" style="display:none;padding: 5px;border: 1px solid #ddd;margin: 5px 0;background: #f7f8f9;border-radius: 8px;font-size: 12px;">
			<p><?php echo esc_html__('分类限制通过分类的id进行分类筛选，可以选择某些分类或者排除某些分类。示例及id列表如下', 'zib_language'); ?></p>
			<b><?php echo esc_html__('分类限制示例：', 'zib_language'); ?></b>
			<p>
			<div><?php echo esc_html__('仅仅显示分类ID为"10"的文章', 'zib_language'); ?></div>
			<div style="padding: 6px;background: #ececec">10</div>
			</p>
			<p>
			<div><?php echo esc_html__('显示包含分类ID为"10、11、12、13"的文章', 'zib_language'); ?></div>
			<div style="padding: 6px;background: #ececec">10,11,12,13</div>
			</p>
			<p>
			<div><?php echo esc_html__('排除分类ID为"10、11、12、13"的文章', 'zib_language'); ?></div>
			<div style="padding: 6px;background: #ececec">-10,-11,-12,-13</div>
			</p>
			<p>
			<div><?php echo esc_html__('排除分类ID为"10"的文章', 'zib_language'); ?></div>
			<div style="padding: 6px;background: #ececec">-10</div>
			</p>
		</div>
	</div>
...
function zib_topics_help()
{
    ?>
	<div>
		<?php echo esc_html__('专题选择：', 'zib_language'); ?><a class="cat-help-button" style="font-weight:bold;color: #ff0039;text-decoration:none;background: #ffe8e8;width: 1.5em;line-height: 1.5em;text-align: center;display: inline-block;border-radius: 50%;" href="javascript:;">?</a>
		<div class="cat-help-con" style="display:none;padding: 5px;border: 1px solid #ddd;margin: 5px 0;background: #f7f8f9;border-radius: 8px;font-size: 12px;">
			<p><?php echo esc_html__('输入专题ID，输入多个ID请用英文逗号分割', 'zib_language'); ?></p>
		</div>
	</div>
<?php
}
function zib_user_help($name = '')
{
	...
	<div>
		...
		<div class="cat-help-con" style="display:none;padding: 5px;border: 1px solid #ddd;margin: 5px 0;background: #f7f8f9;border-radius: 8px;font-size: 12px;">
			<p><?php echo esc_html__('输入用户ID，输入多个ID请用英文逗号分割', 'zib_language'); ?></p>
		</div>
	</div>
<?php
}
...
{
    $html = '<option value="" ' . selected('', $selected, false) . '>' . esc_html__('未选择', 'zib_language') . '</option>';
    if ('cat' == $type) {
        $args = array(
            'orderby'    => 'count',
            'order'      => 'DESC',
        ...
        foreach ($options_cat as $category) {
            $title = rtrim(get_category_parents($category->cat_ID, false, '>'), '>') . sprintf(__('[ID:%1$d][共%2$d篇]', 'zib_language'), $category->cat_ID, $category->count);
            $_id   = $category->cat_ID;
...
function zib_get_widget_show_type_input($args, $name)
{
    ...
    return '<div class=""><p class="mb6">' . esc_html__('显示规则', 'zib_language') . '</p><ul>
    <li><label><input type="radio" name="' . $name . '" value="all"' . (!$type || $type === 'all' ? ' checked="checked"' : '') . '><span class="ml6">' . esc_html__('PC端/移动端均显示', 'zib_language') . '</span></label></li>
    <li><label><input type="radio" name="' . $name . '" value="only_pc"' . ($type === 'only_pc' ? ' checked="checked"' : '') . '><span class="ml6">' . esc_html__('仅在PC端显示', 'zib_language') . '</span></label></li>
    <li><label><input type="radio" name="' . $name . '" value="only_sm"' . ($type === 'only_sm' ? ' checked="checked"' : '') . '><span class="ml6">' . esc_html__('仅在移动端显示', 'zib_language') . '</span></label></li>
    </ul></div>';
}
